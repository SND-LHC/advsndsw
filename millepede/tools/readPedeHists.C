...
void readPedeHists(Option_t *option = "",
                   const char *txtFile = "millepede.his");   //"nodraw" skips drawing, "print" produces ps file, "write"
                                                             //writes object in ROOT file
...
bool ReadPedeHists::readBasicVector(std::ifstream &aStream, std::vector<T> &outVector)
{
    ...
    for (unsigned int i = 0; i < outVector.size(); ++i) {
        if (!readBasic(aStream, outVector[i]))
            return false;
    }
...
//__________________________________________________________________________
void ReadPedeHists::readNumVersTypeTitle(std::ifstream &file, Int_t &num, Int_t &version, Int_t &type, TString &title)
{
    ...
    if (!readBasic(file, type))
        ::Error(method, "failed reading type");
...
TH1 *ReadPedeHists::readNextHist(std::ifstream &file)
{
    ...
    while (readBasic(file, key)) {
        if (key == "bins,") {
            ...
            if (!readBasic(file, key) || key != "limits") {
                ...
                ::Error(
                    method, "failed reading nBins, xLow, xUp (%f %f %f)", nBinsUpLow[0], nBinsUpLow[1], nBinsUpLow[2]);
            } else {
                binContent.resize(static_cast<unsigned int>(nBinsUpLow[0]));
            }
        } else if (key == "out-low") {
            // read under-/overflow with what is 'in between'
            if (!readBasic(file, key) || key != "inside" || !readBasic(file, key) || key != "out-high") {
                ::Error(method, "expected keys 'inside' and 'out-high', got (%s)", key.c_str());
            } else if (!readBasicVector(file, underInOver) || underInOver[0] == -1 || underInOver[1] == -1
                       || underInOver[2] == -1) {
                ::Error(method,
    ...
    TH1 *h = new TH1F(Form("hist%d_version%d", num, version), title, binContent.size(), nBinsUpLow[1], nBinsUpLow[2]);
...
std::pair<TGraph *, Option_t *> ReadPedeHists::readNextGraph(std::ifstream &file)
{
    ...
    if (type == 4 || type == 5) {
        ...
        if (TString(drawOpt).Contains("P"))
            graph->SetMarkerStyle(20);   //
    }   // 'else' not needed, tested above

    if (graph)
        graph->SetNameTitle(Form("graph%d_version%d", num, version), title);
    return std::make_pair(graph, drawOpt);
}

//__________________________________________________________________________
bool ReadPedeHists::readNext(std::ifstream &file, TH1 *&hist, std::pair<TGraph *, Option_t *> &graphOpt)
{
    hist = 0;
    graphOpt.first = 0;
    graphOpt.second = 0;
        ...
        if (file.fail() || (type != "Histogram" && type != "XY-Data")) {
            ...
            if (line != "" && line.Length() != line.CountChar(' ')) {   // not empty
                ::Error("ReadPedeHists::readNext",
                        ...
                        line.Data());
                if (proceedTo(file, "end of"))
                    line.ReadLine(file);   // just skip rest of line...
            }
        }

        if (type == "Histogram")
            hist = readNextHist(file);
        if (type == "XY-Data")
            graphOpt = readNextGraph(file);
        if (hist || graphOpt.first)
            break;
    }
...
void ReadPedeHists::read(std::ifstream &file)
{
    ...
    while (readNext(file, hist, graphOpt)) {
        if (hist)
            theHists.push_back(hist);
        if (graphOpt.first)
            theGraphOpts.push_back(graphOpt);
    }
}
...
void ReadPedeHists::Draw()
{
    ...
    while (iH < theHists.size()) {
        if (last >= nHistX * nHistY) {
            ...
            theCanvases.push_back(new TCanvas(Form("hists%d", iH), "", canCorner, canCorner, nPixelX, nPixelY));
    ...
    while (iH < theGraphOpts.size()) {
        if (last >= nHistX * nHistY) {
            unsigned int canCorner = theCanvases.size() * 20;
            theCanvases.push_back(new TCanvas(Form("graphs%d", iH), "", canCorner, canCorner, nPixelX, nPixelY));
            theCanvases.back()->Divide(nHistX, nHistY);
            last = 0;
        }
        theCanvases.back()->cd(++last);
...
{
    if (theHists.empty() && theGraphOpts.empty())
        return;
    ...
    for (std::vector<TH1 *>::iterator iH = theHists.begin(), iE = theHists.end(); iH != iE; ++iH) {
        (*iH)->Write();
    }

    for (std::vector<std::pair<TGraph *, Option_t *>>::iterator iG = theGraphOpts.begin(), iE = theGraphOpts.end();
         iG != iE;
         ++iG) {
        (*iG).first->Write();
    }
...
void readPedeHists(Option_t *option, const char *txtFile)
{
    ...
    opt.ReplaceAll(" ", "");
    if (!opt.IsNull()) {
        ::Warning("readPedeHists", "Unknown option '%s', know 'nodraw', 'print' and 'write'.", opt.Data());
    }
}
