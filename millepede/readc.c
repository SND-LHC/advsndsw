 ...
 */
{
    ...
    if (fileIndex < 0)
        fileIndex = numAllFiles; /* next one */
...
void readC(double *bufferDouble, float *bufferFloat, int *bufferInt, int *lengthBuffers, int *nFileIn, int *errorFlag)
{
    ...
    *errorFlag = 0;
...
#else
    size_t nCheckR = fread(&recordLength, sizeof(recordLength), 1, files[fileIndex]);
    ...
    if (recordLength / 2 > *lengthBuffers) {
        ...
        if (doublePrec) {
            for (; i < recordLength / 2; ++i) {
                size_t nCheckD = fread(bufferDouble, sizeof(bufferDouble[0]), 1, files[fileIndex]);
                if (ferror(files[fileIndex]) || feof(files[fileIndex]) || nCheckD != *lengthBuffers) {
                    printf("readC: problem with stream or EOF skipping doubles\n");
                    *errorFlag = -32;
                    return;
                }
            ...
            for (; i < recordLength / 2; ++i) {
                size_t nCheckF = fread(bufferFloat, sizeof(bufferFloat[0]), 1, files[fileIndex]);
                if (ferror(files[fileIndex]) || feof(files[fileIndex]) || nCheckF != *lengthBuffers) {
                    printf("readC: problem with stream or EOF skipping floats\n");
                    *errorFlag = -8;
                    return;
                }
        ...
        /* skip ints */
        for (; i < recordLength / 2; ++i) {
            size_t nCheckI = fread(bufferInt, sizeof(bufferInt[0]), 1, files[fileIndex]);
            if (ferror(files[fileIndex]) || feof(files[fileIndex]) || nCheckI != *lengthBuffers) {
    ...
    /* read floats (i.e. derivatives + value + sigma) */
    if (doublePrec) {
        size_t nCheckD = fread(bufferDouble, sizeof(bufferDouble[0]), *lengthBuffers, files[fileIndex]);
        if (ferror(files[fileIndex]) || feof(files[fileIndex]) || nCheckD != *lengthBuffers) {
            printf("readC: problem with stream or EOF reading doubles\n");
            *errorFlag = -32;
            return;
        ...
        size_t nCheckF = fread(bufferFloat, sizeof(bufferFloat[0]), *lengthBuffers, files[fileIndex]);
        if (ferror(files[fileIndex]) || feof(files[fileIndex]) || nCheckF != *lengthBuffers) {
            printf("readC: problem with stream or EOF reading floats\n");
            *errorFlag = -8;
            return;
    ...
    /* read ints (i.e. parameter labels) */
    size_t nCheckI = fread(bufferInt, sizeof(bufferInt[0]), *lengthBuffers, files[fileIndex]);
    if (ferror(files[fileIndex]) || feof(files[fileIndex]) || nCheckI != *lengthBuffers) {
        printf("readC: problem with stream or EOF reading ints\n");
        *errorFlag = -16;
        return;
