...
Bool_t MuDISGenerator::ReadEvent(FairPrimaryGenerator *cpg)
{
    ...
    while (prob2int < gRandom->Uniform(0., 1.)) {
        ...
        if (prob2int > 1.)
            LOG(WARNING) << "***WARNING*** MuDISGenerator: prob2int > Maximum density????" << prob2int
                         << " maxrho:" << mparam[7] << " material: " << mat->GetName();
        count += 1;
    }
    ...
    // a proper implementation however would be to have this better integrated in Geant4, follow the muon, call DIS
    // event, continue
    cpg->AddTrack(
        mu->GetPdgCode(), -(mu->Px()), -(mu->Py()), -(mu->Pz()), xmu, ymu, zmu, 0, true, mu->Energy(), mu->T(), w);
    ...
    for (int i = 0; i < nf; i++) {
        ...
        cpg->AddTrack(
            Part->GetPdgCode(), Part->Px(), Part->Py(), Part->Pz(), xmu, ymu, zmu, 0, true, Part->Energy(), mu->T(), w);
    }
    return kTRUE;
}
// -------------------------------------------------------------------------
