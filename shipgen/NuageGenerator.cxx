...
std::vector<double> NuageGenerator::Rotate(Double_t x, Double_t y, Double_t z, Double_t px, Double_t py, Double_t pz)
{
    ...
    pout.push_back(pyr);
    pout.push_back(pzr);
    // cout << "Info NuageGenerator: rotated" << pout[0] << " " << pout[1] << " " << pout[2] << " " << x << " " << y <<
    // " " << z <<endl;
    return pout;
}
...
Bool_t NuageGenerator::ReadEvent(FairPrimaryGenerator *cpg)
{
    ...
    while (bparam < 1.e-10) {
        ...
        if (pout[2] > 0.) {
            ...
            // cout << "Info NuageGenerator: neutrino xyz-start " << start[0] << "  -  " << start[1] << "  -   " <<
            // start[2] << endl;

            end[0] = txnu * (end[2] - ztarget);
            end[1] = tynu * (end[2] - ztarget);
            // cout << "Info NuageGenerator: neutrino xyz-end " << end[0] << "  -   " << end[1] << "   -  " << end[2] <<
            // endl;
    ...
    // cout << "Info NuageGenerator What was maximum density, mparam[7]= " << mparam[7] << ", " << mparam[7]*1.e8
    // <<endl;
    while (prob2int < gRandom->Uniform(0., 1.)) {
        ...
        if (prob2int > 1.)
            cout << "***WARNING*** NuageGenerator: prob2int > Maximum density????" << prob2int
    ...
    if (not fNuOnly) {
        ...
        if (TMath::Abs(oLPdgCode) != 15) {
            cpg->AddTrack(oLPdgCode,
                          ...
                          0,
                          true,
                          TMath::Sqrt(pp[0] * pp[0] + pp[1] * pp[1] + pp[2] * pp[2]),
                          tof,
                          mparam[0] * mparam[4]);
            nAddTrk++;
        }
                ...
                if (TMath::Abs(parent2) == 15) {
                    ...
                    for (int j = 0; j < nf2; j++) {
                        ...
                        cpg->AddTrack(pdgf2[j],
                                      ...
                                      tof2,
                                      mparam[0] * mparam[4]);
                        //	cout << "pdgf2 " << pdgf2[j] << " p2 "<<
                        //TMath::Sqrt(pp[0]*pp[0]+pp[1]*pp[1]+pp[2]*pp[2]) << endl;
                    }
                    nAddTrk += nf2;
                }
...
// -------------------------------------------------------------------------
Int_t NuageGenerator::GetNevents() { return fNevents; }
