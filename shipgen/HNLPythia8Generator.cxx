    ...
    do {
        ...
        if (iHNL == 0) {
            ...
            tS = tp + LS / beta;   // units ? [mm/c] + [mm/beta] (beta is dimensionless speed, and c=1 here)
                                   // if one would use [s], then tS = tp/(cm*c_light) + (LS/cm)/(beta*c_light) =
                                   // tS/(cm*c_light) i.e. units look consisent
            ...
            // cout << endl << " insert mother pdg=" <<fPythia->event[im].id() << " pmz = " << pmz << " [GeV],  zm = "
            // << zm << " [mm] tm = " << tm << " [mm/c]" << endl; cout << " ----> insert HNL =" << fHNL << " pz = " <<
