...
// -------------------------------------------------------------------------
// -----   Default constructor   -------------------------------------------
Bool_t ALPACAGenerator::Init(const char* fileName) { return Init(fileName, 0); }
// -----   Default constructor   -------------------------------------------
...
Bool_t ALPACAGenerator::ReadEvent(FairPrimaryGenerator* cpg)
{
    if (fn < fNevents) {
        ...
        cout << "Event Number:" << fn / 3 << endl;   // this division is due to the fact that ntuple is recorded in
                                                     // track based and every event has three tracks.
        return kTRUE;
    } else {
...
// -------------------------------------------------------------------------
Int_t ALPACAGenerator::GetNevents() { return fNevents / 3; }
