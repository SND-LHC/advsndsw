...
Bool_t DPPythia8Generator::Init()
{
    ...
    if (fextFile && *fextFile) {
        /*if (0 == strncmp("/eos",fextFile,4) ) {
         ...
         if (!fInputFile) {
          fLogger->Fatal(MESSAGE_ORIGIN, "Error opening input file. You may have forgotten to provide a krb5 token. Try
        kinit username@lxplus.cern.ch"); return kFALSE; } }else{ fLogger->Info(MESSAGE_ORIGIN,"Open external file with
        charm or beauty hadrons: %s",fextFile); fInputFile  = new TFile(fextFile); if (!fInputFile) {
        ...
        */
        ...
        if (fdy)
            fPythia->settings.parm("PhaseSpace:mHatMin", fDPminM);

    } else {
        if (!fpbremPDF) {
            // std::cout << " Failed in retrieving dark photon PDF for production by proton bremstrahlung! Exiting..."
            // << std::endl;
    ...
    if (debug) {
        std::cout << "tau root PDG database " << root_ctau << "[s] ctau root = " << root_ctau * 3e10 << "[cm]"
                  << std::endl;
    }
...
Bool_t DPPythia8Generator::ReadEvent(FairPrimaryGenerator* cpg)
{
    ...
    do {
        ...
        if (fpbrem) {
            ...
            if (debug > 1) {
                std::cout << " Adding DP gun with p " << dpmom << " m " << dpm << " e " << dpe << " theta,phi "
                          << thetain << "," << phiin << std::endl
                          << std::flush;
            }
            fPythia->event.append(fDP,
                                  1,
                                  0,
                                  0,
                                  dpmom * sin(thetain) * cos(phiin),
        ...
        if (iDP == 0) {
            ...
            std::cout << " Debug: decay product of A: " << fPythia->event[fPythia->event[i].daughter1()].id() << " "
                      << fPythia->event[fPythia->event[i].daughter2()].id() << std::endl;
            // hadDecay = abs(fPythia->event[ida].id())!=11 && abs(fPythia->event[ida].id())!=13 &&
            // abs(fPythia->event[ida].id())!=15;
            ...
            // if one would use [s], then tS = tp/(cm*c_light) + (LS/cm)/(beta*c_light) = tS/(cm*c_light) i.e. units
            // look consisent
            ...
            if (fextFile && *fextFile) {
                ...
                // // convert pythia's (x,y,z[mm], t[mm/c]) to ([cm], [s]) cpg->AddTrack(fDP, px, py, pz,
                // xp/cm+dx,yp/cm+dy,zp/cm, 1,false,e,tp/cm/c_light,w);
            } else {
            ...
            if (debug > 1)
                std::cout << std::endl
                          << " insert mother id " << im << " pdg=" << fPythia->event[im].id() << " pmz = " << pmz
                          << " [GeV],  zm = " << zm << " [mm] tm = " << tm << " [mm/c]" << std::endl;
            if (debug > 1)
                std::cout << " ----> insert DP id " << i << " pdg=" << fDP << " pz = " << pz << " [GeV] zp = " << zp
    ...
    if (debug > 1)
        std::cout << "Filling daughter particles" << std::endl;
    // if (!hadDecay){
    for (int k = 0; k < fPythia->event.size(); k++) {
        // if daughter of DP, copy
        if (debug > 1)
            std::cout << k << " pdg =" << fPythia->event[k].id() << " mum " << fPythia->event[k].mother1() << std::endl;
    ...
    for (std::vector<int>::iterator it = dec_chain.begin() + 2; it != dec_chain.end(); ++it) {
        ...
        cpg->AddTrack((Int_t)fPythia->event[k].id(),
                      px,
                      py,
                      pz,
                      xS / cm,
                      ...
                      tS / cm / c_light,
                      w);
        // std::cout <<k<< " insert pdg =" <<fPythia->event[k].id() << " pz = " << pz << " [GeV] zS = " << zS << " [mm]
        // tS = " << tS << "[mm/c]" <<  std::endl;
    }
    return kTRUE;
}
