...
Bool_t Pythia8Generator::Init()
{
    ...
    if (fextFile && *fextFile) {
        if (0 == strncmp("/eos", fextFile, 4)) {
            ...
            if (!fInputFile) {
                LOG(FATAL) << "Error opening input file. You may have forgotten to provide a krb5 token. Try kinit "
...
Bool_t Pythia8Generator::ReadEvent(FairPrimaryGenerator* cpg)
{
    ...
    if (targetName != "") {
        ...
        for (Int_t nI = 0; nI < nInter; nI++) {
            // if (!subprocCodes[nI]<90){continue;}  //if process is not inelastic, go to next. Changed by taking now
            // collision length
            prob2int = -1.;
            ...
            while (prob2int < rndm) {
                ...
                Double_t interLength =
                    mparam[8] * intLengthFactor * 1.7;   // 1.7 = interaction length / collision length from PDG Tables
                ...
                if (node && !gGeoManager->IsOutside()) {
                    ...
                    sigma = 1. / (n * mat->GetIntLen())
                            / mbarn;   // no need to multiply with intLengthFactor, will cancel in next equation.
    ...
    while (lx) {
        ...
        if (mE[0] == 0) {
            lx = true;
            fn++;
            cpg->AddTrack((Int_t)hid[0],
                          hpx[0],
                          hpy[0],
                          hpz[0],
                          (mpx[0] + fPythia->event[0].xProd()) / cm,
                          (mpy[0] + fPythia->event[0].yProd()) / cm,
                          (mpz[0] + fPythia->event[0].zProd()) / cm + zinter / cm,
                          -1,
                          true);
