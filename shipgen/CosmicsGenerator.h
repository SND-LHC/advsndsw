...
class Co3Rng
{
  public:
    Co3Rng()
    {
        ...
        fTheta->SetNpx(10);
        fSpectrumH = new TF1(
            "f4", "1400*TMath::Power(x,-2.7)*(1/(1+x/115)+0.054/(1+x/850))", 100, 1000);   // momentum above 100GeV
    };
    virtual ~Co3Rng()
    {
        delete rng;
...
class CosmicsGenerator : public FairGenerator
{
  public:
    ...
    virtual ~CosmicsGenerator()
    {
        ...
        cout << "There is a total of " << nInside << "/" << nTest << " muons that passed close enough to the detector."
             << endl;
        cout << "Including the given weight this corresponds to ";
        cout << weighttest / FluxIntegral / xdist / zdist * 10000
