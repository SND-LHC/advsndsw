...
// -------------------------------------------------------------------------
// -----   Default constructor   -------------------------------------------
Bool_t MuonBackGenerator::Init(const char* fileName) { return Init(fileName, 0, false); }
// -----   Default constructor   -------------------------------------------
...
Bool_t MuonBackGenerator::checkDiMuon(Int_t muIndex)
{
    ...
    if (strncmp("Hadronic inelastic", pName.Data(), 18) == 0 || strncmp("Positron annihilation", pName.Data(), 21) == 0
                ...
                if (element.first == i) {
                    ...
                    if (not followMuons) {
                        ...
                        if (abspid == 22) {
                            ...
                            e = TMath::Sqrt(lmv.Mag2() + (track->GetMass()) * (track->GetMass()));
                        }
                        px = lmv[0];
                        py = lmv[1];
                        pz = lmv[2];
