...
Bool_t FixedTargetGenerator::InitForCharmOrBeauty(TString fInName, Int_t nev, Double_t npot, Int_t nStart)
{
    ...
    LOG(INFO) << "FixedTargetGenerator::InitForCharmOrBeauty: Input file:" << fInName.Data() << " with " << nEvents
              << " entries, corresponding to nr-pot=" << nrcpot / chicc;
    LOG(INFO) << "FixedTargetGenerator::InitForCharmOrBeauty: weight " << wspill << ", corresponding to " << nrpotspill
...
Bool_t FixedTargetGenerator::Init()
{
    ...
    for (const auto& fPythia : plist) {
        ...
        if (JpsiMainly) {
            ...
            fPythia->readString("553:new  Upsilon  Upsilon  3   0   0    9.46030    0.00005    9.45980    9.46080  "
                                "0.00000e+00   0   1   0   1   0");
        ...
        if (tauOnly) {
            fPythia->readString("431:new  D_s+  D_s-  1   3   0    1.96849    0.00000    0.00000    0.00000  "
                                "1.49900e-01   0   1   0   1   0");
        ...
        while (n != 0) {
            ...
            if (p->tau0() > 1) {
                ...
                LOG(INFO) << "FixedTargetGenerator::Init Made " << p->name().c_str()
        ...
        if (fBoost != 1.) {
            ...
            for (unsigned int i = 0; i < r.size(); ++i) {
                ...
                if (TMath::Abs(ch.product(0)) != 13 || TMath::Abs(ch.product(1)) != 13) {
                    LOG(INFO) << "FixedTargetGenerator::Init this is not the right decay channel:" << r[i] << " "
                              << c[i];
                } else {
                    TString tmp = "";
                    tmp += r[i];
    ...
    if (targetName != "") {
        ...
        if (!fcharmtarget) {
            ...
            Int_t ilast =
                nodes->GetSize() - 5;   // assumes that the last 5 nodes are for the shielding around the target.
            ...
            if (!target) {
                LOG(ERROR) << "FixedTargetGenerator::Init target not found, " << targetName.Data()
                           << ", program will crash";
            }
...
Bool_t FixedTargetGenerator::ReadEvent(FairPrimaryGenerator* cpg)
{
    ...
    if (targetName.Data() != "") {
        ...
        // loop over trajectory between start and end to pick an interaction point, copied from GenieGenerator and
        // adapted to hadrons
        Double_t prob2int = -1.;
        Double_t rndm = 0.;
        Double_t sigma;
    ...
    if (G4only) {
        ...
        cpg->AddTrack(
            int(n_mid), n_mpx, n_mpy, n_mpz, xOff / cm, yOff / cm, zinter / cm, -1, kFALSE, n_mE, 0., wspill, procID);
        // second charm hadron in the event
        nTree->GetEvent(nEntry);
        if (nID1 * n_id > 0) {
            LOG(INFO) << "FixedTargetGenerator::readEvent same sign charm: " << nEntry << " " << nID1 << " " << n_id;
        }
        nEntry += 1;
        fPythiaP->event.append(int(n_id), 1, 0, 0, n_px, n_py, n_pz, n_E, n_M, 0., 9.);
        fPythiaP->next();
