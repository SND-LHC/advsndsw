...
Bool_t GenieGenerator::Init(const char *fileName, const int firstEvent)
{
    ...
    if (fGenOption < 0 || fGenOption > 3) {
        LOG(FATAL) << "Invalid GenieGen Option: " << fGenOption << " Please check the option provided with --Genie "
                   << endl;
        return kFALSE;
    }
    if (fGenOption == 1) {
        ...
        if (!fFLUKANuTree) {
            LOG(FATAL) << "No TTree of interacting neutrinos present. Did you run extract_interacting_neutrinos.py "
                          "from the macro folder?";
            return kFALSE;
        }
...
std::vector<double> GenieGenerator::Rotate(Double_t x, Double_t y, Double_t z, Double_t px, Double_t py, Double_t pz)
{
    ...
    pout.push_back(pyr);
    pout.push_back(pzr);
    // cout << "Info GenieGenerator: rotated" << pout[0] << " " << pout[1] << " " << pout[2] << " " << x << " " << y <<
    // " " << z <<endl;
    return pout;
}
...
Bool_t GenieGenerator::OldReadEvent(FairPrimaryGenerator *cpg)
{
    if (fFirst) {
        ...
        cout << "Info GenieGenerator: vessel length heig<ht width " << Lvessel << " " << Yvessel << " " << Xvessel
             << endl;
        cout << "Info GenieGenerator: scint thickness " << fScintDz << endl;
        cout << "Info GenieGenerator: rextra " << fScintDz / 2. + 2 * fEntrDz_inner << " " << 2 * fEntrDz_outer << " "
             << 2 * fEntrDz_inner << endl;
        for (int j = 0; j < boxs.size(); j++) {
            cout << "Info GenieGenerator: nuMu X" << j << " - " << -boxs[j].X() + dVecs[j].X() << " "
                 << boxs[j].X() + dVecs[j].X() << endl;
            cout << "Info GenieGenerator: nuMu Y" << j << " - " << -boxs[j].Y() + dVecs[j].Y() << " "
                 << boxs[j].Y() + dVecs[j].Y() << endl;
            cout << "Info GenieGenerator: nuMu Z" << j << " - " << -boxs[j].Z() + dVecs[j].Z() << " "
                 << boxs[j].Z() + dVecs[j].Z() << endl;
        }
        fFirst = kFALSE;
    }
    if (fn == fNevents) {
...
Bool_t GenieGenerator::ReadEvent(FairPrimaryGenerator *cpg)
{
    ...
    if (fGenOption == 3) {
        ...
        cpg->AddTrack(neu,   // Neutrino PDG
                      pxv,
                      pyv,
                      pzv,   // Neutrino momentum
                      vtxx * 100,
                      vtxy * 100,
                      vtxz * 100,   // Event vertex [in cm!]
                      -1,           // Parent
        ...
        if (cc)
            outgoing_lepton_pdg = copysign(TMath::Abs(neu) - 1, neu);
        if (nuel)
            outgoing_lepton_pdg = 11;

        bool track_outgoing_lepton = (cc || nuel) ? true : false;
        cpg->AddTrack(outgoing_lepton_pdg, pxl, pyl, pzl, vtxx * 100, vtxy * 100, vtxz * 100, 0, track_outgoing_lepton);
        ...
        if (fGenOption > 0)
            fFLUKANuTree->GetEntry(fn % fNevents);   // 1 or 2
        ...
        while (pout[2] < 0.) {
            ...
            if (pout[2] >= 0.) {
                ...
                // cout << "Info GenieGenerator: neutrino pxyz " << pout[0] << ", " << pout[1] << ", " << pout[2] <<
                // endl;
                ...
                // cout << "Info GenieGenerator: neutrino xyz-start " << start[0] << "-" << start[1] << "-" << start[2]
                // << endl;
                txnu = pout[0] / pout[2];
                tynu = pout[1] / pout[2];
        ...
        while (prob2int < gRandom->Uniform(0., 1.)) {
            ...
            if (mparam[6] < 0.5) {
                // mparam is number of boundaries along path. mparam[6]=0.: uniform material budget along path, use
                // present x,y,z
                prob2int = 2.;
            } else {
                ...
                if (node && !gGeoManager->IsOutside()) {
                    ...
                    // cout << "Info GenieGenerator: mat " <<  count << ", " << mat->GetName() << ", " <<
                    // mat->GetDensity() << endl; density relative to Prob largest density along this trajectory, i.e.
                    // use rho(Pt)
                    prob2int = mat->GetDensity() / mparam[7];
                    if (prob2int > 1.)
                        cout << "***WARNING*** GenieGenerator: prob2int > Maximum density????" << prob2int
                             << " maxrho:" << mparam[7] << " material: " << mat->GetName() << endl;
                    count += 1;
                } else {
                    prob2int = 0.;
                }
