...
Int_t muonHit::DetIDfromXYZ(TVector3 p)
{
    ...
    //
    if (nStat != -1) {
        detID = (Int_t)((p.X() + muStxMax[nStat]) / tileXdim)
                + tileXn[nStat] * (Int_t)((p.Y() + muStyMax[nStat]) / tileYdim) + 10000 * nStat;
    } else {
        detID = -1;
    }
    //
...
TVector3 muonHit::XYZfromDetID(Int_t dID)
{
    ...
    //
    Int_t nStat;
    TVector3 p;   // station number, center tile coordinate
                  //
    if (!onlyOnce) {
        stInit();
        onlyOnce = true;
...
void muonHit::stInit()
{
    ...
    for (Int_t i = 0; i < nodes->GetEntries(); i++) {
        ...
        if (TString(node->GetName()).Contains("muondet")) {
            ...
            muStxMax.push_back(muonBox->GetDX());
            muStyMax.push_back(muonBox->GetDY());
            muStzMax.push_back(muonBox->GetDZ());
            nav->LocalToMaster(loc, global);
            muStZpos.push_back(global[2]);
