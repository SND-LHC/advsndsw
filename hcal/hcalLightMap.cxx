...
hcalLightMap::hcalLightMap(const char* fname, const char* title)
    : TNamed(fname, title)
    , fSSide(-1111.)
    , fS(-1111)
    , fSize(0)
...
void hcalLightMap::Init(const char* filename)
{
    TString fn = filename;
    ...
    while (getline(f, buf)) {
        if (buf.empty())
            continue;
        if (buf.find_first_not_of(" 	") == string::npos)
            continue;
