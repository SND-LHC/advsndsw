...
{
    // cout << "ShipStack:  " << fNParticles << " " << pdgCode << " " << parentId <<    " " << secondparentID<<"
    // "<<proc<< endl;
    ...
    //   Int_t daughter1, Int_t daughter2, Double_t px, Double_t py, Double_t pz, Double_t etot, Double_t vx, Double_t
    //   vy, Double_t vz, Double_t time)
    particle->SetPolarisation(polx, poly, polz);
    particle->SetWeight(weight);
    particle->SetUniqueID(proc);
    // TR August 2014, still trying to understand the logic of FairRoot, due to misuse of secondparentID, all is a big
    // mess
    if (parentId < 0) {
...
void ShipStack::FillTrackArray()
{
    ...
    for (Int_t iPart = 0; iPart < fNParticles; iPart++) {
        ...
        if (store) {
            ShipMCTrack* track = new ((*fTracks)[fNTracks]) ShipMCTrack(GetParticle(iPart));
...
void ShipStack::Register() { FairRootManager::Instance()->Register("MCTrack", "Stack", fTracks, kTRUE); }
// -------------------------------------------------------------------------
...
void ShipStack::SelectTracks()
{
    ...
    for (Int_t i = 0; i < fNParticles; i++) {
        ...
        // independent if tracked or not. apply a dirty trick and use second mother to identify original generator
        // particles. doesn't work, always true: Int_t iMother2 = GetParticle(i)->GetMother(1); maybe should set Mother2
        // to -1 in the generator if (iMother == iMother2) {fStoreMap[i] = kTRUE;}
    }
