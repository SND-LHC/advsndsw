    ...
    : ShipHit()
{
    ...
    point1[1] = &b1;
    point1[2] = &c1;
    point_final[0] = *point1[0];
    point_final[1] = *point1[1];
    point_final[2] = *point1[2];
...
TGeoNode* UpstreamTaggerHit::GetNode(Double_t& hit_final, Int_t& mod)
{
    ...
    c0->ybox_fulldet = 499;   // resize box to define the modules position from active area of the detectors
                              // (discounting the aluminium box + acrilic)

    // First Layer of full Rpc detector covering 2.23 x 4.99 meters with 32 strips
    c0->module[1][0] = 0;
    c0->module[1][1] = ((c0->ybox_fulldet / 2.0) - ((c0->det_yGlassPos) / 2.0));
    c0->module[1][2] = (-(c0->zbox_fulldet / 2.0) + (c0->det_zAlPos / 2.0) + c0->det_dzAlPos);
    c0->module[2][0] = 0;
    c0->module[2][1] = 0;
    c0->module[2][2] = (-(c0->zbox_fulldet / 2.0) + (c0->det_zAlPos / 2.0) + c0->det_dzAlPos);
    c0->module[3][0] = 0;
    c0->module[3][1] = -((c0->ybox_fulldet / 2.0) - ((c0->det_yGlassPos) / 2.0));
    c0->module[3][2] = (-(c0->zbox_fulldet / 2.0) + (c0->det_zAlPos / 2.0) + c0->det_dzAlPos);

    // Second Layer of full Rpc detector covering 2.23 x 4.99 meters with 32 strips
    c0->module[4][0] = 0;
    c0->module[4][1] = ((c0->ybox_fulldet / 2.0) - ((c0->det_yGlassPos)) - ((c0->det_yGlassPos) / 2.0) + c0->extra_y);
    c0->module[4][2] = (-(c0->zbox_fulldet / 2.0) + c0->det_zAlPos + c0->det_dzAlPos * 3.0 + c0->z_space_layers
                        + (c0->det_zAlPos / 2.0));
    c0->module[5][0] = 0;
    c0->module[5][1] = -((c0->ybox_fulldet / 2.0) - ((c0->det_yGlassPos)) - ((c0->det_yGlassPos) / 2.0) + c0->extra_y);
    c0->module[5][2] = (-(c0->zbox_fulldet / 2.0) + c0->det_zAlPos + c0->det_dzAlPos * 3.0 + c0->z_space_layers
                        + (c0->det_zAlPos / 2.0));

    // Third Layer of full Rpc detector covering 2.23 x 4.99 meters with 64 strips
    c0->module[6][0] = 0;
    c0->module[6][1] = ((c0->ybox_fulldet / 2.0) - ((c0->det_yGlassPos) / 2.0));
    c0->module[6][2] = (-(c0->zbox_fulldet / 2.0) + (c0->det_zAlPos * 2.0) + c0->det_dzAlPos * 5.0
                        + (c0->z_space_layers * 2.0) + (c0->det_zAlPos / 2.0));
    c0->module[7][0] = 0;
    c0->module[7][1] = 0;
    c0->module[7][2] = (-(c0->zbox_fulldet / 2.0) + (c0->det_zAlPos * 2.0) + c0->det_dzAlPos * 5.0
                        + (c0->z_space_layers * 3.0) + (c0->det_zAlPos / 2.0));
    c0->module[8][0] = 0;
    c0->module[8][1] = -((c0->ybox_fulldet / 2.0) - ((c0->det_yGlassPos) / 2.0));
    c0->module[8][2] = (-(c0->zbox_fulldet / 2.0) + (c0->det_zAlPos * 2.0) + c0->det_dzAlPos * 5.0
                        + (c0->z_space_layers * 2.0) + (c0->det_zAlPos / 2.0));

    // Fourth Layer of full Rpc detector covering 2.23 x 4.99 meters with 64 strips
    c0->module[9][0] = 0;
    c0->module[9][1] = ((c0->ybox_fulldet / 2.0) - ((c0->det_yGlassPos)) - ((c0->det_yGlassPos) / 2.0) + c0->extra_y);
    c0->module[9][2] = (-(c0->zbox_fulldet / 2.0) + (c0->det_zAlPos * 3.0) + c0->det_dzAlPos * 7.0
                        + (c0->z_space_layers * 3.0) + (c0->det_zAlPos / 2.0));
    c0->module[10][0] = 0;
    c0->module[10][1] = -((c0->ybox_fulldet / 2.0) - ((c0->det_yGlassPos)) - ((c0->det_yGlassPos) / 2.0) + c0->extra_y);
    c0->module[10][2] = (-(c0->zbox_fulldet / 2.0) + (c0->det_zAlPos * 3.0) + c0->det_dzAlPos * 7.0
                         + (c0->z_space_layers * 3.0) + (c0->det_zAlPos / 2.0));

    for (int i = 1; i <= 10; i++) {

        if (XHit1 >= c0->module[i][0] - (c0->det_xGlassPos) / 2.0
            && XHit1 <= c0->module[i][0] + (c0->det_xGlassPos) / 2.0
            && YHit1 >= c0->module[i][1] - (c0->det_yGlassPos) / 2.0
            && YHit1 <= c0->module[i][1] + (c0->det_yGlassPos) / 2.0
            && ZHit1 >= c0->det_zPos + (c0->module[i][2] - (c0->det_zAlPos + 2 * c0->det_dzAlPos) / 2.0)
            && ZHit1 <= c0->det_zPos + (c0->module[i][2] + (c0->det_zAlPos + 2 * c0->det_dzAlPos) / 2.0)) {
    ...
    if (mod >= 6 && mod <= 10 || mod >= 16) {
        path = "/cave_1/Upstream_Tagger_1/UpstreamTagger1_";
        path += mod;
        path += "/glass_upstreamtagger_";
        path += fDetectorID;
        Bool_t rc = nav->cd(path);
    } else {
        path1 = "/cave_1/Upstream_Tagger_1/UpstreamTagger_";
        path1 += mod;
        path1 += "/glass_upstreamtagger_";
        path1 += fDetectorID;
