    ...
    , fStructureId()
{
    ...
    for (i = 1; i < cMaxModuleType; i++) {
        ...
        if (fLightMapNames[i] != "none") {
            ...
            Info("ecal",
                 "Number of modules of type %d is %d (%d channels), lightmap %s",
                 i,
                 fModulesWithType[i],
                 fModulesWithType[i] * i * i,
                 fLightMapNames[i].Data());
        } else
            fLightMaps[i] = NULL;
...
void ecal::FinishPrimary() { fFirstNumber = fLiteCollection->GetEntriesFast(); }
...
Bool_t ecal::ProcessHits(FairVolume* vol)
{
    ...
    // cout <<"Ecal called  "<<volID<<" "<<vol->getVolumeId()<<" "<<fStructureId<<" "<<fELoss*1e10<<"
    // "<<gGeoManager->GetVolume(vol->getVolumeId())->GetName()<<" "<<gMC->CurrentVolName()<<" "<<bz<<endl; if
    // (vol->getVolumeId()==fStructureId) {
    ...
    if (fELoss > 0) {
        ...
        */
        if (fSimpleGeo == 0) {
            gMC->CurrentVolOffID(3, mx);
            mx--;
            gMC->CurrentVolOffID(4, my);
            my--;
            gMC->CurrentVolOffID(2, cell);
            cell--;
        } else {
            gMC->CurrentVolOffID(2, mx);
            mx--;
            gMC->CurrentVolOffID(3, my);
            my--;
        ...
        if (id < 0) {
            if (Ecal.CompareTo(gMC->CurrentVolName()) == 0 && fELoss < 1e-19)
                return kTRUE;
            cout << "neg id " << mx << " " << my << " " << cell << " " << gMC->CurrentVolName() << " "
                 << gMC->CurrentVolOffName(1) << " " << gMC->CurrentVolOffName(2) << " " << fELoss << " "
                 << fELoss * 1e10 << endl;
        }
        /*
...
void ecal::FillWallPoint()
{
    ...
    Double_t ekin =
        TMath::Sqrt(fMom.Px() * fMom.Px() + fMom.Py() * fMom.Py() + fMom.Pz() * fMom.Pz() + mass * mass) - mass;
    fELoss = ekin;
...
Bool_t ecal::FillLitePoint(Int_t volnum)
{
    ...
    while (part->GetFirstMother() >= 0 && part->Vz() >= zmin && part->Vz() <= zmax && TMath::Abs(part->Vx()) <= xecal
           && TMath::Abs(part->Vy()) <= yecal) {
        fTrackID = part->GetFirstMother();
        part = ((ShipStack*)gMC->GetStack())->GetParticle(fTrackID);
        //      cout << "-> " << part->GetFirstMother() << " : " << part->Vx() << ", " << part->Vy() << ", " <<
        //      part->Vz() << endl;
    }
//  if (part->Vz()>500)
...
{
    if (iColl == 0)
        return fEcalCollection;
    if (iColl == 1)
        return fLiteCollection;
    else
        return NULL;
}
// -------------------------------------------------------------------------
...
void ecal::CopyClones(TClonesArray* cl1, TClonesArray* cl2, Int_t offset)
{
    ...
    if (cl1->GetClass() == ecalPoint::Class()) {
        ...
        cout << "-I- ecal: " << cl2->GetEntriesFast() << " merged entries." << endl;
...
TGeoVolume* ecal::ConstructRaw(Int_t num)
{
    ...
    for (i = 0; i < fXSize; i++) {
        ...
        if (fInf->GetType(i, num) == 0)
            continue;
        md = "EcalModule";
        md += (Int_t)fInf->GetType(i, num);
...
ecalPoint* ecal::AddHit(Int_t trackID,
                        Int_t detID,
                        TVector3 pos,
                        TVector3 mom,
                        Double_t time,
                        ...
                        Int_t pdgcode)
{
    ...
    return new (clref[size]) ecalPoint(trackID, detID, pos, mom, time, length, eLoss, pdgcode);
}
// -------------------------------------------------------------------------
...
{
    if (fModules[type] != NULL)
        return;
    ConstructCell(type);

    TString nm = "EcalModule";
    nm += type;
    TString nm1;
    TString cellname = "EcalCell";
    cellname += type;
    Int_t i;
    Int_t j;
    Int_t n;
...
void ecal::ConstructCell(Int_t type)
{
    ...
    for (i = 0; i < fNLayers; i++) {
        gGeoManager->Node(scin.Data(),
                          i + 1,
                          nm.Data(),
                          0.0,
                          0.0,
                          -thickness * fNLayers / 2.0 + fThicknessScin / 2.0 + i * thickness,
                          0,
                          kTRUE,
                          buf,
                          0);
        gGeoManager->Node(lead.Data(),
                          i + 1,
                          nm.Data(),
                          0.0,
                          0.0,
                          -thickness * fNLayers / 2.0 + fThicknessScin + i * thickness + fThicknessTyvk
                              + fThicknessLead / 2.0,
                          0,
                          kTRUE,
                          buf,
        ...
        if (fThicknessTyvk > 0.0) {
            gGeoManager->Node(tyvek.Data(),
                              2 * i + 1,
                              nm.Data(),
                              0.0,
                              0.0,
                              -thickness * fNLayers / 2.0 + fThicknessScin + i * thickness + 1.5 * fThicknessTyvk
                                  + fThicknessLead,
                              0,
                              kTRUE,
                              buf,
            ...
            gGeoManager->Node(tyvek.Data(),
                              ...
                              0.0,
                              0.0,
                              -thickness * fNLayers / 2.0 + fThicknessScin + i * thickness + 0.5 * fThicknessTyvk,
                              0,
                              kTRUE,
                              buf,
                              0);
...
void ecal::ConstructTile(Int_t type, Int_t material)
{
    switch (material) {
        case 0:
            if (fScTiles[type] != NULL)
                return;
            break;
        case 1:
            if (fPbTiles[type] != NULL)
                return;
            break;
        case 2:
            if (fTvTiles[type] != NULL)
                return;
            break;
        default:
            Error("ConstructTile", "Can't construct a tile of type %d.", material);
    }
    Double_t thickness;
    TGeoVolume* hole;
    TGeoVolume* fiber;
    ...
    switch (material) {
        case 0:
            thickness = fThicknessScin / 2.0;
            break;
        case 1:
            thickness = fThicknessLead / 2.0;
            break;
        case 2:
            thickness = fThicknessTyvk / 2.0;
            break;
        default:
            Error("ConstructTile", "Can't construct a tile of type %d.", material);
    }
...
void ecal::ConstructTileSimple(Int_t type, Int_t material)
{
    switch (material) {
        ...
        case 2:
            ...
            break;
        default:
            Error("ConstructTileSimple", "Can't construct a tile of type %d.", material);
    }
    Double_t thickness;
    TGeoVolume* hole;
    TGeoVolume* fiber;
