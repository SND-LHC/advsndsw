...
void ecalDigi::InitPython(ecalStructure* structure) { fStr = structure; }
...
void ecalDigi::Exec(Option_t* option)
{
    ...
    for (; p != cells.end(); ++p) {
        ...
        else if (fChannelMap.find(cell->GetCellNumber()) == fChannelMap.end()) {
            Error("Exec", "Channel %d not found in map. Using default value!", cell->GetCellNumber());
            // TODO: Should we insert Fatal here?
            adc = (Short_t)(gRandom->Gaus(cell->GetEnergy(), fADCNoise) / fADCChannel + fPedestal);
        } else {
            adc =
                (Short_t)(gRandom->Gaus(cell->GetEnergy(), fADCNoise) / fChannelMap[cell->GetCellNumber()] + fPedestal);
        }
        if (adc > fADCMax)
            adc = fADCMax;
        cell->SetEnergy(-1111);
