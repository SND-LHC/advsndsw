...
void ecalReco::Exec(Option_t* option)
{
    ...
    for (i = 0; i < nc; i++) {
        ...
        if (cls->Maxs() == 1) {
            ...
            reco = new ((*fReconstucted)[fN++])
                ecalReconstructed(cls->Energy(), cls->PreCalibrated(), x, y, cls->PeakNum(0), i);
            cls->SetStatus(1);
            continue;
        }
        ...
        /*
            ...
            for(j=0;j<n;j++)
            {
              ...
              reco=new ((*fReconstucted)[fN++]) ecalReconstructed(rawE, fCalib->Calibrate(maxs[j]->GetType(), rawE), x,
           y, cls->PeakNum(j), i);
            }
            cls->SetStatus(1);
        */
    ...
    if (fVerbose > 0)
        Info("Exec", "Event %d. Good %d. Bad %d cls, %d maxs.", fEv, fN, fRejected, fRejectedP);
}
...
void ecalReco::TryReconstruct(ecalCluster* cls, Int_t clsnum)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        for (p = cells.begin(); p != cells.end(); ++p) {
            ...
            for (k = 0; k < i - 1; k++) {
                ...
                tcell->GetNeighborsList(cells2);
                // Have an intersection between 3x3 areas near maximum
                if (find(cells2.begin(), cells2.end(), *p) != cells2.end())
                    isgood[i]--;
            }
            // form 5x5-3x3
            (*p)->GetNeighborsList(cells2);
    ...
    for (i = 0; i < n; i++) {
        ...
        reco = new ((*fReconstucted)[fN++])
            ecalReconstructed(rawE, fCalib->Calibrate(maxs[i]->GetType(), rawE), x, y, cls->PeakNum(i), clsnum);
    }
    cls->SetStatus(1);
    if (rejected > 0) {
