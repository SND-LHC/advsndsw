...
Bool_t simpleTarget::ProcessHits(FairVolume* vol)
{
    ...
    if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
        ...
        if (fOnlyMuons) {
            ...
            new (clref[0]) vetoPoint(
                0, 0, TVector3(0, 0, 0), TVector3(0, 0, 0), 0, 0, fTotalEloss, 0, TVector3(0, 0, 0), TVector3(0, 0, 0));
            return kTRUE;
        }
...
void simpleTarget::Register()
{
    ...
    FairRootManager::Instance()->Register("vetoPoint", "veto", fsimpleTargetPointCollection, kTRUE);
}

TClonesArray* simpleTarget::GetCollection(Int_t iColl) const
{
    if (iColl == 0) {
        return fsimpleTargetPointCollection;
    } else {
        return NULL;
    }
}
void simpleTarget::Reset() { fsimpleTargetPointCollection->Clear(); }
