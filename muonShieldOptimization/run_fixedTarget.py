...
def get_work_dir(run_number, tag=None):
    ...
    if tag:
        out_dir = "{host}_{base}_{runnr}_{comment}".format(
            host=host, base=job_base_name, runnr=run_number, comment=tag
        )
    else:
        out_dir = "{host}_{base}_{runnr}".format(
            host=host, base=job_base_name, runnr=run_number
        )
    return out_dir
...
def init():
    ...
    ap.add_argument(
        "-f", "--force", action="store_true", help="force overwriting output directory"
    )
    ap.add_argument("-r", "--run-number", type=int, dest="runnr", default=runnr)
    ap.add_argument(
        "-e", "--ecut", type=float, help="energy cut", dest="ecut", default=ecut
    )
    ap.add_argument(
        "-n",
        "--num-events",
    ...
    ap.add_argument(
        ...
        dest="G4only",
        default=False,
        help="use Geant4 directly, no Pythia8",
    )
    ap.add_argument(
        "-P",
        "--PythiaDecay",
    ...
    ap.add_argument(
        ...
        dest="withEvtGen",
        default=withEvtGen,
        help="use EvtGen for decays",
    )
    ap.add_argument(
        "-t", "--tau-only", action="store_true", dest="tauOnly", default=False
    )
    ap.add_argument(
        "-J", "--Jpsi-mainly", action="store_true", dest="JpsiMainly", default=False
    )
    ap.add_argument(
        "-b",
        "--boostDiMuon",
        ...
        dest="boostDiMuon",
        default=boostDiMuon,
        help="boost Di-muon branching ratios",
    )
    ap.add_argument(
        "-X",
        "--boostFactor",
        ...
        dest="boostFactor",
        default=boostFactor,
        help="boost Di-muon prod cross sections",
    )
    ap.add_argument(
        "-C",
        "--charm",
    ...
    ap.add_argument(
        ...
        dest="chicc",
        default=chicc,
        help="ccbar over mbias cross section",
    )
    ap.add_argument(
        "-bb",
        "--chibb",
        ...
        dest="chibb",
        default=chibb,
        help="bbbar over mbias cross section",
    )
    ap.add_argument(
        "-p",
        "--pot",
        ...
        dest="npot",
        default=npot,
        help="number of protons on target per spill to normalize on",
    )
    ap.add_argument(
        "-S",
        "--nStart",
        type=int,
        help="first event of input file to start",
        dest="nStart",
        default=nStart,
    )
    ap.add_argument(
    ...
    ap.add_argument(
        ...
        "--seed",
        type=int,
        help="random seed; default value is 0, see TRrandom::SetSeed documentation",
        dest="seed",
        default=0,
    )
    args = ap.parse_args()
    ...
    if args.work_dir is None:
        if charm:
            args.work_dir = get_work_dir(runnr, "charm")
        if beauty:
            args.work_dir = get_work_dir(runnr, "beauty")
        else:
...
if skipNeutrinos:
    sensPlane.SkipNeutrinos()
if FourDP:
    sensPlane.SetOpt4DP()  # in case a ntuple should be filled with pi0,etas,omega
...
if boostFactor > 1:
    ...
    procGMuPair = gProcessTable.FindProcess(
...
if withEvtGen:
    conditions += " V"
if boostDiMuon > 1:
    conditions += " diMu" + str(boostDiMuon)
if boostFactor > 1:
    conditions += " X" + str(boostFactor)
