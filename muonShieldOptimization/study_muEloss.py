...
def makePlot(f, book=True):
    ...
    if book:
        ...
        ut.bookHist(
            h,
            "eloss",
            "rel energy loss as function of momentum GeV/c",
            100,
            0,
            maxTheta,
            10000,
        ...
        ut.bookHist(
            h,
            "elossRaw",
            "energy loss as function of momentum GeV/c",
            100,
            0,
            maxTheta,
            10000,
    ...
    if s == "NA62":
        ...
        for E in [15.0, 20.0, 30.0, 50.0, 80.0]:
            n = h[">eloss"].FindBin(E / 350.0)
...
def NA62():
    ...
    for l in range(N):
        ...
        h["NA62"].SetPointError(l, 0, abs(y1 - y2) / 2.0 * 1e-6)
...
def makeSummaryPlot():
    ...
    NA62()
    for t in [93, 95]:
        h[t] = h["eloss"].ProjectionX(
            str(t), int(h["eloss"].GetNbinsY() * t / 100.0), h["eloss"].GetNbinsY()
        )
        h[t].Sumw2()
        h[t].SetStats(0)
