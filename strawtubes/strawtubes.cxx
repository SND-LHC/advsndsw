...
Bool_t strawtubes::ProcessHits(FairVolume *vol)
{
    ...
    if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
        ...
        if (dist2Wire > fInner_Straw_diameter / 2) {
            std::cout << "addhit " << dist2Wire << " straw id " << straw_uniqueId << " pdgcode " << pdgCode
...
void strawtubes::EndOfEvent() { fstrawtubesPointCollection->Clear(); }
...
void strawtubes::Register()
{
    ...
    */
    ...
    FairRootManager::Instance()->Register("strawtubesPoint", "strawtubes", fstrawtubesPointCollection, kTRUE);
}

TClonesArray *strawtubes::GetCollection(Int_t iColl) const
{
    if (iColl == 0) {
        return fstrawtubesPointCollection;
    } else {
        return NULL;
    }
}

void strawtubes::Reset() { fstrawtubesPointCollection->Clear(); }
...
void strawtubes::ConstructGeometry()
{
    ...
    TGeoBBox *detbox1 = new TGeoBBox(
        "detbox1", fStraw_length + fFrame_lateral_width, ftr34ydim + fFrame_lateral_width, fDeltaz_frame / 2.);
    TGeoBBox *detbox2 = new TGeoBBox("detbox2", fStraw_length + eps, ftr34ydim + eps, fDeltaz_frame / 2. + eps);

    TGeoBBox *detbox1_12 = new TGeoBBox(
        "detbox1_12", fStraw_length_12 + fFrame_lateral_width, ftr12ydim + fFrame_lateral_width, fDeltaz_frame / 2.);
    TGeoBBox *detbox2_12 =
        new TGeoBBox("detbox2_12", fStraw_length_12 + eps, ftr12ydim + eps, fDeltaz_frame / 2. + eps);
    ...
    if (fStraw_length_veto > 1) {
        TGeoBBox *detbox1_veto =
            new TGeoBBox("detbox1_veto", fStraw_length_veto + 1., fvetoydim + 1., fDeltaz_view / 2.);
        TGeoBBox *detbox2_veto =
            new TGeoBBox("detbox2_veto", fStraw_length_veto + eps, fvetoydim + eps, fDeltaz_view / 2. + eps);
    ...
    if (fStraw_length_veto > 1) {
        ...
        for (Int_t vnb = 0; vnb < 2; vnb++) {
            ...
            for (Int_t pnb = 0; pnb < 2; pnb++) {
                // plane loop
                TString nmplane_veto = nmveto + "_plane_";
                nmplane_veto += pnb;
                // width of the planes: z distance between layers + outer straw diameter
                TGeoBBox *plane_veto = new TGeoBBox(
                    "plane box", fStraw_length_veto + eps / 2., fvetoydim + eps / 2., planewidth / 2. + 3. * eps / 2.);
                ...
                for (Int_t lnb = 0; lnb < 2; lnb++) {
                    TString nmlayer_veto = nmplane_veto + "_layer_";
                    nmlayer_veto += lnb;
                    // width of the layer: (plane width-2eps)/2
                    layer_veto = new TGeoBBox("layer box_veto",
                                              fStraw_length_veto + eps / 4.,
                                              fvetoydim + eps / 4.,
                                              layerwidth / 2. + eps / 4.);
                    ...
                    planebox_veto->AddNode(layerbox_veto,
                    ...
                    for (Int_t snb = 1; snb < fStraws_per_layer_veto; snb++) {
                        // straw loop
                        t6v.SetTranslation(
                            0, fvetoydim - fStraw_pitch * snb - fOffset_plane12 * pnb + lnb * fOffset_layer12, 0);
    ...
    Double_t x_prime =
        ...
        + (ftr34ydim + fFrame_lateral_width + 2 * eps) * TMath::Sin(fView_angle * TMath::Pi() / 180.0);
    Double_t y_prime =
        (fVacBox_x + 0.6 * fFrame_lateral_width + 2 * eps) * TMath::Sin(fView_angle * TMath::Pi() / 180.0)
        + (ftr34ydim + fFrame_lateral_width + 2 * eps) * TMath::Cos(fView_angle * TMath::Pi() / 180.0);
    Double_t x_prime_12 =
        (fStraw_length_12 + fFrame_lateral_width + 2 * eps) * TMath::Cos(fView_angle * TMath::Pi() / 180.0)
        + (ftr12ydim + fFrame_lateral_width + 2 * eps) * TMath::Sin(fView_angle * TMath::Pi() / 180.0);
    Double_t y_prime_12 =
        (fStraw_length_12 + fFrame_lateral_width + 2 * eps) * TMath::Sin(fView_angle * TMath::Pi() / 180.0)
        + (ftr12ydim + fFrame_lateral_width + 2 * eps) * TMath::Cos(fView_angle * TMath::Pi() / 180.0);
            ...
            for (Int_t vnb = 0; vnb < 4; vnb++) {
                ...
                for (Int_t pnb = 0; pnb < 2; pnb++) {
                    ...
                    TGeoBBox *plane_12 = new TGeoBBox("plane box_12",
                                                      fStraw_length_12 + eps / 2,
                                                      ftr12ydim + eps / 2,
                                                      planewidth / 2. + 3. * eps / 2);
                    ...
                    for (Int_t lnb = 0; lnb < 2; lnb++) {
                        ...
                        TGeoBBox *layer_12 = new TGeoBBox(
                            "layer box_12", fStraw_length_12 + eps / 4, ftr12ydim + eps / 4, layerwidth / 2. + eps / 4);
                        ...
                        for (Int_t snb = 1; snb < fStraws_per_layer_tr12; snb++) {
                            // straw loop
                            t6s.SetTranslation(
                                0, ftr12ydim - fStraw_pitch * snb - fOffset_plane12 * pnb + lnb * fOffset_layer12, 0);
                            r6s.SetAngles(90, 90, 0);
                            TGeoCombiTrans c6s(t6s, r6s);
                            TGeoHMatrix *h6s = new TGeoHMatrix(c6s);
                            layerbox_12->AddNode(straw_12,
        ...
        if ((statnb == 3) || (statnb == 4)) {
            for (Int_t vnb = 0; vnb < 4; vnb++) {
                ...
                for (Int_t pnb = 0; pnb < 2; pnb++) {
                    ...
                    nmplane += pnb;
                    TGeoBBox *plane = new TGeoBBox(
                        "plane box", fStraw_length + eps / 2, ftr34ydim + eps / 2, planewidth / 2. + 3. * eps / 2);
                    ...
                    for (Int_t lnb = 0; lnb < 2; lnb++) {
                        ...
                        // z translate the layerbox wrt the plane box (which is already rotated)
                        TString nmlayer = nmplane + "_layer_";
                        nmlayer += lnb;
                        TGeoBBox *layer = new TGeoBBox(
                            "layer box", fStraw_length + eps / 4, ftr34ydim + eps / 4, layerwidth / 2. + eps / 4);
                        TGeoVolume *layerbox = new TGeoVolume(nmlayer, layer, med);
                        planebox->AddNode(layerbox,
                                          statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000,
                                          new TGeoTranslation(0, 0, (lnb - 1. / 2.) * fDeltaz_layer12));
                        ...
                        for (Int_t snb = 1; snb < fStraws_per_layer_tr34; snb++) {
                            // straw loop
                            t6s.SetTranslation(
                                0, ftr34ydim - fStraw_pitch * snb - fOffset_plane12 * pnb + lnb * fOffset_layer12, 0);
                            r6s.SetAngles(90, 90, 0);
                            TGeoCombiTrans c6s(t6s, r6s);
                            TGeoHMatrix *h6s = new TGeoHMatrix(c6s);
                            layerbox->AddNode(straw,
                                              statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 1000
                                                  + snb,
                                              h6s);
                            layerbox->AddNode(
...
// method to get end points by emulating the geometry
{
    ...
    // cout << "DetID" << detID << " statnb "<<statnb<<" vnb " << vnb << " pnb " << pnb <<" lnb "<< lnb << " snb " <<
    // snb << endl;
    ...
    Double_t zpos;
    if (statnb < 5) {
        zpos = TStationz + (vnb - 3. / 2.) * fDeltaz_view + (pnb - 1. / 2.) * fDeltaz_plane12
               + (lnb - 1. / 2.) * fDeltaz_layer12;
    } else {
        zpos = TStationz + (vnb - 1. / 2.) * fDeltaz_view + (pnb - 1. / 2.) * fDeltaz_plane12
               + (lnb - 1. / 2.) * fDeltaz_layer12;
    }
    top = TVector3(xtop, ytop, zpos);
    bot = TVector3(xbot, ybot, zpos);
    // cout << "dets="<< xtop << " "<< xbot << " "<<  ytop << " "<< ybot<< " "<< ypos<< " "<< fStraw_length<< "
    // "<<detID<<endl; cout << "top/bot="<< snb << " "<< vnb << " "<<  pnb << " "<< lnb << " "<< ypos<< " "<<
    // fOffset_layer12<< " "<<fOffset_plane12<<endl;
}
                                    ...
                                    Double_t dist2Wire)
{
    ...
    return new (clref[size]) strawtubesPoint(trackID, detID, pos, mom, time, length, eLoss, pdgCode, dist2Wire);
}
