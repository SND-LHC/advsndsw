...
void strawtubes::ConstructGeometry()
{
    ...
    for (Int_t vnb = 0; vnb < 2; vnb++) {
        ...
        for (Int_t pnb = 0; pnb < 2; pnb++) {
            ...
            viewframe->AddNode(planebox,
                               4 * 10000000 + vnb * 1000000 + pnb * 100000,
                               new TGeoTranslation(0, 0, (pnb - 1. / 2.) * fDeltaz_plane12));
            ...
            /*     TGeoRotation r5l;
                 ...
                 for (Int_t lnb=0; lnb<2; lnb++) {
                   ...
                   planebox->AddNode(layerbox, 4*10000000+vnb*1000000+pnb*100000+lnb*10000,new
               TGeoTranslation(0,0,(lnb-1./2.)*fDeltaz_layer12));
                   //layerbox->SetVisibility(kFALSE);
                   //layerbox->VisibleDaughters(kTRUE);
                   //std::cout <<nmlayer<<"veto zpos
               "<<TStationz-(1./2.-vnb)*(fDeltaz_view)+(pnb-1./2.)*fDeltaz_plane12+(lnb-1./2.)*fDeltaz_layer12<<std::endl;
                   //layer loop
                   TGeoRotation r5s;
                   TGeoTranslation t5s;
                   for (Int_t snb=0; snb<fStraws_per_layer; snb++) {
                     //straw loop
                     t5s.SetTranslation(fStraw_length-fStraw_pitch*snb-8*eps+fOffset_plane12*pnb+lnb*fOffset_layer12,
               0,0);
                     //straws are tubes defined along the z-axis, so need to rotate them first by 90 degrees around the
               x-axis to get them vertical
    ...
    for (Int_t statnb = 0; statnb < 4; statnb++) {
        ...
        for (Int_t vnb = 0; vnb < 4; vnb++) {
            ...
            for (Int_t pnb = 0; pnb < 2; pnb++) {
                ...
                // hence z translate the plane wrt to the view
                viewframe->AddNode(planebox,
                                   statnb * 10000000 + vnb * 1000000 + pnb * 100000,
                                   new TGeoTranslation(0, 0, (pnb - 1. / 2.) * fDeltaz_plane12));
                // planebox->SetVisibility(kFALSE);
                // planebox->VisibleDaughters(kTRUE);
                TGeoRotation r5l;
                ...
                for (Int_t lnb = 0; lnb < 2; lnb++) {
                    ...
                    // width of the layer: (plane width-2eps)/2
                    TGeoBBox *layer =
                        new TGeoBBox("layer box", fStraw_length - 2 * eps, fStraw_length - 2 * eps, layerwidth / 2.);
                    TGeoVolume *layerbox = new TGeoVolume(nmlayer, layer, med);
                    // z translate the layerbox wrt the plane box (which is already rotated)
                    ...
                    for (Int_t snb = 0; snb < fStraws_per_layer; snb++) {
                        // straw loop
                        t6s.SetTranslation(fStraw_length - fStraw_pitch * snb - 8 * eps + fOffset_plane12 * pnb
                                               + lnb * fOffset_layer12,
                                           0,
                                           0);
                        // straws are tubes defined along the z-axis, so need to rotate them first by 90 degrees around
                        // the x-axis to get them vertical then x translate the straws according to their plane, layer
                        // and number
                        ...
                        layerbox->AddNode(
                            straw, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 1000 + snb, h6s);
                        layerbox->AddNode(
                                    ...
                                    Double_t eLoss)
{
    ...
    return new (clref[size]) strawtubesPoint(trackID, detID, pos, mom, time, length, eLoss);
}
