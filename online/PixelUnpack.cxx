...
PixelUnpack::PixelUnpack(uint16_t PartitionId)
    : fRawData(new TClonesArray("ShipPixelHit"))
    , fNHits(0)
    , fNHitsTotal(0)
...
Bool_t PixelUnpack::DoUnpack(Int_t *data, Int_t size)
{
    ...
    switch (df->header.frameTime) {
        case SoS:
            LOG(INFO) << "PixelUnpacker: SoS frame.";
            return kTRUE;
        case EoS:
            LOG(INFO) << "PixelUnpacker: EoS frame.";
            return kTRUE;
        default:
            break;
    }
    ...
    for (auto &&hit : hits) {
        ...
        auto detectorID = (df->header.partitionId % 0x0800) * 10000000 + 1000000 * hitData->moduleID
                          + 1000 * channelId->row + channelId->column;
