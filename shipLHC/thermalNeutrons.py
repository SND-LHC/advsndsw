...
parser.add_argument(
    "-n", "--nEvents", dest="nEvents", type=int, help="number of events", default=100
)
parser.add_argument(
...
parser.add_argument(
    "--Estart",
    dest="Estart",
    help="start of energy range of particle gun (default= 0 MeV)",
    required=False,
    default=1e-20,
    type=float,
)
parser.add_argument(
    "--Eend",
    dest="Eend",
    help="end of energy range of particle gun  (default=10 MeV)",
    required=False,
    default=0.01,
    type=float,
)
parser.add_argument(
    "--pID",
    dest="pID",
    help="id of particle used by the gun (default=2112)",
    required=False,
    default=2112,
    type=int,
)
parser.add_argument(
    "--setup",
    dest="neutron",
    help="setup for absorptionLength or neutron in cold box)",
    required=False,
    action="store_true",
)
...
if options.neutron:
    outFile = "thermNeutron_%s_%s_coldbox_%s-%iM.root" % (
        options.targetMaterial,
        str(options.targetLength),
        str(options.run),
        options.nEvents / 1000000.0,
    )
...
target.SetTarget(
    options.targetMaterial, options.targetLength * u.cm, not options.neutron
)
...
def count(hFile=outFile):
    f = ROOT.TFile(hFile)
    ut.bookHist(h, "E", ";log10(Ekin [MeV])", 1000, -10.0, 4.0)
    ut.bookHist(h, "Epassed", ";log10(Ekin [MeV])", 1000, -10.0, 4.0)
    ut.bookHist(
        h, "Lab", ";absorption Length vs logE", 1000, -10.0, 4.0, 500, -0.1, 10.0
    )
    ut.bookHist(
        h, "Labz", ";absorption Length vs logE", 1000, -10.0, 4.0, 500, -0.1, 10.0
    )
...
rate.SetParameter(0, 1.0)
rate.SetParameter(1, 130.0)
...
def debugging():
    ...
    for da in range(world.GetNoDaughters()):
        ...
        mat = lvl.GetMaterial()
        print(
            "%2i  %s  %5.3F g/cm3  %5.2F kg  %s"
            % (
                da,
                vl.GetName(),
                mat.GetDensity() / G4Unit.g * G4Unit.cm3,
                lvl.GetMass() / G4Unit.kg,
                mat.GetName(),
            )
        )
        for n in range(mat.GetNumberOfElements()):
            ...
            print(
                ...
                % (
                    n,
                    element.GetName(),
                    element.GetAtomicMassAmu(),
                    mat.GetFractionVector()[n],
                )
            )
