...
parser.add_argument(
    "-r", "--runNumber", dest="runNumber", help="run number", type=int, required=False
)
parser.add_argument(
    "-p",
    "--path",
...
parser.add_argument(
    ...
    required=False,
    default="/eos/experiment/sndlhc/raw_data/physics/2022/",
)
parser.add_argument(
    "-f",
    "--inputFile",
    dest="inputFile",
    help="input file data and MC",
    default="",
    required=False,
)
parser.add_argument(
...
detSize[0] = [
    ...
    ROOT.advsnd.sensor_width / ROOT.advsnd.strips,
    geo.snd_geo.AdvTarget.TTZ,
]  # check the channel sizes
detSize[1] = detSize[0]
...
if f.FindKey("cbmsim"):
    eventTree = f.cbmsim
    runId = "sim"
    if eventTree.GetBranch("AdvTargetPoint"):
        mc = True
else:
    eventTree = f.rawConv
...
try:
    ...
    fg.Close()
    if runNumber in FSdict:
        fsdict = FSdict[runNumber]
    else:
        fsdict = False
except:
...
def goodEvent(event):
    ...
    return True
    stations = {"Scifi": {}, "Mufi": {}}
    if event.Digi_AdvTargetHits.GetEntries() > 25:
        return False
    ...
    if len(stations["Scifi"]) > 4 and len(stations["Mufi"]) > 6:
        return True
    else:
        False
    if onlyScifi and len(stations["Scifi"]) > Nlimit:
        return True
    elif not onlyScifi and totalN > Nlimit:
        return True
    else:
        return False
...
def drawLegend(max_density, max_QDC, n_legend_points):
    ...
    text_target_legend.SetTextSize(0.15)
    for i in range(n_legend_points):
        if i < (n_legend_points - 1):
            text_target_legend.DrawLatex(
                (i + 0.3) * (1.0 / (n_legend_points + 2)),
                0.2,
                "{:d}".format(int(i * max_density / (n_legend_points - 1))),
            )
            ...
            text_target_legend.DrawLatex(
                (i + 0.3) * (1.0 / (n_legend_points + 2)),
                0.2,
                "{:d} hits/cm".format(int(i * max_density / (n_legend_points - 1))),
            )
            # text_target_legend.DrawLatex((i+0.3)*(1./(n_legend_points+2)), 0.,  "{:.0f} QDC units".format(int(i*max_QDC/(n_legend_points-1))))

        h["markerCollection"].append(
            ROOT.TEllipse(
                (i + 0.15) * (1.0 / (n_legend_points + 2)), 0.26, 0.05 / 4, 0.05
            )
        )
        h["markerCollection"][-1].SetFillColor(
            ROOT.TColor.GetPalette()[
                int(
                    float(i * max_density / (n_legend_points - 1))
                    / max_density
                    * (len(ROOT.TColor.GetPalette()) - 1)
                )
            ]
        )
        h["markerCollection"][-1].Draw("SAME")
...
def loopEvents(
    ...
    h["simpleDisplay"].cd(1)
    zStart = -300.0  # AdvSND MC
    if Setup == "H6":
        zStart = 60.0
    if Setup == "TP":
        zStart = -50.0  # old coordinate system with origin in middle of target
    if "xz" in h:
        h.pop("xz").Delete()
        h.pop("yz").Delete()
    else:
        h["xmin"], h["xmax"] = -100.0, 100.0  # AdvSND MC
        h["ymin"], h["ymax"] = -100.0, 100.0  # AdvSND MC
        h["zmin"], h["zmax"] = zStart, zStart + 500.0
    ...
    for N in range(s, e):
        ...
        if withHoughTrack > 0:
            ...
            if nHoughtracks > 0:
                print("number of tracks by HT:", nHoughtracks)
            if nHoughtracks < nTracks:
                continue
        ...
        digis = []
        if event.FindBranch("Digi_AdvTargetHits"):
            digis.append(event.Digi_AdvTargetHits)
        if event.FindBranch("Digi_AdvMuFilterHits"):
            digis.append(event.Digi_AdvMuFilterHits)
        empty = True
        for x in digis:
            if x.GetEntries() > 0:
                if empty:
        ...
        if empty:
            continue
        h["hitCollectionX"] = {
            "AdvTarget": [0, ROOT.TGraphErrors()],
            "AdvMuFilter": [0, ROOT.TGraphErrors()],
        }
        h["hitCollectionY"] = {
            "AdvTarget": [0, ROOT.TGraphErrors()],
            "AdvMuFilter": [0, ROOT.TGraphErrors()],
        ...
        if hitColour == "q":
            for orientation in ["X", "Y"]:
                ...
                density_AdvTarget = np.clip(
                    0,
                    max_density,
                    getAdvHitDensity(h["hitCollection" + orientation]["AdvTarget"][1]),
                )
                density_AdvMuFilter = np.clip(
                    0,
                    max_density,
                    getAdvHitDensity(
                        h["hitCollection" + orientation]["AdvMuFilter"][1]
                    ),
                )
                for det_name in ["AdvTarget", "AdvMuFilter"]:
                    if det_name == "AdvTarget":
                        density = density_AdvTarget
                    else:
                        density = density_AdvMuFilter
                    for i in range(
                        h["hitCollection" + orientation][det_name][1].GetN()
                    ):
                        h["hitColour" + orientation][det_name].append(
                            ROOT.TColor.GetPalette()[
                                int(
                                    float(density[i])
                                    / max_density
        ...
        if type(start) == type(1):
            rc = event.GetEvent(N - 1)
        else:
            rc = event.GetEvent(start[N] - 1)
        ...
        if option == "2tracks":
            ...
            if not rc:
                rc = twoTrackEvent(sMin=10, dClMin=7, minDistance=0.5, sepDistance=0.75)
            if not rc:
                rc = twoTrackEvent(sMin=10, dClMin=7, minDistance=0.5, sepDistance=1.0)
            if not rc:
                rc = twoTrackEvent(sMin=10, dClMin=7, minDistance=0.5, sepDistance=1.75)
            if not rc:
                rc = twoTrackEvent(sMin=10, dClMin=7, minDistance=0.5, sepDistance=2.5)
            if not rc:
                rc = twoTrackEvent(sMin=10, dClMin=7, minDistance=0.5, sepDistance=3.0)

        if verbose > 0:
            dumpChannels()
        if save:
            h["simpleDisplay"].Print("{:0>2d}-event_{:04d}".format(runId, N) + ".png")
        if auto:
            h["simpleDisplay"].Print(
                options.storePic
                + str(runId)
                + "-event_"
                + str(event.EventHeader.GetEventNumber())
                + ".png"
            )
        if not auto:
...
def addTrack(OT, scifi=False):
    ...
    for aTrack in OT.Reco_MuonTracks:
        ...
        if aTrack.GetUniqueID() == 11:
            trackColor = ROOT.kAzure - 2  # HT AdvTarget track
        # HT cross-system track fit
        if aTrack.GetUniqueID() == 15:
            trackColor = ROOT.kOrange + 7
...
def drawDetectors():
    ...
    if geoVer != 2:
        print(
            "This is an older version of the geometry and it is likely incompatible with this version of the drawDetectors function of the 2dED."
        )
    ...
    for i in range(geo.snd_geo.AdvMuFilter.Nplanes):
        ...
        if i > geo.snd_geo.AdvMuFilter.Nplanes - 2:
            continue
        for j in range(geo.snd_geo.AdvMuFilter.Nplanes - 2):
            if geoVer == 1:
                nodes[
                    "volAdvMuFilter_0/volMuonSysDet_{}_{}/volBar_{}".format(
                        i, i, i * 100 + j
                    )
                ] = ROOT.kGray
            elif geoVer == 2:
    ...
    if geoVer != 2:
        ...
        nodes["volAdvMuFilter_0/volDownstreamMagnet_0/volDownVertCoil1_0"] = (
            ROOT.kOrange + 1
        )
        nodes["volAdvMuFilter_0/volDownstreamMagnet_0/volDownVertCoil2_0"] = (
            ROOT.kOrange + 1
        )
        nodes["volAdvMuFilter_0/volDownstreamMagnet_0/volIronYoke_0"] = ROOT.kGreen - 2
        nodes["volAdvMuFilter_0/volDownstreamMagnet_0/volDownCoil_20000"] = (
            ROOT.kOrange + 1
        )
    ...
    for node_ in nodes:
        ...
        for p in proj:
            if node in h and any(passNode in node for passNode in passNodes):
                ...
                if (
                    N.GetVolume().GetName().find("Coil") > 0
                ...
                if (
                    node
                    == "/cave_1/Detector_0/volAdvMuFilter_0/volDownstreamMagnet_0/volIronCore_0"
                ):
                    ...
                    node
                    == "/cave_1/Detector_0/volAdvMuFilter_0/volDownstreamMagnet_0/volDownCoil_20000"
                    and p == "Y"
                ):
                    X.SetPoint(
                        0, M["LeftBottom"][2], IC_lines[p]["LeftBottom"][c] - 7.0
                    )
                    X.SetPoint(1, M["LeftTop"][2], IC_lines[p]["LeftTop"][c] + 7.0)
                    X.SetPoint(2, M["RightTop"][2], IC_lines[p]["RightTop"][c] + 7.0)
                    X.SetPoint(
                        3, M["RightBottom"][2], IC_lines[p]["RightBottom"][c] - 7.0
                    )
                    X.SetPoint(
                        4, M["LeftBottom"][2], IC_lines[p]["LeftBottom"][c] - 7.0
                    )
                else:
...
def checkGeoVersion():
    ...
    if "volDownstreamMagnet" in pylist:
        return 1
    else:
        return 2
...
def timingOfEvent(makeCluster=False, debug=False):
    ...
    ut.bookHist(h, "evTimeDS", "cor time of hits;[ns]", 70, -5.0, 30)
    ut.bookHist(
        h, "evTimeScifi", "cor time of hits blue DS red Scifi;[ns]", 70, -5.0, 30
    )
    ...
    for siCl in trackTask.clusScifi:
        ...
        if isVertical:
            ...
            L = abs(A[1] - B[1]) / 2.0
        corTime = (
            geo.modules["AdvTarget"].GetCorrectedTime(detID, siCl.GetTime(), 0)
            - (z - firstScifi_z) / u.speedOfLight
        )
        h["evTimeScifi"].Fill(corTime)
        if debug:
            print(detID, corTime, pos)
    for aHit in eventTree.Digi_AdvMuFilterHits:
        detID = aHit.GetDetectorID()
        if not detID // 10000 == 3:
            continue
        if aHit.isVertical():
            nmax = 1
        else:
        ...
        for i in range(nmax):
            corTime = (
                geo.modules["AdvMuFilter"].GetCorrectedTime(
                    detID, i, aHit.GetTime(i) * TDC2ns, 0
                )
                - (z - firstScifi_z) / u.speedOfLight
            )
            h["evTimeDS"].Fill(corTime)
            if debug:
                print(detID, i, corTime, pos)
    tc = h["tevTime"].cd()
    h["evTimeScifi"].Draw()
...
def mufiNoise():
    for s in range(1, 4):
        ut.bookHist(
            h, "mult" + str(s), "hit mult for system " + str(s), 100, -0.5, 99.5
        )
        ut.bookHist(
            h, "multb" + str(s), "hit mult per bar for system " + str(s), 20, -0.5, 19.5
        )
        ut.bookHist(h, "res" + str(s), "residual system " + str(s), 20, -10.0, 10.0)
    OT = sink.GetOutTree()
    N = 0
    for event in eventTree:
        ...
        for aTrack in OT.Reco_MuonTracks:
            mom = aTrack.getFittedState().getMom()
            pos = aTrack.getFittedState().getPos()
            if not aTrack.getFitStatus().isFitConverged():
                continue
            mult = {1: 0, 2: 0, 3: 0}
            for aHit in eventTree.Digi_AdvMuFilterHits:
                if not aHit.isValid():
    ...
    if drawText:
        if k == 1 or len(moreEventInfo) < 5:
            ...
            if hasattr(eventTree.EventHeader, "GetEventNumber"):
                N = eventTree.EventHeader.GetEventNumber()
            else:
                N = event
