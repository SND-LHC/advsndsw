...
def map2Dict(aHit, T="GetAllSignals", mask=True):
    ...
    Value.clear()
    if T == "GetAllTimes":
        ROOT.fixRootT(aHit, key, Value, mask)
    else:
        ROOT.fixRoot(aHit, key, Value, mask)
    theDict = {}
    for k in range(key.size()):
        if T == "SumOfSignals":
            theDict[key[k].Data()] = Value[k]
        else:
...
parser.add_argument(
    "--remakeScifiClusters",
    dest="remakeScifiClusters",
    help="remake ScifiClusters",
    default=False,
)
    ...
    if path.find("eos") > 0:
        ...
        dirlist = str(
            subprocess.check_output(
                "xrdfs " + os.environ["EOSSHIP"] + " ls " + options.path, shell=True
            )
        )
...
def getAverageZpositions():
    ...
    for s in systemAndPlanes:
        for plane in range(systemAndPlanes[s]):
            ...
            zPos["MuFilter"][s * 10 + plane] = (A.Z() + B.Z()) / 2.0
    for s in range(1, 6):
        mat = 2
        sipm = 1
        channel = 64
        for o in range(2):
            Scifi.GetPosition(
                channel + 1000 * sipm + 10000 * mat + 100000 * o + 1000000 * s, A, B
            )
            zPos["Scifi"][s * 10 + o] = (A.Z() + B.Z()) / 2.0
    return zPos
...
if options.runNumber > 0:
    ...
    if partitions == 0:
        ...
        for p in range(partitions):
            eventChain.Add(
                path + "run_" + runNr + "/sndsw_raw-" + str(p).zfill(4) + ".root"
            )
    ...
    if f.Get("rawConv"):
        eventChain = f.rawConv
    else:
        eventChain = f.cbmsim
if options.remakeScifiClusters:
    eventChain.SetBranchStatus("Cluster_Scifi*", 0)
...
def help():
    ...
    print(
        "              Scifi_hitMaps(Nev) and Mufi_hitMaps(Nev)     if no number of events is given, loop over all events in file."
    )
    print(" ")
    ...
    print(
        "              analyze_EfficiencyAndResiduals(readHists=False), with option True, histograms are read from file"
    )
    print(" ")
    print("     Scifi unbiased residuals for an optional set of alignment constants")
    print(
        "              Scifi_residuals(Nev=-1,NbinsRes=100,xmin=-500.,alignPar=False), xmin = low edge of residual histos in microns"
    )
    print(" ")
    ...
    print(" ")
    print(
        "     make histograms with QDC distributions and fit all distributions with Langau "
    )
    print("              mips()")
    ...
    print(" ")
    print(
        "     rough estimate of Scifi resolution by comparing slopes  build with two stations, x and y projection"
    )
...
def Scifi_hitMaps(Nev=options.nEvents):
    ...
    for s in range(10):
        ut.bookHist(h, "posX_" + str(s), "x", 2000, -100.0, 100.0)
        ut.bookHist(h, "posY_" + str(s), "y", 2000, -100.0, 100.0)
        if s % 2 == 1:
            ut.bookHist(
                h,
                "mult_" + str(s),
                "mult vertical station " + str(s // 2 + 1),
                100,
                -0.5,
                99.5,
            )
            ...
            ut.bookHist(
                h,
                "mult_" + str(s),
                "mult horizontal station " + str(s // 2 + 1),
                100,
                -0.5,
                99.5,
            )
    for mat in range(30):
        ...
        ut.bookHist(h, "sig_" + str(mat), "signal / mat", 200, -50.0, 150.0)
        ut.bookHist(h, "tdc_" + str(mat), "tdc / mat", 200, -1.0, 4.0)
    N = -1
    if Nev < 0:
    ...
    for event in eventTree:
        ...
        for aHit in event.Digi_ScifiHits:
            ...
            if aHit.isVertical():
                rc = h["posX_" + str(X[0])].Fill(A[0])
            else:
                rc = h["posY_" + str(X[0])].Fill(A[1])
            mult[X[0]] += 1
        for s in range(10):
...
def Mufi_hitMaps(Nev=options.nEvents):
    ...
    for s in range(1, 4):
        ut.bookHist(
            h, sdict[s] + "Mult", "QDCs vs nr hits", 200, 0.0, 800.0, 200, 0.0, 300.0
        )
        ...
        for aHit in event.Digi_MuFilterHits:
            ...
            for c in allChannels:
                ...
                if s == 2 and smallSiPMchannel(c):
                    rc = h["sigS_" + str(s) + str(l)].Fill(allChannels[c])
                elif c < nSiPMs:
                    rc = h["sigL_" + str(s) + str(l)].Fill(allChannels[c])
                else:
                    rc = h["sigR_" + str(s) + str(l)].Fill(allChannels[c])
...
def smallVsLargeSiPMs(Nev=-1):
        ...
        for side in ["L", "R"]:
            for i1 in range(7):
                for i2 in range(i1 + 1, 8):
                    tag = ""
                    if S == 2 and smallSiPMchannel(i1):
                        tag = "s" + str(i1)
                    else:
                        tag = "l" + str(i1)
                    if S == 2 and smallSiPMchannel(i2):
                        tag += "s" + str(i2)
                    else:
                        tag += "l" + str(i2)
                    ut.bookHist(
                        h,
                        "cor" + tag + "_" + side + str(l),
                        "QDC channel i vs channel j",
                        200,
                        0.0,
                        200.0,
                        200,
    ...
    for event in eventTree:
        ...
        for aHit in event.Digi_MuFilterHits:
            ...
            rc = h["SVSl_" + str(l)].Fill(sumS / 4.0, sumL / 12.0)
            #
            for side in ["L", "R"]:
                offset = 0
                if side == "R":
                ...
                for i1 in range(offset, offset + 7):
                    if not i1 in allChannels:
                        continue
                    qdc1 = allChannels[i1]
                    for i2 in range(i1 + 1, offset + 8):
                        if not (i2) in allChannels:
                            continue
                        if s == 2 and smallSiPMchannel(i1):
                            tag = "s" + str(i1 - offset)
                        else:
                        ...
                        rc = h["cor" + tag + "_" + side + str(l) + str(bar)].Fill(
                            qdc1, qdc2
                        )
            allChannels.clear()
        ...
        for side in ["L", "R"]:
            ...
            for i1 in range(7):
                for i2 in range(i1 + 1, 8):
                    ...
                    for bar in range(systemAndBars[S]):
                        if bar == 0:
                            h["cor" + tag + "_" + side + str(l) + str(bar)].Draw("colz")
                        else:
                            h["cor" + tag + "_" + side + str(l) + str(bar)].Draw(
                                "colzsame"
                            )
                    k += 1
...
def makeIndividualPlots(run=options.runNumber):
    ...
    if not "run" + str(run) in os.listdir():
    ...
    for l in range(5):
        for side in ["L", "R"]:
            ...
            for pad in tcanv.GetListOfPrimitives():
                if not hasattr(pad, "GetListOfPrimitives"):
                    continue
                for aHist in pad.GetListOfPrimitives():
                    if not aHist.ClassName() == "TH2D":
                        continue
                    hname = aHist.GetName()
                    tmp = hname.split("_")
                    bar = tmp[1][2]
...
def eventTime(Nev=options.nEvents):
    ...
    for event in eventTree:
        ...
        if N > 0 and dT > 0:
            ...
            rc = h["time"].Fill(T / freq - t0)  # very first event
        Tprev = T
    ...
    tc.SetLogy(1)
    h["EtimeZ"].Draw()
    rc = h["EtimeZ"].Fit("expo", "S", "", 0.0, 250.0)
    h["T"].Update()
...
def TimeStudy(Nev=options.nEvents, withDisplay=False):
    if Nev < 0:
        Nev = eventTree.GetEntries()
    ut.bookHist(h, "Vetotime", "time", 1000, 0.0, 50.0)
    ut.bookHist(h, "UStime", "time", 1000, 0.0, 50.0)
    ut.bookHist(h, "DStime", "time", 1000, 0.0, 50.0)
    ut.bookHist(h, "Stime", "time", 1000, 0.0, 50.0)
    ut.bookHist(
        h,
        "SvsDStime",
        "; mean Scifi time [ns];mean Mufi time [ns]",
        100,
        0.0,
        50.0,
        100,
    ...
    ut.bookHist(
        h,
        "VEvsUStime",
        "; mean US time [ns];mean VE time [ns]",
        100,
        0.0,
        50.0,
        100,
...
def USshower(Nev=options.nEvents):
    ...
    for x in ["", "-small"]:
        ut.bookHist(h, "shower" + x, "energy vs z", 200, 0.0, 10000.0, 20, zUS0, zUS4)
        ut.bookHist(h, "showerX" + x, "energy vs z", 200, 0.0, 10000.0, 20, zUS0, zUS4)
        ut.bookHist(h, "wshower" + x, "z weighted energy ", 100, -300.0, 0.0)
        ut.bookHist(
            h, "zyshower" + x, "y vs z weighted energy ", 20, zUS0, zUS4, 11, -0.5, 10.5
        )
    for p in range(systemAndPlanes[2]):
        ut.bookHist(
            h,
            "SvsL" + str(p),
            "small vs large Sipms plane" + str(p) + ";large   [QCD]; small   [QCD] ",
            100,
            0.1,
            250.0,
            100,
    ...
    for event in eventTree:
        ...
        for plane in UShits:
            ...
            if 0 in UShits:
                if UShits[0] > 750:
                    rc = h["showerX" + x].Fill(UShits[plane], z)
    ...
    text = ""
    if options.runNumber in energy:
        text += "E=%5.1F" % (energy[options.runNumber])
    if options.runNumber in gain:
        text += "  with gain=%5.2F" % (gain[options.runNumber])
...
def USDSEnergy(Nev=options.nEvents, nproc=15):
    ...
    for b in ["", "lb1", "lip1", "lb2"]:
        ut.bookHist(
            h,
            "energy23" + b,
            "tot energy in DS vs US; US tot.QDC;DS tot.QDC ",
            300,
            0.0,
            15000,
            250,
        ...
        ut.bookHist(
            h,
            "Tenergy23" + b,
            "tot energy in DS vs US with scifi track; US tot.QDC;DS tot.QDC",
            300,
            0.0,
            15000,
            250,
        ...
        ut.bookHist(
            h,
            "Denergy23" + b,
            "tot energy in DS vs US with DS track; US tot.QDC;DS tot.QDC",
            300,
            0.0,
            15000,
            250,
    ...
    for i in range(nproc):
        ...
        if pid != 0:
            ...
            for k in range(nstart, nstop):
                ...
                if lb1:
                    rc = h["energy23lb1"].Fill(Etot[2], Etot[3])
                if lb2:
                    rc = h["energy23lb2"].Fill(Etot[2], Etot[3])
                if lip1:
                    rc = h["energy23lip1"].Fill(Etot[2], Etot[3])
                trackTypes = [0, 0]
                ...
                if trackTypes[0] == 1:
                    ...
                    if lb1:
                        rc = h["Tenergy23lb1"].Fill(Etot[2], Etot[3])
                    if lb2:
                        rc = h["Tenergy23lb2"].Fill(Etot[2], Etot[3])
                    if lip1:
                        rc = h["Tenergy23lip1"].Fill(Etot[2], Etot[3])
                if trackTypes[1] == 1:
                    rc = h["Denergy23"].Fill(Etot[2], Etot[3])
                    if lb1:
                        rc = h["Denergy23lb1"].Fill(Etot[2], Etot[3])
                    if lb2:
                        rc = h["Denergy23lb2"].Fill(Etot[2], Etot[3])
                    if lip1:
                        rc = h["Denergy23lip1"].Fill(Etot[2], Etot[3])
                if Etot[2] > 5000 and Etot[3] > 5000:
                    print(
                        "HE",
                        nstart,
                        k,
                        eventTree.GetChainOffset(),
                        eventTree.GetCurrentFile().GetName(),
                        Etot[2],
                        Etot[3],
                        lb1,
                        lb2,
...
def Mufi_Efficiency(
    ...
    for s in range(1, 6):
        for p in projs:
            ut.bookHist(
                h,
                "dtScifivsX_" + str(s) + projs[p],
                "dt vs x track " + projs[p] + ";X [cm]; dt [ns]",
                100,
                -10.0,
                40.0,
                260,
            ...
            ut.bookHist(
                h, "clN_" + str(s) + projs[p], "cluster size " + projs[p], 10, -0.5, 9.5
            )
        ut.bookHist(
            h,
            "dtScifivsdL_" + str(s),
            "dt vs dL " + str(s) + ";X [cm]; dt [ns]",
            100,
            -40.0,
            0.0,
            200,
    ...
    for s in systemAndPlanes:
        for l in range(systemAndPlanes[s]):
            ...
            ut.bookHist(
                h,
                "atLRvsX_" + sdict[s] + str(s * 10 + l),
                "mean time - T0track vs x " + str(s * 10 + l) + ";X [cm]; dt [ns]",
                20,
                -70.0,
                30.0,
                250,
            ...
            ut.bookHist(
                h,
                "VetoatLRvsX_" + sdict[s] + str(s * 10 + l),
                "mean time - T0Veto vs x " + str(s * 10 + l) + ";X [cm]; dt [ns]",
                20,
                -70.0,
                30.0,
                250,
            ...
            for side in ["", "L", "R", "S"]:
                for proj in ["X", "Y"]:
                    xmin = -X * NbinsRes / 100.0 * scale
                    xmax = -xmin
                    ut.bookHist(
                        h,
                        "res" + proj + "_" + sdict[s] + side + str(s * 10 + l),
                        "residual  " + proj + str(s * 10 + l),
                        NbinsRes,
                        xmin,
                        xmax,
                        100,
                    ...
                    ut.bookHist(
                        h,
                        "gres" + proj + "_" + sdict[s] + side + str(s * 10 + l),
                        "residual  " + proj + str(s * 10 + l),
                        NbinsRes,
                        xmin,
                        xmax,
                    ...
                    if side == "":
                        if s == 1:
                            ut.bookHist(
                                h,
                                "resBar" + proj + "_" + sdict[s] + str(s * 10 + l),
                                "residual " + proj + str(s * 10 + l),
                                NbinsRes,
                                xmin,
                                xmax,
                ...
                if side == "":
                    ut.bookHist(
                        h,
                        "track_" + sdict[s] + str(s * 10 + l),
                        "track x/y " + str(s * 10 + l) + ";X [cm];Y [cm]",
                        100,
                        -90.0,
                        10.0,
                        100,
                    ...
                    ut.bookHist(
                        h,
                        "locEx_" + sdict[s] + str(s * 10 + l),
                        "loc track pos;X [cm];Y [cm]",
                        100,
                        -100,
                        100,
                        100,
    ...
    ut.bookHist(
        h, "trackslxy_badChi2", "track direction", 200, -0.1, 0.1, 200, -0.1, 0.1
    )
    ut.bookHist(h, "tracksChi2Ndof", "chi2/ndof", 100, 0.0, 100.0, 10, -0.5, 9.5)
    ut.bookHist(h, "NdofvsNMeas", "ndof Nmeas", 20, -0.5, 19.5, 20, -0.5, 19.5)

    v = 15.0 * u.cm / u.ns  # signal propagation in fibre, assumption
    if Nev < 0:
        Nev = eventTree.GetEntries()
    N = 0
    for event in eventTree:
        ...
        if (
            not theTrack.getFitStatus().isFitConverged() and optionTrack != "DS"
        ):  # for H8 where only two planes / proj were avaiable
            continue
        ...
        rc = h["NdofvsNMeas"].Fill(
            fitStatus.getNdf(), theTrack.getNumPointsWithMeasurement()
        )
        ...
        if optionTrack == "DS":
            ...
            for nM in range(theTrack.getNumPointsWithMeasurement()):
                ...
                if (0 in allTDCs) and (1 in allTDCs):
                    T0s.append((allTDCs[0] + allTDCs[1]) * TDC2ns / 2.0)
            if len(T0s) == 2:
                ...
                L = ROOT.TMath.Sqrt(
                    (lam * mom.x()) ** 2
                    + (lam * mom.y()) ** 2
                    + (zPos["MuFilter"][32] - pos.z()) ** 2
                )
                ToF = L / u.speedOfLight
                T0track = (T0s[0] + T0s[1] - ToF) / 2.0
                Z0track = pos[2]
        ...
        T0Veto = -999
        if len(avT) == 2:
            T0Veto = (avT[10] + (avT[11] - dZ / u.speedOfLight)) / 2.0
        ...
        for s in sdict:
            ...
            for plane in range(systemAndPlanes[s]):
                ...
                for aHit in muHits[s * 10 + plane]:
                    ...
                    rc = h["locBarPos_" + sdict[s] + str(s * 10 + plane)].Fill(
                        locPos[0], locPos[1]
                    )
                    rc = h["locEx_" + sdict[s] + str(s * 10 + plane)].Fill(
                        locEx[0], locEx[1]
                    )
                    rc = h["resY_" + sdict[s] + str(s * 10 + plane)].Fill(dy, locEx[0])
                    rc = h["resX_" + sdict[s] + str(s * 10 + plane)].Fill(dx, locEx[1])
                    rc = h["gresY_" + sdict[s] + str(s * 10 + plane)].Fill(
                        gdy, globEx[0]
                    )
                    rc = h["gresX_" + sdict[s] + str(s * 10 + plane)].Fill(
                        gdx, globEx[1]
                    )
                    S = map2Dict(aHit, "GetAllSignals")
                    # check for signal in left / right or small sipm
                    left, right, smallL, smallR, Sleft, Sright, SleftS, SrightS = (
                        0,
                        0,
                        0,
                        0,
                    ...
                    if s == 1:
                        ...
                        rc = h["resBarY_" + sdict[s] + str(s * 10 + plane)].Fill(
                            gdy, bar
                        )
                    for x in S:
                        if s == 1:
                        ...
                        if x < nSiPMs:
                            ...
                            if s == 2 and smallSiPMchannel(x):
                                ...
                                right += 1
                    if left > 0:
                        rc = h["resY_" + sdict[s] + "L" + str(s * 10 + plane)].Fill(
                            dy, locEx[1]
                        )
                        rc = h["resX_" + sdict[s] + "L" + str(s * 10 + plane)].Fill(
                            dx, locEx[0]
                        )
                        rc = h["gresY_" + sdict[s] + "L" + str(s * 10 + plane)].Fill(
                            gdy, globEx[1]
                        )
                        rc = h["gresX_" + sdict[s] + "L" + str(s * 10 + plane)].Fill(
                            gdx, globEx[0]
                        )
                    if right > 0:
                        rc = h["resY_" + sdict[s] + "R" + str(s * 10 + plane)].Fill(
                            dy, locEx[1]
                        )
                        rc = h["resX_" + sdict[s] + "R" + str(s * 10 + plane)].Fill(
                            dx, locEx[0]
                        )
                        rc = h["gresY_" + sdict[s] + "R" + str(s * 10 + plane)].Fill(
                            gdy, globEx[1]
                        )
                        rc = h["gresX_" + sdict[s] + "R" + str(s * 10 + plane)].Fill(
                            gdx, globEx[0]
                        )
                    if s == 2 and (smallL > 0 or smallR > 0):
                        rc = h["resY_" + sdict[s] + "S" + str(s * 10 + plane)].Fill(
                            dy, locEx[1]
                        )
                        rc = h["resX_" + sdict[s] + "S" + str(s * 10 + plane)].Fill(
                            dx, locEx[0]
                        )
                        rc = h["gresY_" + sdict[s] + "S" + str(s * 10 + plane)].Fill(
                            gdy, globEx[1]
                        )
                        rc = h["gresX_" + sdict[s] + "S" + str(s * 10 + plane)].Fill(
                            gdx, globEx[0]
                        )
                    dist = abs(dy)
                    if aHit.isVertical():
                    ...
                    if dist < 3.0:  # check channels
                        ...
                        for x in S:
                            ...
                            if x < nSiPMs:
                                if s == 2 and smallSiPMchannel(x):
                                    rc = h["signalSL_" + barName + "-c" + str(x)].Fill(
                                        qcd, dL
                                    )
                                    SleftS += qcd
                                else:
                                    rc = h["signalL_" + barName + "-c" + str(x)].Fill(
                                        qcd, dL
                                    )
                                    Sleft += qcd
                            else:
                                if s == 2 and smallSiPMchannel(x):
                                    rc = h[
                                        "signalSR_" + barName + "-c" + str(x - nSiPMs)
                                    ].Fill(qcd, dR)
                                    SrightS += qcd
                                else:
                                    rc = h[
                                        "signalR_" + barName + "-c" + str(x - nSiPMs)
                                    ].Fill(qcd, dR)
                                    Sright += qcd
                        ...
                        if not aHit.isVertical():
                            ...
                            mtVeto = (
                                aHit.GetImpactT()
                                - T0Veto
                                - (globPos[2] - Z0Veto) / u.speedOfLight
                            )
                            h["dtLRvsX_" + sdict[s] + str(s * 10 + plane)].Fill(
                                xEx, dt * TDC2ns
                            )
                            h["dtLRvsX_" + barName].Fill(xEx, dt * TDC2ns)
                            if (1 in allTDCs) and (9 in allTDCs):
                                h["dtF1LRvsX_" + barName].Fill(
                                    xEx, (allTDCs[1] - allTDCs[9]) * TDC2ns
                                )
                            h["dtfastLRvsX_" + barName].Fill(xEx, dtF * TDC2ns)
                            if Z0track > 0:
                                tcor = (globPos[2] - Z0track) / deltaZ02 * ToF
                                mtTrack = aHit.GetImpactT() - T0track - tcor
                                h["atLRvsX_" + sdict[s] + str(s * 10 + plane)].Fill(
                                    xEx, mtTrack
                                )
                            ...
                            h["VetoatLRvsX_" + sdict[s] + str(s * 10 + plane)].Fill(
                                xEx, mtVeto
                            )
                        ...
                        for i in allTDCs:
                            ...
                            if i < nSiPMs and nL > 0:
                                ...
                                rc = h["sigmaTDCL_" + key].Fill(
                                    (allTDCs[i] - meanL / nL) * TDC2ns
                                )
                                if t0Left < 900:
                                    rc = h["TDCcalibL_" + key].Fill(
                                        (allTDCs[i] - t0Left) * TDC2ns
                                    )
                                ...
                                rc = h["sigmaTDCR_" + key].Fill(
                                    (allTDCs[i] - meanR / nR) * TDC2ns
                                )
                                if t0Right < 900:
                                    rc = h["TDCcalibR_" + key].Fill(
                                        (allTDCs[i] - t0Right) * TDC2ns
                                    )
    ...
    if readHists:
        for x in h:
            if hasattr(x, "Reset"):
                x.Reset()
        ut.readHists(
            h, "MuFilterEff_run" + str(options.runNumber) + ".root", withProj=False
        )
    s = 2
    for plane in range(5):
        for bar in range(10):
            for p in ["L", "R"]:
                for T in ["T", ""]:
                    if T == "T":
                        ...
                        nloop = systemAndChannels[s][1] + systemAndChannels[s][0]
                    for i in range(nloop):
                        ...
                        for n in range(1, h["M" + name].GetNbinsX() + 1):
                            h[name + "-X" + str(n)] = h[name].ProjectionX(
                                name + "-X" + str(n), n, n
                            )
                            tmp = h[name + "-X" + str(n)]
                            ...
                            if tmp.GetEntries() > 50:
                                ...
                                if T == "T":
                                    bmin, bmax = 10, tmp.GetMaximumBin()
                                else:
                                    bmin, bmax = 0, 30
                                ...
                                for k in range(tmp.GetMaximumBin(), tmp.GetNbinsX()):
                                    if (
                                        tmp.GetBinContent(k) + tmp.GetBinContent(k + 1)
                                        < 2
                                    ):
                                        bmax = k
                                        break
                                res = fit_langau(
                                    tmp,
                                    "LQ",
                                    0.8 * tmp.GetBinCenter(bmin),
                                    1.5 * tmp.GetBinCenter(bmax),
                                )
                                if not res:
                                    continue
                                if not res.IsValid():
...
def mipsAfterBurner(readhisto=True):
    if readhisto:
        ut.readHists(
            h, "LandauFits_run" + str(options.runNumber) + ".root", withProj=False
        )
    s = 2
    for plane in range(5):
        for p in ["L", "R"]:
...
def plotMips(readhisto=True):
    ...
    langau = ROOT.TF1("Langau", langaufun, 0.0, 100.0, 4)
    ...
    tc = h["Mpvs"].cd(1)
    h["attLengthL"], h["attLengthR"], h["attLengthLC"], h["attLengthRC"] = (
        ROOT.TGraph(),
        ROOT.TGraph(),
        ROOT.TGraph(),
        ROOT.TGraph(),
    )
    h["mpv0L"], h["mpv0R"], h["mpv0LC"], h["mpv0RC"] = (
        ROOT.TGraph(),
        ROOT.TGraph(),
        ROOT.TGraph(),
        ROOT.TGraph(),
                ...
                for bar in range(10):
                    ...
                    for i in range(nloop):
                        ...
                        for i in range(1, h[chi2N].GetNbinsX() + 1):
                            ...
                            if not funL:
                                continue
                            if not funL.GetParameter(1) > 0:
                                continue
                            mpvRelErr = (
                                funL.GetParError(1)
                                / funL.GetParameter(1)
                                * ROOT.TMath.Sqrt(
                                    h[name[1:] + "-X" + str(i)].GetEntries()
                                )
                            )
                            if mpvRelErr < 0.1:
                                print(
        ...
        for tag in ["", "C"]:
            ...
            for S in side:
                ...
                if tag == "C":
                    ...
                    h[params[P][2] + S + tag].SetMarkerSize(2)
        ...
        for p in ["L", "R"]:
            for t in tMax:
                ...
                for bar in range(10):
                    ...
                    dropt = ""
                    if len(tc.GetListOfPrimitives()) > 0:
                        dropt = "same"
                    h[name].SetMinimum(0.0)
                    h[name].SetMaximum(tMax[t] * 1.1)
                    if name.find("CsignalT") == 0:
                        h[name].SetTitle(
                            "station " + p + " " + str(plane + 1) + ";chi2/nDoF"
                        )
                    if name.find("MsignalT") == 0:
                        ...
                        h[name].SetTitle(
                            "sigma " + p + " " + str(plane + 1) + ";d [cm];QDC"
                        )
                    elif name.find("WsignalT") == 0:
                        h[name].SetTitle(
                            "width " + p + " " + str(plane + 1) + ";d [cm];QDC"
                        )
                    if t == "MsignalT":
                        rc = h[name].Draw("P" + dropt)
                    else:
                        rc = h[name].Draw("PL" + dropt)
                    if t == "NnSiPMs":
                        tc = h["NnSiPMs1"].cd(1)
                        h[name].SetMinimum(3)
                        if k > 1:
                        ...
                        h[name].Draw(dropt)
            k += 1
    for t in ["MsignalT", "NnSiPMs", "WsignalT", "SsignalT"]:
        myPrint(h[t], t)
    ...
    for p in ["L", "R"]:
        for plane in range(4):
            ...
            if p == "R" or plane % 2 == 1:
                rc = hist.Fit("expo", "S", "", 0, 60)
            if p == "L" and plane % 2 == 0:
                rc = hist.Fit("expo", "S", "", 20, 80)
            res = rc.Get()
            if res:
                slopes[str(plane) + p] = [res.Parameter(1), res.ParError(1)]
    ...
    for p in ["L", "R"]:
        for plane in range(4):
            ...
            if plane % 2 == 1:
                pp = "T"
            txt = "%i%s (%5.1F+/-%5.1F)cm" % (
                plane,
                pp,
                -1.0 / slopes[str(plane) + p][0],
                slopes[str(plane) + p][1] / slopes[str(plane) + p][0] ** 2,
            )
        ...
        for plane in range(5):
            for p in ["L", "R"]:
                ...
                for bar in range(10):
                    ...
                    M = h[run]["N" + name].GetBinContent(
                        h[run]["N" + name].GetMaximumBin()
                    )
                    if M > nMax and M < 20:
                        nMax = M
                for bar in range(10):
                ...
                k += 1
        h["MsignalN" + str(run)].Print(
            "MPVgainRatio_" + str(run) + "_" + str(nRef) + ".png"
        )
...
def plotRMS(readHists=True, t0_channel=4):
    if readHists:
        ut.readHists(
            h, options.path + "MuFilterEff_run" + str(r) + ".root", withProj=False
        )
    for s in [1, 2]:
        h["tdcCalib" + sdict[s]] = {}
        for l in range(systemAndPlanes[s]):
        ...
        for l in range(systemAndPlanes[s]):
            tag = sdict[s] + str(l)
            ut.bookCanvas(
                h, "sigmaTDC_" + tag, "TDC RMS", 2000, 600, systemAndBars[s], 2
            )
            ut.bookCanvas(
                h, "TDCcalib_" + tag, "TDC TDCi-T0", 2000, 600, systemAndBars[s], 2
            )
            ut.bookCanvas(
                h, "sigmaQDC_" + tag, "QDC RMS", 2000, 600, systemAndBars[s], 2
            )
            k = 1
            for bar in range(systemAndBars[s]):
                for side in ["L", "R"]:
                    ...
                    for i in range(systemAndChannels[s][1] + systemAndChannels[s][0]):
                        ...
                        for x in [
                            ...
                            if x.find("QDC") > 0:
                                h[aHist].GetXaxis().SetRangeUser(-15, 15)
                            else:
...
def plotMipsTimeResT0(readHists=True, animation=False):
    ...
    for mode in ["t0"]:
        for s in S:
            for side in ["L", "R"]:
                if s == 1:
                    ut.bookCanvas(
                        h, mode + "Time" + side + "-" + str(s), "", 1800, 800, 2, 1
                    )
                if s == 2:
                    ut.bookCanvas(
                        h, mode + "Time" + side + "-" + str(s), "", 1800, 1500, 2, 3
                    )
            nbars = 0
            for plane in range(systemAndPlanes[s]):
                if systemAndOrientation(s, plane) == "vertical":
                    continue
                nbars += systemAndBars[s]
            ut.bookHist(
                h,
                mode + "tsigma_" + str(s),
                ";" + str(systemAndBars[s]) + "*station + bar number;#sigma_{t} [ns]",
                nbars,
                -0.5,
                nbars - 0.5,
            )
        ...
        for l in range(systemAndPlanes[s]):
            ...
            for side in ["L", "R"]:
                ...
                for bar in range(systemAndBars[s]):
                    ...
                    for sipm in range(
                        systemAndChannels[s][0] + systemAndChannels[s][1]
                    ):
                        ...
                        for j in range(1, xax.GetNbins() + 1):
                            ...
                            if h[jname].GetEntries() < minContent:
                                continue
                            ...
                            if res:
                                ...
                                g.SetPointError(
                                    np, xax.GetBinCenter(j), res.ParError(1)
                                )
                                np += 1
                        A = weightedY(h[tname])
                        if A[1] < 100:
                        ...
                        if not res:
                            continue
                        resultsT0[s][l][side][bar][sipm]["velocity"] = (
                            1.0 / res.Parameter(1)
                        )
    #
    h["sideLines"] = []
    for s in resultsT0:
        ut.bookHist(h, "allV_" + str(s), ";sipm channel;[cm/ns]", 800, 0.0, 800.0)
        ut.bookHist(h, "allVdis_" + str(s), ";[cm/ns]", 100, -20.0, 20.0)
        ...
        if s == 2:
            ...
            for l in range(systemAndPlanes[s]):
                for side in ["L", "R"]:
                    xchan += 80
                    h["sideLines"].append(
                        ROOT.TLine(xchan, hist.GetMinimum(), xchan, hist.GetMaximum())
                    )
    ...
    for s in resultsT0:
        ...
        ut.bookHist(h, "allSigm_" + str(s), ";SiPM channel ; [ns]", 800, 0.0, 800.0)
        ...
        if s == 2:
            ...
            for l in range(systemAndPlanes[s]):
                for side in ["L", "R"]:
                    xchan += 80
                    h["sideLines"].append(
                        ROOT.TLine(xchan, 0.0, xchan, hist.GetMaximum())
                    )
                    L = h["sideLines"][len(h["sideLines"]) - 1]
                    L.SetLineColor(ROOT.kGreen)
                    L.Draw()
                ...
                for bar in range(systemAndBars[s]):
                    for side in ["L", "R"]:
                        ...
                        for sipm in range(
                            ...
                            if s == 2 and smallSiPMchannel(sipm):
                                continue
                            X = resultsT0[s][side][bar][sipm]
                            if len(X) == 2:
                                txt += " " + "%5.2F" % (X["meanSigma"])
                            else:
                                txt += " " + "%5.2F" % (-1)
                            key = sdict[s] + str(s * 10 + l) + "_" + str(bar)
                            name = "tvsX" + side + "_" + key + "-c" + str(sipm)
                        ...
                        print(txt, tname)
            os.system(
                "convert -delay 120 -loop 0 *X.png TDCallChannels_" + sdict[s] + ".gif"
            )
            os.system("rm *X.png")
...
def plotMipsTimeRes(readHists=True, animation=False):
    ...
    for mode in ["", "fast", "F1"]:
        ...
        for s in S:
            results[mode][s] = {}
            if s == 1:
                ut.bookCanvas(h, mode + "TimeLR-" + str(s), "", 1800, 800, 2, 1)
            if s == 2:
                ut.bookCanvas(h, mode + "TimeLR-" + str(s), "", 1800, 1500, 2, 3)
            if s == 3:
                ut.bookCanvas(h, mode + "TimeLR-" + str(s), "", 900, 1500, 1, 3)
            nbars = 0
            for plane in range(systemAndPlanes[s]):
                if systemAndOrientation(s, plane) == "vertical":
                    continue
            ...
            for plane in range(systemAndPlanes[s]):
                ...
                for bar in range(systemAndBars[s]):
                    ...
                    ymax = (
                        h[name].GetYaxis().GetBinCenter(h[name].GetYaxis().GetNbins())
                    )
                    minContent = 50
                    for j in range(1, xax.GetNbins() + 1):
                        jname = name + "-X" + str(j)
                        h[jname] = h[name].ProjectionY(jname, j, j)
                        if h[jname].GetSumOfWeights() < minContent:
                            continue
                        xx = h[jname].GetBinCenter(h[jname].GetMaximumBin())
                        xmin = max(ymin, xx - 2.0)
                        xmax = min(ymax, xx + 2.0)
                    ...
                    A = weightedY(h[tname])
                    if A[1] < 100:
                        h[mode + "tsigma_" + str(s)].SetBinContent(
                            plane * systemAndBars[s] + bar, A[0]
                        )
                        h[mode + "tsigma_" + str(s)].SetBinError(
                            plane * systemAndBars[s] + bar, A[1]
                        )
                    rc = g.Fit("pol1", "SQ")
                    res = rc.Get()
                    if not res:
                        continue
                    results[mode][s][plane][bar]["velocity"] = 2.0 / res.Parameter(1)
            ...
            myPrint(
                h[mode + "TimeLR-" + str(s)], mode + "TimeResol-" + sdict[s] + "vsX"
            )
    ...
    if animation:
        ...
        for s in S:
            for l in range(systemAndPlanes[s]):
                if systemAndOrientation(s, l) == "vertical":
                    continue
                if l == 4 and s == 3:
                    continue  # only 2 station for H8
                for bar in range(systemAndBars[s]):
                    name = "dtLRvsX_" + sdict[s] + str(s * 10 + l) + "_" + str(bar)
                    if h[name].GetSumOfWeights() > 50:
                        h[name].Draw("colz")
                    myPrint(
                        h["dummy"],
                        str(l) + "-" + str(bar).zfill(2) + "Xanimat",
                        withRootFile=False,
                    )
            os.system(
    ...
    for s in S:
        ...
        if s == 3:
            ut.bookHist(h, "alldtV_" + str(s), ";bar;[cm/ns]", 180, 0.0, 180.0)
        if s == 2:
            ut.bookHist(h, "alldtV_" + str(s), ";bar;[cm/ns]", 800, 0.0, 800.0)
        if s == 1:
            ut.bookHist(h, "alldtV_" + str(s), ";bar;[cm/ns]", 15, 0.0, 15.0)
        ut.bookHist(h, "alldtVdis_" + str(s), ";[cm/ns]", 100, 10.0, 20.0)
        np = 0
        nx = 0
        for plane in range(systemAndPlanes[s]):
            if systemAndOrientation(s, plane) == "vertical":
...
def weightedY(aHist):
    ...
    for j in range(1, nx + 1):
        ...
        if E > 0:
            mean += N / E**2
            W += 1 / E**2
    return [mean / W, ROOT.TMath.Sqrt(1.0 / W)]
...
def drawArea(s=3, p=0, opt="", color=ROOT.kGreen):
    ...
    for bar in range(systemAndBars[s]):
        ...
        if s < 3 or (s == 3 and bar % 5 == 0):
            latex.DrawLatex(botR.X() - 5.0, botR.Y() + 0.3, barName)
        N = len(lines)
    ...
    for bar in range(systemAndBars[s]):
        ...
        barName = str(bar)
        if bar % 5 == 0:
            latex.DrawLatex(topR.X(), topR.Y() + 2, barName)
    for l in linesV:
...
def analyze_EfficiencyAndResiduals(readHists=False, mode="S", local=True, zoom=False):
    ...
    for proj in ["X", "Y"]:
        if mode == "S":
            if proj == "Y":
                ut.bookCanvas(h, "EVetoUS" + proj, "", 1200, 2000, 2, 7)
            if proj == "Y":
                ut.bookCanvas(h, "EDS" + proj, "", 1200, 2000, 2, 3)
            if proj == "X":
                ut.bookCanvas(h, "EDS" + proj, "", 1200, 2000, 2, 4)
        else:
        ...
        for s in [1, 2, 3]:
            ...
            if s < 3:
                ...
                tname = "EDS" + proj
            for plane in range(systemAndPlanes[s]):
                if s == 3 and proj == "X" and (plane % 2 == 0 and plane < 5):
                    continue
                if s == 3 and proj == "Y" and (plane % 2 == 1 or plane == 6):
                    continue
                tc = h[tname].cd(k)
                ...
                k += 1
                for side in ["L", "R", "S"]:
                    if side == "S" and (s == 3 or mode == "S"):
                        continue
                    if side == "R" and (mode == "S"):
                        continue
                    tc = h[tname].cd(k)
                    k += 1
                    if mode == "S":
                    ...
                    if zoom:
                        if s == 3:
                            resH.GetXaxis().SetRangeUser(-4.0, 4.0)
                        else:
                            resH.GetXaxis().SetRangeUser(-10.0, 10.0)
                    resH.Draw()
                    ...
                    if resH.GetEntries() > 10:
                        ...
                        rc = resH.Fit(myGauss, "SL", "", -15.0, 15.0)
                        fitResult = rc.Get()
                        if not (s * 10 + plane) in residualsAndSigma:
                            residualsAndSigma[s * 10 + plane] = [
                                fitResult.Parameter(1) / u.mm,
                                fitResult.ParError(1) / u.mm,
                                abs(fitResult.Parameter(2) / u.cm),
                                fitResult.ParError(2) / u.cm,
                            ]
                        tc.Update()
                        ...
                        if tracks > 0:
                            ...
                            latex.DrawLatexNDC(
                                0.2, 0.8, "eff=%5.2F+/-%5.2F%%" % (eff, effErr)
                            )
        if "EVetoUS" + proj in h:
    ...
    for s in S:
        ...
        for plane in range(systemAndPlanes[s]):
            ...
            ymax = min(
                hist.GetYaxis().GetBinCenter(hist.GetNbinsY()), 5 * res.Parameter(2)
            )
            ...
            if xproj.GetSumOfWeights() == 0:
                continue
            np = 0
            Lmin = hist.GetSumOfWeights() * 0.005
            for nx in range(
                hist.GetXaxis().FindBin(xmin), hist.GetXaxis().FindBin(xmax)
            ):
                tmp = hist.ProjectionY("tmp", nx, nx)
                if tmp.GetEntries() < Lmin:
                    continue
            ...
            if b < 0:
                sign = "-"
            txt = "X = %5.1F #frac{cm}{ns} #times #frac{1}{2}#Deltat %s %5.1F " % (
                m * 2,
                sign,
                abs(b),
            )
    ...
    for plane in [0, 1]:
        ...
        for nbar in range(1, h[hname].GetNbinsY() + 1):
            ...
            myGauss.SetParameter(1, 0)
            myGauss.SetParameter(2, 2.0)
            rc = h[vname].Fit(myGauss, "SL", "", -15.0, 15.0)
            fitResult = rc.Get()
...
def TimeCalibrationNtuple(Nev=options.nEvents, nStart=0):
    ...
    tdc = array("f", maxD * [0.0])
    qdc = array("f", maxD * [0.0])
    dL = array("f", maxD * [0.0])
    D = array("f", maxD * [0.0])
        ...
        if status.isFitConverged():
            ...
            for nM in range(theTrack.getNumPointsWithMeasurement()):
                ...
                for nh in range(aCl.GetN()):
                    ...
                    D[nHit] = 0
                    if abs(V.Z()) > 0:
                        D[nHit] = V.Mag() / N.Mag() * V.Z() / abs(V.Z())
                    nHit += 1
                    if nHit == maxD:
...
def analyzeTimings(c="", vsignal=15.0, args={}, opt=0):
    print("-->", c)
    if c == "":
        ...
        for nt in tTimes:
            ...
            for nHit in range(nt.nChannels):
                ...
                if opt > 0:
                    hname = "c" + str(detID)
                    if not hname in h:
                        ut.bookHist(h, hname, ";dt [ns]", 200, -10.0, 10.0)
                ...
                for nHit2 in range(nHit + 1, nt.nChannels):
                    ...
                    if abs(detID - detID2) == 1:
                        ...
                        if detID2 > detID:
                            dt = -dt
                        if nt.qdc[nHit] < 15 or nt.qdc[nHit2] < 15:
                            continue
                        rc = h["dTtwin"].Fill(dt)
                        if detID == args["detID0"]:
                            rc = h["dTtwinMax"].Fill(dt)
...
def ScifiChannel_residuals():
    for s in range(1, 6):
        for o in range(2):
            ut.bookHist(
                h,
                "res_Scifi" + str(s * 10 + o),
                "residual  " + str(s * 10 + o) + ";channel; [#mum]",
                512 * 3,
                -0.5,
                512 * 3 - 0.5,
                100,
...
def mergeMuFilterSignals(hstore, tag="signalT", system="", fname=None):
    ...
    for x in s:
        for side in ["L", "R"]:
            ...
            hstore[hname] = hstore[
                tag + side + "_" + sdict[x] + str(x * 10) + "_" + "0"
            ].Clone(hname)
            hstore[hname].Reset()
            ...
            for l in range(systemAndPlanes[x]):
                for bar in range(systemAndBars[x]):
                    xname = (
                        tag + side + "_" + sdict[x] + str(x * 10 + l) + "_" + str(bar)
                    )
                    if F:
                        hstore[xname] = F.Get(xname).Clone(xname)
...
def TwoTrackFinder(
    nstart=0, Nev=-1, sMin=10, dClMin=7, minDistance=1.5, sepDistance=0.5, debug=False
):
    if Nev < 0:
    ...
    for ecounter in range(nstart, nstop):
        ...
        if ecounter % 1000000 == 0:
            print("still here", ecounter)
        ...
        for o in range(0, 2):
            ...
            if k != 1:
                continue
            if abs(seeds[o][0][0][1] - seeds[o][1][0][1]) < sepDistance:
                continue
            for s in range(1, 6):
                if s == S[o]:
                    continue
...
def Scifi_residuals(
            ...
            for k in range(nstart, nstop):
                                    ...
                                    if s > 2:
                                        ...
                                        if bar > 59:
                                            ...
                                            if l < 6:
                                                l += 1
                                    mult[s * 10 + l] += 1
                                    if mult[35] == 1 and mult[34] == 1:
                                        trackTypes[4] += 1  # from IP1 and high momentum

                        if aTrack.GetUniqueID() == 3:
                            trackTypes[3] += 1
                    if not (trackTypes[4] == 1):
                        continue
                ...
                for s in range(1, 6):
                    ...
                    if unbiased or s == 1:
                        ...
                        fitStatus = theTrack.getFitStatus()
                        if (
                            not fitStatus.isFitConverged()
                            or theTrack.getNumPointsWithMeasurement() < 8
                        ):
                            theTrack.Delete()
                            validTrack = False
                            continue
                        rc = h["trackChi2/ndof"].Fill(
                            fitStatus.getChi2() / (fitStatus.getNdf() + 1e-10),
                            fitStatus.getNdf(),
                        )
                        fstate = theTrack.getFittedState()
                        mom = fstate.getMom()
                        rc = h["trackSlopes"].Fill(
                            mom.X() / mom.Z() * 1000, mom.Y() / mom.Z() * 1000
                        )
                    # check residuals
                    if not validTrack and not unbiased:
                        break
                        # test plane
                    for o in range(2):
                        testPlane = s * 10 + o
                        ...
                        for aCl in sortedClusters[testPlane]:
                            ...
                            rc = h["resC" + projs[o] + "_Scifi" + str(testPlane)].Fill(
                                doca / u.um, channel
                            )
                            rc = h["res" + projs[o] + "_Scifi" + str(testPlane)].Fill(
                                doca / u.um
                            )
                            rc = h["resX" + projs[o] + "_Scifi" + str(testPlane)].Fill(
                                doca / u.um, xEx
                            )
                            rc = h["resY" + projs[o] + "_Scifi" + str(testPlane)].Fill(
                                doca / u.um, yEx
                            )

                    if unbiased:
                        theTrack.Delete()
                if not unbiased and validTrack:
                    theTrack.Delete()
        ...
        for s in range(1, 6):
            for o in range(2):
                ...
                if proj == "":
                    ...
                    tc.Update()
                    for p in Par:
                        globalPos[p + projs[o]].SetPoint(
                            s - 1, s, fitResult.Parameter(Par[p])
                        )
                        globalPos[p + projs[o]].SetPointError(
                ...
                if proj == "C":
                    for m in range(3):
                        ...
                        if not fitResult:
                            continue
                        for p in Par:
                            h["globalPosM"][p + projs[o]].SetPoint(
                                j[o], s * 10 + m, fitResult.Parameter(Par[p])
                            )
                            h["globalPosM"][p + projs[o]].SetPointError(
    ...
    for p in globalPos:
        ...
        tc = h[p].cd()
        if p.find("mean") < 0:
            globalPos[p].SetTitle(p + ";station; #sigma [#mum]")
        else:
        ...
        if p.find("mean") == 0:
            for n in range(globalPos[p].GetN()):
                ...
                print(
                    "station %i: offset %s =  %5.2F um   sigma = %5.2F um"
                    % (S.value, p[4:5], M.value, E)
                )
                s = int(S.value * 10)
                if p[4:5] == "V":
                    s += 1
    ...
    for p in h["globalPosM"]:
        ...
        tc = h[p + "M"].cd()
        if p.find("mean") < 0:
            h["globalPosM"][p].SetTitle(p + ";mat ; #sigma  [#mum]")
        else:
...
def printScifi_residuals(tag="v0"):
    ...
    for s in range(1, 6):
        for o in range(2):
            ...
            XM = (mean[0] + mean[1] + mean[2]) / 3.0
            print(
                "      mean value %8.2F            delta s: %8.2F  %8.2F  %8.2F"
                % (XM, mean[0] - XM, mean[1] - XM, mean[2] - XM)
            )
    # for H6 beam
    h6 = False
    if H6:
...
def minimizeAlignScifi(first=True, level=1, migrad=False):
    ...
    if first:
        ...
        if level == 2:
            ...
            vstart[8] = -5.71174e01
            vstart[9] = 1.99880e01
            vstart[10] = 1.57924e02
            vstart[11] = 1.80187e02
            vstart[12] = -1.84760e00
            vstart[13] = 7.89774e01
            vstart[14] = 1.39775e01
            vstart[15] = 7.57700e-01
            vstart[16] = -1.09748e02
            vstart[17] = 7.45420e01
            vstart[18] = -1.67857e01
            vstart[19] = 5.64360e01
            vstart[20] = 9.69426e01
            vstart[21] = 7.10440e01
            vstart[22] = -6.41257e01
            vstart[23] = 1.72473e01
            vstart[24] = 1.76322e02
            vstart[25] = 1.02006e02
            vstart[26] = 8.16677e01
            vstart[27] = -1.62196e02
            vstart[28] = 1.84062e02
            vstart[29] = 5.80966e01
            # rotation, three angles / station
            vstart[30] = 0.00000e00  # s0
            vstart[31] = -9.300e-01
            vstart[32] = 0.00000e00
            vstart[33] = 0.00000e00  # s1
            vstart[34] = -1.900e-01
            vstart[35] = 0.00000e00
            vstart[36] = 0.00000e00  # s2
            vstart[37] = 0.00000e00
            vstart[38] = 0.00000e00
            vstart[39] = 0.00000e00  # s3
            vstart[40] = 0.00000e00
            vstart[41] = 0.00000e00
            vstart[42] = 0.00000e00  # s4
            vstart[43] = -2.200e-01
            vstart[44] = 0.00000e00
            err = 20.0
            h["xmin"] = -2000.0
            h["npar"] = 45
        if level == 3:
    ...
    for s in range(1, 6):
        for r in ["RotPhiS", "RotPsiS", "RotThetaS"]:
            name = "Scifi/" + r + str(s)
            gMinuit.mnparm(p, name, vstart[p], err / 100, 0.0, 0.0, ierflg)
    ...
    for p in range(h["npar"]):
        ...
        if h["npar"] > 29:
            if p < 30:
                ...
                s = (p - 30) // 3 + 1
                if p % 3 == 0:
                    name = "Scifi/RotPhiS" + str(s)
                if p % 3 == 1:
                    name = "Scifi/RotPsiS" + str(s)
                if p % 3 == 2:
                    name = "Scifi/RotThetaS" + str(s)
        h["alignPar"][name] = par[p]
        print("minuit %s %6.4F" % (name, par[p]))
        if h["level"] == 1 and p < 30:  # station alignment
            if m > 0:
                h["alignPar"][name] = par[p - m]
        else:
            h["alignPar"][name] = par[p]
        theArray.append(par[p])

    X = Scifi_residuals(
        Nev=h["Nevents"], NbinsRes=100, xmin=h["xmin"], alignPar=h["alignPar"], nproc=10
    )
    projs = {1: "V", 0: "H"}
    for s in range(1, 6):
        sigma = 15
...
def plotsForCollabMeeting():
    ...
    for s in S:
        ...
        k = 1
        for plane in range(systemAndPlanes[s]):
            if s == 3 and (plane % 2 == 1 or plane > 5):
                continue
            tc = h[tname].cd(k)
            k += 1
            key = sy + str(s * 10 + plane)
