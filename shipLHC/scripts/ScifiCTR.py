...
class Scifi_CTR(ROOT.FairTask):
    ...
    def Init(self, monitor):
        ...
        if (options.geoFile).find("../") < 0:
            self.snd_geo = SndlhcGeo.GeoInterface(options.path + options.geoFile)
        else:
        ...
        for s in range(1, 6):
            ...
            ut.bookHist(
                h,
                "CTR_Scifi_beam" + str(s) + tag,
                "CTR beam" + str(s) + tag + "; dt [ns]; ",
                100,
                -5.0,
                5.0,
            )
            for matH in range(3):
                for matV in range(3):
                    ut.bookHist(
                        h,
                        "CTR_Scifi" + str(s * 100 + 10 * matH + matV) + tag,
                        "CTR " + str(s) + tag + "; dt [ns]; ",
                        100,
                        -5.0,
                        5.0,
                    ...
                    ut.bookHist(
                        h,
                        "CTR_Scifi_beam" + str(s * 100 + 10 * matH + matV) + tag,
                        "CTR beam" + str(s) + tag + "; dt [ns]; ",
                        100,
                        -5.0,
                        5.0,
            ...
            for p in self.projs:
                ut.bookHist(
                    h, "res" + str(s) + self.projs[p], "d;  [cm]; ", 100, -1.0, 1.0
                )
                ut.bookHist(
                    h, "resX" + str(s) + self.projs[p], "d;  [cm]; ", 100, -1.0, 1.0
                )
                ut.bookHist(
                    h,
                    "extrap" + str(s) + self.projs[p],
                    "SiPM distance; L [cm]; ",
                    100,
                    0.0,
                    50.0,
        ...
        for s1 in range(1, 5):
            for s2 in range(s1 + 1, 6):
                ut.bookHist(
                    h,
                    "CTR_ScifiStation" + str(s1 * 10 + s2) + tag,
                    "CTR station" + str(s1 * 10 + s2) + "; dt [ns]; ",
                    100,
                    -5.0,
                    5.0,
                )
                ut.bookHist(
                    h,
                    "CTR_ScifiStation_beam" + str(s1 * 10 + s2) + tag,
                    "CTR station beam" + str(s1 * 10 + s2) + "; dt [ns]; ",
                    100,
                    -5.0,
                    5.0,
                )
        ...
        if self.tag == "v0":
            ...
            for s in range(1, 6):
                self.tdcScifiStationCalib[s] = [
                    0,
                    {"H": {0: 0, 1: 0, 2: 0}, "V": {0: 0, 1: 0, 2: 0}},
                ]
        ...
        for aTrack in self.M.Reco_MuonTracks:
            ...
            for nM in range(aTrack.getNumPointsWithMeasurement()):
                ...
                if aHit.isVertical():
                    ...
                    sortedClusters[s]["V"].append(
                        [clkey, L, B[0], state.getPos()[1], mat, (A[2] + B[2]) / 2.0]
                    )
                    rc = h["res" + str(s) + "V"].Fill(
                        (A[0] + B[0]) / 2.0 - state.getPos()[0]
                    )
                else:
                    L = A[0] - state.getPos()[0]
                    sortedClusters[s]["H"].append(
                        [clkey, L, A[1], state.getPos()[0], mat, (A[2] + B[2]) / 2.0]
                    )
                    rc = h["res" + str(s) + "H"].Fill(
                        (A[1] + B[1]) / 2.0 - state.getPos()[1]
                    )
            # find station with exactly 1 x and 1 y cluster:
            for s in range(1, 6):
                if not (len(sortedClusters[s]["V"]) * len(sortedClusters[s]["H"])) == 1:
                    continue
                timeCorr = {}
                for proj in ["V", "H"]:
                    ...
                    if M.options.check:
                        time = self.M.Scifi.GetCorrectedTime(
                            aCl.GetFirst(), aCl.GetTime(), 0
                        )
                ...
                if abs(slopeX) < 0.1 and abs(slopeY) < 0.1:
                    ...
                    rc = h[
                        "CTR_Scifi_beam" + str(100 * s + 10 * matH + matV) + tag
                    ].Fill(dt)
                ...
                if tag != "v0":
                    ...
                    for s in range(1, 6):
                        if (
                            not (
                                len(sortedClusters[s]["V"])
                                * len(sortedClusters[s]["H"])
                            )
                            == 1
                        ):
                            continue
                        ...
                        stationTimes[s] = [
                            sTime / 2.0,
                            (
                                sortedClusters[s]["H"][0][5]
                                + sortedClusters[s]["V"][0][5]
                            )
                            / 2.0,
                        ]
                    for s1 in range(1, 5):
                        if not s1 in stationTimes:
                            continue
                        for s2 in range(s1 + 1, 6):
                            if not s2 in stationTimes:
                                continue
                            ...
                            dT -= dL / self.C
                            rc = h["CTR_ScifiStation" + str(s1 * 10 + s2) + tag].Fill(
                                dT
                            )
                            if abs(slopeX) < 0.1 and abs(slopeY) < 0.1:
                                rc = h[
    ...
    def Plot(self):
        ...
        for b in ["", "_beam"]:
            ...
            for s in range(1, 6):
                ut.bookCanvas(
                    h, "CTRM" + str(s) + b + tag, "CTR per mat combi", 1800, 1200, 3, 3
                )
                j = 1
                for matH in range(3):
                    for matV in range(3):
                        ...
                        j += 1
                        histo = h[
                            "CTR_Scifi" + b + str(100 * s + 10 * matH + matV) + tag
                        ]
                        histo.Fit("gaus", "SQ")
                        tc.Update()
                        stats = histo.FindObject("stats")
        ...
        if tag != "v0":
            for b in ["", "_beam"]:
                ut.bookCanvas(
                    h, "CTRS" + b, "CTR per station combination" + tag, 1800, 1200, 5, 2
                )
                k = 1
                for s1 in range(1, 5):
                    for s2 in range(s1 + 1, 6):
    ...
    def minimize(self, b=""):
        ...
        ut.bookHist(h, "commonBlock", "", 100, 0.0, 100.0)
    ...
    def minimizeStation(self, b=""):
        ...
        for s1 in range(1, 6):
            for s2 in range(s1 + 1, 6):
                ...
                Fun = histo.GetFunction("gaus")
                self.meanAndSigmaStation[b][key] = [
                    Fun.GetParameter(1),
                    Fun.GetParameter(2),
                ]
        for s1 in range(1, 6):
...
class Scifi_TimeOfTracks(ROOT.FairTask):
    ...
    def Init(self, monitor):
        ...
        ut.bookHist(
            M.h,
            "dTvsZ",
            "dT versus dL; dL/0.5cm [cm];dT/100ps [ns]",
            140,
            0.0,
            70.0,
            120,
        ...
        ut.bookHist(
            M.h,
            "dTvsZ_beam",
            "dT versus dL, beam dL/0.5cm [cm];dT/100ps [ns]",
            140,
            0.0,
            70.0,
            120,
        ...
        for aTrack in self.M.Reco_MuonTracks:
            ...
            for s in range(1, 6):
                ...
                for proj in ["V", "H"]:
                    ...
                    time -= self.tdcScifiStationCalib[s][
                        0
                    ]  # internal station calibration
                    time -= abs(L) / self.V
                ...
                stationTimes[s] = [
                    sTime / 2.0,
                    (sortedClusters[s]["H"][0][5] + sortedClusters[s]["V"][0][5]) / 2.0,
                ]
            # require station 1 to be present which defines T0
            if not 1 in stationTimes:
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="path to data",
        required=False,
        default="/eos/experiment/sndlhc/convertedData/physics/2022/",
    )
    parser.add_argument(
        "-P",
        "--partition",
    ...
    parser.add_argument(
        ...
        help="file name",
        type=str,
        default="geofile_sndlhc_TI18_V0_2022.root",
        required=False,
    )
    parser.add_argument(
        "-c", "--command", dest="command", help="command", required=False, default=""
    )
    parser.add_argument(
        "-o",
        dest="check",
        help="use corrected times",
        action="store_true",
        default=False,
    )
    parser.add_argument(
    ...
    if options.command == "full":
        ...
        for s in range(1, 6):
            ...
            print(
                "station %1i  %5.2F  H: %5.2F  %5.2F  %5.2F   V:  %5.2F  %5.2F  %5.2F "
                % (
                    s,
                    C[0],
                    C[1]["H"][0],
        ...
        for s in range(1, 6):
            C = task.tdcScifiStationCalib[s]
            print(
                "station %1i  %5.3F*u.ns,  %5.3F*u.ns,  %5.3F*u.ns,  %5.3F*u.ns,   %5.3F*u.ns,  %5.3F*u.ns,  %5.3F*u.ns "
                % (
                    s,
                    C[0],
                    C[1]["H"][0],
