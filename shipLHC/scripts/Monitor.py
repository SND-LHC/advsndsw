...
class Monitoring:
    ...
    def __init__(self, options, FairTasks):
        ...
        if (options.geoFile).find("../") < 0:
            ...
            self.snd_geo = SndlhcGeo.GeoInterface(options.geoFile[3:])
        ...
        if options.online:
            ...
            if options.fname:
                ...
                if not eventChain:
                    eventChain = f.cbmsim
                    if eventChain.GetBranch("MCTrack"):
                ...
                if options.partition < 0:
                    ...
                    partitions = [
                        "sndsw_raw-" + str(options.partition).zfill(4) + ".root"
                    ]
            ...
            self.run.Init()
            if len(partitions) > 0:
                self.eventTree = ioman.GetInChain()
            else:
                self.eventTree = ioman.GetInTree()
                # fitted tracks
        ...
        if not self.hasBunchInfo:
            try:
                ...
                print(
                    "continue without knowing filling scheme",
                    options.server + options.path,
                )
            if self.fsdict:
    ...
    def GetEntries(self):
        if self.options.online:
            if self.converter.newFormat:
                return self.converter.fiN.Get("data").GetEntries()
            else:
                return self.converter.fiN.event.GetEntries()
        else:
    ...
    def GetEvent(self, n):
        ...
        if self.options.online:
            ...
            if self.options.FairTask_convRaw:
                self.options.online.sTree.GetEvent(
                    self.options.online.sTree.GetEntries() - 1
                )
            for t in self.FairTasks:
        ...
        if self.hasBunchInfo:
            ...
            if (
                self.xing["B1only"]
                and self.xing["B2noB1"]
                or self.xing["B1only"]
                and self.xing["noBeam"]
            ):
                print("error with b1only assignment", self.xing)
            if self.xing["B2noB1"] and self.xing["noBeam"]:
                print("error with b2nob1 assignment", self.xing)
    ...
    def updateHtml(self):
        ...
        found = False
        for L in oldL:
            if not L.find(self.runNr) < 0:
                return
            if (
                L.find(
                    "https://snd-lhc-monitoring.web.cern.ch/"
                    + destination
                    + "/run.html?file=run"
                )
                > 0
                ...
                Lnew = (
                    '            <li> <a href="https://snd-lhc-monitoring.web.cern.ch/'
                    + destination
                    + "/run.html?file=run"
                )
                Lnew += (
                ...
                if self.options.postScale > 1:
                    Lnew += "  post scaled:" + str(self.options.postScale)
                Lnew += "\n"
                tmp.write(Lnew)
                found = True
            tmp.write(L)
    ...
    def checkAlarm(self, minEntries=1000):
        ...
        if (
            res[0][1] > minEntries
        ):  # choose limit which makes it sensitive to check for empty mats
            for mat in range(30):
                if entries[mat] < 1:
                    s = mat // 6 + 1
                    if mat % 6 < 3:
        ...
        for p in range(2):
            for side in ["L", "R"]:
                entries[str(10 * s + p) + side] = self.h[
                    "mufi-sig" + p + str(10 * s + p)
                ].GetEntries()
        res = sorted(entries.items(), key=operator.itemgetter(1), reverse=True)
        if (
            res[0][1] > minEntries
        ):  # choose limit which makes it sensitive to check for empty boards
            for p in range(2):
                for side in ["L", "R"]:
        ...
        for p in range(5):
            for side in ["L", "R"]:
                entries[str(s * 10 + p) + side] = self.h[
    ...
    def myPrint(self, tc, name, subdir="", withRootFile=True):
        ...
        if withRootFile:
            ...
            if not os.path.isdir(srun):
                os.system("mkdir " + srun)
...
class TrackSelector:
    ...
    def __init__(self, options):
        ...
        self.MuonTracksBranch = self.outTree.Branch(
            "Reco_MuonTracks", self.fittedTracks, 32000, 0
        )

        if (
            self.options.simpleTracking
            and not self.options.trackType.find("Scifi") < 0
            and not eventChain.GetBranch("Cluster_Scifi")
        ):
            self.clusScifi = ROOT.TClonesArray("sndCluster")
            self.clusScifiBranch = self.outTree.Branch(
                "Cluster_Scifi", self.clusScifi, 32000, 1
            )
        if (
            self.options.simpleTracking
            and not self.options.trackType.find("DS") < 0
            and not eventChain.GetBranch("Cluster_Mufi")
        ):
            self.clusMufi = ROOT.TClonesArray("sndCluster")
            self.clusMufiBranch = self.outTree.Branch(
                "Cluster_Mufi", self.clusMufi, 32000, 1
            )
    ...
    def ExecuteEvent(self, event):
        ...
        if self.options.trackType == "ScifiDS":
            if self.options.HoughTracking:
                ...
                track_container_list = [
                    self.muon_reco_task_Sf.kalman_tracks,
                    self.muon_reco_task_DS.kalman_tracks,
                ]
            if self.options.simpleTracking:
    ...
    def Execute(self):
        for n in range(self.options.nStart, self.options.nStart + self.options.nEvents):
            if self.options.scaleFactor > 1:
                if ROOT.gRandom.Rndm() > 1.0 / self.options.scaleFactor:
                    continue
            ...
            if not self.MonteCarlo and self.fittedTracks.GetEntries() == 0:
                continue
            if (
                self.options.simpleTracking
                and not self.options.trackType.find("Scifi") < 0
