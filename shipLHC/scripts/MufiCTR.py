...
def start():
    for x in ["dTScifiDS", "dTcorScifiDS"]:
        for d in ["", "B2noB1", "B1only"]:
            ...
            if d != "":
                dd = d + "/"
            h["mufi-" + x + d] = f.mufilter.Get(dd + "mufi-" + x + d).Clone(
                "mufi-" + x + d
            )
            h["mufi-" + x + d].Draw()
...
def start0(plist=["tmp4705p9"], s=3):
    ...
    for x in ["", "cor"]:
        for d in ["", "B2noB1", "B1only"]:
            ut.bookCanvas(
                h,
                detector + "dT" + x + "Scifi" + system + d,
                "dt rel to scifi " + system,
                S[s][0],
                S[s][1],
                S[s][2],
                S[s][3],
    ...
    for x in ["", "cor"]:
        for d in ["", "B2noB1", "B1only"]:
            ...
            h[detector + "dT" + x + "Scifi" + system + d].Print(
                "mufi-dT" + x + "Scifi" + system + d + ".png"
            )
...
def xCheck():
    ...
    for c in plots:
        ...
        for x in plots[c]:
            if x < 2:
                h["mufi-dTcor_3" + str(x) + "B1"].Draw()
            else:
                h["mufi-dTcor_3" + str(x) + "B1"].Draw("same")
            if x == 6:
                m, s = h["meanAndSig"][b][x + 2]
            else:
                m, s = h["meanAndSig"][b][x + 1]
            h["txt" + str(x)] = txt.DrawLatexNDC(
                0.15,
                0.85 - j * 0.05,
                "DS%i%s  m=%5.2Fns  #sigma=%5.2Fns" % (x // 2, c, m, s),
            )
            h["txt" + str(x)].SetTextColor(
                h["mufi-dTcor_3" + str(x) + "B1"].GetLineColor()
            )
            j += 1
        j += 1
        if c == "H":
            h["txtB2H"] = txt.DrawLatexNDC(
                0.15,
                0.85 - j * 0.05,
                "BW tracks DS0H: %5.2Fns DS1H: %5.2Fns DS2H: %5.2Fns  "
                % (
                    h["meanAndSig"]["B2noB1"][1][0],
                    h["meanAndSig"]["B2noB1"][3][0],
                    h["meanAndSig"]["B2noB1"][5][0],
                ),
            )
            h["txtB2H"].SetTextColor(ROOT.kMagenta)
        else:
            h["txtB2V"] = txt.DrawLatexNDC(
                0.15,
                0.85 - j * 0.05,
                "BW tracks DS0V: %5.2Fns DS1V: %5.2Fns DS2V: %5.2Fns  DS3V: %5.2Fns  "
                % (
                    h["meanAndSig"]["B2noB1"][2][0],
                    h["meanAndSig"]["B2noB1"][4][0],
                    h["meanAndSig"]["B2noB1"][6][0],
                    h["meanAndSig"]["B2noB1"][8][0],
                ),
            )
...
def execute(s=3):
    ...
    ut.bookHist(
        h, "talign", "time alignment B1;channel;offset [ns]", nbins, -0.5, nbins - 0.5
    )
    ut.bookHist(
        h, "talign2", "time alignment B2;channel;offset [ns]", nbins, -0.5, nbins - 0.5
    )
    ut.bookHist(h, "tres", "time diff res;channel;sigma [ns]", nbins, -0.5, nbins - 0.5)
    ut.bookHist(
        h, "t12", "time diff B1 B2;channel;sigma [ns]", nbins, -0.5, nbins - 0.5
    )
    for l in range(S[s][2] * S[s][3]):
        if s == 3 and l > 7:
            continue
        for i in range(hist.GetNbinsY()):
            ...
            if alignTpar[s * 10 + l][i][0] < 99 and alignTparB2[s * 10 + l][i][0] < 99:
                rc = h["t12"].Fill(
                    k + i, alignTparB2[s * 10 + l][i][0] - alignTpar[s * 10 + l][i][0]
                )
            else:
                rc = h["t12"].Fill(k + i, 100)
    if s == 3:
    ...
    if s == 3:
        ...
        for i in range(h["talign"].GetNbinsX()):
            if i < 120 and i % 2 == 0:
                p = 0
            elif i < 120 and i % 2 == 1:
                p = 1
            elif i < 240 and i % 2 == 0:
                p = 2
            elif i < 360 and i % 2 == 0:
                p = 3
            elif i < 360 and i % 2 == 1:
                p = 4
            elif i < 480 and i % 2 == 0:
                p = 5
            elif i < 600 and i % 2 == 0:
                p = 6
            elif i < 600 and i % 2 == 1:
                p = 7
            elif i < 720 and i % 2 == 0:
                p = 8
            elif i < 840 and i % 2 == 0:
                p = 9
            else:
                continue
            if i % 2 == 0:
            ...
            channel = (j + 1 + p * 60) % 60
            if channel < 30:
                X += channel * slope - 15 * slope
            else:
                X -= (channel - 30) * slope - 15 * slope
            h["talignReorder2"].SetBinContent(j + 1 + p * 60, X)

        for l in range(0, 3):
            c = "talignReorder" + str(l)
            ut.bookCanvas(
                h, "results" + c, system + " time alignment " + str(l), 1200, 600, 1, 1
            )
            h["results" + c].SetGridy(1)
            h["results" + c].cd()
            par0[l] = {}
            for x in h[c].GetListOfFunctions():
                x.Delete()
            if l == 2:
                par1 = 0
                planes = 20
            ...
            for p in range(planes):
                ...
                if l > 0:
                    h[theFun].FixParameter(1, sign * par1)
                ...
                while res.Chi2() > 1.5:
                    ...
                    if imax - ix < imin + 2:
                        1 / 0
                    if h[c].GetFunction(theFun):
                        h[c].GetFunction(theFun).Delete()
                    if p == 3:
                        rc = h[c].Fit(h[theFun], "SQw+", "", imin + ix, imax - ix)
                    else:
                        rc = h[c].Fit(
                            h[theFun], "SQw+", "", imin + 2, imax - ix
                        )  # vertical no statistics
                    res = rc.Get()
        ...
        c = "talignReorder3"
        ut.bookHist(h, "resT", "t residuals", 100, -3.0, 3.0)
...
def timingOfEvent(makeCluster=False, debug=False):
    ...
    ut.bookHist(h, "evTimeDS", "cor time of hits;[ns]", 100, -20.0, 20)
    ut.bookHist(
        h, "evTimeScifi", "cor time of hits blue DS red Scifi;[ns]", 100, -20.0, 20
    )
    ...
    for siCl in trackTask.clusScifi:
        ...
        if isVertical:
            ...
            L = abs(A[1] - B[1]) / 2.0
        corTime = (
            geo.modules["Scifi"].GetCorrectedTime(detID, siCl.GetTime(), L)
    ...
    for aHit in eventTree.Digi_MuFilterHits:
        ...
        for i in range(nmax):
            corTime = (
                geo.modules["MuFilter"].GetCorrectedTime(
                    detID, i, aHit.GetTime(i) * TDC2ns, L
                )
                - (z - firstScifi_z) / u.speedOfLight
            )
