...
parser.add_argument(
    "-p", "--path", dest="path", help="path to file", required=False, default=""
)
parser.add_argument(
    "-P",
    "--partition",
...
parser.add_argument(
    "-st", "--simpleTracking", dest="simpleTracking", action="store_true", default=False
)
parser.add_argument(
    "-genfitFormat",
    "--genfitFormat",
    dest="genfitFormat",
    action="store_true",
    help="output track format for simple tracking when only it is run",
    default=False,
)
parser.add_argument(
    "-t", "--trackType", dest="trackType", help="DS, Scifi, ScifiDS", default="ScifiDS"
)
...
parser.add_argument(
    "-ht", "--HoughTracking", dest="HoughTracking", action="store_true", default=False
)
parser.add_argument(
    "-par",
    "--parFile",
...
parser.add_argument(
    ...
    "--scale",
    dest="scaleFactor",
    help="Randomly run reconstruction.",
    required=False,
    default=1,
    type=int,
)
...
if options.simpleTracking:
    ...
    if options.HoughTracking:
        ...
        options.genfitTrack = options.genfitFormat
