...
def loopEvents(start=0, save=False, selection=4):
    ...
    zStart = -40.0  # old coordinate system with origin in middle of target
    ut.bookHist(h, "xz", "x vs z", 500, zStart, zStart + 250.0, 100, -100.0, 10.0)
    ut.bookHist(h, "yz", "y vs z", 500, zStart, zStart + 250.0, 100, 0.0, 80.0)
    proj = {1: "xz", 2: "yz"}
    h["xz"].SetStats(0)
    h["yz"].SetStats(0)
    ...
    while Nstart + 1 < Nev:
        ...
        if rc == "q":
            break
    if save:
        os.system("convert -delay 60 -loop 0 *.png animated.gif")
