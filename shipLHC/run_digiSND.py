...
parser.add_argument(
    ...
    dest="ts",
    type=float,
    help="threshold energy for Scifi [p.e.]",
    default=3.5,
)
parser.add_argument(
    "-ss",
    ...
    dest="ss",
    type=float,
    help="saturation energy for Scifi [p.e.]",
    default=104.0,
)
parser.add_argument(
    "-tML",
    ...
    dest="tml",
    type=float,
    help="threshold energy for Mufi large [p.e.]",
    default=0.0,
)
parser.add_argument(
    "-tMS",
    ...
    dest="tms",
    type=float,
    help="threshold energy for Mufi small [p.e.]",
    default=0.0,
)
parser.add_argument(
    "-cpp",
    ...
    action="store_true",
    dest="FairTask_digi",
    help="perform digitization using DigiTaskSND",
    default=True,
)
...
if mufiDet:
    ...
    mufiDet.SetConfPar(
        "MuFilter/DsSiPMcalibrationS", 25.0 * 1000.0
    )  # in MC: 1.65 keV are about 41.2 qdc
    mufiDet.SetConfPar("MuFilter/VandUpSiPMcalibration", 25.0 * 1000.0)
    mufiDet.SetConfPar("MuFilter/VandUpSiPMcalibrationS", 25.0 * 1000.0)
...
if scifiDet:
    ...
    scifiDet.SetConfPar(
        "Scifi/timeResol", 150.0 * u.picosecond
    )  # time resolution in ps
    scifiDet.SetConfPar(
        "MuFilter/timeResol", 150.0 * u.picosecond
    )  # time resolution in ps, first guess
