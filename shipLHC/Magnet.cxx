...
void Magnet::Initialize() { FairDetector::Initialize(); }
...
void Magnet::ConstructGeometry()
{
    ...
    // Yoke shape
    TGeoCompositeShape *FeYoke =
        new TGeoCompositeShape("FeYoke", "CoilContainer-MagRegion-(Coil:CoilUpPos)-(Coil:CoilDownPos)");
    ...
    MagnetVol->AddNode(
        volTrackPlane, 0, new TGeoTranslation(0, 0, -fMagZ / 2. - fTSpacingZ - fTrackerZ - fLevArm - fTrackerZ / 2.));
    MagnetVol->AddNode(
        volTrackPlane, 3, new TGeoTranslation(0, 0, +fMagZ / 2. + fTSpacingZ + fTrackerZ + fLevArm + fTrackerZ / 2.));
...
void Magnet::EndOfEvent() { fMagnetPointCollection->Clear(); }
...
void Magnet::Register()
{
    ...
    FairRootManager::Instance()->Register("MagnetPoint", "Magnet", fMagnetPointCollection, kTRUE);
}

TClonesArray *Magnet::GetCollection(Int_t iColl) const
{
    if (iColl == 0) {
        return fMagnetPointCollection;
    } else {
        return NULL;
    }
}

void Magnet::Reset() { fMagnetPointCollection->Clear(); }

MagnetPoint *Magnet::AddHit(Int_t trackID,
                            Int_t detID,
                            TVector3 pos,
                            TVector3 mom,
                            Double_t time,
                            ...
                            Int_t pdgCode)
{
    ...
    return new (clref[size]) MagnetPoint(trackID, detID, pos, mom, time, length, eLoss, pdgCode);
}
