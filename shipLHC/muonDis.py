...
normalization[1] = (
    normalization[0] * 137.13 / 78.0
)  # number of p-p collisions in FLUKA simulation 0 / FLUKA simulation 1
normalization[2] = (
    normalization[0] * 137.13 / 14.3
)  # number of p-p collisions in FLUKA simulation 0 / FLUKA simulation 2
normalization[3] = (
    normalization[0] * 137.13 / 50.0
)  # number of p-p collisions in FLUKA simulation 0 / FLUKA simulation 3, muons_VCdown_IR1-LHC

h = {}

SND_Z = 483.262 * u.m
Eloss = 30.0  # energy loss in 75m of rock
...
def G(x):
    return 3.0 / x**3 * (x**2 / 2.0 - 1.0 + ROOT.TMath.Exp(-x) * (1 + x))
...
def nucl_cross(Ebeam, eps, A):
    ...
    nu = eps / Ebeam
    t = Mmu**2 * nu**2 / (1.0 - nu)
    k = 1.0 - 2.0 / nu + 2.0 / nu**2
    x = 0.00282 * ROOT.TMath.Power(A, 1.0 / 3.0) * photoabsorb(eps)
    if eps < 5:
        sigma = 0
    # if eps<3: sigma = 0 # 3.8.2015, test, text (PDG) says, <5 GeV not reliable
        ...
        sigma = (
            ...
            * (
                ...
                * G(x)
                * (
                    k * ROOT.TMath.Log(1 + m1sq / t)
                    - k * m1sq / (m1sq + t)
                    - 2.0 * Mmu**2 / t
                )
                + 0.25 * (k * ROOT.TMath.Log(1 + m2sq / t) - 2.0 * Mmu**2 / t)
                + Mmu**2
                / (2.0 * t)
                * (
                    0.75 * G(x) * m1sq / (m1sq + t)
                    + 0.25 * m2sq / t * ROOT.TMath.Log(1.0 + t / m2sq)
                )
            )
        )
    return sigma
...
def SigmaAnalytic(Ebeam=5000.0, A=1, nsteps=10000):
    gname = "xsec_" + str(A)
...
def SigmaAnalyticVsEnergy(A=1):
    ...
    for Ebeam in EnergyScan:
        ...
        h[gname].SetPoint(
            i, Ebeam, h["xsec_" + str(A)].Integral() / A / 1000.0
        )  # convert to mbarns
        i += 1
...
def SigmaAnalyticVsA(Ebeam=500):
    ...
    for A in range(1, nsteps + 1):
        SigmaAnalytic(Ebeam, A)
        h[gname].SetPoint(
            i, A, h["xsec_" + str(A)].Integral() / 1000.0
        )  # convert to mbarns
        i += 1
...
parser.add_argument(
    "-s", "--firstEvent", dest="nStart", type=int, help="first event", default=0
)
parser.add_argument(
    "-r", "--run", dest="run", type=int, help="production sequence number", default=0
)
...
parser.add_argument(
    "-x", "--nMult", dest="nMult", type=int, help="option to re-use muon", default=1
)
parser.add_argument(
    "-N",
    "--nucleon",
    dest="nucleon",
    type=str,
    help="nucleon for muon DIS pythia6",
    default="p+",
)
parser.add_argument(
    "-z", "--zextrap", dest="z", type=float, help="muon extrapolation", default=-5 * u.m
)
parser.add_argument(
    "-P",
    "--pythia6",
    dest="pythia6",
    type=int,
    help="pythia6 or not pythia6",
    default=1,
)
parser.add_argument(
    "-X", "--PDFpSet", dest="PDFpSet", type=str, help="PDF pSet to use", default="13"
)
...
def convertAscii2Root(fname, version=2):
    ...
    for l in f.readlines():
        ...
        if version == 2:
            ...
            if version == 0:
                column.append(409.0 * u.m)
            if version == 1:
                column.append(
                    418.684 * u.m
                    + line[6] * ROOT.TMath.Tan(2.338 / 180.0 * ROOT.TMath.Pi())
                )
...
def muonPreTransport():
    ...
    for n in range(nt.GetListOfLeaves().GetEntries()):
        ...
        if n < nt.GetListOfLeaves().GetEntries() - 1:
            variables += ":"
    ...
    for n in range(nt.GetEntries()):
        ...
        if E > 0:
            ...
            for n in range(nt.GetListOfLeaves().GetEntries()):
                ...
                if nt.GetListOfLeaves()[n].GetName() == "E":
                    val = E
                if nt.GetListOfLeaves()[n].GetName() == "x":
                    val = nt.x + lam * nt.px
                if nt.GetListOfLeaves()[n].GetName() == "y":
                    val = nt.y + lam * nt.py
                if nt.GetListOfLeaves()[n].GetName() == "z":
                    val = options.z + SND_Z
                column.append(val)
            theTuple = array("d", column)
    ...
    for pid in mutype:
        for t in target:
            ...
            for P in pmom:
                ...
                # Histograms.
                tag = str(pid) + t + str(P)
                ut.bookHist(h, "Qhist" + tag, "Q;[GeV]", 100, 0.0, 50.0)
                ut.bookHist(
                    h, "pTehist" + tag, "pT of scattered muon;[GeV]", 100, 0.0, 50.0
                )
                ut.bookHist(h, "xhist" + tag, "x", 100, 0.0, 1.0)
                ut.bookHist(h, "yhist" + tag, "y", 100, 0.0, 1.0)
                ut.bookHist(
                    h, "pTrhist" + tag, "pT of radiated parton; [GeV]", 100, 0.0, 50.0
                )
                ut.bookHist(
                    h, "pTdhist" + tag, "ratio pT_parton/pT_muon", 100, 0.0, 5.0
                )
                ut.bookHist(h, "nMult" + tag, "particle multiplicity", 50, -0.5, 49.5)
                ut.bookHist(
                    h, "nMultcha" + tag, "charged particle multiplicity", 50, -0.5, 49.5
                )
                ut.bookHist(
                    h, "nMultneu" + tag, "neutral particle multiplicity", 50, -0.5, 49.5
                )
                #
                ...
                for n in range(nstat):
                    ...
                    pProton = ROOT.Pythia8.Vec4(
                        myPythia.GetP(2, 1),
                        myPythia.GetP(2, 2),
                        myPythia.GetP(2, 3),
                        myPythia.GetP(2, 4),
                    )
                    peIn = ROOT.Pythia8.Vec4(
                        myPythia.GetP(1, 1),
                        myPythia.GetP(1, 2),
                        myPythia.GetP(1, 3),
                        myPythia.GetP(1, 4),
                    )
                    peOut = ROOT.Pythia8.Vec4(
                        myPythia.GetP(3, 1),
                        myPythia.GetP(3, 2),
                        myPythia.GetP(3, 3),
                        myPythia.GetP(3, 4),
    ...
    for pid in mutype:
        for g in ["p", "n"]:
            ...
            for P in pmom:
                ...
                if g == "p":
                    generators[g].settings.mode("Beams:idB", 2212)
                else:
                    generators[g].settings.mode("Beams:idB", 2112)
                ...
                generators[g].settings.parm("Beams:eB", 0.0)
...
def makeMuDISEvents(withElossFunction=False, nucleon="p+"):
    ...
    for k in range(options.nStart, nEnd):
        ...
        escale = Peloss / p
        muPart = ROOT.TParticle(
            pid, 0, 0, 0, 0, 0, px * escale, py * escale, pz * escale, E, x, y, z, t
        )
        muPart.SetWeight(w)
        ...
        for n in range(options.nMult):
            ...
            for itrk in range(1, myPythia.GetN() + 1):
                ...
                dPart[nPart] = part
            nMade += 1
            if nMade % options.heartbeat == 0:
                print("made so far  ", options.run, " :", nMade, time.ctime())
            dTree.Fill()
    fout.cd()
    dTree.Write()
...
def checkProdofMuDIS():
    ...
    for r in range(1, 11):
        for z in [0, 1000]:
            f = ROOT.TFile(
                "/home/truf/ubuntu-1710/ship-ubuntu-1710-64/SND/MuonDis/Muons Extended Scoring Plane/muonDis_"
                + str(z + r)
                + ".root"
            )
            for sTree in f.DIS:
    ...
    for x in ["13", "-13"]:
        ...
        h["xy_mu_" + x].SetTitle(
            PDG.GetParticle(int(x)).GetName() + " ;x  [cm]; y  [cm]"
        )
...
def compMuDIS_P6withP8():
    ...
    for x in ["6", "8"]:
        ...
        for aHist in h["p" + x]:
            for t in [
                ...
                if aHist.find(t) < 0:
                    continue
                h["p" + x][aHist].Scale(1.0 / h["p" + x][aHist].GetEntries())
        ...
        for t in ["p", "n"]:
            for g in ["p6", "p8"]:
                ...
                if t == "p" and g == "p6":
                    tx = "p+"
                h["g_" + g + pid + t] = (
                    f[g].Get("g_" + pid + tx).Clone("g_" + g + pid + t)
                )
                ...
                if pid == "13":
                    h["g_" + g + pid + t].SetMarkerStyle(23)
                if pid == "-13":
                    h["g_" + g + pid + t].SetMarkerStyle(22)
    ut.bookCanvas(h, "sec", "xsec", 900, 600, 1, 1)
    ut.bookHist(h, "muDISXsec", ";E  [GeV];#sigma [mb]", 100, 0.0, 10000.0)
    h["sec"].cd(1)
    h["muDISXsec"].SetMinimum(5e-5)
    h["muDISXsec"].SetMaximum(30.0e-3)
    ...
    T.DrawLatexNDC(
        0.5, 0.45, "mean P6: %5.2F" % (h["p6"]["nMultcha13p+" + E].GetMean())
    )
    ...
    T.DrawLatexNDC(
        0.5, 0.45, "mean P6: %5.2F" % (h["p6"]["nMultneu13p+" + E].GetMean())
    )
    ...
    h["p6"]["Qhist13p+500.0"].GetXaxis().SetRangeUser(0.0, 12.0)
...
def analyze(inFile):
    ...
    for pid in ["13", "-13"]:
        ...
        ut.bookHist(h, "inMu_" + str(pid), "Energy", 200, 0.0, 5000.0, 20, 0.0, 19.5)
        ut.bookHist(
            h, "xy_In" + str(pid), " x/y muon In", 120, -60.0, 60.0, 120, -60.0, 60.0
        )
    ut.bookCanvas(h, "sec", "xsec", 900, 600, 1, 1)
    ut.bookHist(h, "muDISXsec", ";E  [GeV];#sigma [mb]", 100, 0.0, 10000.0)
    h["sec"].cd(1)
    h["muDISXsec"].SetMinimum(0.0)
    h["muDISXsec"].SetMaximum(30.0e-3)
    h["muDISXsec"].Draw()
    h["muDISXsec"].SetStats(0)
    ...
    for nt in sTree:
        ...
        pid = muon.GetPdgCode()
        wLHC = (
            5.83388 * muon.GetWeight() / options.nMult / 10.0
        )  # ( options.nMult = number of cycles, 10 events per incoming muon in each cycle)
        wDis = (
            NinteractionLength
            * 97.5
            / 1.67e-24
            * h["g_" + str(pid)].Eval(muonEnergy)
            * 1e-27
        )
        out = nt.Particles
        ...
        for p in out:
            ...
            hname = "E_" + str(pid)
            if not hname in h:
                ut.bookHist(h, hname, "Energy", 1000, 0.0, 5000.0)
                ut.bookHist(
                    h,
                    "2dEsnd_" + str(pid),
                    "E in SND",
                ...
                ut.bookHist(h, "test1_" + str(pid), "E in SND", 100, 0.0, 5000.0)
                ut.bookHist(h, "test2_" + str(pid), "E in SND", 100, 0.0, 5000.0)
                ut.bookHist(
                    h,
                    "origin_xy" + str(pid),
                    "E in SND",
                ...
                ut.bookHist(
                    h, "Veto_mult_" + str(pid), "veto multiplicity", 100, 0.0, 100.0
                )
                ut.bookHist(
                    h, "xy_" + str(pid), "x/y ", 120, -60.0, 60.0, 120, -60.0, 60.0
                )
            ...
            if -8 > xex and xex > -49 and 15 < yex and yex < 57:
                if pid in [130, 2112, -2112]:
                    neutralParticles.append([pid, E, muonEnergy, wDis * wLHC])
        ...
        for x in neutralParticles:
            ...
            if len(chargedParticles) > 0:
                rc = h["origin_xy" + str(x[0])].Fill(mxex, myex, x[3])
            if not veto and len(neutralParticles) == 1:
                rc = h["2dEsnd_Veto_" + str(x[0])].Fill(x[1], x[2], x[3])
        if len(neutralParticles) > 0:
            rc = newTree.Fill()
    ...
    for pid in parts:
        ...
        h["Esnd_Veto_" + str(pid)] = h["2dEsnd_Veto_" + str(pid)].ProjectionX(
            "Esnd_Veto_" + str(pid)
        )
        ut.makeIntegralDistrib(h, hname)
        h["I-" + hname].GetXaxis().SetRangeUser(0.0, 1000.0)
    ...
    for k in range(len(parts)):
        ...
        print(
            "                        %s:   %5.2F N/sec   %5.2F  N fb  with Veto: %5.2F  N fb "
            % (pname, R, R * fbScale, RVeto * fbScale)
        )
        R = hist.GetBinContent(3)
        RVeto = histVeto.GetBinContent(3)
        print(
            "E>10GeV,    %s:  %5.2F N/sec   %5.2F  N fb  with Veto: %5.2F  N fb "
            % (pname, R, R * fbScale, RVeto * fbScale)
        )
        R = hist.GetBinContent(21)
        RVeto = histVeto.GetBinContent(21)
        print(
            "E>100GeV, %s:  %5.2E N/sec  %5.2F N fb  with Veto: %5.2F  N fb "
            % (pname, R, R * fbScale, RVeto * fbScale)
        )
        R = hist.GetBinContent(41)
        RVeto = histVeto.GetBinContent(41)
        print(
            "E>200GeV, %s:  %5.2E N/sec  %5.2F N fb  with Veto: %5.2F  N fb "
            % (pname, R, R * fbScale, RVeto * fbScale)
        )
        R = hist.GetBinContent(101)
        RVeto = histVeto.GetBinContent(101)
        print(
            "E>500GeV, %s:  %5.2E N/sec  %5.2F N fb  with Veto: %5.2F  N fb "
            % (pname, R, R * fbScale, RVeto * fbScale)
        )
...
def muonRateAtSND(withFaser=False, withEff=False, version=1):
    ...
    if withEff:
        ...
        fname = "pol4"
        rc = h["efficiency"].Fit(fname, "S", "", 20, 300.0)
    ...
    for mu in fnames:
        ...
        ut.bookHist(h, "Esco_" + str(mu), "E in SND", 1000, 0.0, 5000.0)
        ut.bookHist(h, "Esnd_" + str(mu), "E in SND", 1000, 0.0, 5000.0)
        ut.bookHist(h, "EsndMS_" + str(mu), "E in SND", 1000, 0.0, 5000.0)
        ut.bookHist(h, "Efaser_" + str(mu), "E in faser", 1000, 0.0, 5000.0)
        ut.bookHist(h, "EfaserMS_" + str(mu), "E in faser", 1000, 0.0, 5000.0)
        ut.bookHist(
            h,
            "tanThetaXY_" + str(mu),
            "tan theta X/Y",
        ...
        ut.bookHist(h, "theta", "mult scattering angle", 200, -1.0, 1.0)
        for sTree in fin.nt:
            px, py, pz = sTree.px, sTree.py, sTree.pz
            m2cm = 1.0
            if version == 0:
            ...
            x, y, z = (
                ...
                sTree.y,
                sTree.z * m2cm,
            )  # how could have this happened? used u.m for conversion!
            ...
            for r in range(Rmult):
                ...
                myexMS = myex
                if Rmult > 1:
                    thetaX = rnr.Gaus(0.0, h["MS"].Eval(p))
                    thetaY = rnr.Gaus(0.0, h["MS"].Eval(p))
                ...
                rc = h["tanThetaXYSND_" + str(mu)].Fill(
                    tanThetaX, tanThetaY, wLHC / Rmult
                )
                if Psnd > 0:
        ...
        for hname in [
            ...
            ROOT.gROOT.cd()
            ut.makeIntegralDistrib(h, hname)
            h["I-" + hname].GetXaxis().SetRangeUser(0.0, 10000.0)
            h["I-" + hname].SetMinimum(1e-6)
            pname = PDG.GetParticle(mu).GetName()
            if hname.find("Esco") == 0:
                print(
                    "at scoring plane    %s, %s:  %5.2F N/sec "
                    % (pname, hname, h["I-" + hname].GetBinContent(1))
                )
            else:
                print(
                    "at SND dE=-30GeV    %s, %s:  %5.2F N/sec "
                    % (pname, hname, h["I-" + hname].GetBinContent(1))
                )
    ...
    for Y in ["xyMSW_", "xysco_"]:
        for mu in fnames:
            ...
            if version == 0:
                h[Y + str(mu)].GetXaxis().SetRangeUser(-60.0, 60.0)
                h[Y + str(mu)].GetYaxis().SetRangeUser(-60.0, 60.0)
            else:
                # h[Y+str(mu)].SetMaximum(10)
                h[Y + str(mu)].GetXaxis().SetRangeUser(-80.0, 80.0)
                h[Y + str(mu)].GetYaxis().SetRangeUser(-80.0, 80.0)
            if mu < 0:
                h[Y + str(mu)].SetTitle("#mu^{+}")
            if mu > 0:
                h[Y + str(mu)].SetTitle("#mu^{-}")
    ...
    print("mu+ and mu- at SND: %5.2F N/sec      %5.2F  Hz/cm2" % (R, R / (41.0 * 42.0)))
    R = h["I-EfaserMS_13"].GetBinContent(1) + h["I-EfaserMS_-13"].GetBinContent(1)
    print(
        "mu+ and mu- at Faser: %5.2F N/sec      %5.2F  Hz/cm2" % (R, R / (24.0 * 24.0))
    )
...
def flukaMuons(version=1, Plimit=False, withFaser=True):
    ...
    if version == 0:
        fnames = {
            13: path + "muons_up/version0/unit30_Nm.root",
            -13: path + "muons_up/version0/unit30_Pm.root",
        }
    if version == 1:
        if Plimit:
            pMin = 27.0
        fnames = {
            13: path + "muons_up/version1/unit30_Nm.root",
            -13: path + "muons_up/version1/unit30_Pm.root",
        }
    if version == 3:
        if Plimit:
            pMin = 27.0
    ...
    for mu in [13, -13]:
        ...
        ut.bookHist(h, "E_" + str(mu), ";E [GeV]; N / 5 GeV", 1000, 0.0, 5000.0)
        ut.bookHist(h, "W_" + str(mu), "w", 100, 0.0, 0.15)
    for mu in fnames:
        ...
        for sTree in fin.nt:
            ...
            if abs(sTree.x) > 50.0 or abs(sTree.y) > 50.0:
                rc = h["xySNDX_" + str(pid)].Fill(Xex[0], Xex[1], wLHC)
    #
    for Y in ["xySCO_", "xySND_", "xySNDX_"]:
        if version == 0 and Y == "xySNDX_":
            continue
        k = 0
        for mu in [13, -13]:
            k += 1
            h[Y + str(mu)].SetStats(0)
            h[Y + str(mu)].SetMaximum(5.0)
            if mu < 0:
                h[Y + str(mu)].SetTitle("#mu^{+}")
            if mu > 0:
                h[Y + str(mu)].SetTitle("#mu^{-}")
    ...
    for mu in fnames:
        ...
        stats[mu]["1x1"] = (
            h["xySND_" + str(mu)].GetSumOfWeights()
            - h["xySNDX_" + str(mu)].GetSumOfWeights()
        )
    for mu in fnames:
        print("-------- " + PDG.GetParticle(mu).GetName() + "   ----------")
        if version == 1 and 0 in h["stats"]:
            print(
                "increae of rates, total: %5.2F    absolute(1x1) %5.2F   new/old=%5.2F"
                % (
                    stats[mu]["1x1"] / stats[mu]["total"],
                    stats[mu]["1x1"],
                    stats[mu]["1x1"] / h["stats"][0][mu]["1x1"],
                )
            )
            newOvOld = (stats[mu]["snd"][0] - stats[mu]["snd"][1]) / (
                h["stats"][0][mu]["snd"][0] - h["stats"][0][mu]["snd"][1]
            )
            newOvOldTOT = stats[mu]["snd"][0] / h["stats"][0][mu]["snd"][0]
            print(
                "increae of rates, SND: %5.2F     absolute(1x1) %5.2F   new/old=%5.2F   new/old(tot)=%5.2F"
                % (
                    stats[mu]["snd"][1] / stats[mu]["snd"][0],
                    stats[mu]["snd"][0] - stats[mu]["snd"][1],
                    newOvOld,
                    newOvOldTOT,
                )
            )
            newOvOld = (stats[mu]["fas"][0] - stats[mu]["fas"][1]) / (
                h["stats"][0][mu]["fas"][0] - h["stats"][0][mu]["fas"][1]
            )
            newOvOldTOT = stats[mu]["fas"][0] / h["stats"][0][mu]["fas"][0]
            print(
                "increae of rates, FAS: %5.2F      absolute(1x1) %5.2F   new/old=%5.2F   new/old(tot)=%5.2F"
                % (
                    stats[mu]["fas"][1] / stats[mu]["fas"][0],
                    stats[mu]["fas"][0] - stats[mu]["fas"][1],
                    newOvOld,
                    newOvOldTOT,
                )
            )
            ...
            print(
                "increae of rates, total: %5.2F    absolute(1x1) %5.2F"
                % (
                    h["xySNDX_" + str(mu)].GetSumOfWeights()
                    / h["xySND_" + str(mu)].GetSumOfWeights(),
                    h["xySND_" + str(mu)].GetSumOfWeights()
            ...
            print(
                "increae of rates, SND: %5.2F     absolute(1x1) %5.2F"
                % (
                    stats[mu]["snd"][1] / stats[mu]["snd"][0],
                    stats[mu]["snd"][0] - stats[mu]["snd"][1],
                )
            ...
            print(
                "increae of rates, FAS: %5.2F      absolute(1x1) %5.2F"
                % (
                    stats[mu]["fas"][1] / stats[mu]["fas"][0],
                    stats[mu]["fas"][0] - stats[mu]["fas"][1],
                )
    ...
    if version == 1 and 0 in h["stats"]:
        for z in ["snd", "fas"]:
            ...
            print(
                "%s: %5.2F /cm2/s    increase: %5.2F "
                % (z, sumMu / h["stats"][1][13][z][2], increase)
            )
...
def muInterGeant4(version=2, njobs=100):
    ...
    if version == 2:
        g = ROOT.TFile("muGeant4_0/geofile_full.conical.Ntuple-TGeant4.root")
    else:
        g = ROOT.TFile("muMinusGeant4_0/geofile_full.conical.Ntuple-TGeant4.root")
    geoManager = g.FAIRGeom
    for subjob in range(njobs):
        if version == 2:
            muons = ["muGeant4_" + str(subjob) + "/ship.conical.Ntuple-TGeant4.root"]
        else:
            muons = [
                "muMinusGeant4_" + str(subjob) + "/ship.conical.Ntuple-TGeant4.root",
                "muPlusGeant4_" + str(subjob) + "/ship.conical.Ntuple-TGeant4.root",
            ]
        pids = {}
        for fname in muons:
            f = ROOT.TFile(fname)
            ...
            for sTree in f.cbmsim:
                ...
                for p in sTree.vetoPoint:
                    ...
                    if not pid in pids:
                        pids[pid] = 0
                    pids[pid] += 1
                    if pid in uninterestingParticles:
                        continue
                    ...
                    rc = h["xyz_origin_" + str(pid)].Fill(
                        exitPoint[0], exitPoint[1], exitPoint[2], w
                    )
                    while track > 0:
                    ...
                    if track > 0:
                        ...
                        rc = h["xyz_muInter_" + str(pid)].Fill(
                            m.GetStartX(), m.GetStartY(), m.GetStartZ(), w
                        )
...
def muondEdX(version=2, njobs=100, path="", withFaser=False, plotOnly=True):
    ...
    if not plotOnly:
        ...
        ut.bookHist(h, "eloss", "energyLoss", 500, 0.0, 5000.0, 500, 0.0, 500.0)
        ut.bookHist(
            h, "dx", "multiple scattering x", 500, 0.0, 5000.0, 1000, -0.02, 0.02
        )
        ut.bookHist(
            h, "dy", "multiple scattering y", 500, 0.0, 5000.0, 1000, -0.02, 0.02
        )
        ut.bookHist(h, "oE", "original energy", 500, 0.0, 5000.0)
        ut.bookHist(h, "oL", "flight path", 100, 0.0, 10000.0)
        ut.bookHist(
            h,
            "3d",
            "exit points",
        ...
        ut.bookHist(h, "z", "last z ", 1000, -1000.0, 1000.0)
        ut.bookHist(
            h, "SNDmuP", "SND entry points", 200, -100.0, 100.0, 200, -100.0, 100.0
        )
        ut.bookHist(
            h, "SNDmuM", "SND entry points", 200, -100.0, 100.0, 200, -100.0, 100.0
        )
        ut.bookHist(h, "SNDmuP_E", "SND muon enery", 200, 0.0, 5000.0)
        ut.bookHist(h, "SNDmuM_E", "SND muon energy", 200, 0.0, 5000.0)
        ut.bookHist(h, "allmuE_13", "all muon enery", 200, 0.0, 5000.0)
        ut.bookHist(h, "allmuE_-13", "all muon energy", 200, 0.0, 5000.0)
        ut.bookHist(
            h,
            "SNDMuFiltermuP",
            "SND entry points",
        ...
        for p in neuPartList:
            ut.bookHist(h, "E_" + str(p), "Energy", 500, 0.0, 5000.0)

        if version == 3:
            g = ROOT.TFile(
                path + "muGeant4_VCdown_0/geofile_full.conical.Ntuple-TGeant4.root"
            )
        elif version == 2:
            g = ROOT.TFile(path + "muGeant4_0/geofile_full.conical.Ntuple-TGeant4.root")
        else:
            g = ROOT.TFile(
                path + "muMinusGeant4_0/geofile_full.conical.Ntuple-TGeant4.root"
            )
        geoManager = g.FAIRGeom
        for subjob in range(njobs):
            if version == 3:
            ...
            for fname in muons:
                ...
                for sTree in h[fname].cbmsim:
                    ...
                    for x in sTree.MCTrack:
                        pid = x.GetPdgCode()
                        if pid in neuPartList:
                            rc = h["E_" + str(pid)].Fill(x.GetEnergy())
                    trajectories = {}
                    for p in sTree.vetoPoint:
                        track = p.GetTrackID()
                        if track < 0:
                            continue  # trajectory not stored
                        if track != 0:
                            continue  # only take original muon for the moment

                        if not track in trajectories:
                            trajectories[track] = {}
                        traj = trajectories[track]
                        end = p.LastPoint()
                        middle = ROOT.TVector3(p.GetX(), p.GetY(), p.GetZ())
                        first = 2 * middle - end
                        Pin = ROOT.Math.PxPyPzMVector(
                            p.GetPx(), p.GetPy(), p.GetPz(), muonMass
                        )
                        Pout = ROOT.Math.PxPyPzMVector(
                            p.LastMom()[0], p.LastMom()[1], p.LastMom()[2], muonMass
                        )
                        if not Pout.Z() > 0:
                            enterCave = False
                            if (
                                sTree.ScifiPoint.GetEntries() > 0
                                or sTree.MuFilterPoint.GetEntries() > 0
                                and end.z() < -24.5
                            ):  # otherwise muon stops in or after SND
                                print(
                                    "something strange here, pout.z<0",
                                    nEv,
                                    fname,
                                    sTree.ScifiPoint.GetEntries(),
                                    sTree.MuFilterPoint.GetEntries(),
                                )
                                p.Print()
                        else:
                            test = end + 1.0 / Pout.Z() * p.LastMom()
                            node = geoManager.FindNode(test[0], test[1], test[2])
                            enterCave = (
                                node.GetName().find("Vrock") < 0
                            )  # particle leaving concrete and entering rock
                    ...
                    for track in trajectories:
                        ...
                        for z in zPos:
                            ...
                            start = ROOT.TVector3(
                                sTree.MCTrack[0].GetStartX(),
                                sTree.MCTrack[0].GetStartY(),
                                sTree.MCTrack[0].GetStartZ(),
                            )
                            L = end - start
        ...
        for det in ["SND", "SNDMuFilter"]:
            ...
            for ix in range(1, nx + 1):
                if projx.GetBinContent(ix) > 0:
                    xmin = projx.GetBinCenter(ix) - projx.GetBinWidth(ix) / 2.0
                    break
            for ix in range(nx, 0, -1):
                if projx.GetBinContent(ix) > 0:
                    xmax = projx.GetBinCenter(ix) + projx.GetBinWidth(ix) / 2.0
                    break
            for iy in range(1, ny + 1):
                if projy.GetBinContent(iy) > 0:
                    ymin = projy.GetBinCenter(iy) - projy.GetBinWidth(iy) / 2.0
                    break
            for iy in range(ny, 0, -1):
                if projy.GetBinContent(iy) > 0:
                    ymax = projy.GetBinCenter(iy) + projy.GetBinWidth(iy) / 2.0
                    break
            sqcm = (xmax - xmin) * (ymax - ymin)
            muP = h[det + "muP"].GetSumOfWeights()
            muM = h[det + "muM"].GetSumOfWeights()
            print(
                "%s       square:  %5.2F,%5.2F,%5.2F,%5.2F"
                % (det, xmin, xmax, ymin, ymax)
            )
            print("         mu+ rate = %5.2F Hz    %5.2F Hz/cm2" % (muP, muP / sqcm))
            print("         mu-  rate = %5.2F Hz    %5.2F Hz/cm2" % (muM, muM / sqcm))
            print(
                "         sum  rate = %5.2F Hz    %5.2F Hz/cm2"
                % (muM + muP, (muM + muP) / sqcm)
            )
        ...
        fname = "pol3"
        h["eloss_mean"].Fit(fname, "S", "", 20.0, 5000.0)
        tc.SetLogx(1)
        h["eloss_mean"].SetTitle(
        ...
        eff.GetXaxis().SetRangeUser(0.0, 500.0)
        eff.Draw()
        ...
        tc.SetLogy(1)
        msDxE.GetXaxis().SetRangeUser(0.0, 1000.0)
        msDxE.Draw("hist")
        rc = msDxE.Fit(h["MS"], "S", "", 20.0, 400.0)
        tc.Update()
        ...
        for mu in [13, -13]:
            ...
            for z in ["snd", "fas"]:
                ...
                stats[mu][z].append(sqcm)
                print(
                    "%s, %s:  total = %5.2F  Hz     %5.2F  Hz/cm2 "
                    % (mu, z, stats[mu][z][0], stats[mu][z][0] / stats[mu][z][1])
                )
        ...
        for pid in L:
            ...
            hname = L[pid] + "_inter"
            if pid == "13":
                h[L[pid] + "_E"].SetLineColor(ROOT.kRed)
            else:
                h[L[pid] + "_E"].SetLineColor(ROOT.kBlue)
            ...
            for k in range(1, h[hname].GetNbinsX() + 1):
                ...
                wDis = (
                    5.9 * 19.3 / 1.67e-24 * h["g_" + str(pid)].Eval(muonEnergy) * 1e-27
                )
...
def drawMuon3D(fname="muondEdX.root", hname="3d", gDir="muMinusGeant4_0"):
    ...
    top = h["top"]
    nx, ny, nz = (
        h[hname].GetXaxis().GetNbins(),
        h[hname].GetYaxis().GetNbins(),
        h[hname].GetZaxis().GetNbins(),
    )
    boxes = []
        ...
        for iy in range(1, ny):
            for iz in range(1, nz):
                ...
                if not C > 0:
                    continue
                x, y, z = (
                    h[hname].GetXaxis().GetBinCenter(ix),
                    h[hname].GetYaxis().GetBinCenter(iy),
                    h[hname].GetZaxis().GetBinCenter(iz),
                )
...
def plotMuDisCrossSection():
    ...
    ut.bookCanvas(h, "sec", "xsec", 900, 600, 1, 1)
    ut.bookHist(h, "muDISXsec", ";E  [GeV];#sigma [mb]", 100, 0.0, 10000.0)
    ut.bookHist(h, "muDISXsecA", ";A ; #sigma [mb]", 100, 0.0, 100.0)
    h["sec"].cd(1)
    h["muDISXsec"].SetMinimum(0.0)
    h["muDISXsec"].SetMaximum(30.0e-3)
    h["muDISXsecA"].SetMinimum(0.0)
    h["muDISXsecA"].SetMaximum(3000.0e-3)
    h["muDISXsec"].Draw()
    h["muDISXsec"].SetStats(0)
    h["g_13"].SetLineColor(ROOT.kGreen)
    ...
    h["lxsec"].AddEntry(
        h["AnalyticCross"], "analytic cross section (Bezrukov and Bugaev, A=1)", "PL"
    )
...
def muonDISfull(
    ...
    if not pythia6:
        ...
        if cycle == 100:
            datafile = "/ship.conical.Ntuple-TGeant4_boost100.0.root"
    ...
    for pid in ["13", "-13"]:
        h["g_" + pid] = fin.Get("g_" + pid).Clone("g_" + pid)
        ut.bookHist(h, "wDIS_" + pid, "muon DIS probability", 100, 0.0, 0.1)
        for z in ["0", "inter"]:
            ut.bookHist(
                h,
                "inMu_" + z + str(pid),
                "muon Energy vs #direct hits",
                200,
                0.0,
                5000.0,
                20,
    ...
    for p in neuPartList:
        for o in ["", "_secondary"]:
            ut.bookHist(
                h, "inE_" + str(p) + o, "energy", 500, 0.0, 5000.0, 600, -500.0, 100.0
            )  # end vertex of primary neutral particle
            ut.bookHist(
                h,
                "inE_Concrete_" + str(p) + o,
                "energy",
                500,
                0.0,
                5000.0,
                ...
                -500.0,
                100.0,
            )  # end vertex of primary neutral particle if in concrete
        ut.bookHist(h, "E_" + str(p), "Energy", 500, 0.0, 5000.0, 200, -30.0, 170.0)
        ut.bookHist(
            h, "E_veto_" + str(p), "Energy", 500, 0.0, 5000.0, 200, -30.0, 170.0
        )
        ut.bookHist(
            h, "E_vetoParticle_" + str(p), "Energy", 500, 0.0, 50.0, 20, -0.5, 19.5
        )
        ut.bookHist(h, "Eprim_" + str(p), "Energy", 500, 0.0, 5000.0, 200, -30.0, 170.0)
        ut.bookHist(
            h, "Eprim_veto_" + str(p), "Energy", 500, 0.0, 5000.0, 200, -30.0, 170.0
        )
        ut.bookHist(h, "startZ_" + str(p), "z", 200, -100.0, 300.0)
        ut.bookHist(
            h,
            "xyz_Inter_" + str(p),
            "x/y/z ",
    ...
    if path.find("eos") < 0:
        g = ROOT.TFile.Open(path + geofileExample)
    else:
        g = ROOT.TFile.Open(os.environ["EOSSHIP"] + path + geofileExample)
    geoManager = g.FAIRGeom
    if path.find("eos") < 0:
        ...
        topDir = str(
            subprocess.check_output(
                "xrdfs " + os.environ["EOSSHIP"] + " ls -l " + path, shell=True
            )
        )
    ...
    for run in range(rMin, rMax):
        for k in muRange:
            for subjob in range(sMin, sMax):
                if pythia6:
                    if options.Emin == 0:
                        prod = "run_" + str(run + cycle * 100 + k) + "_" + str(subjob)
                    else:
                        prod = (
                            "ecut"
                            + str(options.Emin)
                ...
                if path.find("eos") < 0:
                    ...
                    temp = subprocess.check_output(
                        "xrdfs " + os.environ["EOSSHIP"] + " ls -l " + path + prod,
                        shell=True,
                    )
                    if not geofile in str(temp):
                        ...
                        continue
                if pythia6:
                    rc = h["fullStats"].Fill(run + cycle * 100 + k)
                if path.find("eos") < 0:
                    h["f"] = ROOT.TFile.Open(path + prod + datafile)
                else:
                    h["f"] = ROOT.TFile.Open(
                        os.environ["EOSSHIP"] + path + prod + datafile
                    )
                nEv = -1
                for sTree in h["f"].cbmsim:
                    ...
                    muInterVx = ROOT.TVector3(
                        muon.GetStartX(), muon.GetStartY(), muon.GetStartZ()
                    )
                    if pythia6:
                        ...
                        wInter = sTree.MCTrack[
                            2
                        ].GetWeight()  # put density along trajectory as weight, g/cm^2
                        # fast MC used NinteractionLength   * 97.5 g cm-2 interaction length of concrete  --> wInter
                        wDis = (
                            wInter
                            / 1.67e-24
                            * h["g_" + str(mupid)].Eval(muonEnergy)
                            * 1e-27
                        )
                    ...
                    rc = h["xyz_mu_0" + str(mupid)].Fill(
                        muon.GetStartX(), muon.GetStartY(), muon.GetStartZ(), W
                    )
                    ...
                    for m in sTree.MCTrack:
                        ...
                        if Apid in neuPartList:
                            ...
                            nodeOrigin = geoManager.FindNode(
                                m.GetStartX(), m.GetStartY(), m.GetStartZ()
                            )
                            om = nodeOrigin.GetName()
                            if not (
                                om == "VTI18_1" or om == "Vrock_1" or om == "cave_1"
                            ):
                                continue
                                # find end vertex:
                            endZ = 999.0
                            ...
                            if n in daughterOf:
                                d = daughterOf[n]
                                endZ = d.GetStartZ()
                                nodeInter = geoManager.FindNode(
                                    d.GetStartX(), d.GetStartY(), d.GetStartZ()
                                )
                                if (
                                    nodeInter.GetName() == "VTI18_1"
                                    or nodeInter.GetName() == "Vrock_1"
                                ):
                                    concrete = True
                            if not endZ < 999.0:
                                continue
                            ...
                            if m.GetProcID() != 0:
                                origin = "_secondary"
                            rc = h["inE_" + str(m.GetPdgCode()) + origin].Fill(
                                m.GetEnergy(), endZ, W
                            )
                            if concrete:
                                rc = h[
                                    "inE_Concrete_" + str(m.GetPdgCode()) + origin
                                ].Fill(m.GetEnergy(), endZ, W)
                            elif (
                                debug > 2
                                and m.GetEnergy() > 75.0
                                and endZ > -25.0
                                and endZ < 25.0
                                and Apid == 130
                            ):
                    ...
                    if sTree.ScifiPoint.GetEntries() < 1:
                        continue
                    if len(neutrals) < 1:
                        continue
                    ...
                    for x in neutrals:
                        ...
                        if E > eMax:
                            eMax, neu = E, x
                    if debug > 3:
                        print("debug", len(neutrals), E, sTree.MCTrack[x])
                        # look for veto hit
                        # special case, low energy neutrons can also make a hit.
                    vetoPoint, zMinCharged = -1, 999.0
                    for det in [
                        sTree.ScifiPoint,
                        sTree.MuFilterPoint,
                    ]:  # also loop over MuFilterPoint to get hits in veto station
                        nP = -1
                        for p in det:
                            nP += 1
                    ...
                    rc = h["xyz_Inter_" + str(pid)].Fill(
                        neutInterVx[0], neutInterVx[1], neutInterVx[2], W
                    )
                    if E > 100.0:
                        rc = h["xyzE100_Inter_" + str(pid)].Fill(
                            neutInterVx[0], neutInterVx[1], neutInterVx[2], W
                        )
                    rc = h["xyz_origin_" + str(pid)].Fill(
                        Npart.GetStartX(), Npart.GetStartY(), Npart.GetStartZ(), W
                    )
                    rc = h["xyz_muInter_" + str(pid)].Fill(
                        muInterVx.X(), muInterVx.Y(), muInterVx.Z(), W
                    )
                    if veto:
                        rc = h["xyzVeto_Inter_" + str(pid)].Fill(
                            neutInterVx[0], neutInterVx[1], neutInterVx[2], W
                        )
                        ...
                        rc = h["E_vetoParticle_" + str(pid)].Fill(
                            P.Mag(), pidsDictRev[pidVeto], W
                        )
                    ...
                    if Npart.GetProcID() == 0:
                        if veto:
                            rc = h["Eprim_veto_" + str(pid)].Fill(E, neutInterVx[2], W)
                        else:
                            rc = h["Eprim_" + str(pid)].Fill(E, neutInterVx[2], W)
                    #
                    # check for particles in the first muon station to study about increased veto detector
                    for p in sTree.MuFilterPoint:
                        if p.GetZ() < 50.0 and p.GetZ() > 0:
                            track = p.GetTrackID()
                            if track < 0:
                                continue
                            V = sTree.MCTrack[track]
                            if V.GetStartZ() < -35:
                                rc = h["xy_z-30_" + str(pid)].Fill(
                                    p.GetX(), p.GetY(), W
                                )
                                if veto:
                                    rc = h["xy_z-30_veto_" + str(pid)].Fill(
                                        p.GetX(), p.GetY(), W
                                    )
                h["f"].Close()
...
def thermNeutron():
    ...
    f = ROOT.TFile.Open(
        "root://eospublic.cern.ch//eos/experiment/ship/user/truf/SND/muonDis/run_1_0/ship.conical.muonDIS-TGeant4.root"
    )
    ...
    for sTree in f.cbmsim:
        ...
        for m in sTree.MCTrack:
            if abs(m.GetPdgCode()) == 2112:
                m.Get4Momentum(L)
                logE = ROOT.TMath.Log10(1000.0 * (L.Energy() - L.M()))
...
def analyzeDIS(
    ...
    for k in range(len(parts)):
        ...
        h["inE_seco_" + str(pid)] = h["inE_" + str(pid) + "_secondary"].Clone(
            "inE_" + str(pid) + "_secondary_out"
        )
        h["inE_seco_" + str(pid)].Add(h["inE_Concrete_" + str(pid) + "_secondary"], -1)
        for x in ["prim_", "seco_"]:
            zmin = h["inE_" + x + str(pid)].GetYaxis().FindBin(-25.0)
            zmax = h["inE_" + x + str(pid)].GetYaxis().FindBin(+25.0)
    ...
    minMax = {"out_": [9, 1e-6], "Concrete_": [30.0, 1e-5]}
    for k in range(len(parts)):
        pid = parts[k]
    ...
    for c in ["out_", "Concrete_"]:
        ...
        for k in range(len(parts)):
            ...
            for origin in ["_secondary", ""]:
                ...
                if origin == "_secondary":
                    h[hname + "_projz"].Draw("HIST")
                else:
                    h[hname + "_projz"].Draw("HISTsame")
    ...
    for hist in [
        ...
        for pid in parts:
            ...
            if first:
                ...
                h[hist].Add(h[hist + "_" + str(pid)])
    h["inE-noConc"] = h["inE"].Clone("inE-noConc")
    h["inE-noConc"].Add(h["inE_Concrete"], -1.0)
    h["inE-noConc_22"] = h["inE_22"].Clone("inE-noConc_22")
    h["inE-noConc_22"].Add(h["inE_Concrete_22"], -1.0)
    for hist in [
        "xyz_muInter",
        "xyz_Inter",
        "xyzE100_Inter",
        ...
        if hist == "xyzE100_Inter":
            ...
            if h[hist + "_z"].GetEntries() > 10:
                rc = h[hist + "_z"].Fit("expo", "S", "", -25.0, 30.0)
                fitresult = rc.Get()
                txtlam = "#lambda =%4.1Fcm" % (-1.0 / fitresult.GetParams()[1])
                rc = h[hist + "_22_z"].Fit("expo", "SL", "", -30.0, 10.0)
                fitresult = rc.Get()
                funRad = h[hist + "_22_z"].GetFunction("expo")
                funRad.SetBit(ROOT.TF1.kNotDraw)
                funRad.SetLineColor(h[hist + "_22_z"].GetLineColor())
                txtrad = "X_{0}  =%4.1Fcm" % (-1.0 / fitresult.GetParams()[1])
                h[hist + "_z"].GetFunction("expo").SetLineColor(
                    h[hist + "_z"].GetLineColor()
                )
            h[hist + "_z"].Draw()
        ...
        if h[hist].ClassName() == "TH3D":
            for proj in ["xy", "xz", "yz"]:
                ...
                myPrint(
                    h["vertices" + hist + "_" + proj],
                    hist + "_" + proj,
                    pathToPlots=pathToPlots,
                )
    ...
    h["xyz_Inter_z"].GetXaxis().SetRangeUser(-50.0, 100.0)
    ...
    if withMult:
        ...
        mults = {
            ...
            -2112: [23, ROOT.kCyan],
            310: [21, ROOT.kOrange],
            3122: [24, ROOT.kGreen - 1],
            -3122: [25, ROOT.kGreen + 1],
            3322: [26, ROOT.kGreen - 2],
            -3322: [27, ROOT.kGreen + 2],
        }
    ...
    for pid in parts:
        ...
        for o in ["", "prim"]:
            ...
            h["Esnd" + o + "_all_" + str(pid)] = h[
                "E" + o + "_veto_" + str(pid)
            ].ProjectionX("Esnd" + o + "_all_" + str(pid))
    ...
    for k in range(len(parts)):
        ...
        h[hnameScaled].Scale(150.0 * fbScale / runCoverage)
    ...
    myPrint(
        h["muDIS_SNDwithVeto"], "EofNeutralsInSND_vetoApplied", pathToPlots=pathToPlots
    )

    ut.bookCanvas(
        h, "muDIS_SNDnoVetoRequired", "neutrals arriving at SND", 1800, 1200, 3, 3
    )
    for k in range(len(parts)):
        ...
        h[hnameScaled].Draw("hist")
    myPrint(
        h["muDIS_SNDnoVetoRequired"],
        "EofNeutralsInSND_NoVetoApplied",
        pathToPlots=pathToPlots,
    )
    ...
    for o in ["", "prim"]:
        ...
        for k in range(len(parts)):
            ...
            for X in [hname, hnameAll]:
                ...
                h["IFB-" + X].SetTitle(pname + ";> E [GeV/c];N /150 fb^{-1}")
                h["IFB-" + X].GetYaxis().SetTitleOffset(1.4)
                h["IFB-" + X].GetXaxis().SetRangeUser(0.1, 1200.0)
            ...
            for E in [0, 10, 100, 200, 300, 500, 1000]:
                ...
                print(
                    "E>%i GeV,    %s:  %5.2F N/sec   %5.2F  N fb  with Veto: %5.2F  N fb |    %5.2G     veto %5.2G   (%s) "
                    % (
                        E,
                        pname,
                        R,
        ...
        for x in [2112, -2112, 130, 310, 3122, -3122, 3322, 22]:
            ...
            for E in [10, 100, 200, 300, 500, 1000]:
                ...
                if x == 3322:
                    R += (
                        h["I-" + hnameAll.replace("3322", "-3322")].GetBinContent(n)
                        / runCoverage
                    )
                R150 = R * 150.0 * fbScale
                if R150 < 1e-8:
                    R150 = 0
                if R150 < 1000:
                    line += "&  $%5.1F$" % (R150)
        ...
        for x in [2112, -2112, 130, 310, 3122, -3122, 3322, 22]:
            ...
            for E in [10, 100, 200, 300, 500, 1000]:
                ...
                if x == 3322:
                    R += (
                        h["I-" + hname.replace("3322", "-3322")].GetBinContent(n)
                        / runCoverage
                    )
                R150 = R * 150.0 * fbScale
                if R150 < 1e-8:
...
FASERNU[-3322] = {10: 150.0, 100: 10, 300: 1.4, 1000: 0}
scaleFactor = (42.0 * 42.0) / (24.0 * 24.0)
...
def muonDISProduction(cycle, ecut=1.0, strippedEvents=False):
    for run in range(1, 11):
        for k in [0, 1000]:
            prod = str(run + cycle * 100 + k)
            if strippedEvents:
                fn = "muonDis_" + prod + "_stripped.root"
            else:
                fn = "muonDis_" + prod + ".root"
            ...
            while count_python_processes("run_simScript") > ncpus:
                time.sleep(200)
                    ...
                    for P in sTree.Particles:
                        ...
                        if not hname in h:
                            ut.bookHist(
                                h,
                                hname,
                                "Energy " + PDG.GetParticle(P.GetPdgCode()).GetName(),
                                1000,
                                0.0,
                                5000.0,
                            )
    ...
    for cycle in range(10):
        for run in range(1, 11):
            for k in [0, 1000]:
                ...
                f = ROOT.TFile("muonDis_" + str(prod) + ".root")
                sTree = f.DIS
                fstripped = ROOT.TFile(
                    "muonDis_" + str(prod) + "_stripped.root", "recreate"
                )
                newTree = sTree.CloneTree(0)
                for sTree in f.DIS:
                    flag = False
...
def signalNeutrinos():
    ...
    ut.readHists(
        h,
        "/mnt/hgfs/microDisk/CERNBOX/SND@LHC/Neutrinos/SND_Neutrinos_Interacting_CCDIS.root",
    )
    for p in ["", "a"]:
        for nu in ["e", "mu", "tau"]:
            line = ""
    ...
    for ix in range(1, h[hist].GetNbinsX() + 1):
        for iz in range(1, h[hist].GetNbinsZ() + 1):
            ...
            for iy in range(ymin, ymax):
                N += h[hist].GetBinContent(ix, iy, iz)
...
if not options.command.find("muonDIS") < 0:
    if options.pythia6 > 0:
        muonDISfull(
            cycle=options.nMult,
            sMin=options.nStart,
            sMax=options.nStart + options.nEvents,
        )
    else:
        muonDISfull(
            cycle=options.nMult,
...
elif not options.command.find("convert") < 0:
    convertAscii2Root(options.muonIn)
elif not options.command.find("make") < 0:
    makeMuDISEvents(nucleon=options.nucleon)
elif not options.command.find("ana") < 0:
    analyze(options.muonIn)
elif not options.command.find("cross") < 0:
    getPythiaCrossSec(options.nEvents)
elif not options.command.find("muonPreTransport") < 0:
    muonPreTransport()
elif not options.command.find("muon") < 0:
    muonRateAtSND()
