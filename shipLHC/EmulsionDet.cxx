...
void EmulsionDet::Initialize() { FairDetector::Initialize(); }
...
TString EmulsionDet::PathBrickID(Int_t detID)
{
    ...
    TString emulsionpath = TString::Format("/cave_1/Detector_0/volTarget_1/Wall_%i/Row_%i/Brick_%i/Emulsion_%i",
                                           NWall - 1,
                                           NRow - 1,
                                           NColumn - 1,
                                           NPlate - 1);
...
void EmulsionDet::ConstructGeometry()
{
    ...
    for (Int_t n = 0; n < NPlates; n++) {
        volBrick->AddNode(
            volPassive,
            n,
            new TGeoTranslation(0, 0, -BrickZ / 2 + EmPlateWidth + PassiveThickness / 2 + n * AllPlateWidth));   // LEAD
    }
    ...
    for (Int_t n = 0; n < NPlates + 1; n++) {
        volBrick->AddNode(
            volEmulsionFilm, n, new TGeoTranslation(0, 0, -BrickZ / 2 + EmPlateWidth / 2 + n * AllPlateWidth));
    }

    volBrick->SetVisibility(kTRUE);

    // alignment
    double dx_survey[fNWall] = {conf_floats["EmulsionDet/Xpos0"],
                                conf_floats["EmulsionDet/Xpos1"],
                                conf_floats["EmulsionDet/Xpos2"],
                                conf_floats["EmulsionDet/Xpos3"],
                                conf_floats["EmulsionDet/Xpos4"]};
    double dy_survey[fNWall] = {conf_floats["EmulsionDet/Ypos0"],
                                conf_floats["EmulsionDet/Ypos1"],
                                conf_floats["EmulsionDet/Ypos2"],
                                conf_floats["EmulsionDet/Ypos3"],
                                conf_floats["EmulsionDet/Ypos4"]};
    double dz_survey[fNWall] = {conf_floats["EmulsionDet/Zpos0"],
                                conf_floats["EmulsionDet/Zpos1"],
                                conf_floats["EmulsionDet/Zpos2"],
                                conf_floats["EmulsionDet/Zpos3"],
                                conf_floats["EmulsionDet/Zpos4"]};
    ...
    for (int l = 0; l < fNWall; l++) {
        volTarget->AddNode(volWallborder,
                           l,
                           new TGeoTranslation(
                               -dx_survey[l] - XDimension / 2.,
                               dz_survey[l] + YDimension / 2.,
                               dy_survey[l] + TotalWallZDim / 2.));   // the survey points refer to the down-left corner
        volTarget->AddNode(
            volWall,
            l,
            new TGeoTranslation(-dx_survey[l] - XDimension / 2.,
                                dz_survey[l] + YDimension / 2.,
                                dy_survey[l] + TotalWallZDim / 2.
                                    + WallZBorder_offset));   // the survey points refer to the down-left corner
        d_cl_z += BrickZ + TTrackerZ;
    }
    LOG(INFO) << "  EmDet X: " << -dx_survey[0] - XDimension / 2. << "  Y: " << dz_survey[0] + YDimension / 2.
              << "  Z: " << dy_survey[0] + TotalWallZDim / 2. + WallZBorder_offset;
    ...
    if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
        ...
        for (Int_t i = 0; i <= MaxL; i++) {
            ...
            if (strcmp(mumname, "Brick") == 0)
                NColumn = (1 - motherV[i]);   // 0 or 1 (0 higher x, 1 lower x, x are negative, so higher x are closer
                                              // to the beam)
            if (strcmp(mumname, "Row") == 0)
                NRow = motherV[i];   // 0 or 1 (0 lower y, 1 higher y)
            if (strcmp(mumname, "Wall") == 0)
                NWall = motherV[i];   // 0,1,2,3 (increasing along the beam verse)
        }

        detID = (NWall + 1) * 1E4 + (NRow * 2 + NColumn + 1) * 1E3 + (NPlate + 1);
        fVolumeID = detID;
        // found number of row, column and wall
        // if (NColumn > 2 || NRow > 2 || NWall > 5) cout<<"Debug test for detID "<<detID<<" is "<<NColumn<<" "<<NRow<<"
        // "<<NWall<<" "<<NPlate<<endl;
        TParticle *p = gMC->GetStack()->GetCurrentTrack();
        Int_t pdgCode = p->GetPdgCode();
        if (pdgCode == 22) {
            return kFALSE;
        }   // discard only photons
...
void EmulsionDet::EndOfEvent() { fEmulsionDetPointCollection->Clear(); }
...
void EmulsionDet::Register()
{
    ...
    FairRootManager::Instance()->Register("EmulsionDetPoint", "EmulsionDet", fEmulsionDetPointCollection, kTRUE);
}

TClonesArray *EmulsionDet::GetCollection(Int_t iColl) const
{
    if (iColl == 0) {
        return fEmulsionDetPointCollection;
    } else {
        return NULL;
    }
}

void EmulsionDet::Reset() { fEmulsionDetPointCollection->Clear(); }
                                      ...
                                      TVector3 Lmom)
{
    ...
    return new (clref[size]) EmulsionDetPoint(trackID, detID, pos, mom, time, length, eLoss, pdgCode, Lpos, Lmom);
}
