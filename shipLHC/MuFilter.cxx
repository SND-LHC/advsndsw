...
void MuFilter::Initialize() { FairDetector::Initialize(); }
...
void MuFilter::ConstructGeometry()
{
    ...
    Double_t fDownstreamBarX_ver = conf_floats["MuFilter/DownstreamBarX_ver"];   // Staggered bars of upstream section,
    ...
    edge_Veto[1] =
        TVector3(-conf_floats["MuFilter/Veto1Dx"], conf_floats["MuFilter/Veto1Dz"], conf_floats["MuFilter/Veto1Dy"]);
    edge_Veto[2] =
        TVector3(-conf_floats["MuFilter/Veto2Dx"], conf_floats["MuFilter/Veto2Dz"], conf_floats["MuFilter/Veto2Dy"]);
    std::map<int, TVector3> edge_Iron;
    std::map<int, TVector3> edge_MuFilter;
    for (int i = 1; i < 10; i++) {
        string si = to_string(i);
        edge_Iron[i] = TVector3(-conf_floats["MuFilter/Iron" + si + "Dx"],
                                conf_floats["MuFilter/Iron" + si + "Dz"],
                                conf_floats["MuFilter/Iron" + si + "Dy"]);
        edge_MuFilter[i] = TVector3(-conf_floats["MuFilter/Muon" + si + "Dx"],
                                    conf_floats["MuFilter/Muon" + si + "Dz"],
                                    conf_floats["MuFilter/Muon" + si + "Dy"]);
    }
    ...
    TVector3 LocBarVeto =
        TVector3(-conf_floats["MuFilter/VETOLocX"], conf_floats["MuFilter/VETOLocZ"], conf_floats["MuFilter/VETOLocY"]);

    TVector3 VetoBox1 = TVector3(-conf_floats["MuFilter/VETOBoxX1"],
                                 conf_floats["MuFilter/VETOBoxZ1"],
                                 conf_floats["MuFilter/VETOBoxY1"]);   // bottom front left
    TVector3 VetoBox2 = TVector3(-conf_floats["MuFilter/VETOBoxX2"],
                                 conf_floats["MuFilter/VETOBoxZ2"],
                                 conf_floats["MuFilter/VETOBoxY2"]);   // top back right
    TVector3 VetoBoxDim =
        TVector3(VetoBox1.X() - VetoBox2.X(), VetoBox2.Y() - VetoBox1.Y(), VetoBox2.Z() - VetoBox1.Z());
    // support box
    TGeoBBox *supVetoBoxInner = new TGeoBBox("supVetoBoxI", VetoBoxDim.X() / 2, VetoBoxDim.Y() / 2, VetoBoxDim.Z() / 2);
    TGeoBBox *supVetoBoxOuter = new TGeoBBox("supVetoBoxO",
                                             VetoBoxDim.X() / 2 + fSupportBoxVW,
                                             VetoBoxDim.Y() / 2 + fSupportBoxVW,
                                             VetoBoxDim.Z() / 2 + fSupportBoxVW);
    ...
    for (int iplane = 0; iplane < fNVetoPlanes; iplane++) {
        ...
        volVeto->AddNode(
            volVetoPlane, iplane, new TGeoTranslation(displacement.X(), displacement.Y(), displacement.Z()));
        //  VETOBox1 = bottom front left
        displacement =
            edge_Veto[iplane + 1] + VetoBox1 + TVector3(-VetoBoxDim.X() / 2, VetoBoxDim.Y() / 2, VetoBoxDim.Z() / 2);
        volVeto->AddNode(subVetoBox, iplane, new TGeoTranslation(displacement.X(), displacement.Y(), displacement.Z()));

        displacement = TVector3(0, 0, 0);
        for (Int_t ibar = 0; ibar < fNVetoBars; ibar++) {
            Double_t dy_bar = (fVetoBarY + fVetoBarGap) * ibar;
            volVetoPlane->AddNode(volVetoBar,
                                  1e+4 + iplane * 1e+3 + ibar,
    ...
    TVector3 DSBox1 = TVector3(-conf_floats["MuFilter/DSBoxX1"],
                               conf_floats["MuFilter/DSBoxZ1"],
                               conf_floats["MuFilter/DSBoxY1"]);   // bottom front left
    TVector3 DSBox2 = TVector3(-conf_floats["MuFilter/DSBoxX2"],
                               conf_floats["MuFilter/DSBoxZ2"],
                               conf_floats["MuFilter/DSBoxY2"]);   // top back right
    TVector3 DSBoxDim = TVector3(DSBox1.X() - DSBox2.X(), DSBox2.Y() - DSBox1.Y(), DSBox2.Z() - DSBox1.Z());
    TVector3 USBox1 = TVector3(-conf_floats["MuFilter/DSBoxX1"],
                               conf_floats["MuFilter/DSBoxZ1"],
                               conf_floats["MuFilter/USBoxY1"]);   // bottom front left
    TVector3 USBox2 = TVector3(-conf_floats["MuFilter/DSBoxX2"],
                               conf_floats["MuFilter/DSBoxZ2"],
                               conf_floats["MuFilter/USBoxY2"]);   // top back right
    ...
    TGeoVolume *volFeBlockEnd =
        gGeoManager->MakeBox("volFeBlockEnd", Fe, fFeBlockEndX / 2, fFeBlockEndY / 2, fFeBlockEndZ / 2);
    volFeBlockEnd->SetLineColor(kGreen - 4);
    TGeoVolume *volBlockBot =
        gGeoManager->MakeBox("volBlockBot", concrete, fFeBlockBotX / 2, fFeBlockBotY / 2, fFeBlockBotZ / 2);
    volBlockBot->SetLineColor(kGreen - 4);

    // support box
    TGeoBBox *supDSBoxInner = new TGeoBBox("supDSBoxI", DSBoxDim.X() / 2, DSBoxDim.Y() / 2, DSBoxDim.Z() / 2);
    TGeoBBox *supDSBoxOuter = new TGeoBBox(
    ...
    TGeoBBox *supUSBoxOuter = new TGeoBBox(
    ...
    // local position of bottom horizontal bar to survey edge
    TVector3 LocBarUS = TVector3(-conf_floats["MuFilter/DSHLocX"],
                                 conf_floats["MuFilter/DSHLocZ"] - conf_floats["MuFilter/DownstreamBarY"] / 2
                                     + conf_floats["MuFilter/UpstreamBarY"] / 2,
    ...
    TGeoVolume *volMuUpstreamBar =
        gGeoManager->MakeBox("volMuUpstreamBar", Scint, fUpstreamBarX / 2, fUpstreamBarY / 2, fUpstreamBarZ / 2);
    ...
    for (Int_t l = 0; l < fNUpstreamPlanes; l++) {
        ...
        volMuFilter->AddNode(volFeBlock, l, new TGeoTranslation(displacement.X(), displacement.Y(), displacement.Z()));
        ...
        if (edge_Iron[9][2] < 0.1 && l == 0) {
            ...
            volMuFilter->AddNode(
                volFeTarget, 1, new TGeoTranslation(displacement.X(), displacement.Y(), displacement.Z()));
        }
        displacement = edge_MuFilter[l + 1] + LocBarUS + TVector3(-fUpstreamBarX / 2, 0, 0);
        volMuFilter->AddNode(volUpstreamDet,
                             fNVetoPlanes + l,
        ...
        volMuFilter->AddNode(
            subUSBox, l + fNVetoPlanes, new TGeoTranslation(displacement.X(), displacement.Y(), displacement.Z()));

        displacement = TVector3(0, 0, 0);
        for (Int_t ibar = 0; ibar < fNUpstreamBars; ibar++) {
            Double_t dy_bar = (fUpstreamBarY + fUpstreamBarGap) * ibar;
            volUpstreamDet->AddNode(
                volMuUpstreamBar,
                2e+4 + l * 1e+3 + ibar,
                new TGeoTranslation(displacement.X(),
                                    displacement.Y() + conf_floats["MuFilter/USOffZ" + to_string(l + 1)] + dy_bar,
                                    displacement.Z()));
        }
    }
    ...
    // local position of bottom horizontal bar to survey edge
    TVector3 LocBarH =
        TVector3(-conf_floats["MuFilter/DSHLocX"], conf_floats["MuFilter/DSHLocZ"], conf_floats["MuFilter/DSHLocY"]);
    // local position of l left vertical bar to survey edge
    TVector3 LocBarV =
        TVector3(-conf_floats["MuFilter/DSVLocX"], conf_floats["MuFilter/DSVLocZ"], conf_floats["MuFilter/DSVLocY"]);
    ...
    TGeoVolume *volMuDownstreamBar_hor = gGeoManager->MakeBox(
    ...
    // vertical bars, for x measurement
    TGeoVolume *volMuDownstreamBar_ver = gGeoManager->MakeBox(
    ...
    for (Int_t l = 0; l < fNDownstreamPlanes; l++) {
        ...
        if (l < fNDownstreamPlanes - 1) {
            displacement =
                edge_Iron[l + fNUpstreamPlanes + 1] - TVector3(fFeBlockX / 2, -fFeBlockY / 2, -fFeBlockZ / 2);
            volMuFilter->AddNode(volFeBlock,
                                 l + fNUpstreamPlanes + fNVetoPlanes,
                                 new TGeoTranslation(displacement.X(), displacement.Y(), displacement.Z()));
        } else if (edge_Iron[9][2] > 0.1) {
            // more iron
            displacement =
                edge_Iron[l + fNUpstreamPlanes + 1] - TVector3(fFeBlockEndX / 2, -fFeBlockEndY / 2, -fFeBlockEndZ / 2);
            volMuFilter->AddNode(
                volFeBlockEnd, 1, new TGeoTranslation(displacement.X(), displacement.Y(), displacement.Z()));
            displacement = edge_Iron[l + fNUpstreamPlanes + 1]
                           - TVector3(fFeBlockBotX / 2 - 10.0, fFeBlockBotY / 2, fFeBlockBotZ / 2 - fFeBlockEndZ);
            volMuFilter->AddNode(
                volBlockBot, 1, new TGeoTranslation(displacement.X(), displacement.Y(), displacement.Z()));
        }
        ...
        volMuFilter->AddNode(volDownstreamDet,
                             l + fNUpstreamPlanes + fNVetoPlanes,
                             new TGeoTranslation(displacement.X(), displacement.Y(), displacement.Z()));
        ...
        volMuFilter->AddNode(subDSBox,
                             l + fNUpstreamPlanes + fNVetoPlanes,
                             new TGeoTranslation(displacement.X(), displacement.Y(), displacement.Z()));
        // adding vertical bars for x
        displacement =
            LocBarV + TVector3(0, -fDownstreamBarY_ver / 2, 0) - LocBarH - TVector3(-fDownstreamBarX / 2, 0, 0);
        for (Int_t i_vbar = 0; i_vbar < fNDownstreamBars; i_vbar++) {
            ...
            volDownstreamDet->AddNode(
                ...
                3e+4 + l * 1e+3 + i_vbar_rev + 60,
                new TGeoTranslation(displacement.X() + dx_bar, displacement.Y(), displacement.Z()));
            // Andrew added a 60 here to make each horizontal + vertical sub-plane contain bars given detIDs as one
            // plane. So the first bar in the vert. sub plane is the 60th etc.
        }
    }
}
...
void MuFilter::EndOfEvent() { fMuFilterPointCollection->Clear(); }
...
void MuFilter::Register()
{
    ...
    FairRootManager::Instance()->Register("MuFilterPoint", "MuFilter", fMuFilterPointCollection, kTRUE);
}

TClonesArray *MuFilter::GetCollection(Int_t iColl) const
{
    if (iColl == 0) {
        return fMuFilterPointCollection;
    } else {
        return NULL;
    }
}

void MuFilter::Reset() { fMuFilterPointCollection->Clear(); }
                                ...
                                Int_t pdgCode)
{
    ...
    return new (clref[size]) MuFilterPoint(trackID, detID, pos, mom, time, length, eLoss, pdgCode);
}
...
void MuFilter::GetLocalPosition(Int_t fDetectorID, TVector3 &vLeft, TVector3 &vRight)
{
    ...
    nav->MasterToLocal(A, locA);
    nav->MasterToLocal(B, locB);
...
Float_t MuFilter::GetCorrectedTime(Int_t fDetectorID, Int_t channel, Double_t rawTime, Double_t L)
{
    ...
    if (eventHeader) {
        ...
        if (coveredRuns.size() != 0) {
            ...
            // special case
            if (fRunNumber < 5193 && fRunNumber > 5174)
                tag = "t_" + to_string(coveredRuns[0]);
        } else {
    ...
    int p;
    if (channel == 0 && ichannel60 < 30) {
        p = 0 + l * 6;
    }
    if (channel == 0 && ichannel60 > 29 && ichannel60 < 60) {
        p = 1 + l * 6;
    }
    if (channel == 1 && ichannel60 < 30) {
        p = 2 + l * 6;
    }
    if (channel == 1 && ichannel60 > 29 && ichannel60 < 60) {
        p = 3 + l * 6;
    }
    if (channel == 0 && ichannel60 > 59 && ichannel60 < 90) {
        p = 4 + l * 6;
    }
    if (channel == 0 && ichannel60 > 89 && ichannel60 < 120) {
        p = 5 + l * 6;
    }
    if (l == 3) {
        p -= 4;
    ...
    if (ichannel60 < 30) {
        cor += conf_floats["MuFilter/DSTcorslope" + tag] * (ichannel60 - 15);
    } else {
        cor -= conf_floats["MuFilter/DSTcorslope" + tag] * (ichannel60 - 45);
    }
...
void MuFilter::GetPosition(Int_t fDetectorID, TVector3 &vLeft, TVector3 &vRight)
{
    ...
    if (subsystem == 3 and bar_number > 59) {   // vertical bars
        ...
        nav->LocalToMaster(posXend, GposXend);
        nav->LocalToMaster(negXend, GnegXend);
...
Int_t MuFilter::GetnSiPMs(Int_t detID)
{
    ...
    if (subsystem == 0) {
        return conf_ints["MuFilter/VetonSiPMs"];
    }
    if (subsystem == 1) {
        return conf_ints["MuFilter/UpstreamnSiPMs"];
    }
...
Int_t MuFilter::GetnSides(Int_t detID)
{
    int subsystem = floor(detID / 10000) - 1;
    if (subsystem == 0) {
        return conf_ints["MuFilter/VetonSides"];
    }
    if (subsystem == 1) {
