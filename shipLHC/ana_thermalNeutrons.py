...
parser.add_argument(
    ...
    dest="targetMaterial",
    type=str,
    help="target material BoronCarbide or  Boratedpolyethylene",
    default="BoronCarbide",
)
parser.add_argument(
    "--thick", dest="targetLength", type=float, help="target thickness", default=0.1
)
parser.add_argument("-c", "--command", dest="command", help="command", default="none")
parser.add_argument(
    "-f", dest="inputFile", help="Input file", required=False, default=False
)
parser.add_argument(
    "-g",
    dest="geoFile",
    help="geo file",
    required=False,
    default="geofile-thermNeutron_Boratedpolyethylene_coldbox.root",
)
...
def count(hFile):
    f = ROOT.TFile(hFile)
    ut.bookHist(h, "E", ";log10(Ekin [MeV])", 1000, -12.0, 4.0)
    ut.bookHist(h, "Epassed", ";log10(Ekin [MeV])", 1000, -12.0, 4.0)
    ut.bookHist(h, "Esecondary", ";log10(Ekin [MeV])", 1000, -12.0, 4.0)
    ut.bookHist(h, "electrons", ";log10(Ekin [MeV])", 1000, -12.0, 4.0, 100, -0.5, 99.5)
    ut.bookHist(h, "photons", ";log10(Ekin [MeV])", 1000, -12.0, 4.0, 100, -0.5, 99.5)
    ut.bookHist(
        h,
        "Lab",
        "  ; logE[MeV];absorption Length  [cm]",
        1000,
        -12.0,
        4.0,
        100000,
    ...
    ut.bookHist(h, "xyz", "", 100, -0.1, 0.1, 100, -0.1, 0.1, 200, -1.0, 1.0)
    ut.bookHist(h, "dxyz", "", 100, -0.1, 0.1, 100, -0.1, 0.1, 200, -1.0, 1.0)
    Npassed = 0
    for sTree in f.cbmsim:
        N = sTree.MCTrack[0]
        Ekin = N.GetP() ** 2 / (2 * N.GetMass()) * 1000.0
        ...
        for m in sTree.MCTrack:
            ...
            if m.GetStartZ() > 1:
                continue
            for x in Nelectrons:
                if abs(m.GetPdgCode()) == x:
                    Nelectrons[x] += 1
        rc = h["electrons"].Fill(logEkin, Nelectrons[11])
        rc = h["photons"].Fill(logEkin, Nelectrons[22])
        for p in sTree.vetoPoint:
            if not p.PdgCode() == 2112:
                continue
            if not p.GetTrackID() == 0:
                rc = h["Esecondary"].Fill(ROOT.TMath.Log10(Ekin))
...
def beamGas():
    ...
    for l in f.readlines():
        ...
        for x in l.split(" "):
            ...
            p.append(float(x))
        nt.Fill(
            p[0] * 1000.0, p[1] * 1000.0, p[2] / 1000.0 * 10170.0, p[3]
        )  # / 1000 for MeV
...
def absorptionLength(plotOnly=True):
    ...
    B10Parameter = {}
    B10Parameter["Boratedpolyethylene"] = 0.01 * 0.94 / (10.0 * 1.672e-24)
    B10Parameter["BoronCarbide"] = 0.125 * 1.360 / (10.0 * 1.672e-24)
    Xsec = 0.61e-24
    h["Fabsorp"] = {}
    Fabsorp = h["Fabsorp"]
    for m in B10Parameter:
        Fabsorp[m] = ROOT.TF1("fabs" + m, "1./([0]/sqrt(10**x)*[1])", -10.0, 4.0)
        ...
        for material in materials:
            for Erange in [
                ...
                if not fname in os.listdir("."):
                    continue
                count(fname)
                h["Esecondary" + "_" + material + Erange] = h["Esecondary"].Clone(
                    "Esecondary" + "_" + material + Erange
                )
                h["electrons" + "_" + material + Erange] = h["electrons"].Clone(
                    "electrons" + "_" + material + Erange
                )
                h["photons" + "_" + material + Erange] = h["photons"].Clone(
                    "photons" + "_" + material + Erange
                )
                for x in ["Lab", "Labz"]:
    ...
    for material in materials:
        for Erange in [
            ...
            print(
                "secondary neutrons for %s %s %5.2F%%"
                % (
                    material,
                    Erange,
                    100.0
    ...
    for X in ["electrons", "photons"]:
        for material in materials:
            ...
            for p in [1, 2, 5, 10]:
                h[X + str(p) + "Percent_" + material] = h[
                    X + "_" + material
                ].ProjectionX(X + str(p) + "Percent_" + material, 1, p)
        ...
        h[X + "1Percent_" + material].GetYaxis().SetTitle(
            "fraction of events with < N_{" + X + "}"
        )
        h[X + "1Percent_" + material].GetXaxis().SetRangeUser(-12.0, 1)
        ...
        for p in [1, 2, 5, 10]:
            ...
            rc = h["leg" + X].AddEntry(
                h[X + str(p) + "Percent_" + material], "N_{" + X + "}<" + str(p), "PL"
            )
        h["leg" + X].Draw("same")
    ...
    for x in ["Lab", "Labz"]:
        ...
        h["abs" + x].SetLogy()
        hsum = x + "_Concrete"
        h[hsum].GetXaxis().SetRangeUser(-12.0, 1)
        h[hsum].SetMaximum(30.0)
        ...
        for material in materials:
            ...
            if material in Fabsorp:
                Fabsorp[material].Draw("same")
    ...
    h["dangerZone"].SetPoint(0, -5.6, 0.0)
    h["dangerZone"].SetPoint(1, -5.6, 1.0e7)
    h["dangerZone"].SetPoint(2, -5.1, 1.0e7)
    h["dangerZone"].SetPoint(3, -5.1, 0.0)
    ...
    h["Fig12"].Draw("same")
    #
    ut.bookHist(h, "Nr", ";E [MeV];dn/dlogE [cm^{-2}y^{-1}] ", 100, -12.0, 1.0)
    h["Nr"].SetMaximum(2.0e8)
    h["Nr"].SetMinimum(1.0e-4)
    h["Nr"].SetStats(0)
    intRates = {}
    thick = {
        0.0: ROOT.kBlue,
    ...
    for material in ["Boratedpolyethylene", "BoronCarbide"]:
        ...
        for d in thick:  # try different thicknesses
            ...
            for n in range(h["Fig12"].GetN()):
                logE = h["Fig12"].GetX()[n]
                R = ROOT.TMath.Power(10.0, h["Fig12"].GetY()[n])
                dE = h["dE"].GetPointY(n)
                E = ROOT.TMath.Power(10.0, logE)
                absorpt = 0
                if d == 0.0:
            ...
            h[gname].Draw("same")
            rc = h["legthick" + material].AddEntry(
                h[gname], "thickness %3.1Fcm" % (d), "PL"
            )
            ...
            #
            for Elimit in [100.0, 1.0, 0.1, 0.01]:  # MeV
                g = h["IUp-neutronRate_" + material + str(d)]
                gW = h["IUp-neutronRateW_" + material + str(d)]
                N = g.GetN()
                for n in range(N - 1, 1, -1):
                    if ROOT.TMath.Power(10, g.GetX()[n]) < Elimit:
                        break
                reduction = (
                    g.GetY()[n]
                    / h["IUp-neutronRate_" + material + str(0.0)].GetY()[N - 1]
                )
                reductionW = (
                    gW.GetY()[n]
                    / h["IUp-neutronRate_" + material + str(0.0)].GetY()[N - 1]
...
def reactions(hFile, distance=1e10):
    ...
    for sTree in f.cbmsim:
        ...
        if not reaction in stats:
            sTree.MCTrack.Dump()
            stats[reaction] = {}
        if not nphot in stats[reaction]:
            stats[reaction][nphot] = 0
    ...
    for reaction in stats:
        ...
        for nphot in stats[reaction]:
            R += stats[reaction][nphot]
        R = R / n
...
def absorptionLengthOLD():
    ...
    hFiles = {
        ...
        "thermNeutron_Boratedpolyethylene_X.XX_0.root": [1.0, 100.0],
    }
    ...
    for hFile in hFiles:
        ...
        for L in Ls:
            l = str(L)
            if L < 3:
                tmp = hFile.replace("X.XX", l).replace(" _-E_-E", "")
            else:
                tmp = hFile.replace("X.XX", l).replace(" _-E_-E", "_-8_-7")
            count(tmp)
        ...
        ut.bookHist(h, "L", ";logE; L [cm]", 100, -9.0, -6.0)
...
def flukaRateIntegrated(save=False):
    ...
    for n in range(h["neutronRate<E"].GetN()):
        h["neutronRate<E%"].SetPoint(
            n,
            h["neutronRate<E"].GetX()[n],
            h["neutronRate<E"].GetY()[n] / h["neutronRate<E"].GetY()[0],
        )
    ut.bookHist(h, "Nr", ";E [MeV];dn/dlogE [cm^{-2}y^{-1}] ", 100, -12.0, 2.0)
    h["Nr"].SetMaximum(1.1)
    h["Nr"].SetMinimum(0.0)
    h["Nr"].SetStats(0)
...
def coldBox(plotOnly=True, pas=""):
    if 1 > 0:
        ...
        gFile = "geofile-" + (
            tmp[len(tmp) - 1].split("_coldbox")[0] + "_coldbox.root"
        ).replace("histos-", "")
        g = ROOT.TFile(gFile)
        sGeo = g.FAIRGeom
        ROOT.gROOT.cd()
    ...
    if not plotOnly:
        ...
        for o in ["_seco", "_prim"]:
            ...
            for r in Rin:
                ut.bookHist(
                    h, "EkinE" + r + o, "log10(Ekin)", 100, -13.0, 0, 100, -200.0, 200.0
                )
                ut.bookHist(
                    h, "Ekin" + r + o, "log10(Ekin)", 100, -13.0, 0, 100, -200.0, 200.0
                )
            for r in Rsens:
                ut.bookHist(h, "DF" + r + o, "travel  distance", 100, 0.0, 200.0)
                ut.bookHist(
                    h,
                    "EkinF" + r + o,
                    "log10(Ekin) vs distance",
                    100,
                    -13.0,
                    0,
                    100,
        ...
        ut.bookHist(h, "EkinG", "log10(Ekin)", 100, -13.0, 0.0)
        ut.bookHist(h, "EkinW", "log10(Ekin)", 100, -13.0, 0.0)
        ...
        for sTree in f.cbmsim:
            ...
            start = ROOT.TVector3(
                neutron.GetStartX(), neutron.GetStartY(), neutron.GetStartZ()
            )
            if start.y() < boundaries["botSens"]:
                continue
            ...
            for trackID in trajectory:
                ...
                for p in trajectory[trackID]:
                    ...
                    Ekin_entry = (
                        ROOT.TMath.Sqrt(firstMom.Mag2() + neutronMass**2) - neutronMass
                    )
                    if (
                        p.GetDetectorID() == 13 and firstSens
                    ):  # first point inside coldbox
                        firstSens = False
                        rc = h["exit" + origin].Fill(
        ...
        if pas != "":
            ut.readHists(hnorm, "histos-thermNeutron_vacuums_0.0001_coldbox_pass1.root")
        else:
        ...
        for c in ["entry", "exit"]:
            ...
            for p in ["x", "y", "z"]:
                ...
                for imin in range(1, tmp.GetNbinsX() + 1):
                    if tmp.GetBinContent(imin) > 0:
                        break
                for imax in range(tmp.GetNbinsX(), 0, -1):
                    if tmp.GetBinContent(imax) > 0:
                        break
            ...
            for T in ["", "cold", "hot"]:
                for Z in ["entry", "exit", "exit-original"]:
                    ...
                    for p in projections:
                        ...
                        h["X-" + case + p].Scale(1.0 / projections[p]["ydist"])
                        h["Y-" + case + p].Scale(1.0 / projections[p]["xdist"])
        ...
        for r in ["", "top", "bot", "right", "left", "front", "back"]:
            ...
            if r == "":
                ...
                k += 1
                if r == "bot":
                    norm = hnorm["EkintopIn"].ProjectionX("norm")
                else:
                    norm = hnorm["Ekin" + r + "In"].ProjectionX("norm")
            ...
            h[rej].GetXaxis().SetRangeUser(-13.0, -1.0)
            h[rej].SetMaximum(1.2)
            h[rej].SetMinimum(1.0e-4)
            if k == 0:
                tc = h["crosschecks"].cd()
            else:
                tc = h["trej"].cd(k)
            ...
            rc = h["legR" + r].AddEntry(
                h[rej], "reduction as function of E_{kin} when leaving shield", "PL"
            )
            h["legR" + r].Draw("same")
            if k == 0:
        ...
        #
        h["dangerZone"] = ROOT.TGraph()
        h["dangerZone"].SetPoint(0, -8.6, 0.0)
        h["dangerZone"].SetPoint(1, -8.6, 1.0)
        h["dangerZone"].SetPoint(2, -8.1, 1.0)
        h["dangerZone"].SetPoint(3, -8.1, 0.0)
        h["dangerZone"].SetFillStyle(1001)
        h["dangerZone"].SetFillColor(ROOT.kYellow)
        for r in ["rej", "rejo"]:
            ...
            h[r + "TopLeftRightBack"].Scale(1.0 / 4.0)
            h[r + "TopLeftRightBack"].SetTitle("")
            if r == "rej":
                h[r + "TopLeftRightBack"].GetXaxis().SetTitle(
                    "outgoing   " + h[r + "TopLeftRightBack"].GetXaxis().GetTitle()
                )
            if r == "rejo":
                h[r + "TopLeftRightBack"].GetXaxis().SetTitle(
                    "incoming   " + h[r + "TopLeftRightBack"].GetXaxis().GetTitle()
                )
            ...
            h[r + "TopLeftRightBack"].SetMinimum(1.0e-6)
            tc = h[r + "ection2"].cd()
            tc.SetGridx()
            tc.SetGridy()
            tc.SetLogy()
            ...
            rc = h[r + "legR2"].AddEntry(
                h[r + "TopLeftRightBack"], "av. rejection top/left/right/back", "PL"
            )
            rc = h[r + "legR2"].AddEntry(
                h[r + "bot"], "rejction bottom, only concrete", "PL"
            )
            rc = h[r + "legR2"].AddEntry(
                h[r + "front"], "rejction front, with holes", "PL"
            )
            h[r + "legR2"].Draw("same")
            myPrint(
                h[r + "ection2"], material + "Sum " + r + "ections" + thickness + pas
            )
        ...
        for o in ["", "-original"]:
            for T in ["cold", "hot"]:
                ...
                for p in projections:
                    ...
                    if p == "Bot":
                        concrete = h["fluences"]["exit" + T + o + p]
                    elif p == "Front":
    ...
    for ix in range(1, nx + 1, 2):
        for iy in range(1, ny + 1, 2):
            for iz in range(1, nz + 1, 2):
                ...
                h["A"].AddNode(
                    h["x"], ix + 1000 * iy + 1000000 * iz, h["T"][len(h["T"]) - 1]
                )
