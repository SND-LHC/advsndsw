...
void Scifi::Initialize() { FairDetector::Initialize(); }
...
void Scifi::ConstructGeometry()
{
    ...
    //                                                          need y up, z away from IP1, pos X left:  y and z need to
    //                                                          change sign.
    ...
    TVector3 SHfirst = TVector3(conf_floats["Scifi/FirstChannelHX"],
                                -conf_floats["Scifi/FirstChannelHY"],
                                -conf_floats["Scifi/FirstChannelHZ"]);
    TVector3 SVfirst = TVector3(conf_floats["Scifi/FirstChannelVX"],
                                -conf_floats["Scifi/FirstChannelVY"],
                                -conf_floats["Scifi/FirstChannelVZ"]);

    // first channel position in sndsw local plane:
    TVector3 LHfirst = TVector3(conf_floats["Scifi/LfirstChannelHX"],
                                conf_floats["Scifi/LfirstChannelHY"],
                                conf_floats["Scifi/LfirstChannelHZ"]);
    TVector3 LVfirst = TVector3(conf_floats["Scifi/LfirstChannelVX"],
                                conf_floats["Scifi/LfirstChannelVY"],
                                conf_floats["Scifi/LfirstChannelVZ"]);
    ...
    // Carbon Fiber Film
    TGeoVolume *CarbonFiberVolume =
        gGeoManager->MakeBox("CarbonFiber", CarbonComposite, fXDimension / 2, fYDimension / 2, fZCarbonFiber / 2);
    CarbonFiberVolume->SetLineColor(kGray - 2);
    CarbonFiberVolume->SetVisibility(1);

    // Honeycomb Rohacell
    TGeoVolume *HoneycombVolume =
        gGeoManager->MakeBox("Honeycomb", rohacell, fXDimension / 2, fYDimension / 2, fZHoneycomb / 2);
    ...
    TGeoVolume *PlasticAirVolume =
        gGeoManager->MakeBox("PlasticAir", air, fXDimension / 2, fYDimension / 2, fZPlastBar / 2);
    PlasticAirVolume->SetLineColor(kGray - 1);
    PlasticAirVolume->SetVisibility(1);
    PlasticAirVolume->SetVisDaughters(1);

    // Plastic bars
    TGeoVolume *PlasticBarVolume =
        gGeoManager->MakeBox("PlasticBar", PlasticBase, fXPlastBar / 2, fYPlastBar / 2, fZPlastBar / 2);
    PlasticBarVolume->SetLineColor(kGray - 4);
    PlasticBarVolume->SetVisibility(1);

    PlasticAirVolume->AddNode(
        PlasticBarVolume, 0, new TGeoTranslation(-fXDimension / 2 + fXPlastBar / 2, 0, 0));   // bars are placed || to y
    ...
    TGeoVolume *HorMatVolume =
        gGeoManager->MakeBox("HorMatVolume", Epoxy, fLengthScifiMat / 2, fWidthScifiMat / 2, fZEpoxyMat / 2);
    TGeoVolume *VertMatVolume =
        gGeoManager->MakeBox("VertMatVolume", Epoxy, fWidthScifiMat / 2, fLengthScifiMat / 2, fZEpoxyMat / 2);
    ...
    for (int irow = 0; irow < fNFibers_z; irow++) {
        ...
        if (irow % 2 == 0) {
            for (int ifiber = 0; ifiber < fNFibers_Srow; ifiber++) {
                HorMatVolume->AddNode(
                    FiberVolume,
                    1e6 * dummy_station + 1e5 * 0 + 1e4 * dummy_mat + 1e3 * (irow + 1) + ifiber + 1,
                    new TGeoCombiTrans("rottranshor0", 0, offsetS + ifiber * fHorPitch, zPosM, rothorfiber));
            }
        } else {
            for (int ifiber = 0; ifiber < fNFibers_Lrow; ifiber++) {
                HorMatVolume->AddNode(
                    FiberVolume,
                    1e6 * dummy_station + 1e5 * 0 + 1e4 * dummy_mat + 1e3 * (irow + 1) + ifiber + 1,
                    new TGeoCombiTrans("rottranshor1", 0, offsetL + ifiber * fHorPitch, zPosM, rothorfiber));
            }
        }
    }
    ...
    for (int irow = 0; irow < fNFibers_z; irow++) {
        ...
        if (irow % 2 == 0) {
            for (int ifiber = 0; ifiber < fNFibers_Srow; ifiber++) {
                VertMatVolume->AddNode(
                    FiberVolume,
                    1e6 * dummy_station + 1e5 * 1 + 1e4 * dummy_mat + 1e3 * (irow + 1) + ifiber + 1,
                    new TGeoCombiTrans("rottransvert0", offsetS + ifiber * fHorPitch, 0, zPosM, rotvertfiber));
            }
        } else {
            for (int ifiber = 0; ifiber < fNFibers_Lrow; ifiber++) {
                VertMatVolume->AddNode(
                    FiberVolume,
                    1e6 * dummy_station + 1e5 * 1 + 1e4 * dummy_mat + 1e3 * (irow + 1) + ifiber + 1,
                    new TGeoCombiTrans("rottransvert1", offsetL + ifiber * fHorPitch, 0, zPosM, rotvertfiber));
            }
        }
    }
    ...
    for (int istation = 0; istation < fNScifi; istation++) {
        ...
        ScifiVolume->AddNode(
            CarbonFiberVolume, 1, new TGeoTranslation(0, 0, fZCarbonFiber + fZHoneycomb + fZCarbonFiber / 2));
        ScifiVolume->AddNode(
            ScifiHorPlaneVol, node, new TGeoTranslation(0, 0, 2 * fZCarbonFiber + fZHoneycomb + fZEpoxyMat / 2));
        ScifiVolume->AddNode(PlasticAirVolume,
                             0,
                             new TGeoTranslation(0, 0, 2 * fZCarbonFiber + fZHoneycomb + fZEpoxyMat + fZPlastBar / 2));
        ...
        ScifiVolume->AddNode(
            ...
            1,
            new TGeoCombiTrans(
                "rottrans0", 0, 0, 2 * fZCarbonFiber + fZHoneycomb + fZEpoxyMat + 3 * fZPlastBar / 2, rot));
        ScifiVolume->AddNode(
            ScifiVertPlaneVol,
            node,
            new TGeoTranslation(0, 0, 2 * fZCarbonFiber + fZHoneycomb + fZEpoxyMat + 2 * fZPlastBar + fZEpoxyMat / 2));
        ScifiVolume->AddNode(
            CarbonFiberVolume,
            2,
            new TGeoTranslation(
                0, 0, 2 * fZCarbonFiber + fZHoneycomb + fZEpoxyMat + 2 * fZPlastBar + fZEpoxyMat + fZCarbonFiber / 2));
        ScifiVolume->AddNode(
            HoneycombVolume,
            1,
            new TGeoTranslation(
                0, 0, 3 * fZCarbonFiber + fZHoneycomb + fZEpoxyMat + 2 * fZPlastBar + fZEpoxyMat + fZHoneycomb / 2));
        ScifiVolume->AddNode(CarbonFiberVolume,
                             3,
                             new TGeoTranslation(0,
                                                 0,
                                                 3 * fZCarbonFiber + 2 * fZHoneycomb + fZEpoxyMat + 2 * fZPlastBar
                                                     + fZEpoxyMat + fZCarbonFiber / 2));

        Double_t totalThickness = 4 * fZCarbonFiber + 2 * fZHoneycomb + 2 * fZEpoxyMat + 2 * fZPlastBar;

        volTarget->AddNode(
            ScifiVolume, node, new TGeoTranslation(DeltasV[istation][0], DeltasH[istation][1], DeltasH[istation][2]));

        // Creating Scifi planes by appending fiber mats
        for (int imat = 0; imat < fNMats; imat++) {
            // Placing mats along Y
            ScifiHorPlaneVol->AddNode(HorMatVolume,
                                      1e6 * (istation + 1) + 1e4 * (imat + 1),
                                      new TGeoTranslation(0, (imat - 1) * (fWidthScifiMat + fGapScifiMat), 0));

            // Placing mats along X
            ScifiVertPlaneVol->AddNode(VertMatVolume,
                                       1e6 * (istation + 1) + 1e5 + 1e4 * (imat + 1),
                                       new TGeoTranslation((imat - 1) * (fWidthScifiMat + fGapScifiMat), 0, 0));
        }
    }
}
...
{
    if (gGeoManager->FindVolumeFast("SiPMmapVol")) {
        return;
    }
    Double_t fLengthScifiMat = conf_floats["Scifi/scifimat_length"];
    ...
    TGeoVolume *ChannelVol =
        gGeoManager->MakeBox("ChannelVol", 0, fLengthScifiMat / 2, fWidthChannel / 2, fZEpoxyMat / 2);
    ...
    for (int imat = 0; imat < fNMats; imat++) {
        for (int isipms = 0; isipms < fNSiPMs; isipms++) {
            ...
            for (int ichannel = 0; ichannel < fNSiPMChan; ichannel++) {
                SiPMmapVol->AddNode(
                    ChannelVol, imat * 10000 + isipms * 1000 + ichannel, new TGeoTranslation(0, pos, 0));
                pos += fWidthChannel;
                if (ichannel == (fNSiPMChan / 2 - 1)) {
                    pos += fCharrGap;
                }
            }
            pos += fEdge + fBigGap;
...
Bool_t Scifi::ProcessHits(FairVolume *vol)
{
    ...
    if (gMC->IsTrackEntering()) {
        ...
        int fibre_mat_number =
            int((fibre_mat_id_station_removed - int(fibre_mat_id_station_removed / 1e5) * 1e5) / 1e4);

        fVolumeID = fibre_local_id + fibre_station_number * 1e6 + fibre_mat_number * 1e4;

        if (fVolumeID == 0) {
            std::cout << "fiber vol id " << nav->GetMother()->GetName() << std::endl;
        }
    }
...
void Scifi::GetPosition(Int_t fDetectorID, TVector3 &A, TVector3 &B)
{
    ...
    */
    ...
    sID.Form("%i", fDetectorID);
    TString path =
        "/cave_1/Detector_0/volTarget_1/ScifiVolume" + TString(sID(0, 1)) + "_" + TString(sID(0, 1)) + "000000/";
    if (sID(1, 1) == "0") {
    ...
    */
    ...
    if (eventHeader) {
        ...
        if (coveredRuns.size() != 0) {
            ...
            if (fRunNumber < 4575) {
                tag = "A";
            } else if (fRunNumber < 4855) {
                tag = "B";
            } else if (fRunNumber < 5172) {
                tag = "C";
            } else if (fRunNumber < 5431) {
                tag = "D";
            }
        }
    }
    ...
    for (int imat = 0; imat < plane->GetNodes()->GetEntriesFast(); imat++) {
        ...
        for (int ifibre = 0; ifibre < vmat->GetNodes()->GetEntriesFast(); ifibre++) {
            ...
            for (Int_t nChan = 0; nChan < Nodes->GetEntriesFast();
                 nChan++) {   // 12 SiPMs total and 4 SiPMs per mat times 128 channels
...
void Scifi::EndOfEvent() { fScifiPointCollection->Clear(); }
...
void Scifi::Register()
{
    ...
    FairRootManager::Instance()->Register("ScifiPoint", "Scifi", fScifiPointCollection, kTRUE);
}

TClonesArray *Scifi::GetCollection(Int_t iColl) const
{
    if (iColl == 0) {
        return fScifiPointCollection;
    } else {
        return NULL;
    }
}

void Scifi::Reset() { fScifiPointCollection->Clear(); }
