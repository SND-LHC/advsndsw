...
class reversChannelMapping:
    ...
    def Init(self, path):
            ...
            if os.path.isdir(sndswPath + "/geometry"):
                with open(infile, "r") as f:
                    ...
                    for i, row in enumerate(reader):
                        self.SiPMmap[system][int(row["SiPM"])] = [
                            int(row["CN"]),
                            int(row["pin"]),
                            int(row["TOFPET"]),
                            int(row["CHANNEL"]),
        ...
        for board in self.boardMaps["Scifi"]:
            ...
            if not station in self.revBoardMaps["Scifi"]:
                self.revBoardMaps["Scifi"][station] = {}
            self.revBoardMaps["Scifi"][station][mat] = board

    #
    def daqChannel(self, aHit, channel=0):
        if type(aHit).__name__ == "int":
            fDetectorID = aHit
        else:
        ...
        for side in ["Left", "Right"]:
            for bar_number in range(10):
                ...
                for n in range(nSiPMs):
                    ...
                    sipmChannel = (
                        int((fDetectorID - tself.offMap[tag][0]) / direction * nSiPMs)
                        + n
                        + 1
                    )
