        ...
        if subsys == "scifi":
            ...
            for plane, conf in planes.items():
                ...
                if conf["class"] != "multiboard" or conf["type"] != "snd_scifi":
                    raise RuntimeError(
                        "wrong class/type: " + conf["class"] + "/" + conf["type"]
                    )
            ...
            for plane, conf in planes.items():
                ...
                for i in range(2):
                    boardMapsNew["MuFilter"][bString][conf["slots"][i]] = (
                        f'Veto_{plane}{"Left" if i == 0 else "Right"}'
                    )
            ...
            for plane, conf in planes.items():
                ...
                for i in range(2):
                    boardMapsNew["MuFilter"][bString][conf["slots"][i]] = (
                        f'US_{plane}{"Left" if i == 0 else "Right"}'
                    )

        elif subsys == "ds":
            for plane, conf in planes.items():
                if conf["class"] != "multislot" or (
                    conf["type"] != "snd_dsh" and conf["type"] != "snd_dsv"
                ):
                    raise RuntimeError(
                        "wrong class/type: " + conf["class"] + "/" + conf["type"]
                    )
                ...
                if conf["type"] == "snd_dsh":
                    for i in range(2):
                        boardMapsNew["MuFilter"][bString][conf["slots"][i]] = (
                            f'DS_{plane[0]}{"Left" if i == 0 else "Right"}'
                        )
                else:
                    # vertical planes use one slot only
                    boardMapsNew["MuFilter"][bString][conf["slots"][0]] = (
                        f"DS_{plane[0]}Vert"
                    )
