    ...
    fTimeDetPointCollection(new TClonesArray("TimeDetPoint"))
{
    ...
    if (fNCol > 1)
        ...
        fxOv = 0;
    if (fNRow > 1)
        fyOv = (fyBar * fNRow - fySize) / (double)(fNRow - 1);
    else
        fyOv = 0;
}
...
Bool_t TimeDet::ProcessHits(FairVolume* vol)
{
    ...
    if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
        ...
        // cout << uniqueId << " :(" << xmean << ", " << ymean << ", " << zmean << "): " << gMC->CurrentVolName() <<
        // endl;
...
void TimeDet::EndOfEvent() { fTimeDetPointCollection->Clear(); }
...
void TimeDet::Register()
{
    ...
    */
    ...
    FairRootManager::Instance()->Register("TimeDetPoint", "TimeDet", fTimeDetPointCollection, kTRUE);
}

TClonesArray* TimeDet::GetCollection(Int_t iColl) const
{
    if (iColl == 0) {
        return fTimeDetPointCollection;
    } else {
        return NULL;
    }
}

void TimeDet::Reset() { fTimeDetPointCollection->Clear(); }
                              ...
                              TVector3 Lmom)
{
    ...
    return new (clref[size]) TimeDetPoint(trackID, detID, pos, mom, time, length, eLoss, pdgCode, Lpos, Lmom);
}
