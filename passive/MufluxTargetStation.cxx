...
void MufluxTargetStation::ConstructGeometry()
{
    ...
    targettube = gGeoManager->MakeTube("TargetTube",
                                       steel,
                                       fDiameter / 2. + 0.5,
                                       fDiameter / 2. + 1.1,
                                       fTargetLength / 2. - 0.5);   // subtract 1 (front&endcaps)
    ...
    for (Int_t i = 0; i < fnS; i++) {
        ...
        if (fM.at(i) == "tungstenmisis") {
            material = tungstenmisis;
        };
    ...
    absorbercutout =
        gGeoManager->MakeBox("AbsorberCO", iron, fabsorbercutout_x, fabsorbercutout_y, fAbsorberLength / 2.);

    ironshield = gGeoManager->MakeBox("IronShield", iron, fironshield_x, fironshield_y, fironshield_z);
    concreteshield =
        gGeoManager->MakeBox("ConcreteShield", concrete, fconcreteshield_x, fconcreteshield_y, fconcreteshield_z);
    abovetargetshield = gGeoManager->MakeBox(
        "AboveTargetShield", concrete, fabovetargetshield_x, fabovetargetshield_y, fabovetargetshield_z);
    aboveabsorbershield = gGeoManager->MakeBox(
        "AboveBsorberShield", concrete, faboveabsorbershield_x, faboveabsorbershield_y, faboveabsorbershield_z);
    aboveabovetargetshield = gGeoManager->MakeBox(
        "AboveAboveTargetShield", concrete, fAbsorberLength, faboveabovetargetshield_y, fAbsorberLength);
