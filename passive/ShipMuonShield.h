...
enum class FieldDirection : bool
{
    up,
    down
};
...
{
  public:
    ShipMuonShield(const char *name,
    ...
    {
        fSupport = supports;
        LOG(WARNING) << "Setting supports to %s. This will not have any effect if called after the geometry has been "
                        "constructed.",
            fSupport ? "true" : "false";
    }
  ...
  protected:
    ...
    void CreateTube(TString tubeName,
                    TGeoMedium *medium,
                    Double_t dX,
                    Double_t dY,
                    Double_t dZ,
