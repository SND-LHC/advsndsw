...
void MagneticSpectrometer::SetMagneticField(Double_t B) { fField = B; }

void MagneticSpectrometer::SetGapDownstream(Double_t Gap) { fGapDown = Gap; }

void MagneticSpectrometer::SetGapMiddle(Double_t Gap) { fGapMiddle = Gap; }

void MagneticSpectrometer::SetZDimensionArm(Double_t Z) { fZArm = Z; }

void MagneticSpectrometer::SetNFeInArm(Int_t N) { fNFe = N; }

void MagneticSpectrometer::SetNRpcInArm(Int_t N) { fNRpc = N; }
...
void MagneticSpectrometer::Initialize() { FairDetector::Initialize(); }
...
void MagneticSpectrometer::ConstructGeometry()
{
    ...
    if (fDesign == 3) {
        ...
        for (Int_t i = 0; i < fNFe; i++) {
            volMSBox->AddNode(
                volIron, nr + 100 + i, new TGeoTranslation(0, 0, -fZtot / 2 + i * fZFe + fZFe / 2 + i * fZRpc));
        }
        ...
        for (Int_t i = 0; i < fNRpc; i++) {
            volMSBox->AddNode(volRpcContainer,
                              nr + i,
                              new TGeoTranslation(0, 0, -fZtot / 2 + (i + 1) * fZFe + i * fZRpc + fZRpc / 2));
        }
...
void MagneticSpectrometer::EndOfEvent() { fShipRpcPointCollection->Clear(); }
...
void MagneticSpectrometer::Register()
{
    ...
    */
    ...
    FairRootManager::Instance()->Register("ShipRpcPoint", "MagneticSpectrometer", fShipRpcPointCollection, kTRUE);
}
...
void MagneticSpectrometer::Reset() { fShipRpcPointCollection->Clear(); }
