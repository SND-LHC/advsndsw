...
void Hpt::Initialize() { FairDetector::Initialize(); }

// Sets the dimension of the Magnetic Spectrometer volume in which the HPT are placed
void Hpt::SetZsize(const Double_t Mudetsize) { zSizeMudet = Mudetsize; }
...
void Hpt::SetHPTNumber(Int_t nHPT) { fnHPT = nHPT; }

void Hpt::SetDesign(Int_t Design) { fDesign = Design; }

void Hpt::SetSurroundingDetHeight(Double_t height) { fSRHeight = height; }
...
void Hpt::SetSciFiParam(Double_t scifimat_width_,
...
void Hpt::ConstructGeometry()
{
    ...
    if (fDesign < 3) {
        ...
        volMudetBox->AddNode(
            volCbase, 1, new TGeoTranslation(0, -DimY / 2 - fConcreteY / 2, -zSizeMudet / 2 + DimZ / 2));
        ...
        volMudetBox->AddNode(
            volCbase, 2, new TGeoTranslation(0, -DimY / 2 - fConcreteY / 2, -zSizeMudet / 2 + 3 * DimZ / 2 + 55 * cm));
        ...
        volMudetBox->AddNode(
            volCbase, 5, new TGeoTranslation(0, -DimY / 2 - fConcreteY / 2, zSizeMudet / 2 - 3 * DimZ / 2 - 55 * cm));

        volMudetBox->AddNode(volHPT, 6, new TGeoTranslation(0, 0, zSizeMudet / 2 - DimZ / 2));
        volMudetBox->AddNode(
            volCbase, 6, new TGeoTranslation(0, -DimY / 2 - fConcreteY / 2, zSizeMudet / 2 - DimZ / 2));
    }
    if (fDesign == 3) {
        ...
        TGeoBBox *HPT_honeycomb_box =
            new TGeoBBox("HPT_honeycomb_box", HPTrackerX / 2, HPTrackerY / 2, honeycomb_z / 2);
        ...
        // SciFi planes
        TGeoBBox *HPT_scifi_plane_hor_box =
            new TGeoBBox("HPT_scifi_plane_hor_box", HPTrackerX / 2, HPTrackerY / 2, scifimat_z / 2);
        TGeoVolume *HPT_scifi_plane_hor_volume =
            new TGeoVolume("HPT_scifi_plane_hor", HPT_scifi_plane_hor_box, SciFiMat);
        HPT_scifi_plane_hor_volume->SetVisibility(1);

        TGeoBBox *HPT_scifi_plane_vert_box =
            new TGeoBBox("HPT_scifi_plane_vert_box", HPTrackerX / 2, HPTrackerY / 2, scifimat_z / 2);
        TGeoVolume *HPT_scifi_plane_vert_volume =
            new TGeoVolume("HPT_scifi_plane_vert", HPT_scifi_plane_vert_box, SciFiMat);
        HPT_scifi_plane_vert_volume->SetVisibility(1);

        // SciFi mats
        TGeoBBox *HPT_scifimat_hor_box =
            new TGeoBBox("HPT_scifimat_hor_box", scifimat_hor / 2, scifimat_width / 2, scifimat_z / 2);
        ...
        TGeoBBox *HPT_scifimat_vert_box =
            new TGeoBBox("HPT_scifimat_vert_box", scifimat_width / 2, scifimat_vert / 2, scifimat_z / 2);
        ...
        for (int i = 0; i < n_hor_planes; i++) {
            HPT_scifi_plane_hor_volume->AddNode(
                HPT_scifimat_hor_volume,
                i + 1,
                new TGeoTranslation(0, (-(n_hor_planes - 1) / 2.0 + i) * scifimat_width, 0));
        }
        for (int i = 0; i < n_vert_planes; i++) {
            HPT_scifi_plane_vert_volume->AddNode(
                HPT_scifimat_vert_volume,
                100 + i + 1,
                new TGeoTranslation((-(n_vert_planes - 1) / 2.0 + i) * scifimat_width, 0, 0));
        }

        volDT->AddNode(HPT_support_volume, 0, new TGeoTranslation(0, 0, -DimZ / 2 + support_z / 2));
        volDT->AddNode(
            HPT_scifi_plane_hor_volume, 0, new TGeoTranslation(0, 0, -DimZ / 2 + support_z + scifimat_z / 2));
        volDT->AddNode(HPT_scifi_plane_vert_volume,
                       0,
                       new TGeoTranslation(0, 0, -DimZ / 2 + support_z + scifimat_z + scifimat_z / 2));
        volDT->AddNode(HPT_honeycomb_volume,
                       0,
                       new TGeoTranslation(0, 0, -DimZ / 2 + support_z + 2 * scifimat_z + honeycomb_z / 2));
        volDT->AddNode(HPT_support_volume,
                       1,
                       new TGeoTranslation(0, 0, -DimZ / 2 + support_z + 2 * scifimat_z + honeycomb_z + support_z / 2));
        //////////////////////////////////////////////////////////////

        Double_t first_DT_position = -DZMagnetizedRegion / 2 + DZTarget + DimZ / 2;
        for (int i = 0; i < fnHPT; i++) {
            for (int j = 0; j < fntarget; j++) {
                volMagRegion->AddNode(
                    volDT,
                    (i + 1) * 1000 + j * fnHPT,
                    new TGeoTranslation(0,
                                        0,
                                        first_DT_position + i * (fDistance + DimZ)
                                            + j * (DZTarget + fnHPT * DimZ + (fnHPT - 1) * fDistance)));
            }
        }
    }
}
...
void Hpt::EndOfEvent() { fHptPointCollection->Clear(); }
...
void Hpt::Register()
{
    ...
    FairRootManager::Instance()->Register("HptPoint", "Hpt", fHptPointCollection, kTRUE);
}
...
{
    if (iColl == 0) {
        return fHptPointCollection;
    } else {
        return NULL;
    }
}

void Hpt::Reset() { fHptPointCollection->Clear(); }
