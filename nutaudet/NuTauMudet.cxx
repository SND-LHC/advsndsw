...
void NuTauMudet::SetMagneticField(Double_t B) { fField = B; }

void NuTauMudet::SetGapDownstream(Double_t Gap) { fGapDown = Gap; }

void NuTauMudet::SetGapMiddle(Double_t Gap) { fGapMiddle = Gap; }

void NuTauMudet::SetZDimensionArm(Double_t Z) { fZArm = Z; }
...
void NuTauMudet::SetNRpcInArm(Int_t N) { fNRpc = N; }
...
void NuTauMudet::Initialize() { FairDetector::Initialize(); }
...
void NuTauMudet::ConstructGeometry()
{
    ...
    if (fDesign != 3) {
        ...
        for (int i = 0; i < fNCoil; i++) {
            volCoilContainer->AddNode(
                volCoil, i, new TGeoTranslation(0, 0, -40 * cm + fCoilW / 2 + i * (fCoilGap + fCoilW)));
        }
        ...
        for (int i = 0; i < fNCoil; i++) {
            volUpYoke->AddNode(
                volCoilV,
                i,
                new TGeoTranslation(fXRyoke / 2 - fCoilH / 2, 0, -40 * cm + fCoilW / 2 + i * (fCoilGap + fCoilW)));
        }
        for (int i = 0; i < fNCoil; i++) {
            volUpYoke->AddNode(
                volCoilV,
        ...
        for (Int_t i = 0; i < fNFe; i++) {
            volArm1->AddNode(
                volIron, nr + 100 + i, new TGeoTranslation(0, 0, -fZArm / 2 + i * (fZFe + fZRpc) + fZFe / 2));
        }
        ...
        for (Int_t i = 0; i < fNRpc; i++) {
            volArm1->AddNode(
                volRpcContainer,
                nr + i,
                new TGeoTranslation(0, -fYFe / 2 + fYRpc / 2, -fZArm / 2 + (i + 1) * fZFe + i * fZRpc + fZRpc / 2));
        }
        ...
        for (Int_t i = 0; i < fNFe; i++) {
            volArm2->AddNode(
                volIron2, nr + 100 + i, new TGeoTranslation(0, 0, -fZArm / 2 + i * (fZFe + fZRpc) + fZFe / 2));
        }
        ...
        tTauNuDet->AddNode(Pillar1Vol,
                           1,
                           new TGeoTranslation(-fXtot / 2 + fPillarX / 2,
                                               -fYtot / 2 - fPillarY / 2,
                                               fZcenter - fZArm / 2 - fGapMiddle / 2 + fPillarZ / 2));
        tTauNuDet->AddNode(Pillar1Vol,
                           2,
                           new TGeoTranslation(fXtot / 2 - fPillarX / 2,
                                               -fYtot / 2 - fPillarY / 2,
    ...
    if (fDesign == 3) {
        Double_t supportasymmetry = fUpSuppY - fLowSuppY;   // upper and lower support have different dimensions, so the
                                                            // mother box must be large enough to contain both
        Int_t nr = 1E4;
        ...
        volMudetBox->AddNode(
            volUpperCover, 1, new TGeoTranslation(0, fYFe / 2 + fYCov / 2 + fUpSuppY, -fZtot / 2 + fZCov / 2));
        ...
        volMudetBox->AddNode(volLateralCover,
                             1,
                             new TGeoTranslation(-fXFe / 2. - fLatSuppX - fXLateral / 2.,
                                                 -fYFe / 2. + fYLateral / 2. + fYSpacing + 4. * cm,
                                                 -fZtot / 2 + fZLateral / 2));   // low right
        volMudetBox->AddNode(volLateralCover,
                             2,
                             new TGeoTranslation(+fXFe / 2. + fLatSuppX + fXLateral / 2.,
                                                 -fYFe / 2. + fYLateral / 2. + fYSpacing + 4. * cm,
                                                 -fZtot / 2 + fZLateral / 2));   // low left
        volMudetBox->AddNode(volLateralCover,
                             3,
                             new TGeoTranslation(-fXFe / 2. - fLatSuppX - fXLateral / 2.,
                                                 +fYFe / 2. - fYLateral / 2. - fYSpacing - 4. * cm,
                                                 -fZtot / 2 + fZLateral / 2));   // up right
        volMudetBox->AddNode(volLateralCover,
                             4,
                             new TGeoTranslation(+fXFe / 2. + fLatSuppX + fXLateral / 2.,
                                                 +fYFe / 2. - fYLateral / 2. - fYSpacing - 4. * cm,
                                                 -fZtot / 2 + fZLateral / 2));   // up left

        //*********** LATERAL CROSSES************

        Double_t Inclination = TMath::Pi() / 2 - TMath::ATan(fYCross / (fZCross - fWidthArm));
        TGeoRotation *Crossrot1 = new TGeoRotation("Crossrot1", 0., 0., 0.);
        TGeoRotation *Crossrot2 = new TGeoRotation("Crossrot2", 0., 0., 0.);
        Crossrot1->RotateY(-90);
        Crossrot2->RotateY(90);
        Crossrot1->SetName("NegativeRot");
        Crossrot2->SetName("PositiveRot");
        Crossrot1->RegisterYourself();
        Crossrot2->RegisterYourself();

        TGeoPara *ArmCross1 = new TGeoPara("ArmCross1",
                                           fWidthArm / 2.,
                                           fYCross / 2,
                                           fXCross / 2.,
                                           TMath::RadToDeg() * Inclination,
                                           0.,
                                           0.);   // length and height are not x and y here, because it will be rotated!
        ArmCross1->SetName("ARMCROSS1");
        TGeoPara *ArmCross2 = new TGeoPara("ArmCross2",
                                           fWidthArm / 2.,
                                           fYCross / 2,
                                           fXCross / 2.,
                                           TMath::RadToDeg() * Inclination,
        ...
        TGeoCompositeShape *MuCross =
            new TGeoCompositeShape("MUFILTERCROSS", "ARMCROSS1:NegativeRot+ARMCROSS2:PositiveRot");
        TGeoVolume *volMuDetCross = new TGeoVolume("volMuDetCross", MuCross, Al);
        volMuDetCross->SetLineColor(kYellow - 7);
        volMudetBox->AddNode(volMuDetCross,
                             1,
                             new TGeoTranslation(-fXFe / 2. - fLatSuppX - fXLateral + fXCross / 2.,
                                                 0.,
                                                 -fZtot / 2 + fZCross / 2));   // right
        volMudetBox->AddNode(volMuDetCross,
                             2,
                             new TGeoTranslation(+fXFe / 2. + fLatSuppX + fXLateral - fXCross / 2.,
                                                 0.,
                                                 -fZtot / 2 + fZCross / 2));   // left

        //***********************ADDING CUTS AT MID-LATERAL IN WALLS
        IronLayer->SetName("MUDETIRON");
        IronLayer1->SetName("MUDETIRON1");

        Double_t delta = 0.1;   // to avoid border effects in the cuts (cut is not visualized in viewer, I do not know
                                // if it can affect simulation)
        TGeoTrd2 *Model = new TGeoTrd2(
            "Model",
            fCutHeight / 2,
            0,
            (fZFe + delta) / 2,
            (fZFe + delta) / 2,
            (fCutLength + delta) / 2);   // length and height are not x and y here, because it will be rotated!
        ...
        TGeoCompositeShape *mudetcut =
            new TGeoCompositeShape("MUDETCUT", "(MUDETIRON-MUDETTRIANGLE:MuDetcombright)-MUDETTRIANGLE:MuDetcombleft");
        mudetcut->SetName("MUDETTRIANGCUT");
        // same, for the thin layers
        TGeoCompositeShape *mudetcut1 = new TGeoCompositeShape(
            "MUDETCUT1", "(MUDETIRON1-MUDETTRIANGLE:MuDetcombright)-MUDETTRIANGLE:MuDetcombleft");
        ...
        TGeoBBox *UpperSupport1 =
            new TGeoBBox(fUpSuppX / 2., fUpSuppY / 2., fZFethin / 2.);   // The 1 shapes have less z thickness
        ...
        TGeoTranslation *upright =
            new TGeoTranslation("MuDetupright", -fXFe / 2. + fUpSuppX / 2., fYFe / 2 + fUpSuppY / 2., 0);
        TGeoTranslation *upleft =
            new TGeoTranslation("MuDetupleft", +fXFe / 2. - fUpSuppX / 2., fYFe / 2 + fUpSuppY / 2., 0);
        TGeoTranslation *lateralupleft = new TGeoTranslation(
            "MuDetlateralupleft", +fXFe / 2. + fLowSuppX / 2., fYFe / 2 - fLowSuppY / 2. - fYSpacing, 0);
        TGeoTranslation *lateralupright = new TGeoTranslation(
            "MuDetlateralupright", -fXFe / 2. - fLowSuppX / 2., fYFe / 2 - fLowSuppY / 2. - fYSpacing, 0);
        TGeoTranslation *laterallowleft = new TGeoTranslation(
            "MuDetlaterallowleft", +fXFe / 2. + fLowSuppX / 2., -fYFe / 2 + fLowSuppY / 2. + fYSpacing, 0);
        TGeoTranslation *laterallowright = new TGeoTranslation(
            "MuDetlaterallowright", -fXFe / 2. - fLowSuppX / 2., -fYFe / 2 + fLowSuppY / 2. + fYSpacing, 0);
        TGeoTranslation *lowright =
            new TGeoTranslation("MuDetlowright", -fXFe / 2. + fLowSuppX / 2., -fYFe / 2 - fLowSuppY / 2., 0);
        TGeoTranslation *lowleft =
            new TGeoTranslation("MuDetlowleft", +fXFe / 2. - fLowSuppX / 2., -fYFe / 2 - fLowSuppY / 2., 0);
        ...
        TString *supportaddition = new TString(
            "MUDETTRIANGCUT+MUDETUPSUPPORT:MuDetupright+MUDETUPSUPPORT:MuDetupleft+MUDETLOWSUPPORT:MuDetlowright+"
            "MUDETLOWSUPPORT:MuDetlowleft+MUDETLATERALSUPPORT:MuDetlateralupleft+MUDETLATERALSUPPORT:"
            "MuDetlateralupright+MUDETLATERALSUPPORT:MuDetlaterallowleft+MUDETLATERALSUPPORT:MuDetlaterallowright");
        TString *supportaddition1 =
            new TString("MUDETTRIANGCUT1+MUDETUPSUPPORT1:MuDetupright+MUDETUPSUPPORT1:MuDetupleft+MUDETLOWSUPPORT1:"
                        "MuDetlowright+MUDETLOWSUPPORT1:MuDetlowleft+MUDETLOWSUPPORT1:MuDetlowleft+"
                        "MUDETLATERALSUPPORT1:MuDetlateralupleft+MUDETLATERALSUPPORT1:MuDetlateralupright+"
                        "MUDETLATERALSUPPORT1:MuDetlaterallowleft+MUDETLATERALSUPPORT1:MuDetlaterallowright");
        TGeoCompositeShape *SupportedIronLayer = new TGeoCompositeShape("SupportedIronLayer", supportaddition->Data());
        TGeoCompositeShape *SupportedIronLayer1 =
            new TGeoCompositeShape("SupportedIronLayer1", supportaddition1->Data());
        ...
        */
        ...
        TGeoTrd2 *Indentation = new TGeoTrd2("Indentation",
                                             (fYRpc_outer - 2 * (31.2 * cm + 15. * cm)) / 2,
                                             (fYRpc_outer - 2 * 31.2 * cm) / 2,
                                             (fZRpc_outer + 0.1 * cm) / 2,
                                             (fZRpc_outer + 0.1 * cm) / 2,
                                             (15. * cm + 0.1 * cm) / 2);   // (b, B, Z_up, Z_down, h)
        ...
        TGeoCompositeShape *RpcShell = new TGeoCompositeShape(
            "RpcShell", "RPCOUTER-INDENTATION:RIGHTINDENT-INDENTATION:LEFTINDENT-RPCINNER:EXCLUSION");
        ...
        TGeoTranslation *mdown =
            new TGeoTranslation("mdown", 0., -(fYRpcGap + 6 * cm) / 2, 0.);   // 6 cm is the spacing between two gaps
        mdown->SetName("MDOWN");
        ...
        TGeoTrd2 *Indentation_0 = new TGeoTrd2("Indentation_0",
                                               (fYRpc_outer - 2 * (31.2 * cm + 15. * cm)) / 2,
                                               (fYRpc_outer - 2 * 31.2 * cm) / 2,
                                               (fZRpc) / 2,
                                               (fZRpc) / 2,
        ...
        TGeoCompositeShape *RpcContainer = new TGeoCompositeShape(
            "RpcContainer", "RPCCOINTAINER_0-INDENTATION_0:RIGHTINDENT-INDENTATION_0:LEFTINDENT");
        TGeoVolume *volRpcContainer = new TGeoVolume("volRpcContainer", RpcContainer, air);
        //***********************************
        ...
        volRpcContainer->AddNode(
            volRpcShell, 1, new TGeoTranslation(0., 0., -fZGas / 2 - fZRpcGap - fZStrip - fZRpc_outer / 2));
        ...
        for (Int_t i = 0; i < fNRpc; i++) {
            ...
            if (i >= fNFe)
                dz = dz - (i + 1 - fNFe) * (fZFe - fZFethin);
            if (i % 2)
                volMudetBox->AddNode(volRpcContainer, nr + i, new TGeoTranslation(0, -dy, dz));   // staggering
            else {
                volMudetBox->AddNode(volRpcContainer, nr + i, new TGeoTranslation(0, dy, dz));
            }
        }
        ...
        // tTauNuDet->AddNode(Pillar1Vol,1, new
        // TGeoTranslation(-fXtot/2+fPillarX/2,-fYtot/2-fPillarY/2,fZcenter-fZtot/2+fPillarZ/2));
        // tTauNuDet->AddNode(Pillar1Vol,2, new TGeoTranslation(fXtot/2-fPillarX/2,-fYtot/2-fPillarY/2,fZcenter-fZtot/2
        // +fPillarZ/2));
        //       tTauNuDet->AddNode(Pillar1Vol,3, new
        //       TGeoTranslation(-fXtot/2+fPillarX/2,-fYtot/2-fPillarY/2,fZcenter+fZtot/2-fPillarZ/2)); //eventually two
        //       pillars at the end. Now muon det is followed by veto, so its steel pillar supports both
        // tTauNuDet->AddNode(Pillar1Vol,4, new
        // TGeoTranslation(fXtot/2-fPillarX/2,-fYtot/2-fPillarY/2,fZcenter+fZtot/2-fPillarZ/2));
    }
}
...
Bool_t NuTauMudet::ProcessHits(FairVolume *vol)
{
    ...
    if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
        ...
        // cout << name << " ";
        Int_t motherID = 0;
        if (strcmp(name, "volRpc") == 0) {
            motherID = gGeoManager->GetMother(1)->GetNumber();
        } else {
            motherID = gGeoManager->GetMother(0)->GetNumber();
        }
...
void NuTauMudet::EndOfEvent() { fShipRpcPointCollection->Clear(); }
...
void NuTauMudet::Register()
{
    ...
    */
    ...
    FairRootManager::Instance()->Register("ShipRpcPoint", "NuTauMudet", fShipRpcPointCollection, kTRUE);
}
...
{
    if (iColl == 0) {
        return fShipRpcPointCollection;
    } else {
        return NULL;
    }
}

void NuTauMudet::Reset() { fShipRpcPointCollection->Clear(); }
