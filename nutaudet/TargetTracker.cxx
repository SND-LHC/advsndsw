...
void TargetTracker::ConstructGeometry()
{
    ...
    // SciFi planes
    TGeoBBox* TT_scifi_plane_hor_box =
        new TGeoBBox("TT_scifi_plane_hor_box", TTrackerX / 2, TTrackerY / 2, scifimat_z / 2);
    ...
    TGeoBBox* TT_scifi_plane_vert_box =
        new TGeoBBox("TT_scifi_plane_vert_box", TTrackerX / 2, TTrackerY / 2, scifimat_z / 2);
    ...
    TGeoBBox* TT_scifimat_hor_box =
        new TGeoBBox("TT_scifimat_hor_box", scifimat_hor / 2, scifimat_width / 2, scifimat_z / 2);
    ...
    for (int i = 0; i < n_hor_planes; i++) {
        TT_scifi_plane_hor_volume->AddNode(
            TT_scifimat_hor_volume, i + 1, new TGeoTranslation(0, (-(n_hor_planes - 1) / 2.0 + i) * scifimat_width, 0));
    }
    for (int i = 0; i < n_vert_planes; i++) {
        TT_scifi_plane_vert_volume->AddNode(
            TT_scifimat_vert_volume,
            100 + i + 1,
    ...
    TT_volume->AddNode(
        TT_scifi_plane_hor_volume, 0, new TGeoTranslation(0, 0, -TTrackerZ / 2 + support_z + scifimat_z / 2));
    TT_volume->AddNode(TT_scifi_plane_vert_volume,
                       0,
                       new TGeoTranslation(0, 0, -TTrackerZ / 2 + support_z + scifimat_z + scifimat_z / 2));
    TT_volume->AddNode(TT_honeycomb_volume,
                       0,
                       new TGeoTranslation(0, 0, -TTrackerZ / 2 + support_z + 2 * scifimat_z + honeycomb_z / 2));
    TT_volume->AddNode(
        TT_support_volume,
        1,
        new TGeoTranslation(0, 0, -TTrackerZ / 2 + support_z + 2 * scifimat_z + honeycomb_z + support_z / 2));
    ...
    for (int l = 0; l < fNTT; ++l) {
        volTarget->AddNode(
            TT_volume, 1000 * (l + 1), new TGeoTranslation(0, 0, first_tt_position + l * (TTrackerZ + CellWidth)));
    }
}
...
void TargetTracker::EndOfEvent() { fTTPointCollection->Clear(); }
...
void TargetTracker::Reset() { fTTPointCollection->Clear(); }
