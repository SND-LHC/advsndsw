...
void Target::MergeTopBot(Bool_t SingleEmFilm) { fsingleemulsionfilm = SingleEmFilm; }
...
void Target::ConstructGeometry()
{
    ...
    if (fDesign != 2) {
        ...
        if (fDesign == 0)   // TP
            MagnetVol->AddNode(
                volTarget, 1, new TGeoTranslation(0, -fMagnetY / 2 + fColumnY + fCoilH2 + YDimension / 2, 0));
        if (fDesign == 1)   // NEW
        ...
        if (fDesign == 3) {
            ...
            Double_t ZDimMagnetizedRegion =
                ((TGeoBBox *)volMagRegion->GetShape())->GetDZ() * 2.;   // n.d.r. DZ is the semidimension
            for (int i = 0; i < fNTarget; i++) {
                volMagRegion->AddNode(volTarget,
                                      i + 1,
                                      new TGeoTranslation(0,
                                                          0,
                                                          -ZDimMagnetizedRegion / 2 + ZDimension / 2.
                                                              + i * (ZDimension + 3 * fHpTDZ + 2 * fHpTDistance)));
            }
        }
    }
    ...
    for (Int_t n = 0; n < NPlates; n++) {
        volBrick->AddNode(volLead,
                          n,
                          new TGeoTranslation(0,
                                              0,
                                              -BrickZ / 2 + BrickPackageZ / 2 + EmPlateWidth + LeadThickness / 2
    ...
    if (fDesign != 2) {
        ...
        for (Int_t n = 0; n < NRohacellGap; n++) {
            volCES->AddNode(volRohGap,
                            n,
                            new TGeoTranslation(0,
                                                0,
                                                -CESWidth / 2 + CESPackageZ / 2 + EmPlateWidth + RohacellGap / 2
                                                    + n * LayerCESWidth));   // ROHACELL
        }
        if (fsingleemulsionfilm) {   // simplified configuration, unique sensitive layer for the whole emulsion plate
            ...
            for (Int_t n = 0; n < NRohacellGap + 1; n++) {
                volCES->AddNode(
                    volEmulsionFilmCES,
                    n,
                    new TGeoTranslation(0, 0, -CESWidth / 2 + CESPackageZ / 2 + EmPlateWidth / 2 + n * LayerCESWidth));
            }

        } else {   // more accurate configuration, two emulsion films divided by a plastic base

            TGeoBBox *EmulsionFilmCES =
                new TGeoBBox("EmulsionFilmCES", EmulsionX / 2, EmulsionY / 2, EmulsionThickness / 2);
            ...
            for (Int_t n = 0; n < NRohacellGap + 1; n++) {
                volCES->AddNode(
                    ...
                    n,
                    new TGeoTranslation(
                        0, 0, -CESWidth / 2 + CESPackageZ / 2 + EmulsionThickness / 2 + n * LayerCESWidth));   // BOTTOM
                volCES->AddNode(volEmulsionFilmCES,
                                n,
                                new TGeoTranslation(0,
                                                    0,
                                                    -CESWidth / 2 + CESPackageZ / 2 + 3 * EmulsionThickness / 2
                                                        + PlasticBaseThickness + n * LayerCESWidth));   // TOP
                volCES->AddNode(volPlBaseCES,
                                n,
                                new TGeoTranslation(0,
                                                    0,
                                                    -CESWidth / 2 + CESPackageZ / 2 + EmulsionThickness
                                                        + PlasticBaseThickness / 2
                                                        + n * LayerCESWidth));   // PLASTIC BASE
                //	if(n == 2)
                // cout << "-CESWidth/2+3*EmulsionThickness/2+PlasticBaseThickness+n*LayerCESWidth = " <<
                // -CESWidth/2+3*EmulsionThickness/2+PlasticBaseThickness+n*LayerCESWidth << endl;
            }
        }
    ...
    if (fDesign == 2) {
        ...
        if (fDesign == 2) {
            ...
            tTauNuDet->AddNode(PillarVol,
                               1,
                               new TGeoTranslation(-XDimension / 2 + fPillarX / 2,
                                                   -YDimension / 2 - fBaseY - fPillarY / 2,
                                                   fCenterZ - ZDimension / 2 + fPillarZ / 2));
            tTauNuDet->AddNode(PillarVol,
                               2,
                               new TGeoTranslation(XDimension / 2 - fPillarX / 2,
                                                   -YDimension / 2 - fBaseY - fPillarY / 2,
    ...
    if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
        ...
        for (Int_t i = 0; i < MaxL; i++) {
            ...
            if (motherV[0] == 1 && motherV[0] != detID) {
                if (strcmp(mumname, "Brick") == 0 || strcmp(mumname, "CES") == 0)
                    NColumn = motherV[i];
        ...
        detID = (NWall + 1) * 1E7 + (NRow + 1) * 1E6 + (NColumn + 1) * 1E4 + BrickorCES * 1E3 + (NPlate + 1) * 1E1
                + EmTop * 1;
        ...
        //	cout << "ID = "<< fTrackID << "   pdg = " << pdgCode << ";   M = " << fMotherID << "    Npl = " <<
        //NPlate<<";  NCol = " << NColumn << ";  NRow = " << NRow << "; NWall = " << NWall<< "  P = " << fMom.Px() << ",
        //"<< fMom.Py() << ", " << fMom.Pz() << endl;
                           ...
                           Bool_t &EmTop)
{
    ...
    // cout << "NPlate = " << NPlate << ";  NColumn = " << NColumn << ";  NRow = " << NRow << "; NWall = " << NWall <<
    // endl; cout << "BrickorCES = " << BrickorCES <<endl; cout << "EmCES = " << EmCES << ";    EmBrick = " << EmBick <<
    // endl; cout << endl;
}

void Target::EndOfEvent() { fTargetPointCollection->Clear(); }
...
void Target::Reset() { fTargetPointCollection->Clear(); }
