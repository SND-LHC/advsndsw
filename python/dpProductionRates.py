...
def brMesonToMesonDP(mass, epsilon, mumPdg, dauPdg, doprint=False):
    ...
    if mass < (mMeson - mDaughterMeson):
        ...
        fac2 = pow(
            (mass**2.0 - (mMeson + mDaughterMeson) ** 2.0)
            * (mass**2.0 - (mMeson - mDaughterMeson) ** 2.0),
            1.5,
        )
...
def brMesonToDP(mass, epsilon, mumPdg, doprint=False):
    if mumPdg == 223:
        ...
        return brMesonToMesonDP(mass, epsilon, mumPdg, 113, doprint), brMesonToGammaDP(
            mass, epsilon, mumPdg, doprint
        )
    else:
        print(
            "Warning! Unknown mother pdgId %d, not implemented. Setting br to 0."
            % mumPdg
        )
        return 1
