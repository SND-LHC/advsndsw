...
class SndlhcDigi:
    ...
    def __init__(self, fout):
        ...
        self.digiScifi2MCPointsBranch = self.sTree.Branch(
            "Digi_ScifiHits2MCPoints", self.digiScifi2MCPoints, 32000, 1
        )
        ...
        self.digiMuFilterBranch = self.sTree.Branch(
        ...
        self.digiMuFilter2MCPointsBranch = self.sTree.Branch(
            "Digi_MuFilterHits2MCPoints", self.digiMuFilter2MCPoints, 32000, 1
        )

    def digitize(self):
        self.header.SetRunId(self.sTree.MCEventHeader.GetRunID())
        self.header.SetEventNumber(
            self.sTree.MCEventHeader.GetEventID()
        )  # counts from 1
    ...
    def digitizeScifi(self):
        ...
        for p in self.sTree.ScifiPoint:
            ...
            if not locFibreID in self.siPMFibres:
                continue  # fibres in dead areas
        ...
        for detID in hitContainer:
            ...
            if self.digiScifi.GetSize() == index:
                self.digiScifi.Expand(index + 100)
    ...
    def digitizeMuFilter(self):
        ...
        for detID in hitContainer:
            ...
            if self.digiMuFilter.GetSize() == index:
                self.digiMuFilter.Expand(index + 100)
