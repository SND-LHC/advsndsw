...
def configure(run, ship_geo):
    ...
    if hasattr(ship_geo, "tauMudet"):  # don't support old designs
        if ship_geo.muShieldDesign not in [2, 3, 4] and hasattr(
            ...
            if hasattr(ship_geo.tauMudet, "ZFethin"):
                ...
                taumuondetector.SetFeDimensions(
                    ship_geo.tauMudet.XFe, ship_geo.tauMudet.YFe, ship_geo.tauMudet.ZFe
                )
                taumuondetector.SetNFeInArm(ship_geo.tauMudet.NFe)
            taumuondetector.SetRpcDimensions(
                ship_geo.tauMudet.XRpc, ship_geo.tauMudet.YRpc, ship_geo.tauMudet.ZRpc
            )
            taumuondetector.SetRpcGasDimensions(
                ship_geo.tauMudet.XGas, ship_geo.tauMudet.YGas, ship_geo.tauMudet.ZGas
            )
            ...
            taumuondetector.SetRpcElectrodeDimensions(
                ship_geo.tauMudet.XEle, ship_geo.tauMudet.YEle, ship_geo.tauMudet.ZEle
            )
            taumuondetector.SetRpcPETDimensions(
                ship_geo.tauMudet.XPet, ship_geo.tauMudet.YPet, ship_geo.tauMudet.ZPet
            )
            taumuondetector.SetNRpcInArm(ship_geo.tauMudet.NRpc)
            taumuondetector.SetUpperCoverDimensions(
                ship_geo.tauMudet.XCov, ship_geo.tauMudet.YCov, ship_geo.tauMudet.ZCov
            )
    ...
    if not hasattr(ship_geo.Bfield, "fieldMap"):
        ...
        if ship_geo.muShieldDesign == 6:
            fMagField.IncludeTarget(
                ship_geo.target.xy, ship_geo.target.z0, ship_geo.target.length
            )
