...
def readHists(h, fname, wanted=[], withProj=True):
    ...
    for akey in f.GetListOfKeys():
        ...
        if not cln.find("TCanv") < 0:
            h[hname] = obj.Clone()
        if cln.find("TH") < 0 and cln.find("TP") < 0 and cln.find("TE") < 0:
            continue
        if hname in h:
            if hasattr(h[hname], "Add"):
                rc = h[hname].Add(obj)
            if not rc:
...
def bookHist(
    ...
    if key in h:
        ...
        h[key] = TH3D(
            rkey, title, nbinsx, xmin, xmax, nbinsy, ymin, ymax, nbinsz, zmin, zmax
        )
    elif nbinsy > 0:
...
def bookProf(
    h, key=None, title="", nbinsx=100, xmin=0, xmax=1, ymin=None, ymax=None, option=""
):
    if key == None:
        print("missing key")
        return
...
def setAttributes(pyl, leaves, printout=False):
    names = {}
    if printout:
        print("entries", leaves.GetEntries())
...
def container_sizes(sTree, perEvent=False):
    ...
    for l in sTree.GetListOfLeaves():
        ...
        print(
            ...
            % (
                ...
                b.GetZipBytes() / 1.0e6,
                b.GetTotBytes() / 1.0e6,
                b.GetTotalSize() / 1.0e6,
            )
        )
        bnm = nm.split(".")[0]
        if bnm not in counter:
            counter[bnm] = [0, 0, 0]
        counter[bnm][0] += b.GetZipBytes() / 1.0e6
        counter[bnm][1] += b.GetTotBytes() / 1.0e6
        counter[bnm][2] += b.GetTotalSize() / 1.0e6
        counter["total"][0] += b.GetZipBytes() / 1.0e6
        counter["total"][1] += b.GetTotBytes() / 1.0e6
        counter["total"][2] += b.GetTotalSize() / 1.0e6
    print("---> SUMMARY <---------------")
    N = sTree.GetEntries() / 1000.0
    if perEvent:
        print(
    ...
    for i in range(len(sorted_c)):
        ...
        if perEvent:
            print(
                "%30s :%8.3F      %8.3F       %8.3F"
                % (x, counter[x][0] / N, counter[x][1] / N, counter[x][2] / N)
            )
        else:
            print(
                "%30s :%8.3F   %8.3F    %8.3F"
                % (x, counter[x][0], counter[x][1], counter[x][2])
            )
...
def stripOffBranches(fout):
    ...
    if sTree.GetBranch("TargetPoint"):
        if sTree.GetLeaf("cbmroot.Target.TargetPoint.fEmTop"):
            oldTargetClass = True  # old class
        else:
    ...
    if not oldTargetClass:
        newTree = sTree.CloneTree(-1, "fast")
    else:
        newTree = sTree.CloneTree(0)
        for n in range(nEvents):
            sTree.GetEntry(n)
            if oldTargetClass:
                sTree.TargetPoint.Clear()
            rc = newTree.Fill()
    ...
    f = TFile(sFile)
    sTree = f.cbmsim
    if nEvents == sTree.GetEntries():
        print("looks ok, could be deleted", os.path.abspath("."))
    else:
        print("stripping failed, keep old file", os.path.abspath("."))
...
def makeIntegralDistrib(h, key):
    ...
    for n in range(1, h[key].GetNbinsX() + 1):
        if n == 1:
            h[name].SetBinContent(1, h[key].GetSumOfWeights())
        else:
            h[name].SetBinContent(
                n, h[name].GetBinContent(n - 1) - h[key].GetBinContent(n - 1)
            )
