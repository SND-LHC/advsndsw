...
class Task:
    ...
    def __init__(self, t):
        self.SBTefficiency = (
            0.99  # Surrounding Background tagger: 99% efficiency picked up from TP
        )
        self.SVTefficiency = (
            0.995  # Straw Veto tagger: guestimate, including dead channels
        )
    ...
    def RPC_decision(self, mcParticle=None):
        ...
        veto = (
            nHits > 0
        )  # 10  change to >0 since neutrino background ntuple not possible otherwise
        if veto:
            w = 0.0
        # print 'RPC :',nHits
        return veto, w, nHits
