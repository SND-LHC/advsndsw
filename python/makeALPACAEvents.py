...
def Ctau(mres, gax):
    return (
        ...
        * 65.0
        * math.pi
        / ((float(gax) * float(gax)) * (float(mres) * float(mres) * float(mres)))
    )
...
def inputWrite(mres, gax, nev, Lmin, Lmax):  # need to apply Rmin & Rmax as well..
    ...
    with open("input.DAT", "w") as f:
        ...
        f.write(
            "'prot'       ! [btype] : Beam type ('prot' = proton, 'elec' = electron)\n"
        )
        ...
        f.write(
            "1d-2         ! [athetamax] : Maximum ALP theta (when adecay is false)\n"
        )
        f.write(
            "****************************************************************************************\n"
        )
        ...
        f.write(
            "%s           ! [nev] : Number of events ( < 1000000 recommended)\n" % (nev)
        )
        f.write(
        ...
        f.write(
            ".true.       ! [gencuts] : Generate cuts - [rmin], [rmax], [dmin], [emin]\n"
        )
...
def ntupleWrite(ctau, fn, Lmin, Lmax, startZ, endZ, SmearBeam, mres, gax, old):
    ...
    with open(
        wdir + "/outputs/output_" + str(mres) + "_" + str(gax) + ".dat", "r"
    ) as fp:
        for line in fp:
    ...
    fn = fn.readlines()
    inputFile = (
        old + "/" + "alp_m" + str(mres) + "_g" + str(gax) + "_xs" + str(xs) + ".root"
    )
    ...
    for i, j in enumerate(range(5, len(fn), 9)):
        LS = ROOT.gRandom.Uniform(Lmin * 100.0, Lmax * 100.0)
        zinter = ROOT.gRandom.Uniform(startZ, endZ)
        dx, dy = (
            ROOT.gRandom.Uniform(-1, +1) * SmearBeam,
            ROOT.gRandom.Uniform(-1, +1) * SmearBeam,
        )
        tr = fn[j].split()
        dau1 = fn[j + 1].split()
...
def runEvents(mres, gax, nev, Lmin, Lmax, startZ, endZ, SmearBeam):
    print(
        "ALPACA is starting for mass of {} GeV with photon coupling coeffiecient {} GeV^-1.".format(
            mres, gax
        )
    )
    pdg = ROOT.TDatabasePDG.Instance()
    pdg.AddParticle("a", "ALP", mres, False, gax, 0.0, "a", 9900015)
    wdir = os.environ["ALPACABIN"]
    old = os.getcwd()
    os.chdir(wdir)
    ...
    inputFile = ntupleWrite(
        ctau, fn, Lmin, Lmax, startZ, endZ, SmearBeam, mres, gax, old
    )
    os.chdir(old)
