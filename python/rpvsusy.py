...
class constants:
    ...
    def __init__(self):
        ...
        self.gW2 = (
            self.GF / math.sqrt(2) * 8 * self.MW * self.MW
        )  # SU(2)L gauge coupling squared
        self.s2thetaw = 0.23126  # square sine of the Weinberg angle
        self.t2thetaw = self.s2thetaw / (
            1 - self.s2thetaw
        )  # square tan of the Weinberg angle
...
class RPVSUSYbranchings:
    ...
    def AddChannelsToPythia(self, P8Gen, verbose=True):
        ...
        for decay in decays_tmp:
            ...
            if (
                ...
                bf = bf / 2.0
                P8Gen.SetParameters(
                    "9900015:addChannel = 1 {:.12} 0 {}".format(
                        bf, " ".join(str(code) for code in codes)
                    )
                )
                P8Gen.SetParameters(
                    "9900015:addChannel = 1 {:.12} 0 {}".format(
        ...
        if "K*" in H or "D*" in H or "phi" in H:
            if "nu_mu" in L or "nu_e" in L or "nu_tau" in L:
                tmp_width = (
                    ...
                    * c.decayConstant[H] ** 2
                    * (
                        2 * (self.MN**2 - mass(L) ** 2) ** 2
                        - mass(H) ** 2 * (mass(H) ** 2 + self.MN**2 + mass(L) ** 2)
                    )
                )
            else:
                tmp_width = (
    ...
    def NdecayWidth(self):
        ...
        leplist = [
            dlist[1].strip() for dlist in [re.findall(r"\ \w+", dec) for dec in declist]
        ]
        print(leplist, hadlist)
        totalwidth = sum(
            [self.Width_H_L(hadlist[i], leplist[i]) for i in range(0, len(hadlist))]
        )
        return totalwidth
    ...
    def NprodWidth(self):
        ...
        totalwidth = sum(
            [self.Width_N_L(hadlist[i], leplist[i]) for i in range(0, len(hadlist))]
        )
        return totalwidth
    ...
    def findProdBranchingRatio(self, decayString):
        ...
        gooddec = False
        print(
            "findProdBranchingRation() INFO: You have chosen the decay: '",
            corrdecstring,
        )
        for dec in listdecs:
            if corrdecstring in dec:
        ...
        br = self.Width_N_L(had, lep) / (
            self.Width_N_L(had, lep) + c.hGeV / lifetime(had)
        )
        return br
