...
def nextLevel(lv, magnetMass, onlyWithField, exclude, alreadyPrinted):
    ...
    for da in range(lv.GetNoDaughters()):
        ...
        if lvln.GetNoDaughters() > 0:
            xtmp, dummy = nextLevel(
                lvln, magnetMass, onlyWithField, exclude, alreadyPrinted
            )
            magnetMass += xtmp
        else:
...
def addVMCFields(shipGeo, controlFile="", verbose=False, withVirtualMC=True):
    ...
    if hasattr(shipGeo, "Bfield"):
        ...
        if hasattr(shipGeo.EmuMagnet, "WithConstField"):
            withConstFieldNuTauDet = shipGeo.EmuMagnet.WithConstField
        if not withConstFieldNuTauDet:
            fieldMaker.defineFieldMap(
                "NuMap",
                "files/nuTauDetField.root",
                ROOT.TVector3(0.0, 0.0, shipGeo.EmuMagnet.zC),
            )
        ...
        if len(fieldsList) > 1:
            fieldMaker.defineComposite(
                "TotalField", *fieldsList
            )  # fieldsList MUST have length <=4
...
def printVMCFields():
    ...
    for v in vols:
        ...
        if field:
            ...
            print(
                "Volume {0} has B = ({1}, {2}, {3}) T".format(
                    v.GetName(), B[0] / u.tesla, B[1] / u.tesla, B[2] / u.tesla
                )
            )
