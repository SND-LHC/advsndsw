...
parser.add_option(
    ...
    "--cheated",
    dest="chtd",
    help="cheated=1 to use MC hit x,y instead of wire left&right coordinates",
    default=0,
)
parser.add_option(
    "-d", "--debug", dest="dbg", help="debug=1 to print debug information", default=0
)
parser.add_option(
    "-f",
    "--inputFile",
    dest="input",
    help="input file",
    default="$FAIRSHIP/ship.10.0.Pythia8-TGeant4.root",
)
parser.add_option(
    "-g",
    "--geoFile",
...
parser.add_option(
    ...
    "--monitor",
    dest="mntr",
    help="monitor=1 to obtain the PatRec efficiency wrt MC truth",
    default=0,
)
parser.add_option(
    "-o", "--options", dest="helptext", help="Available options:", default=0
)
parser.add_option(
    "-r",
    "--reconstructible",
    dest="rectracks",
    help="number of reconstructible tracks required",
    default=2,
)
parser.add_option(
    "-s", "--saveDisk", dest="saveDisk", help="save file to disk", default=0
)
parser.add_option(
    "-t", "--threeprong", dest="tp", help="threeprong=1 mumunu decay", default=0
)
...
if not inputFile.find("_patrec.root") < 0:
    ...
    if saveDisk:
        os.system("mv " + inputFile + " " + outFile)
    else:
...
bfield = ROOT.genfit.BellField(
    shipPatRec_prev.ship_geo.Bfield.max,
    shipPatRec_prev.ship_geo.Bfield.z,
    2,
    shipPatRec_prev.ship_geo.Yheight / 2.0 * u.m,
)
...
if global_variables.debug:
    ...
    print(
        ...
        shipPatRec_prev.ship_geo.strawtubes.InnerStrawDiameter,
    )
    print("Straw wall thickness   :", shipPatRec_prev.ship_geo.strawtubes.WallThickness)
    print(
        "Outer straw diameter   :",
        shipPatRec_prev.ship_geo.strawtubes.OuterStrawDiameter,
    )
...
def EventLoop(SmearedHits):
    ...
    for n in range(nEvents):
        ...
        if shipPatRec_prev.monitor == True:
            shipPatRec_prev.ReconstructibleMCTracks = (
                shipPatRec_prev.getReconstructibleTracks(n, sTree, sGeo)
            )
            if (
                ...
                if global_variables.debug:
                    print(
                        "Number of reconstructible tracks =",
                        len(shipPatRec_prev.ReconstructibleMCTracks),
        ...
        SmearedHits = shipPatRec_prev.SmearHits(
            n, sTree, modules, SmearedHits, shipPatRec_prev.ReconstructibleMCTracks
        )
    ...
    if global_variables.debug:
        print(
            shipPatRec_prev.falsenegative,
            "matched tracks with wrong negative charge from deflection.",
        )
        print(
            shipPatRec_prev.falsepositive,
...
if global_variables.debug:
    if shipPatRec_prev.threeprong == 1:
        debugrootfile = ROOT.TFile(
            str(shipPatRec_prev.reconstructiblerequired)
            + "track-debug-mumunu-"
            + str(nEvents)
            + ".root",
            "RECREATE",
...
scale = 1.0
if shipPatRec_prev.h["fracsame12"].Integral() != 0:
    scale = 1 / shipPatRec_prev.h["fracsame12"].Integral()
shipPatRec_prev.h["fracsame12"].Scale(scale)
scale = 1.0
if shipPatRec_prev.h["fracsame12-y"].Integral() != 0:
    scale = 1 / shipPatRec_prev.h["fracsame12-y"].Integral()
shipPatRec_prev.h["fracsame12-y"].Scale(scale)
scale = 1.0
if shipPatRec_prev.h["fracsame12-stereo"].Integral() != 0:
    scale = 1 / shipPatRec_prev.h["fracsame12-stereo"].Integral()
shipPatRec_prev.h["fracsame12-stereo"].Scale(scale)
scale = 1.0
if shipPatRec_prev.h["fracsame34"].Integral() != 0:
    scale = 1 / shipPatRec_prev.h["fracsame34"].Integral()
shipPatRec_prev.h["fracsame34"].Scale(scale)
scale = 1.0
if shipPatRec_prev.h["fracsame34-y"].Integral() != 0:
    scale = 1 / shipPatRec_prev.h["fracsame34-y"].Integral()
shipPatRec_prev.h["fracsame34-y"].Scale(scale)
scale = 1.0
if shipPatRec_prev.h["fracsame34-stereo"].Integral() != 0:
    scale = 1 / shipPatRec_prev.h["fracsame34-stereo"].Integral()
shipPatRec_prev.h["fracsame34-stereo"].Scale(scale)

scale = 1.0

if shipPatRec_prev.monitor == 1:
    shipPatRec_prev.totalafterpatrec = shipPatRec_prev.totalafterpatrec / (
        shipPatRec_prev.reconstructiblerequired**2
    )
    shipPatRec_prev.totalaftermatching = (
        shipPatRec_prev.totalaftermatching / shipPatRec_prev.reconstructiblerequired
    )

rc = shipPatRec_prev.h["eventspassed"].SetBinContent(
    1, shipPatRec_prev.reconstructibleevents
)
rc = shipPatRec_prev.h["eventspassed"].SetBinContent(
    2, shipPatRec_prev.reconstructiblehorizontalidsfound12
)
rc = shipPatRec_prev.h["eventspassed"].SetBinContent(
    3, shipPatRec_prev.reconstructiblestereoidsfound12
)
rc = shipPatRec_prev.h["eventspassed"].SetBinContent(
    4, shipPatRec_prev.reconstructibleidsfound12
)
rc = shipPatRec_prev.h["eventspassed"].SetBinContent(
    5, shipPatRec_prev.reconstructiblehorizontalidsfound34
)
rc = shipPatRec_prev.h["eventspassed"].SetBinContent(
    6, shipPatRec_prev.reconstructiblestereoidsfound34
)
rc = shipPatRec_prev.h["eventspassed"].SetBinContent(
    7, shipPatRec_prev.reconstructibleidsfound34
)
rc = shipPatRec_prev.h["eventspassed"].SetBinContent(
    8, shipPatRec_prev.totalafterpatrec
)
rc = shipPatRec_prev.h["eventspassed"].SetBinContent(
    9, shipPatRec_prev.totalaftermatching
)

sTree.Write()

if shipPatRec_prev.threeprong == 1:
    shipPatRec_prev.ut.writeHists(
        shipPatRec_prev.h,
        "patrec-"
        + str(shipPatRec_prev.reconstructiblerequired)
        + "track-mumunu-"
