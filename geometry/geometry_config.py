...
with ConfigRegistry.register_config("basic") as c:
    ...
    z4 = 2438.0 * u.cm + magnetIncrease + extraVesselLength
    if strawDesign != 4 and strawDesign != 10:
        ...
        zset = z4 - 4666.0 * u.cm - magnetIncrease - extraVesselLength
        c.Chamber1 = AttrDict(z=zset)
        zset = z4 - 2628.0 * u.cm - magnetIncrease - extraVesselLength / 2.0
        c.Chamber2 = AttrDict(z=zset)
        zset = z4 - 740.0 * u.cm - magnetIncrease
        c.Chamber3 = AttrDict(z=zset)
        zset = z4 - 420.0 * u.cm - magnetIncrease / 2.0
        c.Chamber4 = AttrDict(z=zset)
        zset = z4 - 100.0 * u.cm
        c.Chamber5 = AttrDict(z=zset)
        zset = z4 + 30.0 * u.cm + windowBulge / 2.0
        c.Chamber6 = AttrDict(z=zset)
        c.Veto = AttrDict(z=0 * u.cm)
        c.Veto.innerSupport = 3.0 * u.cm
        c.Veto.innerSupportMed = "steel"
        if tankDesign > 5:
            c.Veto.outerSupport = 5.0 * u.mm
            c.Veto.outerSupportMed = "steel"
            c.Veto.lidThickness = 16.0 * u.mm
        else:
            c.Veto.outerSupport = 8.0 * u.mm
            c.Veto.outerSupportMed = "Aluminum"
            c.Veto.lidThickness = 80.0 * u.mm
        ...
        zset = z4 - 640.0 * u.cm - magnetIncrease
        c.TrackStation2 = AttrDict(z=zset)
        zset = z4 - 840.0 * u.cm - magnetIncrease
        c.TrackStation1 = AttrDict(z=zset)
        zset = z4 - 4406.0 * u.cm - magnetIncrease - extraVesselLength
    ...
    c.scintillator.Rmin = 251.0 * u.cm
    c.scintillator.Rmax = 260.0 * u.cm
    ...
    if strawDesign == 4:
        ...
        c.strawtubes.YPlaneOffset = c.strawtubes.StrawPitch / 4.0
    ...
    c.strawtubes.DeltazView = 10.0 * u.cm
    c.strawtubes.VacBox_x = 300.0 * u.cm
    ...
    if c.magnetDesign > 3:  # MISIS design
        ...
        VesselThick = 37.0 * u.cm  # full thickness
        c.Bfield.x = 251.0 * u.cm + VesselThick  # half apertures
        c.Bfield.y = 501.0 * u.cm + VesselThick + c.Bfield.CoilThick
    ...
    if c.preshowerOption > 0:
        ...
        c.Preshower.XMax = 300.0 * u.cm
        c.Preshower.YMax = 600.0 * u.cm * c.Yheight / (10.0 * u.m)
        ...
        c.Preshower.FilterThickness1 = 2.0 * u.cm

        presShowerDeltaZ = PreshowerLeverArm + 2 * 10 * u.cm + 2 * 2.0 * u.cm

    c.SplitCal = AttrDict(z=0)
    c.SplitCal.ZStart = c.TimeDet.z + c.TimeDet.DZ + 5 * u.cm + presShowerDeltaZ
    c.SplitCal.XMax = 600.0 * u.cm / 2.0  # 290.*u.cm  #half length
    c.SplitCal.YMax = (
        1200.0 * u.cm / 2.0
    )  # 510.*u.cm * c.Yheight / (10.*u.m)   #half length
    ...
    c.SplitCal.StripHalfWidth = (
        3 * u.cm
    )  # c.SplitCal.XMax/(c.SplitCal.NStripsPerModule*c.SplitCal.NModulesInX)
    ...
    if not c.HcalOption < 0:
        c.hcal = AttrDict(z=c.ecal.z + hcalThickness / 2.0 + 45.0 * u.cm)
    ...
    c.Muon = AttrDict(z=0)
    c.Muon.XMax = 300.0 * u.cm
    c.Muon.YMax = 600.0 * u.cm * c.Yheight / (10.0 * u.m)
    ...
    c.muShield.dZ2 = 6.0 * u.m
    c.muShield.dZ3 = 2.5 * u.m
    c.muShield.dZ4 = 3.0 * u.m
    c.muShield.dZ5 = 0.0 * u.m  # 28Oct #5 removed
    c.muShield.dZ6 = 3.0 * u.m
    c.muShield.dZ7 = 3.0 * u.m
    c.muShield.dZ8 = 3.0 * u.m
    ...
    if muShieldDesign == 7:
        ...
        for index in range(2, 8):
            ...
            c.muShield.half_X_max = max(
                c.muShield.half_X_max, 2 * f_l + g_l, 2 * f_r + g_r
            )
    ...
    if muShieldDesign in range(7, 10):
        ...
        c.muShield.z = -(c.decayVolume.length + c.muShield.length) / 2.0

    if muShieldDesign == 3:
        c.muShield.dZ1 = 3.5 * u.m
        c.muShield.dZ2 = 5.0 * u.m
        c.muShield.dZ3 = 3.5 * u.m
        c.muShield.dZ4 = 2.0 * u.m
        c.muShield.dZ5 = 1.0 * u.m
        c.muShield.dZ6 = 3.0 * u.m
        c.muShield.dZ7 = 3.0 * u.m
        c.muShield.dZ8 = 3.0 * u.m
        c.muShield.dXgap = 0.2 * u.m
    ...
    if muShieldDesign == 2:
        ...
        c.muShield.length = (
            ...
            * (
            ...
            + c.muShield.LE
        )  # leave some space for nu-tau detector
    # for passive design, fDesign==1
    if muShieldDesign == 1:
        c.muShield.length = 70 * u.m
        c.muShield.z = (
            -c.decayVolume.length / 2.0 - c.muShield.length / 2.0 - c.muShield.LE
        )  # leave some space for nu-tau
    if muShieldDesign == 3 or muShieldDesign == 4 or muShieldDesign == 5:
        ...
        c.muShield.z = -c.decayVolume.length / 2.0 - c.muShield.length / 2.0
    if muShieldDesign == 6:
        c.muShield.length = (
            2
            * (
    ...
    if targetOpt < 10:
        ...
        c.target.L12 = 8.0 * u.cm
        c.target.M13 = "molybdenum"
        c.target.L13 = 8.0 * u.cm
        c.target.M14 = "tungsten"
        c.target.L14 = 5.0 * u.cm
        c.target.M15 = "tungsten"
        c.target.L15 = 8.0 * u.cm
        c.target.M16 = "tungsten"
        c.target.L16 = 10.0 * u.cm
        c.target.M17 = "tungsten"
        c.target.L17 = 20.0 * u.cm
        c.target.M18 = "tungsten"
        c.target.L18 = 35.0 * u.cm
        c.target.sl = 0.5 * u.cm  # H20 slit *17 times
        c.target.xy = 30.0 * u.cm  # full length in x and y
        c.target.length = (
            17 * c.target.sl
            + c.target.L1
            + 7 * c.target.L2
    ...
    c.target.z = (
        c.hadronAbsorber.z - c.hadronAbsorber.length / 2.0 - c.target.length / 2.0
    )
    c.target.z0 = c.target.z - c.target.length / 2.0
    ...
    if tankDesign == 5:
        ...
        c.strawtubes.StrawLength12 = (
            c.xMax * (c.TrackStation1.z - 2 * c.strawtubes.DeltazView - zF) / (z4 - zF)
        )
        c.strawtubes.StrawLengthVeto = (
            c.xMax * (c.vetoStation.z - c.strawtubes.DeltazView - zF) / (z4 - zF)
        )
        zF = c.target.z0 + c.zFocusY
        c.strawtubes.vetoydim = (
            c.Yheight
            / 2.0
            * (c.vetoStation.z - c.strawtubes.DeltazView - zF)
            / (z4 - zF)
        )
        c.strawtubes.tr12ydim = (
            c.Yheight
            / 2.0
            * (c.TrackStation1.z - 2 * c.strawtubes.DeltazView - zF)
            / (z4 - zF)
        )
        c.strawtubes.tr34ydim = int(c.Yheight / 2.0)
    else:
        c.strawtubes.StrawLength12 = c.strawtubes.StrawLength
        c.strawtubes.StrawLengthVeto = c.strawtubes.StrawLength
        if tankDesign > 5:
            c.strawtubes.StrawLengthVeto = 0.5  # switch of veto strawtracker
        c.strawtubes.vetoydim = int(c.Yheight / 2.0)
        c.strawtubes.tr12ydim = int(c.Yheight / 2.0)
        c.strawtubes.tr34ydim = int(c.Yheight / 2.0)
    ...
    if nuTauTargetDesign != 2:
        ...
        if c.EmuMagnet.Design == 3:
            ...
            c.EmuMagnet.ColY = (
                scale * c.EmuMagnet.Y - 2 * scale * c.EmuMagnet.BaseY
            )  # avoid overlapping between bases and columns
            ...
            c.EmuMagnet.PillarY = (
                10 * u.m - c.EmuMagnet.Y / 2 - 0.1 * u.mm - c.cave.floorHeightMuonShield
            )
        if c.EmuMagnet.Design < 2:
            c.EmuMagnet.WithConstField = (
                True  # older geometries still use constant fields
            )
    ...
    if nuTauTargetDesign <= 2:
        ...
        c.tauMudet.ZFe = 5.0 * u.cm
        c.tauMudet.ZRpc = 2.0 * u.cm
        c.tauMudet.ZArm = (
            c.tauMudet.NFe * c.tauMudet.ZFe + c.tauMudet.NRpc * c.tauMudet.ZRpc
        )
        c.tauMudet.GapD = 27.0 * u.cm
        ...
        c.tauMudet.N = 20
        c.tauMudet.zMudetC = (
            -c.decayVolume.length / 2.0 - c.tauMudet.GapD - c.tauMudet.Ztot / 2
        )
    ...
    if nuTauTargetDesign == 3:
        ...
        c.tauMudet.ZFethick = 15.0 * u.cm
        c.tauMudet.ZFethin = 10.0 * u.cm

        c.tauMudet.XRpc = c.tauMudet.XFe
        c.tauMudet.YRpc = c.tauMudet.YFe
        c.tauMudet.ZRpc = 8.0 * u.cm
        # support structure
        ...
        c.tauMudet.Xtot = (
            c.tauMudet.XFe + 2 * c.tauMudet.LateralSupportX
        )  # now we need to include also supports.
        c.tauMudet.Ytot = (
            c.tauMudet.YFe + c.tauMudet.UpperSupportY + c.tauMudet.LowerSupportY
        )
        c.tauMudet.Ztot = (
            c.tauMudet.NRpc * c.tauMudet.ZRpc
            + c.tauMudet.NFethick * c.tauMudet.ZFethick
            + c.tauMudet.NFethin * c.tauMudet.ZFethin
        )
        # c.tauMudet.zMudetC = -c.decayVolume.length/2. - c.tauMudet.Ztot/2
        c.tauMudet.zMudetC = (
            c.Chamber1.z - c.chambers.Tub1length - c.tauMudet.Ztot / 2 - 31 * u.cm
        )
        # lateral cuts
        c.tauMudet.CutHeight = 78.548 * u.cm
        c.tauMudet.CutLength = (c.tauMudet.CutHeight / 2) / (
            r.TMath.Tan(r.TMath.DegToRad() * 55)
        )
        # upper cover
        ...
        c.tauMudet.YCross = (
            c.tauMudet.YFe
            - 2 * c.tauMudet.YLateral
            - 2 * c.tauMudet.YSpacing
            - 8 * u.cm
        )
        ...
        c.tauMudet.PillarZ = 50 * u.cm
        c.tauMudet.PillarY = (
            10 * u.m - c.cave.floorHeightMuonShield - c.tauMudet.Ytot / 2 - 0.1 * u.mm
        )
    ...
    if nuTauTargetDesign == 2:
        c.NuTauTarget.zC = (
            -c.decayVolume.length / 2.0 - c.tauMudet.GapD - c.tauMudet.Ztot - 2.5 * u.m
        )
    ...
    if (
        c.NuTauTarget.Design == 3
    ):  # One unique magnet, eventually more than one target volume
    ...
    c.NuTauTarget.BrZ = (
        c.NuTauTarget.n_plates * c.NuTauTarget.AllPW
        + c.NuTauTarget.EPlW
        + c.NuTauTarget.BrPackZ
    )
    ...
    c.NuTauTT.scifimat_hor = (
        94.315 * u.cm
    )  # 13.045 * 7 + endpieces = length of hor. mats
    c.NuTauTT.scifimat_vert = (
        146.495 * u.cm
    )  # 13.045 * 11 + endpieces = length of vert. mats
    ...
    c.NuTauTT.TTZ = (
        2 * c.NuTauTT.support_z + 2 * c.NuTauTT.scifimat_z + c.NuTauTT.honeycomb_z
    )
    ...
    c.NuTauTarget.WallYDim = (
        c.NuTauTarget.row * c.NuTauTarget.BrY
        + (c.NuTauTarget.row - 1) * c.NuTauTarget.Ydist
    )
    ...
    c.tauHPT.TZ = (
        2 * c.tauHPT.support_z + 2 * c.tauHPT.scifimat_z + c.tauHPT.honeycomb_z
    )
    ...
    if nuTauTargetDesign != 2:  # TP or NEW with magnet
        ...
        c.NuTauTarget.CESW = (
            2 * c.NuTauTarget.LayerCESW + c.NuTauTarget.EPlW + c.NuTauTarget.CESPack
        )
        c.NuTauTarget.CellW = c.NuTauTarget.BrZ + c.NuTauTarget.CESW
        if nuTauTargetDesign != 3:
            c.NuTauTarget.zdim = (
                c.NuTauTarget.wall * c.NuTauTarget.CellW
                + (c.NuTauTarget.wall + 1) * c.NuTauTT.TTZ
            )
        if nuTauTargetDesign == 3:
            c.NuTauTarget.zdim = (
                c.NuTauTarget.wall * c.NuTauTarget.CellW
                + c.NuTauTarget.wall * c.NuTauTT.TTZ
            )
            c.NuTauTarget.zC = c.EmuMagnet.zC - c.NuTauTarget.zdim / 2.0
            c.tauHPT.TotalDZ = (
                c.EmuMagnet.Z - c.EmuMagnet.Height1
            ) - c.NuTauTarget.zdim  # MagRegion-Target
            c.tauHPT.distHPT = (c.tauHPT.TotalDZ - c.tauHPT.nHPT * c.tauHPT.DZ) / (
                c.tauHPT.nHPT - 1
            )
    ...
    c.NuTauTarget.PillarY = (
        10 * u.m
        - c.NuTauTarget.ydim / 2
        - c.NuTauTarget.BaseY
        - 0.1 * u.mm
        - c.cave.floorHeightMuonShield
    )
    ...
    c.UpstreamTagger.Z_Position = (
        c.tauMudet.zMudetC + (c.tauMudet.Ztot) / 2 + 12.0 * u.cm
    )
