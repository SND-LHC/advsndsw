...
/*void  ShipGoliathField::get(const double& posX, const double& posY, const double& posZ, double& Bx, double& By,
double& Bz) const { if ((posX < coords[0][0]) && (posX > coords[0][3]) && (posY < coords[0][1]) && (posY > coords[0][4])
&& (posZ < coords[0][2]+5.) && (posZ>coords[0][5]+5.) ) { Bx = GetBx(posX, posY, posZ); By = GetBy(posX, posY, posZ); Bz
= GetBz(posX, posY, posZ);
  }
  else {
    for (Int_t i=1;i<13;i++){
     if ((posX < coords[i][0]) && (posX > coords[i][3]) && (posY < coords[i][1]) && (posY > coords[i][4]) && (posZ <
coords[i][2]) && (posZ>coords[i][5])) { Bx = 0.; By = 0.5; Bz = 0.; break;
     }
    }
  }
}
...
void ShipGoliathField::Init(const char* fieldfile)
{
    ...
    std::vector<TString>
    volume={"/volGoliath_1/VolVacuum_1","/volGoliath_1/volLateralS1_1","/volGoliath_1/volLateralS2_1","/volGoliath_1/volLateralSurface1low_1",\
    ...
    posY "<< posY <<" "<<  coords[i][1] << ";" << coords[i][5] << " posZ " << posZ<<" "<< coords[i][2]<<std::endl;
    }
    */
}
...
void ShipGoliathField::getpos(TString volname, TVector3& vbot, TVector3& vtop) const
{
    ...
    if (gGeoManager) {
        ...
        std::cout << "No geomanager" << std::endl;
    }
...
Double_t ShipGoliathField::GetBx(Double_t x, Double_t y, Double_t z)
{
    ...
    // cout << "GetBX " << x << ", binx " << binx <<" y "<< y << " biny "<<biny<<" z "<< z << " binz "<<binz<<" Bx= " <<
    // bx <<  endl;
    return bx;
}
