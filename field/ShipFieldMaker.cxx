...
ShipFieldMaker::ShipFieldMaker(Bool_t verbose)
    ...
    , verbose_(verbose)
    , Tesla_(10.0)
    ,   // To convert T to kGauss for VMC/FairRoot
    theNode_(0)
    , gotNode_(kFALSE)
{
}
...
void ShipFieldMaker::defineUniform(const TString& name, const TVector3& BVector)
{
    ...
    if (!this->gotField(name)) {
        ...
        if (verbose_) {
            std::cout << "We already have a uniform field with the name " << name.Data() << std::endl;
        }
    }
}
                                    ...
                                    const TVector3& BVector)
{
    ...
    if (!this->gotField(name)) {
        ...
        ShipConstField* theField = new ShipConstField(name.Data(), xMin, xMax, yMin, yMax, zMin, zMax, Bx, By, Bz);
        theFields_[name] = theField;

    } else {

        if (verbose_) {
            std::cout << "We already have a constant field with the name " << name.Data() << std::endl;
        }
    }
}
                                ...
                                Double_t L)
{
    ...
    if (!this->gotField(name)) {
        ...
        if (verbose_) {
            std::cout << "We already have a Bell field with the name " << name.Data() << std::endl;
        }
    }
}
                                    ...
                                    Bool_t useSymmetry)
{
    ...
    if (!this->gotField(name)) {
        ...
        if (verbose_) {
            if (useSymmetry) {
                std::cout << "Creating symmetric field map called " << name.Data() << " using " << fullFileName
                          << std::endl;
            } else {
        ...
        ShipBFieldMap* mapField =
            new ShipBFieldMap(name.Data(), fullFileName, x0, y0, z0, phi, theta, psi, scale, useSymmetry);
        theFields_[name] = mapField;

    } else {

        if (verbose_) {
            std::cout << "We already have a field map with the name " << name.Data() << std::endl;
        }
    }
}
                                        ...
                                        const TVector3& eulerAngles)
{
    ...
    if (!this->gotField(name)) {
        ...
        ShipBFieldMap* fieldToCopy = dynamic_cast<ShipBFieldMap*>(this->getField(mapNameToCopy));

        if (fieldToCopy) {

            if (verbose_) {
                std::cout << "Creating map field copy " << name.Data() << " based on " << mapNameToCopy.Data()
                          << std::endl;
            }
            ...
            ShipBFieldMap* copiedMap = new ShipBFieldMap(name.Data(), *fieldToCopy, x0, y0, z0, phi, theta, psi, scale);
            theFields_[name] = copiedMap;
        }

    } else {

        std::cout << "We already have a copied field map with the name " << name.Data() << std::endl;
    }
}
...
void ShipFieldMaker::defineComposite(const TString& name, std::vector<TString> fieldNames)
{
    ...
    if (!this->gotField(name)) {
        ...
        std::cout << "We already have a composite field with the name " << name.Data() << std::endl;
    }
}
...
void ShipFieldMaker::defineGlobalField(std::vector<TString> fieldNames)
{
    ...
    if (gMC) {
        ...
        std::cout << "The virtual MC pointer gMC is null! The global field can't be used by Geant4 but will work for "
                     "track fitting and track extrapolation"
                  << std::endl;
    }
}
...
void ShipFieldMaker::defineRegionField(const TString& volName, const TString& fieldName, Double_t scale)
{
    ...
    if (verbose_) {
        std::cout << "ShipFieldMaker::defineRegionField for volume " << volName.Data() << " and field "
                  << fieldName.Data() << " with scale = " << scale << std::endl;
    }
                    ...
                    if (!lgField) {
                        ...
                        if (verbose_) {
                            std::cout << "Creating the combined field " << lgName.Data() << ", with local field "
                                      << fieldName.Data() << " with the global field for volume " << volName.Data()
                                      << std::endl;
                        }
                        ...
                        if (verbose_) {
                            std::cout << "Setting the field " << lgName.Data() << " for volume " << volName.Data()
                                      << std::endl;
                        }
...
void ShipFieldMaker::defineLocalField(const TString& volName, const TString& fieldName, Double_t scale)
{
    ...
    if (verbose_) {
        std::cout << "ShipFieldMaker::defineLocalField for volume " << volName.Data() << " and field "
                  << fieldName.Data() << " with scale = " << scale << std::endl;
    }
    ...
    for (localIter = localInfo_.begin(); localIter != localInfo_.end(); ++localIter) {
        ...
        if (theVol) {
            ...
            if (localField) {
                ...
                if (verbose_) {
                    std::cout << "Setting local field " << localField->GetName() << " for volume " << volName
                              << std::endl;
                }
...
void ShipFieldMaker::checkLocalFieldMap(TVirtualMagField*& localField, const TString& volName, Double_t scale)
{
    ...
    if (mapField) {
        ...
        if (verbose_) {
            std::cout << "Checking local field map " << fieldName << " co-ord transformation for volume " << volName
                      << std::endl;
        }
        ...
        if (!localMap && volName.Length() > 0) {
            ...
            if (verbose_) {
                std::cout << "Creating field map copy for " << localName << " based on " << mapField->GetName()
                          << ": x0 = " << theInfo.x0_ << ", y0 = " << theInfo.y0_ << ", z0 = " << theInfo.z0_
                          << ", phi = " << theInfo.phi_ << ", theta = " << theInfo.theta_ << ", psi = " << theInfo.psi_
                          << ", scale = " << scale << " and symmetry = " << mapField->HasSymmetry() << std::endl;
            }

            localMap = new ShipBFieldMap(localName.Data(),
                                         *mapField,
                                         theInfo.x0_,
                                         theInfo.y0_,
                                         theInfo.z0_,
...
void ShipFieldMaker::plotXYField(const TVector3& xAxis, const TVector3& yAxis, const std::string& plotFile) const
{
    this->plotField(0, xAxis, yAxis, plotFile);
}

void ShipFieldMaker::plotZXField(const TVector3& zAxis, const TVector3& xAxis, const std::string& plotFile) const
{
    this->plotField(1, zAxis, xAxis, plotFile);
}

void ShipFieldMaker::plotZYField(const TVector3& zAxis, const TVector3& yAxis, const std::string& plotFile) const
{
                               ...
                               const std::string& plotFile) const
{
    ...
    for (int icomponent = 0; icomponent < nhistograms; icomponent++) {
        theHist[icomponent] =
            TH2D(Form("theHist[%i]", icomponent), titles[icomponent].data(), Nx, xMin, xMax, Ny, yMin, yMax);
                                      ...
                                      const float zShift)
{
    ...
    myfile << xSteps << "    " << ySteps << "    " << zSteps << "    " << step << "    " << xRange << "    " << yRange
           << "    " << zRange << std::endl;
    for (int i = 0; i < xSteps; i++) {
        for (int k = 0; k < ySteps; k++) {
            for (int m = 0; m < zSteps; m++) {
                ...
                myfile << x << "    " << y << "    " << z << "    " << B[0] / Tesla_ << "    " << B[1] / Tesla_
                       << "    " << B[2] / Tesla_ << std::endl;
            }
        }
    }
