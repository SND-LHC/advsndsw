...
{
    if (!fieldPar) {
        cerr << "-W- ShipBellField::ShipBellField: empty parameter container!" << endl;
        fType = 0;
    } else {
...
Double_t ShipBellField::GetBy(Double_t x, Double_t y, Double_t z)
{
    ...
    if (fInclTarget && z < targetZ0 + targetL && z > targetZ0) {
        ...
        if (fabs(x) < targetXY && fabs(y) < targetXY) {
            by = 1 * tesla;
        }
        if (fabs(x) > 2 * targetXY && fabs(x) < 3 * targetXY && fabs(y) < targetXY) {
            by = -1 * tesla;
        }
    } else if (fOrient == 1) {
...
Double_t ShipBellField::GetBz(Double_t x, Double_t y, Double_t z) { return 0.; }
// -------------------------------------------------------------------------
