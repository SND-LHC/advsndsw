...
void sndAnalysis::getSciFiHitsPerStation(TClonesArray *digiHits,
...
int sndAnalysis::getTotalSciFiHits(std::vector<int> &horizontal_hits, std::vector<int> &vertical_hits)
{
    return std::accumulate(
        horizontal_hits.begin(), horizontal_hits.end(), std::accumulate(vertical_hits.begin(), vertical_hits.end(), 0));
}
...
std::vector<float> sndAnalysis::getFractionalHitsPerPlane(std::vector<int> &horizontal_hits,
                                                          std::vector<int> &vertical_hits)
{
    ...
    std::transform(horizontal_hits.begin(),
                   horizontal_hits.end(),
                   vertical_hits.begin(),
