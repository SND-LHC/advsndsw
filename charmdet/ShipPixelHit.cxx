...
ShipPixelHit::ShipPixelHit(Int_t detIDX, Float_t digi)
    : ShipHit(detIDX, digi)
{
}
...
void ShipPixelHit::GetPixelXYZ(TVector3 &pixel, int detIDX)
...
{
    // map unique detectorID to x,y,z position in LOCAL coordinate system. xy (0,0) is on the bottom left of each Front
    // End, the raw data counts columns from 1-80 from left to right and rows from 1-336 FROM TOP TO BOTTOM.
        ...
        for (int frontEndID = 0; frontEndID < 8; frontEndID++) {
            for (int column = 1; column < 81; column++) {
                for (int row = 1; row < 337; row++) {
                    ...
                    (*positionMap)[map_index] = TVector3{x - Xref[moduleID], y - Yref[moduleID], Zref[moduleID]};
                }
            }
        }
    }
