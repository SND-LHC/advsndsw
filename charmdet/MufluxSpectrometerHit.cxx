...
MufluxSpectrometerHit::MufluxSpectrometerHit(Int_t detID,
                                             Float_t ftdc,
                                             Float_t signal_width,
                                             uint16_t flag,
                                             uint16_t ch)
    : ShipHit(detID, ftdc)
    , time_over_threshold(signal_width)
    , flags(flag)
    , channel(ch)
{
}
...
MufluxSpectrometerHit::MufluxSpectrometerHit(MufluxSpectrometerPoint* p, Double_t t0)
{
    ...
    MufluxSpectrometer* module =
        dynamic_cast<MufluxSpectrometer*>(FairRunSim::Instance()->GetListOfModules()->FindObject("MufluxSpectrometer"));
...
void MufluxSpectrometerHit::MufluxSpectrometerEndPoints(TVector3& vbot, TVector3& vtop, bool charm = false)
{
    ...
    // TString path =
    // "/volProva_1";path+="/";path+=stat;path+="/";path+=plane;path+="/";path+=layer;path+="/";path+=wire;
    TString path = "";
    path += "/";
    path += stat;
    path += "/";
...
std::vector<int> MufluxSpectrometerHit::StationInfo()
{
    ...
    if (statnb == 1 && iview == 0) {
        tdcId = 0;
    } else if (statnb == 1 && iview == 1) {
        tdcId = 0;
    } else if (statnb == 2 && iview == 2 && pnb == 0) {
        tdcId = 0;
    } else if (statnb == 2 && iview == 2 && pnb == 1) {
        tdcId = 1;
    } else if (statnb == 2 && iview == 0) {
        tdcId = 1;
    } else if (statnb == 3 && channelID < 25) {
        tdcId = 4;
    } else if (statnb == 3 && !(channelID < 25)) {
        tdcId = 3;
    } else if (statnb == 4 && channelID < 13 && pnb == 0) {
        tdcId = 3;
    } else if (statnb == 4 && channelID < 13 && pnb == 1) {
        tdcId = 2;
    } else if (statnb == 4 && channelID < 37 && !(channelID < 13)) {
        tdcId = 2;
    } else if (statnb == 4 && channelID > 36) {
        tdcId = 1;
    }
    Int_t moduleId = 1;
    if (statnb > 2) {
        moduleId = int((channelID - 1) / 12) + 1;
    }
