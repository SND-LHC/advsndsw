...
MuonTagger::MuonTagger(const char *name,
                       const Double_t BX,
                       const Double_t BY,
                       const Double_t BZ,
                       const Double_t zBox,
...
void MuonTagger::Initialize() { FairDetector::Initialize(); }
...
void MuonTagger::SetRPCThickness(Double_t RPCThickness) { fRPCThickness = RPCThickness; }

void MuonTagger::SetElectrodeThickness(Double_t ElectrodeThickness) { fElectrodeThickness = ElectrodeThickness; }

void MuonTagger::SetGapThickness(Double_t GapThickness) { fGapThickness = GapThickness; }
...
void MuonTagger::ConstructGeometry()
{
        ...
        if (n == 0) {
            ...
            if (n == 1) {
                VMuonBox->AddNode(
                    ...
                    n + 1,
                    new TGeoTranslation(
                        0, 0, fRPCz[n] - PasThicknessz[n] / 2. - (fRPCz[n] - fRPCz[n - 1] - PasThicknessz[n]) / 2.));
            }
            // else if (n==npassive-1) VMuonBox->AddNode(VPassive1, n+1, new
            // TGeoTranslation(0,walldisalignment,fRPCz[n]-PasThicknessz[n]/2.-(fRPCz[n]-fRPCz[n-1]-PasThicknessz[n])/2.));
            // //last wall had been disaligned with respect to the others
            else {
                VMuonBox->AddNode(
                    VPassive1,
                    n + 1,
    ...
    for (int n = 1; n < nsensitive + 1; n++) {
        ...
        VSensitive[n - 1]->AddNode(
            Ground,
            10000 * n + 2000,
            new TGeoTranslation(
                0, 0, -fGapThickness / 2 - fElectrodeThickness - 0.025 - fStripz - fStripfoamz - fStripz / 2));
        VSensitive[n - 1]->AddNode(Electrode,
                                   10000 * n + 3000,
                                   new TGeoTranslation(0, 0, -fGapThickness / 2 - 0.025 - fElectrodeThickness / 2));
            ...
            if (m == 1) {
                VSensitive[n - 1]->AddNode(
                    ...
                    new TGeoTranslation(0,
                                        SensY / 2 - fHstrip_ext / 2,
                                        -fGapThickness / 2 - fElectrodeThickness - 0.025 - fStripz - fStripfoamz / 2));
                VSensitive[n - 1]->AddNode(
                    Hstrip_ext,
                    10000 * n + m,
                    new TGeoTranslation(0,
                                        SensY / 2 - fHstrip_ext / 2,
                                        -fGapThickness / 2 - fElectrodeThickness - 0.025 - fStripz / 2));
            } else {
                if (m == fNHstrips) {
                    VSensitive[n - 1]->AddNode(
                        HFoam_ext,
                        ...
                        new TGeoTranslation(
                            0,
                            SensY / 2 - fHstripoffset * (m - 1) - fHstripy * (m - 2) - 3 * fHstrip_ext / 2,
                            -fGapThickness / 2 - fElectrodeThickness - 0.025 - fStripz - fStripfoamz / 2));
                    VSensitive[n - 1]->AddNode(
                        Hstrip_ext,
                        10000 * n + m,
                        new TGeoTranslation(0,
                                            SensY / 2 - fHstripoffset * (m - 1) - fHstripy * (m - 2)
                                                - 3 * fHstrip_ext / 2,
                                            -fGapThickness / 2 - fElectrodeThickness - 0.025 - fStripz / 2));
                } else {
                    VSensitive[n - 1]->AddNode(
                        ...
                        new TGeoTranslation(
                            0,
                            SensY / 2 - fHstrip_ext - fHstripoffset * (m - 1) - fHstripy * (m - 2) - fHstripy / 2,
                            -fGapThickness / 2 - fElectrodeThickness - 0.025 - fStripz - fStripfoamz / 2));
                    VSensitive[n - 1]->AddNode(
                        Hstrip,
                        10000 * n + m,
                        new TGeoTranslation(0,
                                            SensY / 2 - fHstrip_ext - fHstripoffset * (m - 1) - fHstripy * (m - 2)
                                                - fHstripy / 2,
                                            -fGapThickness / 2 - fElectrodeThickness - 0.025 - fStripz / 2));
                }
            }
        ...
        for (int m = 1; m < fNVstrips + 1; m++) {
            if (m == 1) {
                VSensitive[n - 1]->AddNode(
                    ...
                    10000 * n + 1000 + m,
                    new TGeoTranslation(
                        SensX / 2 - fVstrip_R / 2, 0, fGapThickness / 2 + fElectrodeThickness + 0.025 + fStripz / 2));
                VSensitive[n - 1]->AddNode(
                    VFoam_R,
                    10000 * n + 8000 + m,
                    new TGeoTranslation(SensX / 2 - fVstrip_R / 2,
                                        0,
                                        fGapThickness / 2 + fElectrodeThickness + 0.025 + fStripz + fStripfoamz / 2));
            } else {
                if (m == fNVstrips) {
                    VSensitive[n - 1]->AddNode(
                        Vstrip_L,
                        10000 * n + 1000 + m,
                        new TGeoTranslation(SensX / 2 - fVstripoffset * (m - 1) - fVstripx * (m - 2) - fVstrip_R
                                                - fVstrip_L / 2,
                                            0,
                                            fGapThickness / 2 + fElectrodeThickness + 0.025 + fStripz / 2));
                    VSensitive[n - 1]->AddNode(
                        ...
                        10000 * n + 8000 + m,
                        new TGeoTranslation(
                            SensX / 2 - fVstripoffset * (m - 1) - fVstripx * (m - 2) - fVstrip_R - fVstrip_L / 2,
                            0,
                            fGapThickness / 2 + fElectrodeThickness + 0.025 + fStripz + fStripfoamz / 2));
                } else {
                    VSensitive[n - 1]->AddNode(
                        Vstrip,
                        10000 * n + 1000 + m,
                        new TGeoTranslation(SensX / 2 - fVstripoffset * (m - 1) - fVstripx * (m - 2) - fVstripx / 2
                                                - fVstrip_R,
                                            0,
                                            fGapThickness / 2 + fElectrodeThickness + 0.025 + fStripz / 2));
                    VSensitive[n - 1]->AddNode(
                        ...
                        10000 * n + 8000 + m,
                        new TGeoTranslation(
                            SensX / 2 - fVstripoffset * (m - 1) - fVstripx * (m - 2) - fVstripx / 2 - fVstrip_R,
                            0,
                            fGapThickness / 2 + fElectrodeThickness + 0.025 + fStripz + fStripfoamz / 2));
                }
            }
...
void MuonTagger::EndOfEvent() { fMuonTaggerPointCollection->Clear(); }
...
void MuonTagger::Register()
{
    ...
    FairRootManager::Instance()->Register("MuonTaggerPoint", "MuonTagger", fMuonTaggerPointCollection, kTRUE);
}

TClonesArray *MuonTagger::GetCollection(Int_t iColl) const
{
    if (iColl == 0) {
        return fMuonTaggerPointCollection;
    } else {
        return NULL;
    }
}

void MuonTagger::Reset() { fMuonTaggerPointCollection->Clear(); }

MuonTaggerPoint *MuonTagger::AddHit(Int_t trackID,
                                    Int_t detID,
                                    TVector3 pos,
                                    TVector3 mom,
                                    Double_t time,
                                    ...
                                    Int_t pdgCode)
{
    ...
    return new (clref[size]) MuonTaggerPoint(trackID, detID, pos, mom, time, length, eLoss, pdgCode);
}
...
void MuonTagger::EndPoints(Int_t fDetectorID, TVector3 &vbot, TVector3 &vtop)
{
    ...
    TString stat = "VMuonBox_1/VSensitive";
    stat += +statnb;
    stat += "_";
    stat += statnb;
    TString striptype;
    if (orientationnb == 0) {
        striptype = "Hstrip_";
        if (fDetectorID % 1000 == 116 || fDetectorID % 1000 == 1) {
            striptype = "Hstrip_ext_";
        }
    }
    if (orientationnb == 1) {
        striptype = "Vstrip_";
        if (fDetectorID % 1000 == 184) {
            striptype = "Vstrip_L_";
        }
        if (fDetectorID % 1000 == 1) {
            striptype = "Vstrip_R_";
        }
    }
