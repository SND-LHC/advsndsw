...
setup = {}
#
setup["muflux"] = {"thickness": 240 * u.cm / 2.0, "material": "iron"}
s = "muflux"
...
parser.add_argument(
    "-n", "--Nevents", dest="nevents", help="number of events", required=False
)
parser.add_argument(
    "-pg", "--ParticleGun", dest="PG", help="particle gun", required=False
)
...
def run():
    ...
    if options.PG:
        ...
        myPgun.SetPRange(0.0, 10.0)
        myPgun.SetPhiRange(0, 0)
        myPgun.SetXYZ(0.0 * u.cm, 0.0 * u.cm, -thickness)
...
def analyze():
    ut.bookHist(h, "PinPout", "pin pout", 105, -0.5, 10.0, 105, -0.5, 10.0)
