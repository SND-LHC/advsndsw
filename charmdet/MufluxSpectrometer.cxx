...
MufluxSpectrometer::MufluxSpectrometer(const char *name,
                                       const Double_t DX,
                                       const Double_t DY,
                                       const Double_t DZ,
                                       Bool_t Active,
...
void MufluxSpectrometer::Initialize() { FairDetector::Initialize(); }
...
void MufluxSpectrometer::ChooseDetector(Bool_t muflux) { fMuonFlux = muflux; }
...
void MufluxSpectrometer::SetTubePitch_T1u(Double_t tubepitch_T1u,
                                          Double_t T1u_const,
                                          Double_t T1u_const_2,
                                          Double_t T1u_const_3,
                                          Double_t T1u_const_4)
{
...
void MufluxSpectrometer::SetTubePitch_T2v(Double_t tubepitch_T2v,
                                          Double_t T2v_const,
                                          Double_t T2v_const_2,
                                          Double_t T2v_const_3,
                                          Double_t T2v_const_4)
{
...
void MufluxSpectrometer::SetGoliathCentreZ(Double_t goliathcentre) { fgoliathcentre = goliathcentre; }

void MufluxSpectrometer::SetTStationsZ(Double_t T1z,
                                       Double_t T1x_z,
                                       Double_t T1u_z,
                                       Double_t T2z,
                                       Double_t T2v_z,
...
void MufluxSpectrometer::SetTStationsX(Double_t T1x_x,
                                       Double_t T1u_x,
                                       Double_t T2x_x,
                                       Double_t T2v_x,
                                       Double_t T3x,
...
void MufluxSpectrometer::SetTStationsY(Double_t T1x_y,
                                       Double_t T1u_y,
                                       Double_t T2x_y,
                                       Double_t T2v_y,
                                       Double_t T3y,
...
void MufluxSpectrometer::ConstructGeometry()
{
    ...
    TGeoUniformMagField *magField = new TGeoUniformMagField(
        0., MagneticField, 0.);   // The magnetic field must be only in the vacuum space between the stations
    ...
    if (fMuonFlux) {
        ...
        TGeoBBox *platebox_12 = new TGeoBBox("platebox_12",
                                             ftr12xdim / 2. + fTubes_pitch,
                                             plate_thickness / 2.,
                                             fDeltaz_view / 2. + fTubes_pitch / 2. - 0.1);
        // Volume: tube
        ...
        TGeoBBox *DriftTube1 = new TGeoBBox(
            "DriftTube1", DimX / 2 + 1. * m / 2, DimY / 2 + 0.9 * m / 2, DimZ + 2 * cm + fdiststereoT1 / 2 + eps);
        TGeoVolume *volDriftTube1 = new TGeoVolume("volDriftTube1", DriftTube1, air);
        volDriftTube1->SetLineColor(kBlue - 5);

        TGeoBBox *DriftTube2 = new TGeoBBox(
            "DriftTube2", DimX / 2 + 1. * m / 2, DimY / 2 + 1. * m / 2, DimZ + 2 * cm + fdiststereoT2 / 2 + eps);
        ...
        for (Int_t statnb = 1; statnb < 3; statnb++) {
            ...
            for (Int_t vnb = 0; vnb < 2; vnb++) {
                ...
                if (angle == 0.) {
                    t5.SetTranslation(fT1x_x - fTubes_pitch / 2.,
                                      fT1x_y + ftr12ydim / 2. + eps + plate_thickness / 2. + 0.5 * cm,
                                      (vnb - 1) * fDeltaz_view);
                    t6.SetTranslation(fT1x_x - fTubes_pitch / 2.,
                                      fT1x_y - ftr12ydim / 2. - eps - plate_thickness / 2. - 0.5 * cm,
                                      (vnb - 1) * fDeltaz_view);
                    t5b.SetTranslation(fT2x_x - fTubes_pitch / 2.,
                                       fT2x_y + ftr12ydim / 2. + eps + plate_thickness / 2. + 0.5 * cm,
                                       (vnb - 1) * fDeltaz_view + fdiststereoT2);
                    t6b.SetTranslation(fT2x_x - fTubes_pitch / 2.,
                                       fT2x_y - ftr12ydim / 2. - eps - plate_thickness / 2. - 0.5 * cm,
                                       (vnb - 1) * fDeltaz_view + fdiststereoT2);
                } else {
                    if (statnb == 1) {
                        t5.SetTranslation(
                            fT1u_x
                                - (ftr12ydim / 2. + eps + plate_thickness / 2 + fTubes_pitch)
                                      * sin(TMath::Pi() * angle / 180.),
                            fT1u_y + (ftr12ydim / 2. + eps + plate_thickness / 2) * cos(TMath::Pi() * angle / 180.),
                            fDeltaz_view + fTubes_pitch / 4.);
                        t6.SetTranslation(
                            fT1u_x + (ftr12ydim / 2. + eps + plate_thickness / 2) * sin(TMath::Pi() * angle / 180.),
                            fT1u_y - (ftr12ydim / 2. + eps + plate_thickness / 2) * cos(TMath::Pi() * angle / 180.),
                            fDeltaz_view + fTubes_pitch / 4.);
                    }
                    if (statnb == 2) {
                        t5b.SetTranslation(
                            fT2v_x
                                - (ftr12ydim / 2. + eps + plate_thickness / 2 + fTubes_pitch)
                                      * sin(TMath::Pi() * angle / 180.),
                            fT2v_y + (ftr12ydim / 2. + eps + plate_thickness / 2) * cos(TMath::Pi() * angle / 180.),
                            (vnb - 1) * fDeltaz_view);
                        t6b.SetTranslation(
                            fT2v_x + (ftr12ydim / 2. + eps + plate_thickness / 2) * sin(TMath::Pi() * angle / 180.),
                            fT2v_y - (ftr12ydim / 2. + eps + plate_thickness / 2) * cos(TMath::Pi() * angle / 180.),
                            (vnb - 1) * fDeltaz_view);
                    }
                }
                ...
                for (Int_t pnb = 0; pnb < 2; pnb++) {
                    ...
                    nmplane_12 += pnb;
                    TGeoBBox *plane_12 = new TGeoBBox("plane box_12",
                                                      ftr12xdim / 2. + eps / 2. + 2 * fTubes_pitch,
                                                      ftr12ydim / 2. + eps / 2.,
                                                      planewidth / 2. + eps / 2 + 0.0111);
                    ...
                    if (statnb == 1) {
                        if (angle == 0.) {
                            t3.SetTranslation(
                                fT1x_x, fT1x_y, (vnb - 1.) * (fDeltaz_view) + (pnb - 1. / 2.) * fDeltaz_plane12);
                        } else {
                            t3.SetTranslation(fT1u_x,
                                              fT1u_y,
                                              (vnb - 1.) * (fDeltaz_view) + (pnb - 1. / 2.) * fDeltaz_plane12
                                                  + fdiststereoT1);
                        }
                    }
                    if (statnb == 2) {
                        if (angle == 0.) {
                            t3.SetTranslation(fT2x_x,
                                              fT2x_y,
                                              (vnb - 1.) * (fDeltaz_view) + (pnb - 1. / 2.) * fDeltaz_plane12
                                                  + fdiststereoT2);
                        } else {
                            t3.SetTranslation(
                                fT2v_x, fT2v_y, (vnb - 1.) * (fDeltaz_view) + (pnb - 1. / 2.) * fDeltaz_plane12);
                        }
                    }
                    ...
                    if (statnb == 1) {
                        volDriftTube1->AddNode(planebox_12, statnb * 10000000 + vnb * 1000000 + pnb * 100000, j3);
                    }
                    if (statnb == 2) {
                        volDriftTube2->AddNode(planebox_12, statnb * 10000000 + vnb * 1000000 + pnb * 100000, j3);
                    }

                    for (Int_t lnb = 0; lnb < 2; lnb++) {
                        // z translate the layerbox wrt the plane box (which is already rotated)
                        TString nmlayer_12 = nmplane_12 + "_layer_";
                        nmlayer_12 += lnb;
                        TGeoBBox *layer_12 = new TGeoBBox(
                            "layer box_12", ftr12xdim / 2. + 2 * fTubes_pitch, ftr12ydim / 2., layerwidth / 2. + 0.011);
                        TGeoVolume *layerbox_12 = new TGeoVolume(nmlayer_12, layer_12, air);
                        layerbox_12->SetVisibility(kFALSE);
                        if ((statnb == 1 && pnb == 0) && (lnb == 0)) {
                            planebox_12->AddNode(layerbox_12,
                        ...
                        for (Int_t snb = 1; snb < fTubes_per_layer_tr12 + 1; snb++) {
                            // tubes loop
                            if ((statnb == 1) && (angle == 0.)) {
                                t6s.SetTranslation(xT1[lnb * 1 + pnb * 2] - ftr12xdim / 2. + fTubes_pitch * (snb - 1)
                            ...
                            if ((statnb == 2) && (angle == 0.)) {
                                t6s.SetTranslation(xT2[lnb * 1 + pnb * 2] - ftr12xdim / 2. + fTubes_pitch * (snb - 1)
                                                       - fOffset_plane12 * pnb
                                                       + fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                                   0,
                                                   fT2x_z);
                            ...
                            if (angle != 0.) {
                                if (statnb == 2) {
                                    if (lnb == 0 && pnb == 0) {
                                        t6s.SetTranslation(fT2v_const - ftr12xdim / 2.
                                                               + (fTubes_pitch + fTubes_pitch_T2v) * (snb - 1)
                                                               - fOffset_plane12 * (pnb - 1)
                                                               - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                                           0,
                                                           fT2v_z);
                                    ...
                                    if (lnb == 1 && pnb == 0) {
                                        t6s.SetTranslation(fT2v_const_2 - ftr12xdim / 2.
                                                               + (fTubes_pitch + fTubes_pitch_T2v) * (snb - 1)
                                                               - fOffset_plane12 * (pnb - 1)
                                                               - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                                           0,
                                    ...
                                    if (lnb == 0 && pnb == 1) {
                                        t6s.SetTranslation(fT2v_const_3 - ftr12xdim / 2.
                                                               + (fTubes_pitch + fTubes_pitch_T2v) * (snb - 1)
                                                               - fOffset_plane12 * (pnb - 1)
                                                               - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                                           0,
                                    ...
                                    if (lnb == 1 && pnb == 1) {
                                        t6s.SetTranslation(fT2v_const_4 - ftr12xdim / 2.
                                                               + (fTubes_pitch + fTubes_pitch_T2v) * (snb - 1)
                                                               - fOffset_plane12 * (pnb - 1)
                                                               - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                                           0,
                                ...
                                if (statnb == 1) {
                                    if (lnb == 0 && pnb == 0) {
                                        t6s.SetTranslation(fT1u_const - ftr12xdim / 2.
                                                               + (fTubes_pitch + fTubes_pitch_T1u) * (snb - 1)
                                                               - fOffset_plane12 * (pnb - 1)
                                                               - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                                           0,
                                                           fT1u_z);
                                    ...
                                    if (lnb == 1 && pnb == 0) {
                                        t6s.SetTranslation(fT1u_const_2 - ftr12xdim / 2.
                                                               + (fTubes_pitch + fTubes_pitch_T1u) * (snb - 1)
                                                               - fOffset_plane12 * (pnb - 1)
                                                               - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                                           0,
                                    ...
                                    if (lnb == 0 && pnb == 1) {
                                        t6s.SetTranslation(fT1u_const_3 - ftr12xdim / 2.
                                                               + (fTubes_pitch + fTubes_pitch_T1u) * (snb - 1)
                                                               - fOffset_plane12 * (pnb - 1)
                                                               - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                                           0,
                                    ...
                                    if (lnb == 1 && pnb == 1) {
                                        t6s.SetTranslation(fT1u_const_4 - ftr12xdim / 2.
                                                               + (fTubes_pitch + fTubes_pitch_T1u) * (snb - 1)
                                                               - fOffset_plane12 * (pnb - 1)
                                                               - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                                           0,
                            ...
                            layerbox_12->AddNode(drifttube_12,
                                                 statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 1000
                                                     + snb,
                                                 h6s);
                            layerbox_12->AddNode(gas_12,
                                                 statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 2000
                                                     + snb,
                                                 h6s);
                            layerbox_12->AddNode(wire_12,
                                                 statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 3000
                                                     + snb,
                                                 h6s);
    ...
    TGeoTranslation *tr1 = new TGeoTranslation(-TransversalSize / 2 + side1 / 2,
                                               Height / 2 - BasisHeight - UpCoilHeight / 2,
                                               -LongitudinalSize / 2 + base1 / 2);
    ...
    // LONGER RECTANGLE
    TGeoBBox *LateralSurface1low =
        new TGeoBBox("LateralSurface1low", side1 / 2, (CoilDistance + LowCoilHeight) / 2, side2 / 2);
    ...
    TGeoTranslation *tr1low =
        new TGeoTranslation(-TransversalSize / 2 + side1 / 2,
                            Height / 2 - BasisHeight - UpCoilHeight - (CoilDistance + LowCoilHeight) / 2,
                            -LongitudinalSize / 2 + side2 / 2);
    volGoliath->AddNode(volLateralSurface1low, 1, tr1low);
    ;

    // SHORTER RECTANGLE
    TGeoBBox *LateralSurface2low =
        new TGeoBBox("LateralSurface2low", side3 / 2, (CoilDistance + LowCoilHeight) / 2, base2 / 2);
    ...
    TGeoTranslation *tr2low =
        new TGeoTranslation(-TransversalSize / 2 + side1 + side3 / 2,
                            Height / 2 - BasisHeight - UpCoilHeight - (CoilDistance + LowCoilHeight) / 2,
                            -LongitudinalSize / 2 + base2 / 2);
    ...
    // LONGER RECTANGLE
    TGeoTranslation *tr1_b = new TGeoTranslation(-TransversalSize / 2 + side1 / 2,
                                                 Height / 2 - BasisHeight - UpCoilHeight / 2,
                                                 LongitudinalSize / 2 - base1 / 2);
    ...
    // LONGER RECTANGLE
    TGeoBBox *LateralS1_d =
        new TGeoBBox("LateralS1_d", side1 / 2, (UpCoilHeight + LowCoilHeight + CoilDistance) / 2, base1 / 2);
    TGeoTranslation *tr1_d =
        new TGeoTranslation(TransversalSize / 2 - side1 / 2,
                            Height / 2 - BasisHeight - (UpCoilHeight + LowCoilHeight + CoilDistance) / 2,
                            -LongitudinalSize / 2 + base1 / 2);
    ...
    TGeoCombiTrans tr2_d(TransversalSize / 2,
                         Height / 2 - BasisHeight - (UpCoilHeight + LowCoilHeight + CoilDistance) / 2,
                         -LongitudinalSize / 2,
                         r2_d);
    ...
    // LONGER RECTANGLE
    TGeoBBox *LateralS1_c =
        new TGeoBBox("LateralS1_c", side1 / 2, (UpCoilHeight + LowCoilHeight + CoilDistance) / 2, base1 / 2);
    TGeoTranslation *tr1_c =
        new TGeoTranslation(TransversalSize / 2 - side1 / 2,
                            Height / 2 - BasisHeight - (UpCoilHeight + LowCoilHeight + CoilDistance) / 2,
                            LongitudinalSize / 2 - base1 / 2);
    ...
    TGeoCombiTrans tr2_c(TransversalSize / 2,
                         Height / 2 - BasisHeight - (UpCoilHeight + LowCoilHeight + CoilDistance) / 2,
                         LongitudinalSize / 2,
                         r2_c);
    ...
    if (fMuonFlux) {
        ...
        for (Int_t statnb = 3; statnb < 5; statnb++) {
            ...
            // TGeoBBox *platebox_34 = new TGeoBBox("platebox_34", ftr34xdim/2.+1.+2*fTubes_pitch,  plate_thickness/2. ,
            // fDeltaz_view/2.);
            TGeoBBox *platebox_34 = new TGeoBBox(
                "platebox_34", ftr34xdim / 2. + fTubes_pitch / 2., plate_thickness / 2., fDeltaz_view / 2.);

            TGeoBBox *DriftTube3 =
                new TGeoBBox("DriftTube3", DimX / 2 + 1 * m / 2, DimY / 2 + 0.68 * m / 2, DimZ / 2 + eps);
            TGeoVolume *volDriftTube3 = new TGeoVolume("volDriftTube3", DriftTube3, air);
            volDriftTube3->SetLineColor(kBlue - 5);

            TGeoBBox *DriftTube4 =
                new TGeoBBox("DriftTube4", DimX / 2 + 1 * m / 2, DimY / 2 + 0.68 * m / 2, DimZ / 2 + eps);
            ...
            for (Int_t pnb = 0; pnb < 2; pnb++) {
                ...
                TGeoBBox *plane_34 = new TGeoBBox("plane box_34",
                                                  ftr34xdim / 2. + eps / 2. + 2 * fTubes_pitch,
                                                  ftr34ydim / 2 + eps / 2.,
                                                  planewidth / 2. + eps / 2 + 0.137);
                ...
                if (statnb == 3) {
                    volDriftTube3->AddNode(planebox_34, statnb * 10000000 + vnb * 1000000 + pnb * 100000, j3);
                }
                if (statnb == 4) {
                    volDriftTube4->AddNode(planebox_34, statnb * 10000000 + vnb * 1000000 + pnb * 100000, j3);
                }
                ...
                for (Int_t lnb = 0; lnb < 2; lnb++) {
                    ...
                    TGeoBBox *layer_34 = new TGeoBBox(
                        "layer box_34", ftr34xdim / 2. + 2 * fTubes_pitch, ftr34ydim / 2., layerwidth / 2. + 0.136);
                    ...
                    for (Int_t snb = 1; snb < fTubes_per_layer_tr34 + 1; snb++) {
                        ...
                        if (statnb == 3) {
                            ...
                            t6s.SetTranslation(Tx_translation + xT3[pnb * 2 + lnb * 1] - ftr34xdim / 2.
                                                   + fTubes_pitch * (snb - 1) - fOffset_plane12 * (pnb - 1)
                                                   - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                               0,
                                               Tz_translation);
                        }
                        if (statnb == 4) {
                            ...
                            t6s.SetTranslation(Tx_translation + xT4[pnb * 2 + lnb * 1] - ftr34xdim / 2.
                                                   + fTubes_pitch * (snb - 1) - fOffset_plane12 * (pnb - 1)
                                                   - fOffset_layer12 * (pnb * lnb + (pnb - 1) * (lnb - 1)),
                                               0,
                                               Tz_translation);
                        ...
                        layerbox_34->AddNode(drifttube,
                                             ...
                                             h6s);
                        layerbox_34->AddNode(
                            gas, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 2000 + snb, h6s);
                        layerbox_34->AddNode(
                            wire, statnb * 10000000 + vnb * 1000000 + pnb * 100000 + lnb * 10000 + 3000 + snb, h6s);
                        // end of straw loop
                    }
                    // end of layer loop
                }
...
Bool_t MufluxSpectrometer::ProcessHits(FairVolume *vol)
{
    ...
    if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
        ...
        if (fVolumeID == tube_uniqueId) {
            // std::cout << pdgCode<< " same volume again ? "<< tube_uniqueId << " exit:" << gMC->IsTrackExiting() << "
            // stop:" << gMC->IsTrackStop() << " disappeared:" << gMC->IsTrackDisappeared()<< std::endl;
            return kTRUE;
        }
        ...
        AddHit(fTrackID,
               ...
               dist2Wire);
        if (dist2Wire > 2.23) {
            std::cout << "addhit " << dist2Wire << " tube id " << tube_uniqueId << " pdgcode " << pdgCode
                      << " dot prod " << pq.Dot(uCrossv) << std::endl;
            std::cout << " exit:" << gMC->IsTrackExiting() << " stop:" << gMC->IsTrackStop()
...
void MufluxSpectrometer::EndOfEvent() { fMufluxSpectrometerPointCollection->Clear(); }
...
void MufluxSpectrometer::Register()
{
    ...
    FairRootManager::Instance()->Register(
        "MufluxSpectrometerPoint", "MufluxSpectrometer", fMufluxSpectrometerPointCollection, kTRUE);
}

// -----   Public method to Decode volume info  -------------------------------------------
// -----   returns hpt, arm, rpc numbers -----------------------------------
void MufluxSpectrometer::DecodeVolumeID(Int_t detID, int &nHPT) { nHPT = detID; }

TClonesArray *MufluxSpectrometer::GetCollection(Int_t iColl) const
{
    if (iColl == 0) {
        return fMufluxSpectrometerPointCollection;
    } else {
        return NULL;
    }
}

void MufluxSpectrometer::Reset() { fMufluxSpectrometerPointCollection->Clear(); }
...
MufluxSpectrometerPoint *MufluxSpectrometer::AddHit(Int_t trackID,
                                                    Int_t detID,
                                                    TVector3 pos,
                                                    TVector3 mom,
                                                    Double_t time,
...

{
    ...
    return new (clref[size]) MufluxSpectrometerPoint(trackID, detID, pos, mom, time, length, eLoss, pdgCode, dist2Wire);
}
