...
enum class view
{
    ...
    v,
    y
};   // to avoid using std::string to index
typedef std::map<std::string, float> StringFloatMap;
typedef std::map<std::string, std::vector<int>> StringVecIntMap;
typedef std::unordered_map<int, std::unordered_map<int, std::unordered_map<int, std::vector<MufluxSpectrometerHit *>>>>
    nestedList;
...
class MufluxReco
{
  public:
    ...
    {
        DTPositionsTop[c] = TVector3(tx, ty, tz);
        DTPositionsBot[c] = TVector3(bx, by, bz);
    }
