...
void SciFi::SetStationNumber(Int_t nSciFistations) { nSciFi = nSciFistations; }
...
void SciFi::ConstructGeometry()
{
    ...
    for (int istation = 0; istation < nSciFi; istation++) {
        if (TMath::Abs(xs[istation]) > offsetxmax)
            offsetxmax = TMath::Abs(xs[istation]);
        if (TMath::Abs(ys[istation]) > offsetymax)
            offsetymax = TMath::Abs(ys[istation]);
    }
    ...
    for (int iscifi = 0; iscifi < nSciFi; iscifi++) {
        if (vertical[iscifi])
            volSciFiBox->AddNode(volSciFiy,
                                 SciFiIDlist[iscifi],
                                 new TGeoTranslation(xs[iscifi],
                                                     ys[iscifi],
                                                     -SBoxZ / 2. + zs[iscifi]));   // compensation for the Node offset
        else
            volSciFiBox->AddNode(
                volSciFix, SciFiIDlist[iscifi], new TGeoTranslation(xs[iscifi], ys[iscifi], -SBoxZ / 2. + zs[iscifi]));
    }
}
