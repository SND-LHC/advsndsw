...
void Scintillator::Initialize() { FairDetector::Initialize(); }
...
void Scintillator::EndOfEvent() { fScintillatorPointCollection->Clear(); }
...
void Scintillator::Register()
{
    ...
    FairRootManager::Instance()->Register("ScintillatorPoint", "Scintillator", fScintillatorPointCollection, kTRUE);
}

// -----   Public method to Decode volume info  -------------------------------------------
// -----   returns hpt, arm, rpc numbers -----------------------------------
void Scintillator::DecodeVolumeID(Int_t detID, int &nHPT) { nHPT = detID; }

TClonesArray *Scintillator::GetCollection(Int_t iColl) const
{
    if (iColl == 0) {
        return fScintillatorPointCollection;
    } else {
        return NULL;
    }
}

void Scintillator::Reset() { fScintillatorPointCollection->Clear(); }

ScintillatorPoint *Scintillator::AddHit(Int_t trackID,
                                        Int_t detID,
                                        TVector3 pos,
                                        TVector3 mom,
                                        Double_t time,
                                        ...
                                        Int_t pdgCode)
{
    ...
    return new (clref[size]) ScintillatorPoint(trackID, detID, pos, mom, time, length, eLoss, pdgCode);
}
