...
PixelModules::PixelModules(const char *name,
                           const Double_t DX,
                           const Double_t DY,
                           const Double_t DZ,
                           Bool_t Active,
...
void PixelModules::Initialize() { FairDetector::Initialize(); }
...
void PixelModules::SetBoxParam(Double_t SX,
                               Double_t SY,
                               Double_t SZ,
                               Double_t zBox,
                               Double_t SZPixel,
                               Double_t D1short,
                               Double_t D1long,
                               Double_t SiliconDZthin,
                               Double_t SiliconDZthick,
...
void PixelModules::ConstructGeometry()
{
    ...
    // TGeoBBox *Pixelythin = new TGeoBBox("Pixelythin", Dim1Short / 2, Dim1Long / 2, DimZThinSlice / 2); // long along
    // y TGeoVolume *volPixelythin = new TGeoVolume("volPixelythin", Pixelythin, Silicon);
    // volPixelythin->SetLineColor(kBlue - 5);
    // AddSensitiveVolume(volPixelythin);

    TGeoBBox *Pixelxthin =
        new TGeoBBox("Pixelxthin", (Dim1Long) / 2, (Dim1Short) / 2, DimZThinSlice / 2);   // long along
                                                                                          // x
    ...
    for (Int_t module = 0; module < nSi; module += nSlices) {
        if ((module) % 4 == 0 || (module) % 4 == 1) {
            ...
            if ((module) % 4 == 0) {
                ...
                z_tmp_cu = (-DimZPixelBox / 2.) + zs[module] + z_offset - (DimZThickSlice / 2.) - FlexKapthick
                           - (FlexCuthick / 2.);
                ...
                z_tmp_kap = (-DimZPixelBox / 2.) + zs[module + nSlices - 1] + z_offset + (DimZThickSlice / 2.)
                            + FlexKapthick / 2.;
            ...
            if (module == 2) {
                z_tmp_cu = (-DimZPixelBox / 2.) + zs[module] + z_offset - (DimZThinSlice / 2.) - FlexKapthick
                           - (FlexCuthick / 2.);
                z_tmp_kap = (-DimZPixelBox / 2.) + zs[module] + z_offset - (DimZThinSlice / 2.) - FlexKapthick / 2.;
                z_tmp_fe = (-DimZPixelBox / 2.) + zs[module + nSlices - 1] + z_offset + (DimZThinSlice / 2.)
                           + (FrontEndthick / 2.);
            } else if (module == 11) {
                z_tmp_fe = (-DimZPixelBox / 2.) + zs[module] + z_offset - (DimZThinSlice / 2.) - (FrontEndthick / 2.);
                z_tmp_kap = (-DimZPixelBox / 2.) + zs[module + nSlices - 1] + z_offset + (DimZThinSlice / 2.)
                            + FlexKapthick / 2.;
...
void PixelModules::EndOfEvent() { fPixelModulesPointCollection->Clear(); }
...
void PixelModules::DecodeVolumeID(Int_t detID, int &nHPT) { nHPT = detID; }
...
void PixelModules::Reset() { fPixelModulesPointCollection->Clear(); }

PixelModulesPoint *PixelModules::AddHit(Int_t trackID,
                                        Int_t detID,
                                        TVector3 pos,
                                        TVector3 mom,
                                        Double_t time,
