...
Bool_t splitcal::ProcessHits(FairVolume *vol)
{
    ...
    if (gMC->IsTrackExiting() || gMC->IsTrackStop() || gMC->IsTrackDisappeared()) {
        ...
        //    cout << " "<< gGeoManager->FindVolumeFast(vol->GetName())->GetNumber()<< "  " << gMC->CurrentVolID() <<
        //    endl;
...
void splitcal::EndOfEvent() { fsplitcalPointCollection->Clear(); }
...
void splitcal::Register()
{
    ...
    */
    ...
    FairRootManager::Instance()->Register("splitcalPoint", "splitcal", fsplitcalPointCollection, kTRUE);
}

TClonesArray *splitcal::GetCollection(Int_t iColl) const
{
    if (iColl == 0) {
        return fsplitcalPointCollection;
    } else {
        return NULL;
    }
}

void splitcal::Reset() { fsplitcalPointCollection->Clear(); }
...
void splitcal::ConstructGeometry()
{
    ...
    stripGivingX =
        gGeoManager->MakeBox("stripGivingX", A1, fStripHalfWidth, fStripHalfLength, fActiveECALThickness / 2);
    stripGivingX->SetVisibility(kTRUE);
    AddSensitiveVolume(stripGivingX);
    stripGivingX->SetLineColor(kGreen);

    stripGivingY =
        gGeoManager->MakeBox("stripGivingY", A1, fStripHalfLength, fStripHalfWidth, fActiveECALThickness / 2);
    ...
    for (Int_t i_nlayECAL = 0; i_nlayECAL < fnECALSamplings; i_nlayECAL++) {
        ...
        if (i_nlayECAL == 0)
            z_splitcal += fEmpty;   // space after first layer? set to 0 in the config file? for whar is it for?
        if (i_nlayECAL == 7)
            z_splitcal += fBigGap;

        // position high precision sensitive layers
        i_nlayECAL_gas = -1000;
        if (i_nlayECAL == ffirst_precision_layer or i_nlayECAL == fsecond_precision_layer
            or i_nlayECAL == fthird_precision_layer) {
                ...
                for (int mx = 0; mx < fNModulesInX; mx++) {
                    for (int my = 0; my < fNModulesInY; my++) {
                        for (int j = 0; j < fNStripsPerModule; j++) {
                            ...
                            double xCoordinate =
                                -fXMax
                                + (fNStripsPerModule * mx + j + 0.5) * fStripHalfWidth
                                      * 2;   // the times 2 is to get the total width from the half-width
                            double yCoordinate =
                                -fYMax
                                + (my + 0.5) * fStripHalfLength
                                      * 2;   // the times 2 is to get the total length from the half-length
                            tSplitCal->AddNode(
                                stripGivingX, index, new TGeoTranslation(xCoordinate, yCoordinate, z_splitcal));
                ...
                for (int mx = 0; mx < fNModulesInX; mx++) {
                    for (int my = 0; my < fNModulesInY; my++) {
                        for (int j = 0; j < fNStripsPerModule; j++) {
                            ...
                            double xCoordinate =
                                -fXMax
                                + (mx + 0.5) * fStripHalfLength
                                      * 2;   // the times 2 is to get the total length from the half-length
                            double yCoordinate =
                                -fYMax
                                + (fNStripsPerModule * my + j + 0.5) * fStripHalfWidth
                                      * 2;   // the times 2 is to get the total width from the half-width
                            tSplitCal->AddNode(
                                stripGivingY, index, new TGeoTranslation(xCoordinate, yCoordinate, z_splitcal));
                            // std::cout<< "--- index = "<< index << std::endl;
                        }   // end loop on strips
                    }   // end loop on modules in y
                }   // end loop on modules in x
    ...
    for (Int_t i_nlayHCAL = 0; i_nlayHCAL < 1; i_nlayHCAL++) {
        ...
        newHCALfilter[i_nlayHCAL] =
            gGeoManager->MakeBox(char_labelHCALfilter[i_nlayHCAL], A2, fXMax, fYMax, fFilterHCALThickness / 2);
        if (fActiveHCAL) {
            newHCALdet[i_nlayHCAL] =
                gGeoManager->MakeBox(char_labelHCALdet[i_nlayHCAL], A4, fXMax, fYMax, fActiveHCALThickness / 2);
                                ...
                                Int_t pdgCode)
{
    ...
    return new (clref[size]) splitcalPoint(trackID, detID, pos, mom, time, length, eLoss, pdgCode);
}
