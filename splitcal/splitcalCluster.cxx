...
splitcalCluster::splitcalCluster(splitcalHit* h)
{
    ...
    if (_vectorOfHits.size() == 1) {   // first element added to the cluster
        ...
        if (_start.Z() > h->GetZ())
            SetStartPoint(h);   // start point is the hit with the smalllest z
        if (_end.Z() < h->GetZ())
            SetEndPoint(h);   // end point is the hit with the largest z
    }
}
...
void splitcalCluster::ComputeEtaPhiE()
{
    ...
    // FIXME: regression fit seems instable --> for the moment commented it out in favour of simple direction from
...
void splitcalCluster::SetStartPoint(splitcalHit*& h) { SetStartPoint(h->GetX(), h->GetY(), h->GetZ()); }

void splitcalCluster::SetEndPoint(splitcalHit*& h) { SetEndPoint(h->GetX(), h->GetY(), h->GetZ()); }
...
void splitcalCluster::Print() const
{
    ...
    std::cout << "    (eta,phi,energy) = " << _eta << " ,  " << _phi << " ,  " << _energy << std::endl;
    std::cout << "    start(x,y,z) = " << _start.X() << " ,  " << _start.Y() << " ,  " << _start.Z() << std::endl;
    std::cout << "    end(x,y,z) = " << _end.X() << " ,  " << _end.Y() << " ,  " << _end.Z() << std::endl;
    std::cout << "------- " << std::endl;
}
// -------------------------------------------------------------------------
