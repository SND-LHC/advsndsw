...
GblTrajectory::GblTrajectory(const std::vector<std::pair<std::vector<GblPoint>, TMatrixD>> &aPointsAndTransList)
    ...
    , numPoints()
    , numOffsets(0)
    , numInnerTrans(aPointsAndTransList.size())
    , numParameters(0)
    , numLocals(0)
    , numMeasurements(0)
    , externalPoint(0)
...
unsigned int GblTrajectory::getResults(int aSignedLabel, TVectorD &localPar, TMatrixDSym &localCov) const
{
    if (not fitOK)
        return 1;
    std::pair<std::vector<unsigned int>, TMatrixD> indexAndJacobian = getJacobian(aSignedLabel);
                                           ...
                                           TVectorD &aDownWeights)
{
    ...
    for (unsigned int i = 0; i < numData; ++i) {
        getResAndErr(firstData + i, aResiduals[i], aMeasErrors[i], aResErrors[i], aDownWeights[i]);
    }
    return 0;
}
 ...
 */
void GblTrajectory::getLabels(std::vector<std::vector<unsigned int>> &aLabelList)
{
    unsigned int aLabel = 0;
                                 ...
                                 double &aDownWeight)
{
    ...
    theData[aData].getResidual(aResidual, aMeasVar, aDownWeight, indLocal, derLocal);
...
void GblTrajectory::prepare()
{
    unsigned int nDim = theDimension.size();
    // upper limit
    unsigned int maxData = numMeasurements + nDim * (numOffsets - 2) + externalSeed.GetNrows();
        ...
        for (itPoint = thePoints[iTraj].begin(); itPoint < thePoints[iTraj].end(); ++itPoint) {
            ...
            if (measDim) {
                ...
                if (measDim > 2) {
                    ...
                    matPDer.Place_at(matP.Sub<SMatrix22>(3, 3) * matDer.Sub<SMatrix25>(3, 0), 3, 0);
                }
                ...
                for (unsigned int i = iOff; i < 5; ++i) {
                    if (aPrec(i) > 0.) {
                        ...
                        aData.addDerivatives(
                            i, labDer, matPDer, iOff, localDer, globalLab, globalDer, numLocals, transDer);
    ...
    // external seed
    if (externalPoint > 0) {
        std::pair<std::vector<unsigned int>, TMatrixD> indexAndJacobian = getJacobian(externalPoint);
    ...
    if (nExt > 0) {
        ...
        for (unsigned int iExt = 0; iExt < nExt; ++iExt) {
            ...
            GblData aData(1U, externalMeasurements(iExt), externalPrecisions(iExt));
 ...
 */
unsigned int GblTrajectory::fit(double &Chi2, int &Ndf, double &lostWeight, std::string optionList)
{
...
void GblTrajectory::milleOut(MilleBinary &aMille)
{
    ...
    std::vector<GblData>::iterator itData;
    for (itData = theData.begin(); itData != theData.end(); ++itData) {
        itData->getAllData(aValue, aErr, indLocal, derLocal, labGlobal, derGlobal);
        aMille.addData(aValue, aErr, *indLocal, *derLocal, *labGlobal, *derGlobal);
    }
    aMille.writeRecord();
}
...
void GblTrajectory::printTrajectory(unsigned int level)
{
    ...
    if (externalMeasurements.GetNrows()) {
        std::cout << " Number of ext. measurements  : " << externalMeasurements.GetNrows() << std::endl;
    }
    if (externalPoint) {
