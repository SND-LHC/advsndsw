...
bool writeHistoDataForLabel(double label,
                            TVectorD res,
                            TVectorD measErr,
                            TVectorD resErr,
                            TVectorD downWeights,
                            TVectorD localPar,
...
GFGbl::GFGbl()
    : AbsFitter()
    , m_milleFileName("millefile.dat")
    , m_gblInternalIterations("THC")
    , m_pValueCut(0.)
    , m_minNdf(1)
    , m_chi2Cut(0.)
...
void GFGbl::beginRun()
{
    ...
    stats = new TH1I(
        "stats", "0: NDF>0 | 1: fTel&VXD | 2: all | 3: VXD | 4: SVD | 5: all - PXD | 6: fTel&SVD | 7: bTel", 10, 0, 10);
...
void GFGbl::processTrackWithRep(Track* trk, const AbsTrackRep* rep, bool resortHits)
{
    ...
    for (int ipoint_meas = 0; ipoint_meas < npoints_meas; ipoint_meas++) {
        ...
        if (!fi) {
            cout << " ERROR: KalmanFitterInfo (with reference state) for measurement does not exist. Track will be "
                    "skipped."
                 << endl;
            return;
        }
        ...
        ReferenceStateOnPlane* reference = new ReferenceStateOnPlane(
            *fi->getReferenceState());   //(dynamic_cast<const
                                         //genfit::ReferenceStateOnPlane&>(*fi->getReferenceState()));
        ...
        if (measPlanar)
            sensorId = measPlanar->getPlaneId();
        ...
        if (point_meas->getRawMeasurement(0)->getDim() != 2
            ...
            && point_meas->getRawMeasurement(0)->getDim() == 1 && point_meas->getRawMeasurement(1)->getDim() == 1) {
            ...
            if (measPlanar2)
                sensorId2 = measPlanar->getPlaneId();
            ...
            if (sensorId != sensorId2) {
                ...
                cout << " ERROR: Incompatible sensorIDs at measurement point " << ipoint_meas
            ...
            if (raw_meas1->constructHMatrix(rep)->isEqual(genfit::HMatrixU())
                ...
                cout << " ERROR: Incompatible 1D measurements at meas. point " << ipoint_meas
        ...
        if (raw_meas->getRawHitCoords().GetNoElements() != 2) {
            skipMeasurement = true;
#ifdef DEBUG
            cout << " WARNING: Measurement " << (ipoint_meas + 1) << " is not 2D. Measurement Will be skipped. "
                 << endl;
#endif
        }
        ...
        if (!skipMeasurement) {
            ...
            int offset = 0;
            // if (sensorId > 16704) offset = 20; // svd, but needs to introduce new 6 constraints: sum rot&shifts of
            // pxd&svd = 0

            derGlobal(0, 6) = drldg(0, 0);
            labGlobal.push_back(offset + 1);
            derGlobal(0, 7) = drldg(0, 1);
            labGlobal.push_back(offset + 2);
            derGlobal(0, 8) = drldg(0, 2);
            labGlobal.push_back(offset + 3);
            derGlobal(0, 9) = drldg(0, 3);
            labGlobal.push_back(offset + 4);
            derGlobal(0, 10) = drldg(0, 4);
            labGlobal.push_back(offset + 5);
            derGlobal(0, 11) = drldg(0, 5);
            labGlobal.push_back(offset + 6);
        ...
        try {
            ...
            if (ipoint_meas < npoints_meas - 1) {
                ...
                KalmanFitterInfo* fi_i_plus_1 =
                    dynamic_cast<KalmanFitterInfo*>(trk->getPoint(ipoint_meas + 1)->getFitterInfo(rep));
                if (!fi_i_plus_1) {
                    cout << " ERROR: KalmanFitterInfo (with reference state) for measurement does not exist. Track "
                            "will be skipped."
                         << endl;
                    return;
                }
                ...
                rep->extrapolateToPlane(
                    refCopy, trk->getPoint(ipoint_meas + 1)->getFitterInfo(rep)->getPlane(), false, false);
                ...
                theta1 = sqrt(scatTheta * scatTheta * scatDeltaS * scatDeltaS
                              / (scatDeltaS * scatDeltaS + (scatSMean - s1) * (scatSMean - s1)));
                theta2 = sqrt(scatTheta * scatTheta * (scatSMean - s1) * (scatSMean - s1)
                              / (scatDeltaS * scatDeltaS + (scatSMean - s1) * (scatSMean - s1)));
                ...
                if (s2 < 1.e-4 || s2 >= trackLen - 1.e-4 || s2 <= 1.e-4) {
                    cout << " WARNING: GBL points will be too close. GBLTrajectory construction might fail. Let's try "
                            "it..."
                         << endl;
                }
            }
            ...
            // If not last measurement, extrapolate and get jacobians for scattering points between this and next
            // measurement
            if (ipoint_meas < npoints_meas - 1) {
                if (theta2 > scatEpsilon) {
                    ...
                    double nextStep = rep->extrapolateToPlane(
                        *reference, trk->getPoint(ipoint_meas + 1)->getFitterInfo(rep)->getPlane(), false, true);
                    if (0. > nextStep) {
                        cout << " ERROR: The extrapolation to measurement point " << (ipoint_meas + 2)
                             << " stepped back by " << nextStep << "cm !!! Track will be skipped." << endl;
                        return;
                    }
                    rep->getForwardJacobianAndNoise(jacPointToPoint, noise, deltaState);
    ...
    if (n_gbl_meas_points > 1) {
        ...
        if (traj->isValid() && pvalue >= m_pValueCut && Ndf >= m_minNdf) {
            ...
            for (unsigned int p = 0; p < listOfPoints.size(); p++) {
                ...
                if (m_chi2Cut
                    && (fabs(residuals[0] / resErrors[0]) > m_chi2Cut || fabs(residuals[1] / resErrors[1]) > m_chi2Cut))
                    return;
// Write layer-wise data
#ifdef OUTPUT
                if (!writeHistoDataForLabel(
                        listOfLayers.at(p), residuals, measErrors, resErrors, downWeights, localPar, localCov))
                    return;
#endif
            ...
            if (hittedLayers[0] && hittedLayers[1] && hittedLayers[2] && hittedLayers[3] && hittedLayers[4]
                && hittedLayers[5] && hittedLayers[6] && hittedLayers[7] && hittedLayers[8]) {
                // front tel + pxd + svd
                stats->Fill(1);
            }

            if (hittedLayers[0] && hittedLayers[1] && hittedLayers[2] && hittedLayers[3] && hittedLayers[4]
                && hittedLayers[5] && hittedLayers[6] && hittedLayers[7] && hittedLayers[8] && hittedLayers[9]
                && hittedLayers[10] && hittedLayers[11]) {
                // all layers
                stats->Fill(2);
            }
            if (hittedLayers[3] && hittedLayers[4] && hittedLayers[5] && hittedLayers[6] && hittedLayers[7]
                && hittedLayers[8]) {
                // vxd
                stats->Fill(3);
            }
            if (hittedLayers[5] && hittedLayers[6] && hittedLayers[7] && hittedLayers[8]) {
                // svd
                stats->Fill(4);
            }
            if (hittedLayers[0] && hittedLayers[1] && hittedLayers[2] &&
            ...
            if (hittedLayers[9] && hittedLayers[10] && hittedLayers[11]) {
                // backward tel
                stats->Fill(7);
            }
#endif
