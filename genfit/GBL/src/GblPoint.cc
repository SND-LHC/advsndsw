                              ...
                              double minPrecision)
{
    ...
    for (unsigned int i = 0; i < measDim; ++i) {
        ...
        measPrecision(iOff + i) = (aPrecision[i] >= minPrecision ? aPrecision[i] : 0.);
                              ...
                              double minPrecision)
{
    ...
    for (unsigned int i = 0; i < measDim; ++i) {
        ...
        measPrecision(iOff + i) = (transPrecision[i] >= minPrecision ? transPrecision[i] : 0.);
        for (unsigned int j = 0; j < measDim; ++j) {
 ...
 */
void GblPoint::getMeasurement(SMatrix55 &aProjection, SVector5 &aResiduals, SVector5 &aPrecision) const
{
 ...
 */
void GblPoint::getScatterer(SMatrix22 &aTransformation, SVector2 &aResiduals, SVector2 &aPrecision) const
{
...
unsigned int GblPoint::getNumLocals() const { return localDerivatives.GetNcols(); }

/// Retrieve local derivatives from a point.
const TMatrixD &GblPoint::getLocalDerivatives() const { return localDerivatives; }
...
unsigned int GblPoint::getNumGlobals() const { return globalDerivatives.GetNcols(); }

/// Retrieve global derivatives labels from a point.
std::vector<int> GblPoint::getGlobalLabels() const { return globalLabels; }

/// Retrieve global derivatives from a point.
const TMatrixD &GblPoint::getGlobalDerivatives() const { return globalDerivatives; }
...
const SMatrix55 &GblPoint::getP2pJacobian() const { return p2pJacobian; }
...
void GblPoint::addPrevJacobian(const SMatrix55 &aJac)
{
    ...
    SMatrix23 CA = aJac.Sub<SMatrix23>(3, 0) * aJac.Sub<SMatrix33>(0, 0).InverseFast(ifail);   // C*A^-1
 ...
 * \param [in] aJac Jacobian
 */
void GblPoint::addNextJacobian(const SMatrix55 &aJac) { nextJacobian = aJac; }
 ...
 */
void GblPoint::getDerivatives(int aDirection, SMatrix22 &matW, SMatrix22 &matWJ, SVector2 &vecWd) const
{
    ...
    if (!matW.InvertFast()) {
        std::cout << " GblPoint::getDerivatives failed to invert matrix: " << matW << std::endl;
...
void GblPoint::printPoint(unsigned int level) const
{
    ...
    if (localDerivatives.GetNcols()) {
        std::cout << ", " << localDerivatives.GetNcols() << " local derivatives";
    }
    if (globalDerivatives.GetNcols()) {
        std::cout << ", " << globalDerivatives.GetNcols() << " global derivatives";
    }
    std::cout << std::endl;
    if (level > 0) {
        if (measDim) {
            std::cout << "  Measurement" << std::endl;
            std::cout << "   Projection: " << std::endl << measProjection << std::endl;
        ...
        std::cout << "   Point-to-point " << std::endl << p2pJacobian << std::endl;
        if (theLabel) {
            std::cout << "   To previous offset " << std::endl << prevJacobian << std::endl;
            std::cout << "   To next offset " << std::endl << nextJacobian << std::endl;
        }
    }
}
