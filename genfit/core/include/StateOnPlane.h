 ...
 * #sharedPlane_ is a shared_pointer, the ownership over that plane is shared between all StateOnPlane objects defined
 * in that plane. The definition of the state is bound to the TrackRep #rep_. Therefore, the StateOnPlane contains a
 * pointer to a AbsTrackRep. It will provide functionality to extrapolate it and translate the state it into cartesian
 * coordinates. Shortcuts to all functions of the AbsTrackRep which use this StateOnPlane are also provided here.
 */
class StateOnPlane : public TObject
{
  ...
  public:
    ...
    {
        return rep_->extrapolateBy(*this, step, stopAtBoundary, calcJacobianNoise);
    }
  ...
  protected:
    ...
    SharedPlanePtr
        sharedPlane_;   //! Shared ownership.  '!' in order to silence ROOT, custom streamer writes and reads this.
...
inline StateOnPlane::StateOnPlane(const TVectorD& state,
