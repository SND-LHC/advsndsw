...
class TrackPoint : public TObject
{
  ...
  public:
    ...
    bool hasFitterInfo(const AbsTrackRep* rep) const { return (fitterInfos_.find(rep) != fitterInfos_.end()); }
    ...
    {
        assert(rawMeasurement != NULL);
        rawMeasurement->setTrackPoint(this);
        rawMeasurements_.push_back(rawMeasurement);
    }
  ...
  private:
    ...
    //! Can be more than one, e.g. multiple measurements in the same Si detector, left and right measurements of a wire
    //! detector etc.
