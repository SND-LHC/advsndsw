    ...
    , thinScatterer_(NULL)
{
    ...
    for (std::vector<AbsMeasurement*>::const_iterator it = rhs.rawMeasurements_.begin();
         it != rhs.rawMeasurements_.end();
         ++it) {
    ...
    // copy fitterInfos
    for (std::map<const AbsTrackRep*, AbsFitterInfo*>::const_iterator it = rhs.fitterInfos_.begin();
         it != rhs.fitterInfos_.end();
         ++it) {
...
TrackPoint& TrackPoint::operator=(TrackPoint rhs)
{
    ...
    for (std::vector<AbsMeasurement*>::const_iterator it = rawMeasurements_.begin(); it != rawMeasurements_.end();
         ++it) {
...
void TrackPoint::Print(const Option_t*) const
{
    std::cout << "genfit::TrackPoint, belonging to Track " << track_ << "; sorting parameter = " << sortingParameter_
              << "\n";
    std::cout << "contains " << rawMeasurements_.size() << " rawMeasurements and " << getFitterInfos().size()
              << " fitterInfos for " << fitterInfos_.size() << " TrackReps.\n";
