...
DetPlane::DetPlane(const TVector3& o, const TVector3& u, const TVector3& v, AbsFinitePlane* finite)
    : o_(o)
    , u_(u)
    , v_(v)
    , finitePlane_(finite)
...
DetPlane::DetPlane(const TVector3& o, const TVector3& n, AbsFinitePlane* finite)
    : o_(o)
    , finitePlane_(finite)
{
    setNormal(n);
...
void DetPlane::set(const TVector3& o, const TVector3& u, const TVector3& v)
{
    o_ = o;
    u_ = u;
    v_ = v;
...
void DetPlane::setO(const TVector3& o) { o_ = o; }

void DetPlane::setO(double X, double Y, double Z) { o_.SetXYZ(X, Y, Z); }
...
TVector3 DetPlane::getNormal() const { return u_.Cross(v_); }

void DetPlane::setNormal(double X, double Y, double Z) { setNormal(TVector3(X, Y, Z)); }
...
TVector2 DetPlane::project(const TVector3& x) const { return TVector2(u_ * x, v_ * x); }

TVector2 DetPlane::LabToPlane(const TVector3& x) const { return project(x - o_); }
...
TVector3 DetPlane::dist(const TVector3& x) const { return toLab(LabToPlane(x)) - x; }
...
void DetPlane::Print(const Option_t* option) const
{
    std::cout << "DetPlane: "
              ...
              << "u(" << u_.X() << ", " << u_.Y() << ", " << u_.Z() << ") "
              << "v(" << v_.X() << ", " << v_.Y() << ", " << v_.Z() << ") "
              << "n(" << getNormal().X() << ", " << getNormal().Y() << ", " << getNormal().Z() << ") " << std::endl;
...
bool operator==(const DetPlane& lhs, const DetPlane& rhs)
{
    ...
    if (fabs((lhs.o_.X() - rhs.o_.X())) > detplaneEpsilon || fabs((lhs.o_.Y() - rhs.o_.Y())) > detplaneEpsilon
        || fabs((lhs.o_.Z() - rhs.o_.Z())) > detplaneEpsilon)
        return false;
    else if (fabs((lhs.u_.X() - rhs.u_.X())) > detplaneEpsilon || fabs((lhs.u_.Y() - rhs.u_.Y())) > detplaneEpsilon
             || fabs((lhs.u_.Z() - rhs.u_.Z())) > detplaneEpsilon)
        return false;
    else if (fabs((lhs.v_.X() - rhs.v_.X())) > detplaneEpsilon || fabs((lhs.v_.Y() - rhs.v_.Y())) > detplaneEpsilon
             || fabs((lhs.v_.Z() - rhs.v_.Z())) > detplaneEpsilon)
        return false;
    return true;
}

bool operator!=(const DetPlane& lhs, const DetPlane& rhs) { return !(lhs == rhs); }
...
{
    // |(point - o_)*(u_ x v_)|
    return fabs((x - o_.X()) * (u_.Y() * v_.Z() - u_.Z() * v_.Y()) + (y - o_.Y()) * (u_.Z() * v_.X() - u_.X() * v_.Z())
...
void DetPlane::straightLineToPlane(const double& posX,
                                   const double& posY,
                                   const double& posZ,
                                   const double& dirX,
                                   const double& dirY,
                                   ...
                                   double& v) const
{
    ...
    double t = 1. / dirTimesN * ((o_.X() - posX) * W.X() + (o_.Y() - posY) * W.Y() + (o_.Z() - posZ) * W.Z());
