    ...
    , covSeed_(6)
{
    ...
    for (unsigned int i = 0; i < factoryHits.size(); ++i) {
        ...
        if (lastPlanarMeas != NULL && planarMeas != NULL && lastPlanarMeas->getDetId() == planarMeas->getDetId()
    ...
    , covSeed_(rhs.covSeed_)
{
    ...
    for (std::map<const AbsTrackRep*, FitStatus*>::const_iterator it = rhs.fitStatuses_.begin();
         it != rhs.fitStatuses_.end();
         ++it) {
...
{
    int i(0);
    for (std::vector<TrackPoint*>::const_iterator it = trackPointsWithMeasurement_.begin();
         it != trackPointsWithMeasurement_.end();
         ++it) {
...
void Track::deletePoint(int id)
{
    ...
    std::vector<TrackPoint*>::iterator it =
        std::find(trackPointsWithMeasurement_.begin(), trackPointsWithMeasurement_.end(), trackPoints_[id]);
...
void Track::mergeTrack(const Track* other, int id)
{
    ...
    for (std::vector<AbsTrackRep*>::const_iterator otherRep = other->trackReps_.begin();
         otherRep != other->trackReps_.end();
         ++otherRep) {
        bool found(false);
        for (std::vector<AbsTrackRep*>::const_iterator thisRep = trackReps_.begin(); thisRep != trackReps_.end();
             ++thisRep) {
    ...
    for (std::vector<TrackPoint*>::const_iterator otherTp = other->trackPoints_.begin();
         otherTp != other->trackPoints_.end();
         ++otherTp) {
...
void Track::deleteTrackRep(int id)
{
    ...
    for (std::vector<TrackPoint*>::const_iterator pointIt = trackPoints_.begin(); pointIt != trackPoints_.end();
         ++pointIt) {
...
void Track::setCardinalRep(int id)
{
    ...
    if (id >= 0 && (unsigned int)id < trackReps_.size())
        ...
        cardinalRep_ = 0;
        std::cerr << "Track::setCardinalRep: Attempted to set cardinalRep_ to a value out of bounds. Resetting  "
                     "cardinalRep_ to 0."
                  << std::endl;
    }
}
...
void Track::determineCardinalRep()
{
    ...
    for (std::map<const AbsTrackRep*, FitStatus*>::const_iterator it = fitStatuses_.begin(); it != fitStatuses_.end();
         ++it) {
...
void Track::deleteForwardInfo(int startId, int endId, const AbsTrackRep* rep)
{
    ...
    for (std::vector<TrackPoint*>::const_iterator pointIt = trackPoints_.begin() + startId;
         pointIt != trackPoints_.begin() + endId;
         ++pointIt) {
        if (rep != NULL) {
            ...
            for (std::vector<AbsFitterInfo*>::const_iterator fitterInfoIt = fitterInfos.begin();
                 fitterInfoIt != fitterInfos.end();
                 ++fitterInfoIt) {
...
void Track::deleteFitterInfo(int startId, int endId, const AbsTrackRep* rep)
{
    ...
    for (std::vector<TrackPoint*>::const_iterator pointIt = trackPoints_.begin() + startId;
        ...
        if (rep != NULL) {
            ...
            for (std::vector<AbsTrackRep*>::const_iterator repIt = trackReps_.begin(); repIt != trackReps_.end();
                 ++repIt) {
...
void Track::fixWeights(AbsTrackRep* rep, int startId, int endId)
{
    ...
    for (std::vector<TrackPoint*>::iterator tp = trackPoints_.begin() + startId; tp != trackPoints_.begin() + endId;
         ++tp) {
        fis.clear();
        if (rep == NULL) {
...
void Track::Print(const Option_t* option) const
{
    ...
    std::cout << "=======================================================================================\n";
    std::cout << "genfit::Track, containing " << trackPoints_.size() << " TrackPoints and " << trackReps_.size()
              << " TrackReps.\n";
    std::cout << " Seed state: ";
    stateSeed_.Print();
...
bool Track::checkConsistency() const
{
    ...
    for (std::vector<AbsTrackRep*>::const_iterator rep = trackReps_.begin(); rep != trackReps_.end(); ++rep) {
        ...
        if (particle == NULL) {
            std::cerr << "Track::checkConsistency(): TrackRep pdg ID " << (*rep)->getPDG() << " is not valid"
                      << std::endl;
            retVal = false;
        }
    }
    ...
    for (std::vector<TrackPoint*>::const_iterator tp = trackPoints_.begin(); tp != trackPoints_.end(); ++tp) {
        ...
        for (std::vector<AbsMeasurement*>::const_iterator m = rawMeasurements.begin(); m != rawMeasurements.end();
             ++m) {
            // check for NULL
            if ((*m) == NULL) {
            ...
            if ((*m)->getTrackPoint() != *tp) {
                std::cerr << "Track::checkConsistency(): Measurement does not point back to correct TrackPoint"
                          << std::endl;
                retVal = false;
            }
        }
        ...
        for (std::vector<AbsFitterInfo*>::const_iterator fi = fitterInfos.begin(); fi != fitterInfos.end(); ++fi) {
            ...
            if (mycount == 0) {
                std::cerr << "Track::checkConsistency(): fitterInfo points to TrackRep which is not in Track"
                          << std::endl;
                retVal = false;
            }

            if (dynamic_cast<KalmanFitterInfo*>(*fi) != NULL) {
                if (prevFis[(*fi)->getRep()] != NULL && static_cast<KalmanFitterInfo*>(*fi)->hasReferenceState()
                    ...
                    if (fabs(prevLen + len) > 1E-10) {
                        std::cerr << "Track::checkConsistency(): segment lengths of reference states for rep "
                                  << (*fi)->getRep() << " (id " << getIdForRep((*fi)->getRep()) << ") at TrackPoint "
                                  << (*tp) << " don't match" << std::endl;
                        std::cerr << prevLen << " + " << len << " = " << prevLen + len << std::endl;
                        std::cerr << "TrackPoint " << *tp << ", FitterInfo " << *fi << ", rep "
