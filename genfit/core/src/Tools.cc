...
void tools::invertMatrix(const TMatrixDSym& mat, TMatrixDSym& inv, double* determinant)
{
    ...
    if (!(mat < 1.E100) || !(mat > -1.E100)) {
        Exception e("Tools::invertMatrix() - cannot invert matrix, entries too big (>1e100)", __LINE__, __FILE__);
        e.setFatal();
        throw e;
    }
    ...
    if (mat.GetNrows() == 2) {
        ...
        if (determinant != NULL)
            *determinant = det;
        if (fabs(det) < 1E-50) {
            Exception e("Tools::invertMatrix() - cannot invert matrix , determinant = 0", __LINE__, __FILE__);
            e.setFatal();
            throw e;
        }
        det = 1. / det;
    ...
    if (useCramer && mat.GetNrows() <= 6) {
        ...
        switch (mat.GetNrows()) {
            case 3:
                inversion = TMatrixTSymCramerInv::Inv3x3;
                break;
            case 4:
                inversion = TMatrixTSymCramerInv::Inv4x4;
                break;
            case 5:
                inversion = TMatrixTSymCramerInv::Inv5x5;
                break;
            case 6:
                inversion = TMatrixTSymCramerInv::Inv6x6;
                break;
        }
    ...
    if (status == 0) {
        Exception e("Tools::invertMatrix() - cannot invert matrix, status = 0", __LINE__, __FILE__);
        e.setFatal();
        throw e;
    }
...
void tools::safeAverage(const TMatrixDSym& C1, const TMatrixDSym& C2, TMatrixDSym& result)
{
    /*
