...
Exception::Exception(std::string excString, int line, std::string file)
    : excString_(excString)
    , line_(line)
    , file_(file)
    , fatal_(false)
{
    std::ostringstream ErrMsgStream;
    ErrMsgStream << "genfit::Exception thrown with excString:" << std::endl
                 << excString_ << std::endl
...
void Exception::setNumbers(std::string _numbersLabel, const std::vector<double>& _numbers)
{
    numbersLabel_ = _numbersLabel;
    numbers_ = _numbers;
}

void Exception::setMatrices(std::string _matricesLabel, const std::vector<TMatrixD>& _matrices)
{
...
void Exception::info()
{
    if (quiet_)
        return;
    if (numbers_.empty() && matrices_.empty())
        return;   // do nothing
