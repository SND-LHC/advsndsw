...
class StepLimits
{
  ...
  public:
    StepLimits()
        : limits_(ENUM_NR_ITEMS, maxLimit_)
        , stepSign_(1)
    {
        ;
    }
    ...
    double getLimitSigned(StepLimitType type) const { return stepSign_ * getLimit(type); }
    ...
    double getLowestLimitSignedVal(double margin = 1.E-3) const { return getLowestLimitVal(margin) * stepSign_; }
