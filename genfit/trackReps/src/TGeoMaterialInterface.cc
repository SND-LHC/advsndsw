...
bool TGeoMaterialInterface::initTrack(double posX, double posY, double posZ, double dirX, double dirY, double dirZ)
{
#ifdef DEBUG
...
void TGeoMaterialInterface::getMaterialParameters(double& density,
                                                  double& Z,
                                                  double& A,
...
void TGeoMaterialInterface::getMaterialParameters(MaterialProperties& parameters)
{

    TGeoMaterial* mat = gGeoManager->GetCurrentVolume()->GetMedium()->GetMaterial();

    parameters.setMaterialProperties(
        mat->GetDensity(), mat->GetZ(), mat->GetA(), mat->GetRadLen(), MeanExcEnergy_get(mat));
}

double TGeoMaterialInterface::findNextBoundary(const RKTrackRep* rep,
                                               ...
                                               bool varField)
{
    ...
    while (1) {
        if (++it > maxIt) {
            Exception exc("TGeoMaterialInterface::findNextBoundary ==> maximum number of iterations exceeded",
                          __LINE__,
                          __FILE__);
            exc.setFatal();
            throw exc;
        ...
        if (step > safety && maxDeviation2 > epsilon * epsilon) {
            ...
            bool volChanged = initTrack(
                state7[0], state7[1], state7[2], stepSign * state7[3], stepSign * state7[4], stepSign * state7[5]);
...
double TGeoMaterialInterface::findNextBoundaryAndStepStraight(double sMax)
{
...
double MeanExcEnergy_get(TGeoMaterial* mat)
{
    if (mat->IsMixture()) {
        ...
        for (int i = 0; i < mix->GetNelements(); ++i) {
            ...
            logMEE +=
                1. / (mix->GetAmixt())[i] * (mix->GetWmixt())[i] * (mix->GetZmixt())[i] * log(MeanExcEnergy_get(index));
