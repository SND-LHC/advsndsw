...
/**
 *  @brief Collects information needed and produced by a AbsKalmanFitter implementations and is specific to one
...
class KalmanFitterInfo : public AbsFitterInfo
{
  ...
  public:
    ...
    //! @param ignoreWeights If set, the weights of the individual measurements will be ignored (they will be treated as
    //! if they all had weight 1)
    ...
    MeasurementOnPlane getResidual(unsigned int iMeasurement = 0,
                                   bool biased = false,
                                   bool onlyMeasurementErrors = true)
        const;   // calculate residual, track and measurement errors are added if onlyMeasurementErrors is false
    ...
    /* class owned_pointer_vector : private std::vector<MeasuredStateOnPlane*> {
     public:
      ...
      const  MeasuredStateOnPlane* at(size_t i)  const { return this->at(i); };
          ...
          //owned_pointer_vector::iterator erase(owned_pointer_vector::iterator first, owned_pointer_vector::iterator
  last);
  };
          */
