...
enum eMultipleMeasurementHandling
{
    ...
    weightedClosestToReferenceWire,   /**<  if corresponding TrackPoint has one WireMeasurement, select closest to
                                         reference, weighted with its weight_. Otherwise use weightedAverage. */
    unweightedClosestToReferenceWire, /**<  if corresponding TrackPoint has one WireMeasurement, select closest to
                                         reference, weighted with 1. Otherwise use unweightedAverage. */
    weightedClosestToPredictionWire,  /**<  if corresponding TrackPoint has one WireMeasurement, select closest to
                                         prediction, weighted with its weight_. Otherwise use weightedAverage. */
    unweightedClosestToPredictionWire /**<  if corresponding TrackPoint has one WireMeasurement, select closest to
                                         prediction, weighted with 1. Otherwise use unweightedAverage. Recommended for
                                         KalmanFitter to 'resolve' l/r ambiguities */
};
...
class AbsKalmanFitter : public AbsFitter
{
  ...
  public:
    ...
    void getChiSquNdf(const Track* tr, const AbsTrackRep* rep, double& bChi2, double& fChi2, double& bNdf, double& fNdf)
        const;
  ...
  protected:
    ...
    const std::vector<MeasurementOnPlane*> getMeasurements(const KalmanFitterInfo* fi,
                                                           const TrackPoint* tp,
                                                           int direction) const;
    ...
    //! after how many failed hits (exception during construction of plane, extrapolation etc.) should the fit be
    //! cancelled. -1 means don't cancel
    int maxFailedHits_;
