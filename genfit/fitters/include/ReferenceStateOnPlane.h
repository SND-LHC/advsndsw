...
/**
 * @brief #StateOnPlane with linearized transport to that #ReferenceStateOnPlane from previous and next
...
class ReferenceStateOnPlane : public StateOnPlane
{
  ...
  public:
    ...
    {
        forwardTransportMatrix_.ResizeTo(mat);
        forwardTransportMatrix_ = mat;
    }
    void setBackwardTransportMatrix(const TMatrixD& mat)
    {
        backwardTransportMatrix_.ResizeTo(mat);
        backwardTransportMatrix_ = mat;
    }
    void setTransportMatrix(const TMatrixD& mat, int direction)
    {
        if (direction >= 0)
            setForwardTransportMatrix(mat);
        else
            setBackwardTransportMatrix(mat);
    }
    void setForwardNoiseMatrix(const TMatrixDSym& mat)
    {
        forwardNoiseMatrix_.ResizeTo(mat);
        forwardNoiseMatrix_ = mat;
    }
    void setBackwardNoiseMatrix(const TMatrixDSym& mat)
    {
        backwardNoiseMatrix_.ResizeTo(mat);
        backwardNoiseMatrix_ = mat;
    }
    void setNoiseMatrix(const TMatrixDSym& mat, int direction)
    {
        if (direction >= 0)
            setForwardNoiseMatrix(mat);
        else
            setBackwardNoiseMatrix(mat);
    }
    void setForwardDeltaState(const TVectorD& mat)
    {
        forwardDeltaState_.ResizeTo(mat);
        forwardDeltaState_ = mat;
    }
    void setBackwardDeltaState(const TVectorD& mat)
    {
        backwardDeltaState_.ResizeTo(mat);
        backwardDeltaState_ = mat;
    }
    void setDeltaState(const TVectorD& mat, int direction)
    {
        if (direction >= 0)
            setForwardDeltaState(mat);
        else
            setBackwardDeltaState(mat);
    }
    ...
    {
        if (direction >= 0)
            return forwardTransportMatrix_;
        return backwardTransportMatrix_;
    }
    ...
    {
        if (direction >= 0)
            return forwardNoiseMatrix_;
        return backwardNoiseMatrix_;
    }
    ...
    {
        if (direction >= 0)
            return forwardDeltaState_;
        return backwardDeltaState_;
    }
