...
KalmanFitterInfo::~KalmanFitterInfo() { deleteMeasurementInfo(); }
...
KalmanFitterInfo* KalmanFitterInfo::clone() const
{
    ...
    for (std::vector<MeasurementOnPlane*>::const_iterator it = this->measurementsOnPlane_.begin();
         it != this->measurementsOnPlane_.end();
         ++it) {
...
MeasurementOnPlane* KalmanFitterInfo::getClosestMeasurementOnPlane(const StateOnPlane* sop) const
{
    ...
    for (unsigned int i = 0; i < getNumMeasurements(); ++i) {
        if (*(measurementsOnPlane_[i]->getHMatrix()) != *H) {
            Exception e("KalmanFitterInfo::getClosestMeasurementOnPlane: Cannot compare measurements with different "
                        "H-Matrices. Maybe you have to select a different multipleMeasurementHandling.",
                        __LINE__,
                        __FILE__);
            e.setFatal();
            throw e;
    ...
    if (biased) {
        ...
        if (this->getTrackPoint()->getTrack()->getPointWithMeasurementAndFitterInfo(-1, this->getRep())
            ...
            if (forwardUpdate_.get() == NULL) {
                Exception e(
                    "KalmanFitterInfo::getFittedState: Needed updates/redictions not available in this FitterInfo.",
                    __LINE__,
                    __FILE__);
                e.setFatal();
                throw e;
        ...
        if (forwardUpdate_.get() == NULL || backwardPrediction_.get() == NULL) {
            Exception e("KalmanFitterInfo::getFittedState: Needed updates/redictions not available in this FitterInfo.",
                        __LINE__,
                        __FILE__);
            e.setFatal();
            throw e;
        ...
        fittedStateUnbiased_.reset(
            new MeasuredStateOnPlane(calcAverageState(*forwardPrediction_, *backwardPrediction_)));
        return *fittedStateUnbiased_;
    }
}

MeasurementOnPlane KalmanFitterInfo::getResidual(unsigned int iMeasurement,
                                                 bool biased,
                                                 bool onlyMeasurementErrors) const
{
    ...
    if (*(smoothedState.getPlane()) != *plane) {
        Exception e("KalmanFitterInfo::getResidual: smoothedState and measurement are not defined in the same plane.",
                    __LINE__,
                    __FILE__);
        throw e;
    }
    if (smoothedState.getRep() != measurement->getRep()) {
        Exception e(
            "KalmanFitterInfo::getResidual: smoothedState and measurement are not defined wrt the same TrackRep.",
            __LINE__,
            __FILE__);
        throw e;
    }
    ...
    if (onlyMeasurementErrors) {
        return MeasurementOnPlane(
            res, measurement->getCov(), plane, smoothedState.getRep(), H->clone(), measurement->getWeight());
    }
...
void KalmanFitterInfo::setMeasurementsOnPlane(const std::vector<genfit::MeasurementOnPlane*>& measurementsOnPlane)
{
    ...
    for (std::vector<MeasurementOnPlane*>::const_iterator m = measurementsOnPlane.begin(),
...
void KalmanFitterInfo::setRep(const AbsTrackRep* rep)
{
    ...
    for (std::vector<MeasurementOnPlane*>::iterator it = measurementsOnPlane_.begin(); it != measurementsOnPlane_.end();
         ++it) {
        (*it)->setRep(rep);
    }
}
...
void KalmanFitterInfo::setWeights(const std::vector<double>& weights)
{
    ...
    if (weights.size() != getNumMeasurements()) {
        Exception e("KalmanFitterInfo::setWeights: weights do not have the same size as measurementsOnPlane",
                    __LINE__,
                    __FILE__);
        throw e;
    }
    ...
    if (fixWeights_) {
        std::cerr << "KalmanFitterInfo::setWeights - WARNING: setting weights even though weights are fixed!"
                  << std::endl;
    }
...
void KalmanFitterInfo::Print(const Option_t*) const
{
    ...
    for (unsigned int i = 0; i < measurementsOnPlane_.size(); ++i) {
        std::cout << "MeasurementOnPlane Nr " << i << ": ";
        measurementsOnPlane_[i]->Print();
    }

    if (referenceState_) {
        std::cout << "Reference state: ";
        referenceState_->Print();
    }
    if (forwardPrediction_) {
        std::cout << "Forward prediction_: ";
        forwardPrediction_->Print();
    }
    if (forwardUpdate_) {
        std::cout << "Forward update: ";
        forwardUpdate_->Print();
    }
    if (backwardPrediction_) {
        std::cout << "Backward prediction_: ";
        backwardPrediction_->Print();
    }
    if (backwardUpdate_) {
        std::cout << "Backward update: ";
        backwardUpdate_->Print();
    }
}
...
bool KalmanFitterInfo::checkConsistency() const
{
    ...
    if (measurementsOnPlane_.size() > 1) {
        ...
        for (unsigned int i = 1; i < measurementsOnPlane_.size(); ++i) {
            if (measurementsOnPlane_[i]->getState().GetNrows() != dim) {
                std::cerr << "KalmanFitterInfo::checkConsistency(): measurementsOnPlane_ do not all have the same "
                             "dimensionality"
                          << std::endl;
                retVal = false;
            }
    ...
    if (referenceState_) {
        if (referenceState_->getPlane() != plane) {
            std::cerr << "KalmanFitterInfo::checkConsistency(): referenceState_ is not defined with the correct plane "
                      << referenceState_->getPlane().get() << " vs. " << plane.get() << std::endl;
            retVal = false;
        }
        if (referenceState_->getRep() != rep_) {
            std::cerr
                << "KalmanFitterInfo::checkConsistency(): referenceState_ is not defined with the correct TrackRep"
                << std::endl;
            retVal = false;
        }
        if (referenceState_->getState().GetNrows() != dim) {
            std::cerr << "KalmanFitterInfo::checkConsistency(): referenceState_ does not have the right dimension!"
                      << std::endl;
            retVal = false;
        }
    }
    ...
    if (forwardPrediction_) {
        if (forwardPrediction_->getPlane() != plane) {
            std::cerr
                << "KalmanFitterInfo::checkConsistency(): forwardPrediction_ is not defined with the correct plane"
                << std::endl;
            retVal = false;
        }
        if (forwardPrediction_->getRep() != rep_) {
            std::cerr
                << "KalmanFitterInfo::checkConsistency(): forwardPrediction_ is not defined with the correct TrackRep"
                << std::endl;
            retVal = false;
        }
        if (forwardPrediction_->getState().GetNrows() != dim || forwardPrediction_->getCov().GetNrows() != dim) {
            std::cerr << "KalmanFitterInfo::checkConsistency(): forwardPrediction_ does not have the right dimension!"
                      << std::endl;
            retVal = false;
        }
    }
    if (forwardUpdate_) {
        if (forwardUpdate_->getPlane() != plane) {
            std::cerr << "KalmanFitterInfo::checkConsistency(): forwardUpdate_ is not defined with the correct plane"
                      << std::endl;
            retVal = false;
        }
        if (forwardUpdate_->getRep() != rep_) {
            std::cerr << "KalmanFitterInfo::checkConsistency(): forwardUpdate_ is not defined with the correct TrackRep"
                      << std::endl;
            retVal = false;
        }
        if (forwardUpdate_->getState().GetNrows() != dim || forwardUpdate_->getCov().GetNrows() != dim) {
            std::cerr << "KalmanFitterInfo::checkConsistency(): forwardUpdate_ does not have the right dimension!"
                      << std::endl;
            retVal = false;
        }
    }
    ...
    if (backwardPrediction_) {
        if (backwardPrediction_->getPlane() != plane) {
            std::cerr
                << "KalmanFitterInfo::checkConsistency(): backwardPrediction_ is not defined with the correct plane"
                << std::endl;
            retVal = false;
        }
        if (backwardPrediction_->getRep() != rep_) {
            std::cerr
                << "KalmanFitterInfo::checkConsistency(): backwardPrediction_ is not defined with the correct TrackRep"
                << std::endl;
            retVal = false;
        }
        if (backwardPrediction_->getState().GetNrows() != dim || backwardPrediction_->getCov().GetNrows() != dim) {
            std::cerr << "KalmanFitterInfo::checkConsistency(): backwardPrediction_ does not have the right dimension!"
                      << std::endl;
            retVal = false;
        }
    }
    if (backwardUpdate_) {
        if (backwardUpdate_->getPlane() != plane) {
            std::cerr << "KalmanFitterInfo::checkConsistency(): backwardUpdate_ is not defined with the correct plane"
                      << std::endl;
            retVal = false;
        }
        if (backwardUpdate_->getRep() != rep_) {
            std::cerr
                << "KalmanFitterInfo::checkConsistency(): backwardUpdate_ is not defined with the correct TrackRep"
                << std::endl;
            retVal = false;
        }
        if (backwardUpdate_->getState().GetNrows() != dim || backwardUpdate_->getCov().GetNrows() != dim) {
            std::cerr << "KalmanFitterInfo::checkConsistency(): backwardUpdate_ does not have the right dimension!"
                      << std::endl;
            retVal = false;
        }
    }

    for (std::vector<MeasurementOnPlane*>::const_iterator it = measurementsOnPlane_.begin();
         it != measurementsOnPlane_.end();
         ++it) {
        if ((*it)->getPlane() != plane) {
            std::cerr << "KalmanFitterInfo::checkConsistency(): measurement is not defined with the correct plane"
                      << std::endl;
            retVal = false;
        }
        if ((*it)->getRep() != rep_) {
            std::cerr << "KalmanFitterInfo::checkConsistency(): measurement is not defined with the correct TrackRep"
                      << std::endl;
            retVal = false;
        }
...
void KalmanFitterInfo::Streamer(TBuffer& R__b)
{
    ...
    if (R__b.IsReading()) {
        ...
        {
            std::vector<genfit::MeasurementOnPlane*, std::allocator<genfit::MeasurementOnPlane*>>& R__stl =
        ...
        int flag = (!!referenceState_ | (!!forwardPrediction_ << 1) | (!!forwardUpdate_ << 2)
                    | (!!backwardPrediction_ << 3) | (!!backwardUpdate_ << 4));
        R__b << flag;
        if (flag & 1)
            referenceState_->Streamer(R__b);
        if (flag & (1 << 1))
