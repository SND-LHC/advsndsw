    ...
    for (;;) {
        ...
        try {
            ...
            if (!prepareTrack(tr, rep, resortHits, nFailedHits) && !refitAll_) {
                ...
                status->setCharge(
                    rep->getCharge(*static_cast<KalmanFitterInfo*>(tr->getPointWithMeasurement(0)->getFitterInfo(rep))
                                        ->getBackwardUpdate()));
            ...
            double PvalFW =
                (debugLvl_ > 0)
                    ? ROOT::Math::chisquared_cdf_c(chi2FW, ndfFW)
                    : 0;   // Don't calculate if not debugging as this function potentially takes a lot of time.

            if (debugLvl_ > 0) {
                std::cout << "KalmanFitterRefTrack::Track after fit:";
                tr->Print("C");

                std::cout << "old chi2s: " << oldChi2BW << ", " << oldChi2FW << " new chi2s: " << chi2BW << ", "
                          << chi2FW << std::endl;
                std::cout << "old pVals: " << oldPvalBW << ", " << oldPvalFW << " new pVals: " << PvalBW << ", "
                          << PvalFW << std::endl;
            }
...
bool KalmanFitterRefTrack::prepareTrack(Track* tr, const AbsTrackRep* rep, bool setSortingParams, int& nFailedHits)
{
    ...
    for (; i < nPoints; ++i) {
        ...
        try {
            ...
            if (fitterInfo->hasReferenceState()) {
                ...
                if (!prevNewRefState) {
                    if (debugLvl_ > 0) {
                        std::cout << "TrackPoint already has referenceState and previous referenceState has not been "
                                     "altered -> continue \n";
                    }
                ...
                if (prevReferenceState == NULL) {
                    if (debugLvl_ > 0) {
                        std::cout << "TrackPoint already has referenceState but previous referenceState is NULL -> "
                                     "reset forward info of current reference state and continue \n";
                    }
                ...
                if (debugLvl_ > 0) {
                    std::cout << "TrackPoint already has referenceState but previous referenceState has been altered "
                                 "-> update transport matrices and continue \n";
                }
                ...
                double segmentLen = rep->extrapolateToPlane(
                    stateToExtrapolate, fitterInfo->getReferenceState()->getPlane(), false, true);
                if (debugLvl_ > 0) {
            ...
            } else if (rep != tr->getCardinalRep() && trackPoint->hasFitterInfo(tr->getCardinalRep())
                       && dynamic_cast<KalmanFitterInfo*>(trackPoint->getFitterInfo(tr->getCardinalRep())) != NULL
                       && static_cast<KalmanFitterInfo*>(trackPoint->getFitterInfo(tr->getCardinalRep()))
    ...
    if (firstBackwardUpdate && tr->getPointWithMeasurementAndFitterInfo(0, rep)) {
        KalmanFitterInfo* fi =
            static_cast<KalmanFitterInfo*>(tr->getPointWithMeasurementAndFitterInfo(0, rep)->getFitterInfo(rep));
    ...
    for (unsigned int i = 0; i < nPoints; ++i) {
        ...
        if (fitterInfo->hasReferenceState()) {
            ...
            if (chi2 < deltaChi2Ref_) {
                ...
                if (debugLvl_ > 0) {
                    std::cout << "reference state is near smoothed state ->  do not update reference state, chi2 = "
                              << chi2 << "\n";
                }
                continue;
            } else {
                                             ...
                                             int direction)
{
    ...
    if (prevFi != NULL) {
        ...
        C_.Similarity(F);
        C_ += N;
        fi->setPrediction(new MeasuredStateOnPlane(p_,
                                                   C_,
        ...
        if (debugLvl_ > 1) {
            std::cout << "\033[31m";
            std::cout << "F (Transport Matrix) ";
            F.Print();
            std::cout << "p_{k,r} (reference state) ";
            fi->getReferenceState()->getState().Print();
            std::cout << "c (delta state) ";
            fi->getReferenceState()->getDeltaState(direction).Print();
            std::cout << "F*p_{k-1,r} + c ";
        ...
        if (fi->hasPrediction(direction)) {
            ...
            MeasuredStateOnPlane mop(
                p_, dummy, fi->getReferenceState()->getPlane(), rep, fi->getReferenceState()->getAuxInfo());
            TVector3 pos, mom;
    ...
    if (debugLvl_ > 1) {
        std::cout << " p_{k|k-1} (state prediction)";
        p_.Print();
        std::cout << " C_{k|k-1} (covariance prediction)";
        C_.Print();
        std::cout << "\033[0m";
    }
    ...
    for (std::vector<MeasurementOnPlane*>::const_iterator it = measurements.begin(); it != measurements.end(); ++it) {
        ...
        // (weighted) cov
        const TMatrixDSym& V((!canIgnoreWeights() && m.getWeight() < 0.99999) ? 1. / m.getWeight() * m.getCov()
                                                                              : m.getCov());
        ...
        if (debugLvl_ > 1) {
            std::cout << "\033[34m";
            std::cout << "m (measurement) ";
            m.getState().Print();
            std::cout << "V ((weighted) measurement covariance) ";
            (1. / m.getWeight() * m.getCov()).Print();
            std::cout << "residual        ";
            res_.Print();
            std::cout << "\033[0m";
        }
        ...
        if (debugLvl_ > 1) {
            std::cout << "\033[32m";
            std::cout << " update";
            (TMatrixD(CHt, TMatrixD::kMult, covSumInv_) * res_).Print();
            std::cout << "\033[0m";
        }
        ...
        if (debugLvl_ > 1) {
            ...
            std::cout << "\033[32m";
            std::cout << " p_{k|k} (updated state)";
            p_.Print();
            std::cout << " C_{k|k} (updated covariance)";
            C_.Print();
            std::cout << "\033[0m";
        }
    ...
    KalmanFittedStateOnPlane* upState = new KalmanFittedStateOnPlane(p_,
                                                                     C_,
                                                                     fi->getReferenceState()->getPlane(),
                                                                     fi->getReferenceState()->getRep(),
                                                                     fi->getReferenceState()->getAuxInfo(),
