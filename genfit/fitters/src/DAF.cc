...
DAF::DAF(bool useRefKalman, double deltaPval, double deltaWeight)
    : AbsKalmanFitter(10, deltaPval)
    , deltaWeight_(deltaWeight)
{
    if (useRefKalman) {
...
void DAF::processTrackWithRep(Track* tr, const AbsTrackRep* rep, bool resortHits)
{
    ...
    for (unsigned int iBeta = 0;; ++iBeta) {
        ...
        if (debugLvl_ > 0) {
            std::cout << "DAF::processTrack, trackRep  " << rep << ", iteration " << iBeta + 1
        ...
        try {
            ...
            if (!converged && iBeta >= minIterations_ - 1 && status->getBackwardPVal() != 0
                && fabs(lastPval - status->getBackwardPVal()) < this->deltaPval_) {
                if (debugLvl_ > 0) {
                    std::cout << "converged by Pval = " << status->getBackwardPVal()
        ...
        if (iBeta >= minIterations_ - 1 && converged) {
            if (debugLvl_ > 0) {
                std::cout << "DAF::convergence reached in iteration " << iBeta + 1
    ...
    if (status->getForwardPVal() == 0. && status->getBackwardPVal() == 0.) {
        status->setIsFitConvergedFully(false);
        status->setIsFitConvergedPartially(false);
    }
}
                   ...
                   double b10)
{
    ...
    betas_.resize(
        maxIterations_,
        betas_.back());   // make sure main loop has a maximum of maxIterations_ and also make sure the last beta value
                          // is used for if more iterations are needed then the ones set by the user.
}
...
void DAF::setAnnealingScheme(double bStart, double bFinal, unsigned int nSteps)
{
    ...
    betas_.resize(
        maxIterations_,
        betas_.back());   // make sure main loop has a maximum of 10 iterations and also make sure the last beta value
                          // is used for if more iterations are needed then the ones set by the user.
    ...
    for (std::vector<TrackPoint*>::const_iterator tp = trackPoints.begin(); tp != trackPoints.end(); ++tp) {
        ...
        for (unsigned int j = 0; j < nMeas; j++) {
            ...
            try {
                ...
                assert(cutVal > 1.E-6);
                // the following assumes that in the competing hits could have different V otherwise calculation could
                // be simplified
