                            ...
                            int& nFailedHits)
{
    ...
    if (multipleMeasurementHandling_ == unweightedClosestToReference
        ...
        Exception exc("KalmanFitter::fitTrack ==> cannot use (un)weightedClosestToReference(Wire) as multiple "
...
void KalmanFitter::processTrackWithRep(Track* tr, const AbsTrackRep* rep, bool)
{
    ...
    if (trackPoint->hasFitterInfo(rep) && dynamic_cast<KalmanFitterInfo*>(trackPoint->getFitterInfo(rep)) != NULL
    ...
    if (rep != tr->getCardinalRep() && trackPoint->hasFitterInfo(tr->getCardinalRep())
        ...
        try {
            ...
            if (debugLvl_ > 0) {
                ...
                std::cout << "old chi2s: " << oldChi2BW << ", " << oldChi2FW << " new chi2s: " << chi2BW << ", "
                          << chi2FW << " oldPvals " << oldPvalFW << ", " << oldPvalBW << std::endl;
            }
            ++nIt;
...
void KalmanFitter::processTrackPartially(Track* tr, const AbsTrackRep* rep, int startId, int endId)
{
    ...
    if (prevTrackPoint != NULL && prevTrackPoint->hasFitterInfo(rep)
        ...
        currentState_.reset(new MeasuredStateOnPlane(
            *(static_cast<KalmanFitterInfo*>(prevTrackPoint->getFitterInfo(rep))->getUpdate(direction))));
        if (debugLvl_ > 0)
...
void KalmanFitter::processTrackPoint(TrackPoint* tp, const AbsTrackRep* rep, double& chi2, double& ndf, int direction)
{
    ...
    if (newFi) {
        const std::vector<genfit::AbsMeasurement*>& rawMeasurements = tp->getRawMeasurements();
        for (std::vector<genfit::AbsMeasurement*>::const_iterator it = rawMeasurements.begin();
             it != rawMeasurements.end();
             ++it) {
    ...
    if (debugLvl_ > 0) {
        std::cout << "its plane is at R = " << plane->getO().Perp() << " with normal pointing along ("
    ...
    for (std::vector<MeasurementOnPlane*>::const_iterator it = measurements.begin(); it != measurements.end(); ++it) {
        ...
        // (weighted) cov
        const TMatrixDSym& V((!canIgnoreWeights() && weight < 0.99999) ? 1. / weight * mOnPlane.getCov()
                                                                       : mOnPlane.getCov());
        if (debugLvl_ > 1) {
            std::cout << "\033[31m";
            std::cout << "State prediction: ";
            stateVector.Print();
            std::cout << "Cov prediction: ";
            state->getCov().Print();
            std::cout << "\033[0m";
            std::cout << "\033[34m";
            std::cout << "measurement: ";
            measurement.Print();
            std::cout << "measurement covariance V: ";
            V.Print();
            // cov.Print();
            // measurement.Print();
        }
        ...
        if (squareRootFormalism_) {
            ...
            TMatrixD pre(S.GetNrows() + Q.GetNrows() + V.GetNrows(), S.GetNcols() + V.GetNcols());
            TMatrixD SFt(S, TMatrixD::kMultTranspose, F);
            pre.SetSub(0, 0, decompR.GetU()); /*           upper right block is zero               */
            pre.SetSub(V.GetNrows(), 0, H->MHt(SFt));
            pre.SetSub(V.GetNrows(), V.GetNcols(), SFt);
            if (Q.GetNrows() > 0) {   // needed to suppress warnings when inserting an empty Q
                pre.SetSub(S.GetNrows() + V.GetNrows(), 0, H->MHt(Q));
                pre.SetSub(S.GetNrows() + V.GetNrows(), V.GetNcols(), Q);
            }
            ...
            // K.Print();
            //(K*R).Print();
            TMatrixD Snew(r.GetSub(V.GetNrows(), V.GetNrows() + S.GetNrows() - 1, V.GetNcols(), pre.GetNcols() - 1));
            // Snew.Print();
