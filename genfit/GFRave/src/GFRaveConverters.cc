                                          ...
                                          int startID)
{
    ...
    for (unsigned int i = 0; i < ntracks; ++i) {
        ...
        if (!GFTracks[i]->getFitStatus(GFTracks[i]->getCardinalRep())->isFitConverged())
            continue;
        ...
        if (GFStates == NULL)
            IdGFTrackStateMap[startID].state_ = new MeasuredStateOnPlane(
                GFTracks[i]->getFittedState());   // here clones are made so that the state of the original GFTracks and
                                                  // their TrackReps will not be altered by the vertexing process
        else
...
rave::Track GFTrackToTrack(trackAndState trackAndState, int id, std::string tag)
{
    ...
    // covariance
    rave::Covariance6D ravecov(cov(0, 0),
                               cov(1, 0),
                               cov(2, 0),
                               cov(1, 1),
                               cov(2, 1),
    ...
    rave::Track ret(id,
                    ...
                    static_cast<void*>(const_cast<Track*>(trackAndState.track_)),
                    tag);
...
GFRaveVertex* RaveToGFVertex(const rave::Vertex& raveVertex,
                             const std::map<int, genfit::trackAndState>& IdGFTrackStateMap)
{
    ...
    for (unsigned int i = 0; i < nTrks; ++i) {
        ...
        if (smoothing) {
            ...
            trackparams = new GFRaveTrackParameters(
                IdGFTrackStateMap.at(id).track_,                                   // track
            ...
            trackparams =
                new GFRaveTrackParameters(IdGFTrackStateMap.at(id).track_,                                   // track
                                          IdGFTrackStateMap.at(id).state_,                                   // state
                                          raveWeightedTracks[i].first,                                       // weight
    ...
    return new GFRaveVertex(Point3DToTVector3(raveVertex.position()),
                            ...
                            trackParameters,
                            raveVertex.ndf(),
                            raveVertex.chiSquared(),
                            raveVertex.id());
}

void RaveToGFVertices(std::vector<GFRaveVertex*>* GFVertices,
...
{
    return SharedPlanePtr(
        new DetPlane(Point3DToTVector3(rplane.position()), Vector3DToTVector3(rplane.normalVector())));
}

TVector3 Point3DToTVector3(const rave::Point3D& v) { return TVector3(v.x(), v.y(), v.z()); }

TVector3 Vector3DToTVector3(const rave::Vector3D& v) { return TVector3(v.x(), v.y(), v.z()); }
...
rave::Point3D TVector3ToPoint3D(const TVector3& vec) { return rave::Point3D(vec.X(), vec.Y(), vec.Z()); }
...
rave::Covariance3D TMatrixDSymToCovariance3D(const TMatrixDSym& matrix)
{
    ...
    return rave::Covariance3D(matrix(0, 0), matrix(0, 1), matrix(0, 2), matrix(1, 1), matrix(1, 2), matrix(2, 2));
}
