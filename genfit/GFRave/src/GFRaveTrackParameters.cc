...
GFRaveTrackParameters::GFRaveTrackParameters(const Track* track,
...
TVector3 GFRaveTrackParameters::getPos() const { return TVector3(state_[0], state_[1], state_[2]); }

TVector3 GFRaveTrackParameters::getMom() const { return TVector3(state_[3], state_[4], state_[5]); }

double GFRaveTrackParameters::getCharge() const { return getTrack()->getFitStatus()->getCharge(); }

double GFRaveTrackParameters::getPdg() const
{
    if (hasTrack())
...
void GFRaveTrackParameters::Print(const Option_t*) const
{
    std::cout << "weight: " << getWeight() << "\n";
    if (!hasSmoothedData_)
        std::cout << "state and cov are NOT smoothed! \n";
    std::cout << "state: ";
    getState().Print();
    std::cout << "cov: ";
    getCov().Print();
    if (hasTrack()) {
        ...
        std::cout << "NO genfit::Track pointer \n";
}
