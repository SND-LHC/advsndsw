...
GFRaveVertexFactory::GFRaveVertexFactory(int verbosity, bool useVacuumPropagator)
{
    ...
    if (verbosity > 0)
        ++verbosity;   // verbosity has to be >1 for rave

    factory_ = new rave::VertexFactory(*magneticField_,
                                       *propagator_,
                                       "kalman-smoothing:1",
...
void GFRaveVertexFactory::findVertices(std::vector<genfit::GFRaveVertex*>* GFvertices,
                                       ...
                                       bool use_beamspot)
{
    ...
    try {
        RaveToGFVertices(GFvertices,
                         factory_->create(GFTracksToTracks(GFTracks, NULL, IdGFTrackStateMap_, 0), use_beamspot),
                         IdGFTrackStateMap_);
    } catch (Exception& e) {
        std::cerr << e.what();
    }
                                       ...
                                       bool use_beamspot)
{
    ...
    try {
        RaveToGFVertices(GFvertices,
                         factory_->create(GFTracksToTracks(GFTracks, &GFStates, IdGFTrackStateMap_, 0), use_beamspot),
                         IdGFTrackStateMap_);
    } catch (Exception& e) {
        std::cerr << e.what();
    }
...
void GFRaveVertexFactory::setBeamspot(const TVector3& pos, const TMatrixDSym& cov)
{
    factory_->setBeamSpot(rave::Ellipsoid3D(TVector3ToPoint3D(pos), TMatrixDSymToCovariance3D(cov)));
}

void GFRaveVertexFactory::setMethod(const std::string& method)
{
