...
class GFRaveVertexFactory
{
  public:
    ...
    //! MeasuredStateOnPlanes will be used (instead of the tracks fitted states) to calculate the rave::Track
    //! parameters. takes ownership of MeasuredStateOnPlanes.
    void findVertices(std::vector<genfit::GFRaveVertex*>*,
                      const std::vector<genfit::Track*>&,
                      std::vector<genfit::MeasuredStateOnPlane*>&,
                      bool use_beamspot = false);
  ...
  private:
    ...
    std::map<int, genfit::trackAndState>
        IdGFTrackStateMap_;   // map of copies of the cardinal MeasuredStateOnPlanes for the GFRavePropagator; ownership
                              // of MeasuredStateOnPlanes is HERE!!!
