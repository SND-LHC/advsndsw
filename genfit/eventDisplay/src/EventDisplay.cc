    ...
    namespace genfit
{
        ...
        for (unsigned int i = 0; i < events_.at(id)->size(); i++) {   // loop over all tracks in an event
            ...
            if (refit_) {
                ...
                switch (fitterId_) {
                    ...
                    case DafSimple:
                        ...
                        (static_cast<KalmanFitter*>((static_cast<DAF*>(fitter.get()))->getKalman()))
                ...
                int microseconds =
                    1000000 * (endcputime.tv_sec - startcputime.tv_sec) + (endcputime.tv_usec - startcputime.tv_usec);
            ...
            for (unsigned int j = 0; j < numhits; j++) {   // loop over all hits in the track
                ...
                if (tp->getNumRawMeasurements() > 1) {
                    ...
                    if (!sameTypes) {
                        std::cerr << "cannot draw trackpoint containing multiple Measurements of differend types"
                                  << std::endl;
                        continue;
                    }
                }
                ...
                if (dynamic_cast<const FullMeasurement*>(m) != NULL) {
                    ...
                    std::cout << "Track " << i << ", Hit " << j << ": Unknown measurement type: skipping hit!"
                              << std::endl;
                    continue;
                }
                ...
                for (unsigned int iMeas = 0; iMeas < fi->getNumMeasurements(); ++iMeas) {
                    ...
                    if (iMeas == 0 && (drawPlanes_ || (drawDetectors_ && planar_hit))) {
                        ...
                        if (wire_hit)
                            move = v
                                   * (v
                                      * (track_pos
                                         - o));   // move the plane along the wire until the track goes through it
                    ...
                    if (j > 0 && prevFi != NULL) {
                        ...
                        if (drawForward_)
                            makeLines(prevFi->getForwardUpdate(),
                                      fi->getForwardPrediction(),
                                      rep,
                                      kCyan,
                                      1,
                                      drawTrackMarkers_,
                        ...
                        if (drawRefTrack_ && fi->hasReferenceState() && prevFi->hasReferenceState())
                            makeLines(prevFi->getReferenceState(),
                                      fi->getReferenceState(),
                                      rep,
                                      charge > 0 ? kRed + 2 : kBlue + 2,
                                      2,
                                      drawTrackMarkers_,
                                      false,
                                      3);
                    ...
                    if (drawDetectors_) {
                        ...
                        if (wire_hit) {
                            ...
                            det_shape->SetShape(new TGeoTube(
                                std::max(0., (double)(hit_u - 0.0105 / 2.)), hit_u + 0.0105 / 2., plane_size));

                            TVector3 norm = u.Cross(v);
                            TGeoRotation det_rot(
                                "det_rot",
                                (u.Theta() * 180) / TMath::Pi(),
                                (u.Phi() * 180) / TMath::Pi(),
                                (norm.Theta() * 180) / TMath::Pi(),
                                (norm.Phi() * 180) / TMath::Pi(),
                                (v.Theta() * 180) / TMath::Pi(),
                                (v.Phi() * 180)
                                    / TMath::Pi());   // move the tube to the right place and rotate it correctly
                            TVector3 move =
                                v
                                * (v
                                   * (track_pos - o));   // move the tube along the wire until the track goes through it
                            TGeoCombiTrans det_trans(o(0) + move.X(), o(1) + move.Y(), o(2) + move.Z(), &det_rot);
                    ...
                    if (drawHits_) {
                        ...
                        if (full_hit) {
                            ...
                            StateOnPlane dummy2(TVectorD(rep->getDim()),
                                                static_cast<const FullMeasurement*>(m)->constructPlane(dummy),
                                                rep);
                            MeasuredStateOnPlane sop =
                                *(static_cast<const FullMeasurement*>(m)->constructMeasurementsOnPlane(dummy2)[0]);
                        ...
                        if (planar_hit) {
                            if (!planar_pixel_hit) {
                                ...
                                hit_box = boxCreator((o + move + hit_u * stripDir3),
                                                     stripDir3,
                                                     stripDir3perp,
                                ...
                                if (drawAutoScale_) {
                                    ...
                                    if (min_cov < 1e-5) {
                                        std::cout << "Track " << i << ", Hit " << j
                                                  << ": Invalid covariance matrix (Eigenvalue < 1e-5), autoscaling not "
                                                     "possible!"
                                                  << std::endl;
                                    } else {
                                        if (min_cov < 0.049) {
                        ...
                        if (space_hit) {
                            {
                                ...
                                if (!det_rot.IsValid()) {
                                    ...
                                    det_rot.SetAngles(eVec1.Theta() * radDeg,
                                                      eVec1.Phi() * radDeg,
                                                      eVec2.Theta() * radDeg,
                                                      eVec2.Phi() * radDeg,
                                                      eVec3.Theta() * radDeg,
                                ...
                                TGeoGenTrans det_trans(
                                    ...
                                    // std::sqrt(pseudo_res_2/pseudo_res_0/pseudo_res_1), // this workaround is
                                    // necessary due to the "normalization" performed in  TGeoGenTrans::SetScale
                        ...
                        if (wire_hit) {
                            ...
                            if (drawAutoScale_) {
                                if (pseudo_res_0 < 1e-5) {
                                    std::cout << "Track " << i << ", Hit " << j
                                              << ": Invalid wire resolution (< 1e-5), autoscaling not possible!"
                                              << std::endl;
                                } else {
                            ...
                            hit_box = boxCreator((o + move + hit_u * u),
                                                 u,
                                                 v,
                                                 errorScale_ * std::sqrt(hit_cov(0, 0)),
                                                 pseudo_res_1,
                                 ...
                                 int markerPos)
    {
        ...
        lineSet->AddLine(
            intermediate1(0), intermediate1(1), intermediate1(2), intermediate2(0), intermediate2(1), intermediate2(2));
        ...
        if (drawErrors) {
            ...
            if (measuredState != NULL) {
                ...
                for (int i = 0; i < nEdges; ++i) {
                    ...
                    error_shape->SetTriangle(
                        iTri++, (i + 1) % nEdges + 1, i + 1 + nEdges, (i + 1) % nEdges + 1 + nEdges);
    ...
    void EventDisplay::makeGui()
    {
        ...
        hf = new TGHorizontalFrame(frmMain);
        {
            guiErrorScale_ = new TGNumberEntry(hf,
                                               errorScale_,
                                               6,
                                               999,
        ...
        hf = new TGHorizontalFrame(frmMain);
        {
            guiRepId_ = new TGNumberEntry(hf,
                                          repId_,
                                          6,
                                          999,
                                          TGNumberFormat::kNESInteger,
        ...
        hf = new TGHorizontalFrame(frmMain);
        {
            guiTrackId_ = new TGNumberEntry(hf,
                                            trackId_,
                                            6,
                                            999,
                                            TGNumberFormat::kNESInteger,
        ...
        hf = new TGHorizontalFrame(frmMain2);
        {
            ...
            lbl = new TGLabel(hf, "Go to event: ");
            hf->AddFrame(lbl);
            guiEvent2 = new TGNumberEntry(hf,
                                          0,
                                          9,
                                          999,
                                          TGNumberFormat::kNESInteger,
        ...
        hf = new TGHorizontalFrame(frmMain2);
        {
            guiDebugLvl_ = new TGNumberEntry(hf,
                                             debugLvl_,
                                             6,
                                             999,
                                             TGNumberFormat::kNESInteger,
        ...
        hf = new TGHorizontalFrame(frmMain2);
        {
            guiDPVal_ = new TGNumberEntry(hf,
                                          dPVal_,
                                          6,
                                          9999,
                                          TGNumberFormat::kNESReal,
        ...
        hf = new TGHorizontalFrame(frmMain2);
        {
            guiRelChi2_ = new TGNumberEntry(hf,
                                            dRelChi2_,
                                            6,
                                            9999,
                                            TGNumberFormat::kNESReal,
        ...
        hf = new TGHorizontalFrame(frmMain2);
        {
            guiNMinIter_ = new TGNumberEntry(hf,
                                             nMinIter_,
                                             6,
                                             999,
                                             TGNumberFormat::kNESInteger,
        ...
        hf = new TGHorizontalFrame(frmMain2);
        {
            guiNMaxIter_ = new TGNumberEntry(hf,
                                             nMaxIter_,
                                             6,
                                             999,
                                             TGNumberFormat::kNESInteger,
        ...
        hf = new TGHorizontalFrame(frmMain2);
        {
            guiNMaxFailed_ = new TGNumberEntry(hf,
                                               nMaxFailed_,
                                               6,
                                               999,
                                               TGNumberFormat::kNESInteger,
