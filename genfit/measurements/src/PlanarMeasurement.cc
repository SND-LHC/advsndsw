...
PlanarMeasurement::PlanarMeasurement(const TVectorD& rawHitCoords,
...
std::vector<MeasurementOnPlane*> PlanarMeasurement::constructMeasurementsOnPlane(const StateOnPlane& state) const
{
    ...
    MeasurementOnPlane* mop = new MeasurementOnPlane(
        rawHitCoords_, rawHitCov_, state.getPlane(), state.getRep(), constructHMatrix(state.getRep()));
...
const AbsHMatrix* PlanarMeasurement::constructHMatrix(const AbsTrackRep* rep) const
{
    ...
    switch (rawHitCoords_.GetNrows()) {
        ...
        default:
            Exception exc(
                "PlanarMeasurement default implementation can only handle 1D (strip) or 2D (pixel) measurements!",
                __LINE__,
                __FILE__);
            throw exc;
    }
