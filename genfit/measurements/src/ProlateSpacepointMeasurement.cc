...
ProlateSpacepointMeasurement::ProlateSpacepointMeasurement(int nDim)
    : SpacepointMeasurement(nDim)
    , largestErrorDirection_(0, 0, 1)
{
    ;
}

ProlateSpacepointMeasurement::ProlateSpacepointMeasurement(const TVectorD& rawHitCoords,
                                                           const TMatrixDSym& rawHitCov,
                                                           int detId,
                                                           int hitId,
                                                           TrackPoint* trackPoint)
    : SpacepointMeasurement(rawHitCoords, rawHitCov, detId, hitId, trackPoint)
    , largestErrorDirection_(0, 0, 1)
{
    ;
}
...
SharedPlanePtr ProlateSpacepointMeasurement::constructPlane(const StateOnPlane& state) const
{
    ...
    if (fabs(largestErrorDirection_.Angle(dirInPoca)) < 0.01) {
        Exception exc("ProlateSpacepointMeasurement::constructPlane(): Cannot construct detector plane, track "
                      "direction is parallel to largest error direction",
                      __LINE__,
                      __FILE__);
        throw exc;
    }
