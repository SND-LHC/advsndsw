...
SpacepointMeasurement::SpacepointMeasurement(int nDim)
    : AbsMeasurement(nDim)
    , weightedPlaneContruction_(true)
    , G_(3)
    , cutCov_(true)
{
    assert(nDim >= 3);
...
SpacepointMeasurement::SpacepointMeasurement(const TVectorD& rawHitCoords,
    ...
    , weightedPlaneContruction_(weightedPlaneContruction)
    , cutCov_(cutCov)
{
...
std::vector<MeasurementOnPlane*> SpacepointMeasurement::constructMeasurementsOnPlane(const StateOnPlane& state) const
{
    ...
    // m
    m(0) = (rawHitCoords_(0) - o.X()) * u.X() + (rawHitCoords_(1) - o.Y()) * u.Y() + (rawHitCoords_(2) - o.Z()) * u.Z();

    m(1) = (rawHitCoords_(0) - o.X()) * v.X() + (rawHitCoords_(1) - o.Y()) * v.Y() + (rawHitCoords_(2) - o.Z()) * v.Z();
