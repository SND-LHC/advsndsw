...
SharedPlanePtr WireMeasurement::constructPlane(const StateOnPlane& state) const
{
    ...
    // std::cout << " wire1(" << rawHitCoords_(0) << ", " << rawHitCoords_(1) << ", " << rawHitCoords_(2) << ")" <<
    // std::endl; std::cout << " wire2(" << rawHitCoords_(3) << ", " << rawHitCoords_(4) << ", " << rawHitCoords_(5) <<
    // ")" << std::endl;
    ...
    // std::cout << " wireDirection(" << wireDirection.X() << ", " << wireDirection.Y() << ", " << wireDirection.Z() <<
    // ")" << std::endl;
    ...
    if (fabs(wireDirection.Angle(dirInPoca)) < 0.01) {
        Exception exc("WireMeasurement::detPlane(): Cannot construct detector plane, direction is parallel to wire",
                      __LINE__,
                      __FILE__);
        throw exc;
    }
...
std::vector<MeasurementOnPlane*> WireMeasurement::constructMeasurementsOnPlane(const StateOnPlane& state) const
{
    ...
    MeasurementOnPlane* mopL = new MeasurementOnPlane(
        TVectorD(1, &mL), TMatrixDSym(1, &V), state.getPlane(), state.getRep(), constructHMatrix(state.getRep()));

    MeasurementOnPlane* mopR = new MeasurementOnPlane(
        TVectorD(1, &mR), TMatrixDSym(1, &V), state.getPlane(), state.getRep(), constructHMatrix(state.getRep()));
...
const AbsHMatrix* WireMeasurement::constructHMatrix(const AbsTrackRep* rep) const
{
    if (dynamic_cast<const RKTrackRep*>(rep) == NULL) {
        Exception exc("WireMeasurement default implementation can only handle state vectors of type RKTrackRep!",
                      __LINE__,
                      __FILE__);
        throw exc;
    }
