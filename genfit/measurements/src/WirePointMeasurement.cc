...
WirePointMeasurement::WirePointMeasurement(const TVectorD& rawHitCoords,
...
std::vector<MeasurementOnPlane*> WirePointMeasurement::constructMeasurementsOnPlane(const StateOnPlane& state) const
{
    MeasurementOnPlane* mopR = new MeasurementOnPlane(
        TVectorD(2), TMatrixDSym(2), state.getPlane(), state.getRep(), constructHMatrix(state.getRep()));
...
const AbsHMatrix* WirePointMeasurement::constructHMatrix(const AbsTrackRep* rep) const
{
    if (dynamic_cast<const RKTrackRep*>(rep) == NULL) {
        Exception exc("WirePointMeasurement default implementation can only handle state vectors of type RKTrackRep!",
                      __LINE__,
                      __FILE__);
        throw exc;
    }
