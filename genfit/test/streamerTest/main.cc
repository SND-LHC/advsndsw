...
int main()
{
        ...
        try {
            for (unsigned int i = 0; i < measurementTypes.size(); ++i) {
                std::vector<genfit::AbsMeasurement*> measurements =
                    measurementCreator.create(measurementTypes[i], i * 5.);
                genfit::TrackPoint* tp = new genfit::TrackPoint(measurements, fitTrack);
                // test scatterers
                genfit::ThinScatterer* sc = new genfit::ThinScatterer(
                    genfit::SharedPlanePtr(new genfit::DetPlane(TVector3(1, 1, 1), TVector3(1, 1, 1))),
    ...
    for (Long_t nEntry = 0; nEntry < tResults->GetEntries(); ++nEntry) {
        ...
        if (*pState == fitTrack->getFittedState().getState() && *pMatrix == fitTrack->getFittedState().getCov()
