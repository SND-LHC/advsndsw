...
std::vector<genfit::AbsMeasurement*> MeasurementCreator::create(eMeasurementType type,
                                                                double tracklength,
                                                                bool& outlier,
                                                                int& lr)
{
    ...
    if (outlierProb_ > gRandom->Uniform(1.)) {
        outlier = true;
        if (debug_)
            std::cerr << "create outlier" << std::endl;
    }

    switch (type) {
        case Pixel: {
            if (debug_)
                std::cerr << "create PixHit" << std::endl;

            genfit::SharedPlanePtr plane(
                new genfit::DetPlane(point, planeNorm.Cross(z), (planeNorm.Cross(z)).Cross(planeNorm)));
        ...
        case Spacepoint: {
            if (debug_)
                std::cerr << "create SpacepointHit" << std::endl;
        ...
        case ProlateSpacepoint: {
            if (debug_)
                std::cerr << "create ProlateSpacepointHit" << std::endl;
            ...
            rot(0, 0) = xp.X();
            rot(0, 1) = yp.X();
            rot(0, 2) = currentWireDir.X();
            rot(1, 0) = xp.Y();
            rot(1, 1) = yp.Y();
            rot(1, 2) = currentWireDir.Y();
            rot(2, 0) = xp.Z();
            rot(2, 1) = yp.Z();
            rot(2, 2) = currentWireDir.Z();
            ...
            measurement =
                new genfit::ProlateSpacepointMeasurement(hitCoords, hitCov, int(type), measurementCounter_, nullptr);
        ...
        case StripUV: {
            if (debug_)
                std::cerr << "create StripHit" << std::endl;
        ...
        case Wire: {
            if (debug_)
                std::cerr << "create WireHit" << std::endl;
        ...
        case WirePoint: {
            if (debug_)
                std::cerr << "create WirePointHit" << std::endl;
            ...
            if (outlier) {
                hitCoords(6) = gRandom->Uniform(outlierRange_);
                hitCoords(7) =
                    gRandom->Uniform(currentWireDir.Mag() - outlierRange_, currentWireDir.Mag() + outlierRange_);
            } else {
                hitCoords(6) = gRandom->Gaus(wirePerp.Mag(), resolution_);
