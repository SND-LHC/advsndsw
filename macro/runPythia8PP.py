...
parser.add_argument(
    "-n", "--pot", dest="Np", type=int, help="proton collisions", default=1000000
)
parser.add_argument(
    "-Ecm",
    "--energyCM",
    dest="eCM",
    type=float,
    help="center of mass energy [GeV]",
    default=13000.0,
)
parser.add_argument(
    "-C",
...
parser.add_argument(
    ...
    dest="eta_min",
    type=float,
    help="minimum eta for neutrino to pass",
    default=6.0,
)
parser.add_argument(
    "-EtaMax",
    dest="eta_max",
    type=float,
    help="maximum eta for neutrino to pass",
    default=10.0,
)
...
for i in range(0, L):
    if i == L - 1:
        nunames += nudict[options.nu_flavour[i]]
    else:
        nunames += nudict[options.nu_flavour[i]] + "_"
...
for n in range(int(options.Np)):
    ...
    for ii in range(1, py.event.size()):
        ...
        if py.event[ii].isFinal() and py.event[ii].id() in options.nu_flavour:
            evt = py.event[ii]
            eta = evt.eta()
            if (eta > options.eta_min and eta < options.eta_max) or (
                eta < -options.eta_min and eta > -options.eta_max
            ):
                dAnc.Clear()
                neut = ROOT.TParticle(
                    evt.id(),
...
print(
    "Saving to output %s neutrino flavour(s) having PDG ID(s)" % (nunames),
    options.nu_flavour,
)
print(
    "simulated events = %i, equivalent to integrated luminosity of %5.2G fb-1. Real time %6.1Fs, CPU time %6.1Fs"
    % (options.Np, IntLumi, rtime, ctime)
)
# neutrino CC cross section about 0.7 E-38 cm2 GeV-1 nucleon-1, SND@LHC: 59mm tungsten
# sigma_CC(100 GeV) = 4.8E-12
print(
    "corresponding to effective luminosity (folded with neutrino CC cross section at 100GeV) of %5.2G fb-1."
    % (IntLumi / 4.8e-12)
)
