...
def main():
    ...
    ut.bookHist(h, "NuTauMu_mu_pt", "#mu#pm;p_t[GeV];", 100, 0, maxpt)
    ut.bookHist(
        h, "NuTauMu_mu_ppt", "#mu#pm;p[GeV];p_t[GeV];", 100, 0, maxp, 100, 0, maxpt
    )
    ut.bookHist(h, "NuTauMu_all_p", "#mu#pm;p[GeV];", 100, 0, maxp)
    ut.bookHist(h, "NuTauMu_all_pt", "#mu#pm;p_t[GeV];", 100, 0, maxpt)
    ut.bookHist(
        h, "NuTauMu_all_ppt", "#mu#pm;p[GeV];p_t[GeV];", 100, 0, maxp, 100, 0, maxpt
    )

    for suffix in ["", "_original"]:
        ut.bookHist(h, "mu_p{}".format(suffix), "#mu#pm;p[GeV];", 100, 0, maxp)
        ut.bookHist(h, "mu_pt{}".format(suffix), "#mu#pm;p_t[GeV];", 100, 0, maxpt)
        ut.bookHist(
            h,
    ...
    ut.bookHist(
        h,
        "ECAL_TP_e",
        "e#pm with E#geq 250 MeV;x[cm];y[cm]",
        167,
        -501,
        +501,
        334,
    ...
    ut.bookHist(
        h, "ECAL_TP_gamma", "#gamma;x[cm];y[cm]", 167, -501, +501, 334, -1002, 1002
    )
    ut.bookHist(
        h, "ECAL_Alt_gamma", "#gamma;x[cm];y[cm]", 50, -500, +500, 100, -1000, 1000
    )
    ...
    for event in ch:
        ...
        if muon:
            ...
            h["mu_ppt_original"].Fill(
                original_muon.GetP(), original_muon.GetPt(), weight
            )
