...
if not options.geoFile:
    options.geoFile = options.inputFile.replace("ship.", "geofile_full.").replace(
        "_rec.", "."
    )
...
ut.bookHist(
    h,
    "delPOverP2",
    "delP / P chi2/nmeas<" + str(chi2CutOff),
    400,
    0.0,
    200.0,
    100,
...
ut.bookHist(
    h,
    "delPOverP2z",
    "delPz / Pz chi2/nmeas<" + str(chi2CutOff),
    400,
    0.0,
    200.0,
    100,
...
for x in ["", "_pi0"]:
    ...
    ut.bookHist(
        h,
        "IP0/mass" + x,
        "Impact Parameter to target vs mass",
        100,
        0.0,
        2.0,
        100,
...
ut.bookHist(
    h,
    "measVSchi2",
    "number of measurements vs chi2/meas",
    40,
    -0.5,
    39.5,
    100,
...
ut.bookHist(h, "ecalClusters", "x/y and energy", 50, -3.0, 3.0, 50, -6.0, 6.0)
        ...
        for k in range(4):
            for j in range(3):
                ...
                if k == 0 or k == 2:
                    ...
                    aZAN = (
                        (ca.Dot(u) * Vsq - ca.Dot(v) * UV)
                        * 2
                        * (u[j] * Vsq - v[j] * UV)
                    )
                    bNAZ = denom * (
                        ca[j] * UV + (u.Dot(ca) * v[j]) - 2 * ca.Dot(v) * u[j]
                    )
                    bZAN = (
                        (ca.Dot(u) * UV - ca.Dot(v) * Usq)
                        * 2
                        * (u[j] * Vsq - v[j] * UV)
                    )
                    T[i][3 * k + j] = 0.5 * (
                        Va * KD
                        + u[i] / denom**2 * (aNAZ - aZAN)
                        + v[i] / denom**2 * (bNAZ - bZAN)
                    )
                elif k == 3:
                    # covv
                    aNAZ = denom * (
                        2 * ca.Dot(u) * v[j] - ca.Dot(v) * u[j] - ca[j] * UV
                    )
                    aZAN = (
                        (ca.Dot(u) * Vsq - ca.Dot(v) * UV)
                        * 2
...
def fitSingleGauss(x, ba=None, be=None):
    ...
    if not myGauss:
        ...
        myGauss = ROOT.TF1(
            name,
            "[0]*" + str(bw) + "/([2]*sqrt(2*pi))*exp(-0.5*((x-[1])/[2])**2)+[3]",
            4,
        )
...
def ecalCluster2MC(aClus):
    ...
    for i in range(aClus.Size()):
        mccell = ecalStructure.GetHitCell(
            aClus.CellNum(i)
...
def makePlots():
    ut.bookCanvas(
        h, key="ecalanalysis", title="cluster map", nx=800, ny=600, cx=1, cy=1
    )
    ...
    h["meas2"].Draw()
    ut.bookCanvas(
        h, key="fitresults", title="Fit Results", nx=1600, ny=1200, cx=2, cy=2
    )
    ...
    for x in ["", "_pi0"]:
        ...
        if x == "":
            ...
            h["pi0Mass"].SetYTitle(
                "N/" + str(int(h["pi0Mass"].GetBinWidth(1) * 1000)) + "MeV"
            )
            h["pi0Mass"].Draw()
    ...
    ut.bookCanvas(
        h, key="vxpulls", title="Vertex resol and pulls", nx=1600, ny=1200, cx=3, cy=2
    )
    cv = h["vxpulls"].cd(4)
    h["Vxpull"].Draw()
    cv = h["vxpulls"].cd(5)
    ...
    ut.bookCanvas(
        h, key="vetodecisions", title="Veto Detectors", nx=1600, ny=600, cx=4, cy=1
    )
...
def myEventLoop(n):
    ...
    if hasattr(sTree, "EcalClusters"):
        ...
        for aClus in ecalReconstructed:
            ...
            if pName not in h:
                ut.bookHist(
                    h,
                    pName,
                    "x/y and energy for " + pName.split("_")[1],
                    50,
                    -3.0,
                    3.0,
                    50,
            ...
            for fT in sTree.FitTracks:
                ...
                if rc:
                    ...
                    if tName not in h:
                        ...
                        ut.bookHist(
                            h,
                            tName.replace("dist", "disty"),
                            "Ecal cluster distance to " + p + " in Y ",
                            100,
                            -50.0 * u.cm,
                            50.0 * u.cm,
                        )
                    dist = ROOT.TMath.Sqrt(
                        (aClus.X() - pos.X()) ** 2 + (aClus.Y() - pos.Y()) ** 2
                    )
        ...
        for aClus in sTree.EcalClusters:
            rc = h["ecalClusters"].Fill(
                aClus.X() / u.m, aClus.Y() / u.m, aClus.Energy() / u.GeV
            )
    ...
    for atrack in sTree.FitTracks:
        ...
        Px, Py, Pz = (
            fittedState.getMom().x(),
            fittedState.getMom().y(),
    ...
    for HNL in sTree.Particles:
        ...
        st1, st2 = (
            sTree.FitTracks[t1].getFittedState(),
        ...
        rep1, rep2 = (
            ROOT.genfit.RKTrackRep(st1.getPDG()),
            ROOT.genfit.RKTrackRep(st2.getPDG()),
        )
        ...
        dist = HNL.GetDoca()
        h["Vzpull"].Fill(
            (mctrack.GetStartZ() - HNLPos.Z()) / ROOT.TMath.Sqrt(covX[2][2])
        )
        h["Vxpull"].Fill(
            (mctrack.GetStartX() - HNLPos.X()) / ROOT.TMath.Sqrt(covX[0][0])
        )
        h["Vypull"].Fill(
            (mctrack.GetStartY() - HNLPos.Y()) / ROOT.TMath.Sqrt(covX[1][1])
        )
...
def HNLKinematics():
    ...
    for x in HNLorigin:
        print(
            "%4i : %5.4F relative fraction: %5.4F "
            % (x, HNLorigin[x], HNLorigin[x] / theSum)
        )
...
if ecal:
    ...
    if hasattr(sTree, "EcalReconstructed"):
        ...
        ecalCl3PhS.SetParameters(6.77797e-04, 5.75385e00, 3.42690e-03, -1.16383e-04)
        ecalClusterCalib.SetStraightCalibration(3, ecalCl3PhS)
        ecalCl3Ph = ROOT.TFormula(
            "ecalCl3Ph", "[0]+x*([1]+x*([2]+x*[3]))+[4]*x*y+[5]*x*y*y"
        )
        ecalCl3Ph.SetParameters(
            0.000750975, 5.7552, 0.00282783, -8.0025e-05, -0.000823651, 0.000111561
        )
        ecalClusterCalib.SetCalibration(3, ecalCl3Ph)
        # 6x6 cm cells
        ecalCl2PhS = ROOT.TFormula("ecalCl2PhS", "[0]+x*([1]+x*([2]+x*[3]))")
        ecalCl2PhS.SetParameters(8.14724e-04, 5.67428e00, 3.39030e-03, -1.28388e-04)
        ecalClusterCalib.SetStraightCalibration(2, ecalCl2PhS)
        ecalCl2Ph = ROOT.TFormula(
            "ecalCl2Ph", "[0]+x*([1]+x*([2]+x*[3]))+[4]*x*y+[5]*x*y*y"
        )
        ecalCl2Ph.SetParameters(
            0.000948095, 5.67471, 0.00339177, -0.000122629, -0.000169109, 8.33448e-06
        )
        ...
        ecalReconstructed = ecalReco.InitPython(
            sTree.EcalClusters, ecalStructure, ecalCalib
        )
